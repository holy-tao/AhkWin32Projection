...
class IPhotoConfirmationControl extends IInspectable{
    ...
    /**
     * The interface identifier for IPhotoConfirmationControl
     * @type {Guid}
     */
    static IID => Guid("{c8f3f363-ff5e-4582-a9a8-0550f85a4a76}")
     ...
     * @type {Array<String>}
     */
    static VTableNames => ["get_Supported", "get_Enabled", "put_Enabled", "get_PixelFormat", "put_PixelFormat"]
     ...
     */
    get_Supported() {
        result := ComCall(6, this, "int*", &pbSupported := 0, "int")
        if(result != 0) {
     ...
     */
    get_PixelFormat() {
        result := ComCall(9, this, "int*", &format := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    put_PixelFormat(format) {
        result := ComCall(10, this, "int", format, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
