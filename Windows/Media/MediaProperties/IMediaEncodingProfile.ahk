#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include .\AudioEncodingProperties.ahk
#Include .\VideoEncodingProperties.ahk
#Include .\ContainerEncodingProperties.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Media.MediaProperties
 * @version WindowsRuntime 1.4
 */
class IMediaEncodingProfile extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IMediaEncodingProfile
     * @type {Guid}
     */
    static IID => Guid("{e7dbf5a8-1db9-4783-876b-3dfe12acfdb3}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["put_Audio", "get_Audio", "put_Video", "get_Video", "put_Container", "get_Container"]

    /**
     * @type {AudioEncodingProperties} 
     */
    Audio {
        get => this.get_Audio()
        set => this.put_Audio(value)
    }

    /**
     * @type {VideoEncodingProperties} 
     */
    Video {
        get => this.get_Video()
        set => this.put_Video(value)
    }

    /**
     * @type {ContainerEncodingProperties} 
     */
    Container {
        get => this.get_Container()
        set => this.put_Container(value)
    }

    /**
     * 
     * @param {AudioEncodingProperties} value 
     * @returns {HRESULT} 
     */
    put_Audio(value) {
        result := ComCall(6, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {AudioEncodingProperties} 
     */
    get_Audio() {
        result := ComCall(7, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return AudioEncodingProperties(value)
    }

    /**
     * 
     * @param {VideoEncodingProperties} value 
     * @returns {HRESULT} 
     */
    put_Video(value) {
        result := ComCall(8, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {VideoEncodingProperties} 
     */
    get_Video() {
        result := ComCall(9, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return VideoEncodingProperties(value)
    }

    /**
     * 
     * @param {ContainerEncodingProperties} value 
     * @returns {HRESULT} 
     */
    put_Container(value) {
        result := ComCall(10, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {ContainerEncodingProperties} 
     */
    get_Container() {
        result := ComCall(11, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return ContainerEncodingProperties(value)
    }
}
