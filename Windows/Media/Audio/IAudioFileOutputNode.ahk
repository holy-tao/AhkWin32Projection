#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Storage\IStorageFile.ahk
#Include ..\MediaProperties\MediaEncodingProfile.ahk
#Include ..\..\Foundation\IAsyncOperation.ahk
#Include ..\..\Foundation\IPropertyValue.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Media.Audio
 * @version WindowsRuntime 1.4
 */
class IAudioFileOutputNode extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IAudioFileOutputNode
     * @type {Guid}
     */
    static IID => Guid("{50e01980-5166-4093-80f8-ada00089e9cf}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_File", "get_FileEncodingProfile", "FinalizeAsync"]

    /**
     * @type {IStorageFile} 
     */
    File {
        get => this.get_File()
    }

    /**
     * @type {MediaEncodingProfile} 
     */
    FileEncodingProfile {
        get => this.get_FileEncodingProfile()
    }

    /**
     * 
     * @returns {IStorageFile} 
     */
    get_File() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IStorageFile(value)
    }

    /**
     * 
     * @returns {MediaEncodingProfile} 
     */
    get_FileEncodingProfile() {
        result := ComCall(7, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return MediaEncodingProfile(value)
    }

    /**
     * 
     * @returns {IAsyncOperation<Integer>} 
     */
    FinalizeAsync() {
        result := ComCall(8, this, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation((ptr) => IPropertyValue(ptr).GetInt32(), result_)
    }
}
