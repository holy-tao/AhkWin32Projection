 ...
 */
class IHttpMediaTypeWithQualityHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpMediaTypeWithQualityHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{5b070cd9-b560-4fc8-9835-7e6c0a657b24}")
    ...
    Parse(input_) {
        ...
        return HttpMediaTypeWithQualityHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpMediaTypeWithQualityHeaderValue>} mediaTypeWithQualityHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, mediaTypeWithQualityHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", mediaTypeWithQualityHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
