 ...
 */
class IHttpContentCodingWithQualityHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpContentCodingWithQualityHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{e8c9357c-8f89-4801-8e75-4c9abfc3de71}")
    ...
    Parse(input_) {
        ...
        return HttpContentCodingWithQualityHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpContentCodingWithQualityHeaderValue>} contentCodingWithQualityHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, contentCodingWithQualityHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", contentCodingWithQualityHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
