 ...
 */
class IHttpLanguageRangeWithQualityHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpLanguageRangeWithQualityHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{2541e146-f308-46f5-b695-42f54024ec68}")
    ...
    Parse(input_) {
        ...
        return HttpLanguageRangeWithQualityHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpLanguageRangeWithQualityHeaderValue>} languageRangeWithQualityHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, languageRangeWithQualityHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", languageRangeWithQualityHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
