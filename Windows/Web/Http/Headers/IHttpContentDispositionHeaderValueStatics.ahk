 ...
 */
class IHttpContentDispositionHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpContentDispositionHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{29c56067-5a37-46e4-b074-c5177d69ca66}")
    ...
    Parse(input_) {
        ...
        return HttpContentDispositionHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpContentDispositionHeaderValue>} contentDispositionHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, contentDispositionHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", contentDispositionHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
