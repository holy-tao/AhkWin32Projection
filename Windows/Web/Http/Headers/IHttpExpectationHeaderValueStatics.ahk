 ...
 */
class IHttpExpectationHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpExpectationHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{3019abe2-cfe5-473b-a57f-fba5b14eb257}")
    ...
    Parse(input_) {
        ...
        return HttpExpectationHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpExpectationHeaderValue>} expectationHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, expectationHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", expectationHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
