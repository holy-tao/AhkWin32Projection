 ...
 */
class IHttpProductHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpProductHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{90c33e29-befc-4337-be62-49f097975f53}")
    ...
    Parse(input_) {
        ...
        return HttpProductHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpProductHeaderValue>} productHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, productHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", productHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
