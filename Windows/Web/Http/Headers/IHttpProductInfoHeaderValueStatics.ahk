 ...
 */
class IHttpProductInfoHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpProductInfoHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{db7fd857-327a-4e73-81e5-7059a302b042}")
    ...
    Parse(input_) {
        ...
        return HttpProductInfoHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpProductInfoHeaderValue>} productInfoHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, productInfoHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", productInfoHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
