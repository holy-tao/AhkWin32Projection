 ...
 */
class IHttpCookiePairHeaderValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IHttpCookiePairHeaderValueStatics
     * @type {Guid}
     */
    static IID => Guid("{6e866d48-06af-4462-8158-99388d5dca81}")
    ...
    Parse(input_) {
        ...
        return HttpCookiePairHeaderValue(result_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<HttpCookiePairHeaderValue>} cookiePairHeaderValue 
     * @returns {Boolean} 
     */
    TryParse(input_, cookiePairHeaderValue) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        ...
        result := ComCall(7, this, "ptr", input_, "ptr", cookiePairHeaderValue, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
