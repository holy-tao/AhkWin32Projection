#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Perception.Spatial
 * @version WindowsRuntime 1.4
 */
class ISpatialAnchorExportSufficiency extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for ISpatialAnchorExportSufficiency
     * @type {Guid}
     */
    static IID => Guid("{77c25b2b-3409-4088-b91b-fdfd05d1648f}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_IsMinimallySufficient", "get_SufficiencyLevel", "get_RecommendedSufficiencyLevel"]

    /**
     * @type {Boolean} 
     */
    IsMinimallySufficient {
        get => this.get_IsMinimallySufficient()
    }

    /**
     * @type {Float} 
     */
    SufficiencyLevel {
        get => this.get_SufficiencyLevel()
    }

    /**
     * @type {Float} 
     */
    RecommendedSufficiencyLevel {
        get => this.get_RecommendedSufficiencyLevel()
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_IsMinimallySufficient() {
        result := ComCall(6, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @returns {Float} 
     */
    get_SufficiencyLevel() {
        result := ComCall(7, this, "double*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @returns {Float} 
     */
    get_RecommendedSufficiencyLevel() {
        result := ComCall(8, this, "double*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }
}
