 ...
 */
class IResourceContext extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IResourceContext
     * @type {Guid}
     */
    static IID => Guid("{2fa22f4b-707e-4b27-ad0d-d0d8cd468fd2}")
     ...
     * @type {Array<String>}
     */
    static VTableNames => ["get_QualifierValues", "Reset", "ResetQualifierValues", "OverrideToMatch", "Clone", "get_Languages", "put_Languages"]
    ...
    get_QualifierValues() {
        ...
        return IObservableMap((ptr) => HSTRING({ Value: ptr }), (ptr) => HSTRING({ Value: ptr }), value)
    }
    ...
    ResetQualifierValues(qualifierNames) {
        result := ComCall(8, this, "ptr", qualifierNames, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    Clone() {
        result := ComCall(10, this, "ptr*", &clone := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return ResourceContext(clone)
    }
    ...
    put_Languages(languages) {
        result := ComCall(12, this, "ptr", languages, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
