 ...
 * @version v4.0.30319
 */
class FAST_IO_WRITE extends IUnknown {
     ...
     */
    Invoke(FileObject, FileOffset, Length, Wait, LockKey, Buffer_, IoStatus, DeviceObject) {
        FileOffsetMarshal := FileOffset is VarRef ? "int64*" : "ptr"
        Buffer_Marshal := Buffer_ is VarRef ? "ptr" : "ptr"

        result := ComCall(3, this, "ptr", FileObject, FileOffsetMarshal, FileOffset, "uint", Length, "char", Wait, "uint", LockKey, Buffer_Marshal, Buffer_, "ptr", IoStatus, "ptr", DeviceObject, "char")
        return result
    }
}
