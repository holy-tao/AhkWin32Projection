 ...
 * @version v4.0.30319
 */
class FAST_IO_PREPARE_MDL_WRITE extends IUnknown {
     ...
     */
    Invoke(FileObject, FileOffset, Length, LockKey, MdlChain, IoStatus, DeviceObject) {
        FileOffsetMarshal := FileOffset is VarRef ? "int64*" : "ptr"
        MdlChainMarshal := MdlChain is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, "ptr", FileObject, FileOffsetMarshal, FileOffset, "uint", Length, "uint", LockKey, MdlChainMarshal, MdlChain, "ptr", IoStatus, "ptr", DeviceObject, "char")
        return result
    }
}
