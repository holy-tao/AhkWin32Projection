 ...
 */
export default struct FAST_IO_PREPARE_MDL_WRITE {
    value : IntPtr

    __value {
        set {
            if (value is FAST_IO_PREPARE_MDL_WRITE) {
    ...
    /**
     * A FAST_IO_PREPARE_MDL_WRITE that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends FAST_IO_PREPARE_MDL_WRITE {
        /**
         * Creates a FAST_IO_PREPARE_MDL_WRITE pointer that invokes the given AHK function when called.
         * @param {Func(FILE_OBJECT, "int64*", UInt32, UInt32, "ptr*", IO_STATUS_BLOCK, DEVICE_OBJECT) => BOOLEAN} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 7)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 7 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [FILE_OBJECT.Ptr, "int64*", UInt32, UInt32, "ptr*", IO_STATUS_BLOCK.Ptr, DEVICE_OBJECT.Ptr, BOOLEAN])
        }
