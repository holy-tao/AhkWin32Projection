 ...
 */
export default struct PROTOCOL_CM_MAKE_CALL {
    value : IntPtr

    __value {
        set {
            if (value is PROTOCOL_CM_MAKE_CALL) {
    ...
    /**
     * A PROTOCOL_CM_MAKE_CALL that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PROTOCOL_CM_MAKE_CALL {
        /**
         * Creates a PROTOCOL_CM_MAKE_CALL pointer that invokes the given AHK function when called.
         * @param {Func("ptr", CO_CALL_PARAMETERS, "ptr", "ptr*") => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
