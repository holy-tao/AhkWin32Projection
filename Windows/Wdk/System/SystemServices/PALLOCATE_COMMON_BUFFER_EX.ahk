 ...
 * @version v4.0.30319
 */
class PALLOCATE_COMMON_BUFFER_EX extends IUnknown {
     ...
     */
    Invoke(DmaAdapter, MaximumAddress, Length, LogicalAddress, CacheEnabled, PreferredNode) {
        MaximumAddressMarshal := MaximumAddress is VarRef ? "int64*" : "ptr"
        LogicalAddressMarshal := LogicalAddress is VarRef ? "int64*" : "ptr"

        result := ComCall(3, this, "ptr", DmaAdapter, MaximumAddressMarshal, MaximumAddress, "uint", Length, LogicalAddressMarshal, LogicalAddress, "char", CacheEnabled, "uint", PreferredNode, "ptr")
        return result
    }
}
