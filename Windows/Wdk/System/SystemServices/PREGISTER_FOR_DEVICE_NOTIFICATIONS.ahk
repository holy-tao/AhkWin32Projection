...
export default struct PREGISTER_FOR_DEVICE_NOTIFICATIONS {
    ...
    struct From extends PREGISTER_FOR_DEVICE_NOTIFICATIONS {
         ...
         * @param {Func(DEVICE_OBJECT, PDEVICE_NOTIFY_CALLBACK, "ptr") => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 3)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 3 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [DEVICE_OBJECT.Ptr, PDEVICE_NOTIFY_CALLBACK, "ptr", NTSTATUS])
        }
