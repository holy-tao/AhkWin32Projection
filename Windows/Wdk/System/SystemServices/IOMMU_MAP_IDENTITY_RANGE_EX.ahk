 ...
 */
export default struct IOMMU_MAP_IDENTITY_RANGE_EX {
    value : IntPtr

    __value {
        set {
            if (value is IOMMU_MAP_IDENTITY_RANGE_EX) {
    ...
    /**
     * A IOMMU_MAP_IDENTITY_RANGE_EX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends IOMMU_MAP_IDENTITY_RANGE_EX {
        /**
         * Creates a IOMMU_MAP_IDENTITY_RANGE_EX pointer that invokes the given AHK function when called.
         * @param {Func(IOMMU_DMA_DOMAIN, UInt32, IOMMU_MAP_PHYSICAL_ADDRESS) => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 3)) {
