 ...
 */
export default struct IOMMU_DOMAIN_ATTACH_DEVICE {
    value : IntPtr

    __value {
        set {
            if (value is IOMMU_DOMAIN_ATTACH_DEVICE) {
    ...
    /**
     * A IOMMU_DOMAIN_ATTACH_DEVICE that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends IOMMU_DOMAIN_ATTACH_DEVICE {
        /**
         * Creates a IOMMU_DOMAIN_ATTACH_DEVICE pointer that invokes the given AHK function when called.
         * @param {Func(IOMMU_DMA_DOMAIN, DEVICE_OBJECT, UInt32, UInt32) => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
