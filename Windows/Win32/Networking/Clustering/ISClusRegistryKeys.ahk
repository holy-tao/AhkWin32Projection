...
class ISClusRegistryKeys extends IDispatch{
    ...
    get_Item(varIndex) {
        ...
        result := ComCall(10, this, "ptr", varIndex, "ptr", pbstrRegistryKey, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    AddItem(bstrRegistryKey) {
        if(bstrRegistryKey is String) {
            pin := BSTR.Alloc(bstrRegistryKey)
            bstrRegistryKey := pin.Value
        }

        result := ComCall(11, this, "ptr", bstrRegistryKey, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
