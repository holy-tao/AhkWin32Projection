 ...
 * @version v4.0.30319
 */
class PRESUTIL_GET_FILETIME_PROPERTY extends IUnknown {
     ...
     */
    Invoke(pftOutValue, pValueStruct, ftOldValue, ftMinimum, ftMaximum, ppPropertyList, pcbPropertyListSize) {
        ppPropertyListMarshal := ppPropertyList is VarRef ? "ptr*" : "ptr"
        pcbPropertyListSizeMarshal := pcbPropertyListSize is VarRef ? "uint*" : "ptr"

        result := ComCall(3, this, "ptr", pftOutValue, "ptr", pValueStruct, "ptr", ftOldValue, "ptr", ftMinimum, "ptr", ftMaximum, ppPropertyListMarshal, ppPropertyList, pcbPropertyListSizeMarshal, pcbPropertyListSize, "uint")
        return result
    }
}
