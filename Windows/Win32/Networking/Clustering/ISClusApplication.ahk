...
class ISClusApplication extends IDispatch{
    ...
    get_DomainNames(ppDomains) {
        result := ComCall(7, this, "ptr", ppDomains, "int")
        if(result != 0)
    ...
    get_ClusterNames(bstrDomainName, ppClusters) {
        ...
        result := ComCall(8, this, "ptr", bstrDomainName, "ptr", ppClusters, "int")
        if(result != 0)
            throw OSError(result)
    ...
    OpenCluster(bstrClusterName, pCluster) {
        ...
        result := ComCall(9, this, "ptr", bstrClusterName, "ptr", pCluster, "int")
        if(result != 0)
            throw OSError(result)
