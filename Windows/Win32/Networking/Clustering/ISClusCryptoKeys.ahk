 ...
 */
export default struct ISClusCryptoKeys extends IDispatch {
    /**
     ...
     * used for ISClusCryptoKeys interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISClusCryptoKeys.Vtbl()
        }
     ...
     */
    get_Item(varIndex) {
        pbstrCyrptoKey := BSTR.Owned()
        result := ComCall(10, this, VARIANT, varIndex, BSTR.Ptr, pbstrCyrptoKey, "HRESULT")
    ...
    AddItem(bstrCryptoKey) {
        ...
        result := ComCall(11, this, BSTR, bstrCryptoKey, "HRESULT")
        return result
    }
    ...
    Query(iid) {
        if (ISClusCryptoKeys.IID.Equals(iid)) {
            return true
        }
