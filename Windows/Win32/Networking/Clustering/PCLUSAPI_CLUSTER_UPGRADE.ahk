 ...
 * @version v4.0.30319
 */
class PCLUSAPI_CLUSTER_UPGRADE extends IUnknown {
     ...
     */
    Invoke(hCluster_, perform, pfnProgressCallback, pvCallbackArg) {
        hCluster_ := hCluster_ is Win32Handle ? NumGet(hCluster_, "ptr") : hCluster_

        pvCallbackArgMarshal := pvCallbackArg is VarRef ? "ptr" : "ptr"

        result := ComCall(3, this, "ptr", hCluster_, "int", perform, "ptr", pfnProgressCallback, pvCallbackArgMarshal, pvCallbackArg, "uint")
        return result
    }
}
