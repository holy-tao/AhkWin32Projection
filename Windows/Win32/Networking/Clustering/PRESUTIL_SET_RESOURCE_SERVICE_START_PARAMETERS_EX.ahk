...
export default struct PRESUTIL_SET_RESOURCE_SERVICE_START_PARAMETERS_EX {
    ...
    Call(pszServiceName, schSCMHandle, phService, dwDesiredAccess, pfnLogEvent, hResourceHandle) {
        ...
        result := DllCall(this.value, "ptr", pszServiceName, SC_HANDLE, schSCMHandle, SC_HANDLE.Ptr, phService, UInt32, dwDesiredAccess, PLOG_EVENT_ROUTINE, pfnLogEvent, IntPtr, hResourceHandle, UInt32)
        return result
    }
    ...
    struct From extends PRESUTIL_SET_RESOURCE_SERVICE_START_PARAMETERS_EX {
         ...
         * @param {Func(PWSTR, SC_HANDLE, SC_HANDLE, UInt32, PLOG_EVENT_ROUTINE, IntPtr) => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [PWSTR, SC_HANDLE, SC_HANDLE.Ptr, UInt32, PLOG_EVENT_ROUTINE, IntPtr, UInt32])
        }
