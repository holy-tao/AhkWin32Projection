...
class ISClusResTypes extends IDispatch{
     ...
     */
    get_Item(varIndex) {
        result := ComCall(10, this, "ptr", varIndex, "ptr*", &ppClusResType := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    CreateItem(bstrResourceTypeName, bstrDisplayName, bstrResourceTypeDll, dwLooksAlivePollInterval, dwIsAlivePollInterval) {
        if(bstrResourceTypeName is String) {
            pin := BSTR.Alloc(bstrResourceTypeName)
            bstrResourceTypeName := pin.Value
        }
        if(bstrDisplayName is String) {
            pin := BSTR.Alloc(bstrDisplayName)
            bstrDisplayName := pin.Value
        }
        if(bstrResourceTypeDll is String) {
            pin := BSTR.Alloc(bstrResourceTypeDll)
            bstrResourceTypeDll := pin.Value
        }

        result := ComCall(11, this, "ptr", bstrResourceTypeName, "ptr", bstrDisplayName, "ptr", bstrResourceTypeDll, "int", dwLooksAlivePollInterval, "int", dwIsAlivePollInterval, "ptr*", &ppResourceType := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
