...
class ISClusResGroups extends IDispatch{
     ...
     */
    get_Item(varIndex) {
        result := ComCall(10, this, "ptr", varIndex, "ptr*", &ppClusResGroup := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    CreateItem(bstrResourceGroupName) {
        if(bstrResourceGroupName is String) {
            pin := BSTR.Alloc(bstrResourceGroupName)
            bstrResourceGroupName := pin.Value
        }

        result := ComCall(11, this, "ptr", bstrResourceGroupName, "ptr*", &ppResourceGroup := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
