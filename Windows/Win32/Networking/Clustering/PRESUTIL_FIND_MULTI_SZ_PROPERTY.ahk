 ...
 */
export default struct PRESUTIL_FIND_MULTI_SZ_PROPERTY {
    value : IntPtr

    __value {
        set {
            if (value is PRESUTIL_FIND_MULTI_SZ_PROPERTY) {
    ...
    /**
     * A PRESUTIL_FIND_MULTI_SZ_PROPERTY that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PRESUTIL_FIND_MULTI_SZ_PROPERTY {
        /**
         * Creates a PRESUTIL_FIND_MULTI_SZ_PROPERTY pointer that invokes the given AHK function when called.
         * @param {Func(IntPtr, UInt32, PWSTR, PWSTR, "uint*") => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 5)) {
