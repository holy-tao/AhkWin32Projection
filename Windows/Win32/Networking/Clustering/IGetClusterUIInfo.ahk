 ...
 */
export default struct IGetClusterUIInfo extends IUnknown {
    /**
     ...
     * used for IGetClusterUIInfo interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IGetClusterUIInfo.Vtbl()
        }
     ...
     */
    GetFont() {
        result := ComCall(5, this, HFONT.Owned)
        return result
    }
     ...
     */
    GetIcon() {
        result := ComCall(6, this, HICON.Owned)
        return result
    }

    Query(iid) {
        if (IGetClusterUIInfo.IID.Equals(iid)) {
            return true
        }
