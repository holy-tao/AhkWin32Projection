 ...
 */
export default struct IGetClusterResourceInfo extends IUnknown {
    /**
     ...
     * used for IGetClusterResourceInfo interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetResourceHandle      : IntPtr
        GetResourceTypeName    : IntPtr
        GetResourceNetworkName : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IGetClusterResourceInfo.Vtbl()
        }
    ...
    Query(iid) {
        if (IGetClusterResourceInfo.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetResourceHandle)
        CallbackFree(this.vtbl.GetResourceTypeName)
        CallbackFree(this.vtbl.GetResourceNetworkName)
    }
}
