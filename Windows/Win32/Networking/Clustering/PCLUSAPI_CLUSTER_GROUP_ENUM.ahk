 ...
 */
export default struct PCLUSAPI_CLUSTER_GROUP_ENUM {
    value : IntPtr

    __value {
        set {
            if (value is PCLUSAPI_CLUSTER_GROUP_ENUM) {
    ...
    /**
     * A PCLUSAPI_CLUSTER_GROUP_ENUM that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PCLUSAPI_CLUSTER_GROUP_ENUM {
        /**
         * Creates a PCLUSAPI_CLUSTER_GROUP_ENUM pointer that invokes the given AHK function when called.
         * @param {Func(HGROUPENUM, UInt32, "uint*", PWSTR, "uint*") => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 5)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 5 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [HGROUPENUM, UInt32, "uint*", PWSTR, "uint*", UInt32])
        }
