 ...
 * @version v4.0.30319
 */
class PCLUSAPI_CREATE_CLUSTER_NAME_ACCOUNT extends IUnknown {
     ...
     */
    Invoke(hCluster_, pConfig, pfnProgressCallback, pvCallbackArg) {
        hCluster_ := hCluster_ is Win32Handle ? NumGet(hCluster_, "ptr") : hCluster_

        pvCallbackArgMarshal := pvCallbackArg is VarRef ? "ptr" : "ptr"

        result := ComCall(3, this, "ptr", hCluster_, "ptr", pConfig, "ptr", pfnProgressCallback, pvCallbackArgMarshal, pvCallbackArg, "uint")
        return result
    }
}
