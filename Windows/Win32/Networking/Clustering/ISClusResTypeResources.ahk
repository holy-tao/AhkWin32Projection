 ...
 */
export default struct ISClusResTypeResources extends IDispatch {
    /**
     * The interface identifier for ISClusResTypeResources
     * @type {Guid}
     */
    static IID := Guid("{f2e60714-2631-11d1-89f1-00a0c90d061e}")
     ...
     * used for ISClusResTypeResources interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISClusResTypeResources.Vtbl()
        }
     ...
     * @returns {ISClusResource} 
     */
    CreateItem(bstrResourceName, bstrGroupName, dwFlags) {
        ...
        result := ComCall(11, this, BSTR, bstrResourceName, BSTR, bstrGroupName, CLUSTER_RESOURCE_CREATE_FLAGS, dwFlags, "ptr*", &ppClusterResource := 0, "HRESULT")
    ...
    _Query(iid) {
        if (ISClusResTypeResources.IID.Equals(iid)) {
            return true
        }
    ...
    Implement(implObj, flags := "") {
        ...
        this.vtbl.CreateItem := CallbackCreate(ObjBindMethod(implObj, "CreateItem"), flags, 5)
