 ...
 * @see https://learn.microsoft.com/windows/win32/api/mswsock/nc-mswsock-lpfn_connectex
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPFN_CONNECTEX {
    value : IntPtr

    __value {
        set {
            if (value is LPFN_CONNECTEX) {
     ...
     * @param {Integer} lpSendBuffer A pointer to the buffer to be transferred after a connection is established. This parameter is optional. If the TCP_FASTOPEN option is enabled  on <i>s</i> before <b>ConnectEx</b> is called, then some of this data may be sent during connection establishment.
     * @param {Integer} dwSendDataLength The length, in bytes, of data pointed to by the <i>lpSendBuffer</i> parameter. This parameter is ignored when the <i>lpSendBuffer</i> parameter is <b>NULL</b>.
     ...
     * @returns {BOOL} On success,  the <b>ConnectEx</b> function returns <b>TRUE</b>. On failure, the function returns <b>FALSE</b>. Use the 
    ...
    /**
     * A LPFN_CONNECTEX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPFN_CONNECTEX {
        /**
         * Creates a LPFN_CONNECTEX pointer that invokes the given AHK function when called.
         * @param {Func(SOCKET, IntPtr, Int32, IntPtr, UInt32, "uint*", OVERLAPPED) => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 7)) {
