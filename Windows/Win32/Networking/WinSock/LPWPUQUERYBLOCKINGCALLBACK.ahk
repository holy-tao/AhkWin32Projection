 ...
 * @version v4.0.30319
 */
class LPWPUQUERYBLOCKINGCALLBACK extends IUnknown {
     ...
     */
    Invoke(dwCatalogEntryId, lplpfnCallback, lpdwContext, lpErrno) {
        lplpfnCallbackMarshal := lplpfnCallback is VarRef ? "ptr*" : "ptr"
        lpdwContextMarshal := lpdwContext is VarRef ? "ptr*" : "ptr"
        lpErrnoMarshal := lpErrno is VarRef ? "int*" : "ptr"

        result := ComCall(3, this, "uint", dwCatalogEntryId, lplpfnCallbackMarshal, lplpfnCallback, lpdwContextMarshal, lpdwContext, lpErrnoMarshal, lpErrno, "int")
        return result
    }
}
