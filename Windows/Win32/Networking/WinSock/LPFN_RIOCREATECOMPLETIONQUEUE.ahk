...
#Import ".\RIO_NOTIFICATION_COMPLETION.ahk" { RIO_NOTIFICATION_COMPLETION }
 ...
 * @see https://learn.microsoft.com/windows/win32/api/mswsock/nc-mswsock-lpfn_riocreatecompletionqueue
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPFN_RIOCREATECOMPLETIONQUEUE {
    value : IntPtr

    __value {
        set {
            if (value is LPFN_RIOCREATECOMPLETIONQUEUE) {
     ...
     * @returns {RIO_CQ} If no error occurs, the **RIOCreateCompletionQueue** function returns a descriptor referencing a new completion queue. Otherwise, a value of **RIO\_INVALID\_CQ** is returned, and a specific error code can be retrieved by calling the [**WSAGetLastError**](../winsock/nf-winsock-wsagetlasterror.md) function.
     * 
    ...
    /**
     * A LPFN_RIOCREATECOMPLETIONQUEUE that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPFN_RIOCREATECOMPLETIONQUEUE {
        /**
         * Creates a LPFN_RIOCREATECOMPLETIONQUEUE pointer that invokes the given AHK function when called.
         * @param {Func(UInt32, RIO_NOTIFICATION_COMPLETION) => RIO_CQ} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 2)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 2 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [UInt32, RIO_NOTIFICATION_COMPLETION.Ptr, RIO_CQ])
        }
