 ...
 * @see https://learn.microsoft.com/windows/win32/api/mswsock/nc-mswsock-lpfn_rioreceiveex
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPFN_RIORECEIVEEX {
    value : IntPtr

    __value {
        set {
            if (value is LPFN_RIORECEIVEEX) {
     ...
     * @param {Integer} DataBufferCount A data buffer count parameter that indicates if data is to be received in the buffer pointed to by the *pData* parameter.
     * 
     * This parameter should be set to zero if the *pData* is NULL. Otherwise, this parameter should be set to 1.
     * @param {Pointer<RIO_BUF>} pLocalAddress A buffer segment that on completion will hold the local address on which the network data was received.
     * 
     ...
     * @param {Pointer<RIO_BUF>} pRemoteAddress A buffer segment that on completion will hold the remote address from which the network data was received.
     * 
     ...
     * @param {Pointer<RIO_BUF>} pControlContext A buffer slice that on completion will hold additional control information about the receive operation.
     * 
     * This parameter may be **NULL** if the application does not want to receive the additional control information.
     * @param {Pointer<RIO_BUF>} pFlags This parameter may be **NULL** if the application does not want to receive the additional flags information.
     * @param {Integer} Flags A set of flags that modify the behavior of the **RIOReceiveEx** function.
     * 
     ...
     * @param {Pointer<Void>} RequestContext The request context to associate with this receive operation.
     * @returns {Integer} If no error occurs, the **RIOReceiveEx** function returns **TRUE**. In this case, the receive operation is successfully initiated and the completion will have already been queued or the operation has been successfully initiated and the completion will be queued at a later time.
     * 
    ...
    /**
     * A LPFN_RIORECEIVEEX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPFN_RIORECEIVEEX {
        /**
         * Creates a LPFN_RIORECEIVEEX pointer that invokes the given AHK function when called.
         * @param {Func(RIO_RQ, RIO_BUF, UInt32, RIO_BUF, RIO_BUF, RIO_BUF, RIO_BUF, UInt32, "ptr") => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 9)) {
