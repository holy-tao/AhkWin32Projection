 ...
 * @see https://learn.microsoft.com/windows/win32/api/ws2spi/nc-ws2spi-lpwsprecv
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPWSPRECV {
    value : IntPtr

    __value {
        set {
            if (value is LPWSPRECV) {
     ...
     * @param {Pointer<Integer>} lpNumberOfBytesRecvd A pointer to the number of bytes received by this call.
     * @param {Pointer<Integer>} lpFlags A pointer to flags that specify the way in which the call is made.
     * @param {Pointer<OVERLAPPED>} lpOverlapped A pointer to a <b><a href="https://docs.microsoft.com/windows/win32/api/winsock2/ns-winsock2-wsaoverlapped">WSAOverlapped</a></b> structure (ignored for nonoverlapped structures).
     ...
     * @returns {Integer} If no error occurs and the receive operation has completed immediately, **LPWSPRecv** returns zero. Note that in this case the completion routine, if specified, will have already been queued. Otherwise, a value of SOCKET_ERROR is returned, and a specific error code is available in <i>lpErrno</i>. The error code WSA_IO_PENDING indicates that the overlapped operation has been successfully initiated and that completion will be indicated at a later time. Any other error code indicates that no overlapped operations was initiated and no completion indication will occur.
     * 
     * <table>
     * <tr>
     * <th>Error Code</th>
    ...
    Call(s, lpBuffers, dwBufferCount, lpNumberOfBytesRecvd, lpFlags, lpOverlapped, lpCompletionRoutine, lpThreadId, lpErrno) {
        ...
        result := DllCall(this.value, SOCKET, s, WSABUF.Ptr, lpBuffers, UInt32, dwBufferCount, lpNumberOfBytesRecvdMarshal, lpNumberOfBytesRecvd, lpFlagsMarshal, lpFlags, OVERLAPPED.Ptr, lpOverlapped, "ptr", lpCompletionRoutine, WSATHREADID.Ptr, lpThreadId, lpErrnoMarshal, lpErrno, Int32)
        return result
    }

    /**
     * A LPWSPRECV that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPWSPRECV {
        /**
         * Creates a LPWSPRECV pointer that invokes the given AHK function when called.
         * @param {Func(SOCKET, WSABUF, UInt32, "uint*", "uint*", OVERLAPPED, "ptr", WSATHREADID, "int*") => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 9)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 9 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [SOCKET, WSABUF.Ptr, UInt32, "uint*", "uint*", OVERLAPPED.Ptr, "ptr", WSATHREADID.Ptr, "int*", Int32])
        }
