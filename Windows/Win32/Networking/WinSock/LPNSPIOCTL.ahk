 ...
 * <div> </div>
 * @see https://learn.microsoft.com/windows/win32/api/ws2spi/nc-ws2spi-lpnspioctl
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPNSPIOCTL {
    value : IntPtr

    __value {
        set {
            if (value is LPNSPIOCTL) {
     ...
     * @param {Pointer<WSACOMPLETION>} lpCompletion A pointer to a <a href="https://docs.microsoft.com/windows/desktop/api/winsock2/ns-winsock2-wsacompletion">WSACOMPLETION</a> structure, used for asynchronous processing. Set <i>lpCompletion</i> to **NULL** to force blocking (synchronous) execution.
     * @param {Pointer<WSATHREADID>} lpThreadId A pointer to a 
     ...
     * @returns {Integer} If no error occurs and the operation has completed immediately, 
    ...
    /**
     * A LPNSPIOCTL that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPNSPIOCTL {
        /**
         * Creates a LPNSPIOCTL pointer that invokes the given AHK function when called.
         * @param {Func(HANDLE, UInt32, IntPtr, UInt32, IntPtr, UInt32, "uint*", WSACOMPLETION, WSATHREADID) => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 9)) {
