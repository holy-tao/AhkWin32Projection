 ...
 * </tr>
 * </table>
 * @see https://learn.microsoft.com/windows/win32/api/ws2spi/nc-ws2spi-lpnsplookupservicenext
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPNSPLOOKUPSERVICENEXT {
    value : IntPtr

    __value {
        set {
            if (value is LPNSPLOOKUPSERVICENEXT) {
     ...
     * @param {Integer} lpqsResults A pointer to a memory block that will contain, on return, one result set in a 
    ...
    /**
     * A LPNSPLOOKUPSERVICENEXT that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPNSPLOOKUPSERVICENEXT {
        /**
         * Creates a LPNSPLOOKUPSERVICENEXT pointer that invokes the given AHK function when called.
         * @param {Func(HANDLE, UInt32, "uint*", IntPtr) => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
