 ...
 * @see https://learn.microsoft.com/windows/win32/api/mswsock/nc-mswsock-lpfn_riosendex
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPFN_RIOSENDEX {
    value : IntPtr

    __value {
        set {
            if (value is LPFN_RIOSENDEX) {
     ...
     * @param {Pointer<RIO_BUF>} pLocalAddress This parameter is reserved and must be **NULL**.
     * @param {Pointer<RIO_BUF>} pRemoteAddress A buffer segment from a registered buffer that on input holds the remote address to which the network data is to be sent.
     * 
     * This parameter may be **NULL** if the socket is connected.
     * @param {Pointer<RIO_BUF>} pControlContext A buffer slice that on completion will hold additional control information about the send operation.
     * 
     * This parameter may be **NULL** if the application does not want to receive the additional control information.
     * @param {Pointer<RIO_BUF>} pFlags A buffer slice that on completion will hold additional information about the set of flags for the send operation.
     * 
     * This parameter may be **NULL** if the application does not want to receive the additional flags information.
     * @param {Integer} Flags A set of flags that modify the behavior of the **RIOSendEx** function.
     * 
     ...
     * @returns {BOOL} If no error occurs, the **RIOSendEx** function returns **TRUE**. In this case, the send operation is successfully initiated and the completion will have already been queued or the operation has been successfully initiated and the completion will be queued at a later time.
     * 
    ...
    /**
     * A LPFN_RIOSENDEX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPFN_RIOSENDEX {
        /**
         * Creates a LPFN_RIOSENDEX pointer that invokes the given AHK function when called.
         * @param {Func(RIO_RQ, RIO_BUF, UInt32, RIO_BUF, RIO_BUF, RIO_BUF, RIO_BUF, UInt32, "ptr") => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 9)) {
