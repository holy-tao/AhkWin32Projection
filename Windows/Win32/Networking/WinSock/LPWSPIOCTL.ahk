 ...
 * </dt> <dd>
 * 
 * When issued, this IOCTL requests that the route to the remote address specified as a <b><a href="https://docs.microsoft.com/windows/win32/winsock/sockaddr-2">sockaddr</a></b> in the input buffer be discovered. If the address already exists in the local cache, its entry is invalidated. In the case of Novell's IPX, this call initiates an IPX GetLocalTarget (GLT), that queries the network for the given remote address.
 * 
 * </dd> <dt>
 * 
 ...
 * </dt> <dd>
 * 
 * This IOCTL fills the output buffer with a <b><a href="https://docs.microsoft.com/windows/win32/winsock/sockaddr-2">sockaddr</a></b> structure containing a suitable broadcast address for use with <b><a href="https://docs.microsoft.com/windows/win32/api/ws2spi/nc-ws2spi-lpwspsendto">LPWSPSendTo</a></b>.
 * 
 * </dd> <dt>
 * 
 ...
 * </dt> <dd>
 * 
 * To obtain the address of the local interface (represented as <b><a href="https://docs.microsoft.com/windows/win32/winsock/sockaddr-2">sockaddr</a></b> structure) that should be used to send to the remote address specified in the input buffer (as **sockaddr**). Remote multicast addresses may be submitted in the input buffer to get the address of the preferred interface for multicast transmission. In any case, the interface address returned may be used by the application in a subsequent <b><a href="https://docs.microsoft.com/windows/win32/api/winsock/nf-winsock-bind">Bind</a></b> request.
 * 
 ...
 * 
 * <b><a href="https://docs.microsoft.com/windows/win32/api/ws2spi/nf-ws2spi-wpuqueueapc">WPUQueueApc</a></b> takes as input parameters a pointer to a <b><a href="https://docs.microsoft.com/windows/win32/api/ws2spi/ns-ws2spi-wsathreadid">WSATHREADID</a></b> structure (supplied to the provider through the <i>lpThreadId</i> input parameter), a pointer to an APC function to be invoked, and a 32-bit context value that is subsequently passed to the APC function. Because only a single 32-bit context value is available, the APC function itself cannot be the client specified–completion routine. The service provider must instead supply a pointer to its own APC function that uses the supplied context value to access the needed result information for the overlapped operation, and then invokes the client specified–completion routine.
 * 
 ...
 * @see https://learn.microsoft.com/windows/win32/api/ws2spi/nc-ws2spi-lpwspioctl
 * @namespace Windows.Win32.Networking.WinSock
 */
export default struct LPWSPIOCTL {
    value : IntPtr

    __value {
        set {
            if (value is LPWSPIOCTL) {
     ...
     * @param {Integer} lpvOutBuffer A pointer to the output buffer.
     ...
     * @param {Pointer<OVERLAPPED>} lpOverlapped A pointer to a <b><a href="https://docs.microsoft.com/windows/win32/api/winsock2/ns-winsock2-wsaoverlapped">WSAOverlapped</a></b> structure (ignored for non-overlapped sockets).
     ...
     * @param {Pointer<WSATHREADID>} lpThreadId A pointer to a <b><a href="https://docs.microsoft.com/windows/win32/api/ws2spi/ns-ws2spi-wsathreadid">WSATHREADID</a></b> structure to be used by the provider in a subsequent call to <b><a href="https://docs.microsoft.com/windows/win32/api/ws2spi/nf-ws2spi-wpuqueueapc">WPUQueueApc</a></b>. The provider should store the referenced **WSATHREADID** structure (not the pointer) until after the **WPUQueueApc** function returns.
     * @param {Pointer<Integer>} lpErrno A pointer to the error code.
     * @returns {Integer} If no error occurs and the operation has completed immediately, **LPWSPIoctl** returns zero. Note that in this case the completion routine, if specified, will have already been queued. Otherwise, a value of SOCKET_ERROR is returned, and a specific error code is available in <i>lpErrno</i>. The error code WSA_IO_PENDING indicates that an overlapped operation has been successfully initiated and that completion will be indicated at a later time. Any other error code indicates that no overlapped operation was initiated and no completion indication will occur.
     * 
     * | Error code                                                                                                                                          | Meaning                                                                                                                                                                                                        |
     * |-----------------------------------------------------------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
    ...
    Call(s, dwIoControlCode, lpvInBuffer, cbInBuffer, lpvOutBuffer, cbOutBuffer, lpcbBytesReturned, lpOverlapped, lpCompletionRoutine, lpThreadId, lpErrno) {
        ...
        result := DllCall(this.value, SOCKET, s, UInt32, dwIoControlCode, IntPtr, lpvInBuffer, UInt32, cbInBuffer, IntPtr, lpvOutBuffer, UInt32, cbOutBuffer, lpcbBytesReturnedMarshal, lpcbBytesReturned, OVERLAPPED.Ptr, lpOverlapped, "ptr", lpCompletionRoutine, WSATHREADID.Ptr, lpThreadId, lpErrnoMarshal, lpErrno, Int32)
        return result
    }

    /**
     * A LPWSPIOCTL that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPWSPIOCTL {
        /**
         * Creates a LPWSPIOCTL pointer that invokes the given AHK function when called.
         * @param {Func(SOCKET, UInt32, IntPtr, UInt32, IntPtr, UInt32, "uint*", OVERLAPPED, "ptr", WSATHREADID, "int*") => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 11)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 11 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [SOCKET, UInt32, IntPtr, UInt32, IntPtr, UInt32, "uint*", OVERLAPPED.Ptr, "ptr", WSATHREADID.Ptr, "int*", Int32])
        }
