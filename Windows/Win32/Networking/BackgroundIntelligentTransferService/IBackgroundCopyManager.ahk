...
class IBackgroundCopyManager extends IUnknown{
     ...
     */
    GetJob(jobID, ppJob) {
        result := ComCall(4, this, "ptr", jobID, "ptr", ppJob, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumJobs(dwFlags, ppEnum) {
        result := ComCall(5, this, "uint", dwFlags, "ptr", ppEnum, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetErrorDescription(hResult, LanguageId, pErrorDescription) {
        result := ComCall(6, this, "int", hResult, "uint", LanguageId, "ptr", pErrorDescription, "int")
        if(result != 0)
            throw OSError(result)
