...
class IRdcFileReader extends IUnknown{
    ...
    GetFileSize(fileSize) {
        result := ComCall(3, this, "uint*", fileSize, "int")
        if(result != 0)
     ...
     * @param {Pointer<UInt32>} bytesActuallyRead 
     * @param {Pointer<Byte>} buffer 
     * @param {Pointer<BOOL>} eof 
     * @returns {HRESULT} 
     */
    Read(offsetFileStart, bytesToRead, bytesActuallyRead, buffer, eof) {
        result := ComCall(4, this, "uint", offsetFileStart, "uint", bytesToRead, "uint*", bytesActuallyRead, "char*", buffer, "ptr", eof, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt64>} offsetFromStart 
     * @returns {HRESULT} 
     */
    GetFilePosition(offsetFromStart) {
        result := ComCall(5, this, "uint*", offsetFromStart, "int")
        if(result != 0)
            throw OSError(result)
