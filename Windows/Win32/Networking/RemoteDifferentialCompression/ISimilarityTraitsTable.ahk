 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nn-msrdc-isimilaritytraitstable
...
class ISimilarityTraitsTable extends IUnknown{
     ...
     * @param {PWSTR} path_ A pointer to a null-terminated string that specifies the name of the file that will contain the similarity traits table. The alternate stream name ":Traits" will be appended to the end of this file name. For more information, see <a href="https://docs.microsoft.com/windows/desktop/FileIO/naming-a-file">Naming a File</a>.
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-createtable
     */
    CreateTable(path_, truncate, securityDescriptor_) {
        path_ := path_ is String ? StrPtr(path_) : path_

        securityDescriptor_Marshal := securityDescriptor_ is VarRef ? "char*" : "ptr"

        result := ComCall(3, this, "ptr", path_, "int", truncate, securityDescriptor_Marshal, securityDescriptor_, "int*", &isNew := 0, "int")
        if(result != 0) {
     ...
     * @param {BOOL} truncate <b>TRUE</b> if a new similarity traits table should always be created or truncated. If <b>FALSE</b> is specified and the table exists and is valid, it may be used; otherwise, if the table is not valid or does not exist, the existing table is overwritten.
     * @returns {Integer} A pointer to a variable that receives an  <a href="https://docs.microsoft.com/windows/win32/api/msrdc/ne-msrdc-rdccreatedtables">RdcCreatedTables</a> enumeration value that describes the state of the similarity traits table. If a new table is created, this variable receives <b>RDCTABLE_New</b>. If an existing table is used, this variable receives <b>RDCTABLE_Existing</b>. If this method fails, this variable receives <b>RDCTABLE_InvalidOrUnknown</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-createtableindirect
     */
    CreateTableIndirect(mapping, truncate) {
        result := ComCall(4, this, "ptr", mapping, "int", truncate, "int*", &isNew := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-closetable
     */
    CloseTable(isValid) {
     ...
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-append
     */
     ...
     * @param {Pointer<SimilarityData>} similarityData_ A pointer to a <a href="https://docs.microsoft.com/windows/win32/api/msrdc/ns-msrdc-similaritydata">SimilarityData</a> structure that contains similarity information for the file.
     * @param {Integer} numberOfMatchesRequired TBD
     * @param {Pointer<FindSimilarFileIndexResults>} findSimilarFileIndexResults_ A pointer to a buffer that receives an array of <a href="https://docs.microsoft.com/windows/win32/api/msrdc/ns-msrdc-findsimilarfileindexresults">FindSimilarFileIndexResults</a> structures that contain the requested information.
     ...
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-findsimilarfileindex
     */
    FindSimilarFileIndex(similarityData_, numberOfMatchesRequired, findSimilarFileIndexResults_, resultsSize, resultsUsed) {
        resultsUsedMarshal := resultsUsed is VarRef ? "uint*" : "ptr"

        result := ComCall(7, this, "ptr", similarityData_, "ushort", numberOfMatchesRequired, "ptr", findSimilarFileIndexResults_, "uint", resultsSize, resultsUsedMarshal, resultsUsed, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-begindump
     */
    BeginDump() {
        result := ComCall(8, this, "ptr*", &similarityTableDumpState_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return ISimilarityTableDumpState(similarityTableDumpState_)
    }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/msrdc/nf-msrdc-isimilaritytraitstable-getlastindex
     */
    GetLastIndex() {
        result := ComCall(9, this, "uint*", &fileIndex := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
