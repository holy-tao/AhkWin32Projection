...
class IRdcGenerator extends IUnknown{
    ...
    GetGeneratorParameters(level, iGeneratorParameters) {
        result := ComCall(3, this, "uint", level, "ptr", iGeneratorParameters, "int")
        if(result != 0)
     ...
     * @param {Integer} depth 
     * @param {Pointer<RdcBufferPointer>} outputBuffers 
    ...
    Process(endOfInput, endOfOutput, inputBuffer, depth, outputBuffers, rdc_ErrorCode) {
        result := ComCall(4, this, "int", endOfInput, "ptr", endOfOutput, "ptr", inputBuffer, "uint", depth, "ptr", outputBuffers, "int*", rdc_ErrorCode, "int")
        if(result != 0)
            throw OSError(result)
