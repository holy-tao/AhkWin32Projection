 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nn-dsadmin-idsadminnewobjext
...
class IDsAdminNewObjExt extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-initialize
     */
     ...
     * @param {LPARAM} lParam_ Contains data that is private to the administrative snap-in. This value is passed as the second parameter to <i>lpfnAddPage</i>.
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-addpages
     */
    AddPages(lpfnAddPage, lParam_) {
        lParam_ := lParam_ is Win32Handle ? NumGet(lParam_, "ptr") : lParam_

        result := ComCall(4, this, "ptr", lpfnAddPage, "ptr", lParam_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-setobject
     */
    SetObject(pADsObj) {
     ...
     * @param {HWND} hWnd_ The window handle used as the parent window for possible error messages.
     * @param {Integer} uContext 
     * @returns {HRESULT} Returns <b>S_OK</b> if successful or an OLE-defined error code otherwise.
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-writedata
     */
    WriteData(hWnd_, uContext) {
        hWnd_ := hWnd_ is Win32Handle ? NumGet(hWnd_, "ptr") : hWnd_

        result := ComCall(6, this, "ptr", hWnd_, "uint", uContext, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-onerror
     */
    OnError(hWnd_, hr, uContext) {
        hWnd_ := hWnd_ is Win32Handle ? NumGet(hWnd_, "ptr") : hWnd_

        result := ComCall(7, this, "ptr", hWnd_, "int", hr, "uint", uContext, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dsadmin/nf-dsadmin-idsadminnewobjext-getsummaryinfo
     */
