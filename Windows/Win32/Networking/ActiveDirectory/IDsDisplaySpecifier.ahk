...
class IDsDisplaySpecifier extends IUnknown{
    ...
    GetDisplaySpecifier(pszObjectClass, riid, ppv) {
        ...
        result := ComCall(5, this, "ptr", pszObjectClass, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetIconLocation(pszObjectClass, dwFlags, pszBuffer, cchBuffer, presid) {
        ...
        result := ComCall(6, this, "ptr", pszObjectClass, "uint", dwFlags, "ptr", pszBuffer, "int", cchBuffer, "int*", presid, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * 
     * @param {PWSTR} pszObjectClass 
     * @param {Pointer<DSCLASSCREATIONINFO>} ppdscci 
     * @returns {HRESULT} 
     */
    GetClassCreationInfo(pszObjectClass, ppdscci) {
        pszObjectClass := pszObjectClass is String ? StrPtr(pszObjectClass) : pszObjectClass

        result := ComCall(11, this, "ptr", pszObjectClass, "ptr", ppdscci, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumClassAttributes(pszObjectClass, pcbEnum, lParam) {
        ...
        result := ComCall(12, this, "ptr", pszObjectClass, "ptr", pcbEnum, "ptr", lParam, "int")
        if(result != 0)
            throw OSError(result)
