...
/**
 * Used with the DsGetDcName function to receive data about a domain controller.
 * @remarks
 * 
  * > [!NOTE]
  ...
  * 
 * @see https://docs.microsoft.com/windows/win32/api//dsgetdc/ns-dsgetdc-domain_controller_infow
...
class DOMAIN_CONTROLLER_INFOW extends Win32Struct
{
    ...
    DomainControllerName{
        get {
            if(!this.HasProp("__DomainControllerName"))
                this.__DomainControllerName := PWSTR(this.ptr + 0)
            return this.__DomainControllerName
        }
    }
    ...
    DomainControllerAddress{
        get {
            if(!this.HasProp("__DomainControllerAddress"))
                this.__DomainControllerAddress := PWSTR(this.ptr + 8)
            return this.__DomainControllerAddress
        }
    }
    ...
    DomainName{
        get {
            if(!this.HasProp("__DomainName"))
                this.__DomainName := PWSTR(this.ptr + 32)
    ...
    DnsForestName{
        get {
            if(!this.HasProp("__DnsForestName"))
                this.__DnsForestName := PWSTR(this.ptr + 40)
    ...
    DcSiteName{
        get {
            if(!this.HasProp("__DcSiteName"))
                this.__DcSiteName := PWSTR(this.ptr + 56)
            return this.__DcSiteName
        }
    }
    ...
    ClientSiteName{
        get {
            if(!this.HasProp("__ClientSiteName"))
                this.__ClientSiteName := PWSTR(this.ptr + 64)
            return this.__ClientSiteName
        }
    }
}
