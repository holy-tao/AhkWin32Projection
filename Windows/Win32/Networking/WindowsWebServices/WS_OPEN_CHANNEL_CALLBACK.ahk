 ...
 *                 of this API.
 * @see https://learn.microsoft.com/windows/win32/api/webservices/nc-webservices-ws_open_channel_callback
 * @namespace Windows.Win32.Networking.WindowsWebServices
 */
export default struct WS_OPEN_CHANNEL_CALLBACK {
    value : IntPtr

    __value {
        set {
            if (value is WS_OPEN_CHANNEL_CALLBACK) {
     ...
     * </table>
     */
    Call(channelInstance, _endpointAddress, asyncContext, _error) {
        channelInstanceMarshal := channelInstance is VarRef ? "ptr" : "ptr"
        _errorMarshal := _error is VarRef ? "ptr*" : "ptr"

        result := DllCall(this.value, channelInstanceMarshal, channelInstance, WS_ENDPOINT_ADDRESS.Ptr, _endpointAddress, WS_ASYNC_CONTEXT.Ptr, asyncContext, _errorMarshal, _error, "HRESULT")
        return result
    }

    /**
     * A WS_OPEN_CHANNEL_CALLBACK that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends WS_OPEN_CHANNEL_CALLBACK {
        /**
         * Creates a WS_OPEN_CHANNEL_CALLBACK pointer that invokes the given AHK function when called.
         * @param {Func("ptr", WS_ENDPOINT_ADDRESS, WS_ASYNC_CONTEXT, WS_ERROR) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr", WS_ENDPOINT_ADDRESS.Ptr, WS_ASYNC_CONTEXT.Ptr, WS_ERROR.Ptr, "int"])
        }
