 ...
 *                 of this API.
 * @see https://learn.microsoft.com/windows/win32/api/webservices/nc-webservices-ws_accept_channel_callback
 * @namespace Windows.Win32.Networking.WindowsWebServices
 */
export default struct WS_ACCEPT_CHANNEL_CALLBACK {
    value : IntPtr

    __value {
        set {
            if (value is WS_ACCEPT_CHANNEL_CALLBACK) {
     ...
     * @param {Pointer<WS_ASYNC_CONTEXT>} asyncContext Information on how to invoke the function asynchronously, or NULL if invoking synchronously.
    ...
    /**
     * A WS_ACCEPT_CHANNEL_CALLBACK that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends WS_ACCEPT_CHANNEL_CALLBACK {
        /**
         * Creates a WS_ACCEPT_CHANNEL_CALLBACK pointer that invokes the given AHK function when called.
         * @param {Func("ptr", "ptr", WS_ASYNC_CONTEXT, WS_ERROR) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
