...
class IMLangString extends IUnknown{
     ...
     * @returns {HRESULT} 
     * @see https://learn.microsoft.com/windows/win32/ktop-src/WMP/sync-attributes
     */
    Sync(fNoAccess) {
     ...
     */
    GetLength() {
        result := ComCall(4, this, "int*", &plLen := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetMLStr(lSrcPos, lSrcLen, pUnkOuter, dwClsContext, piid, ppDestMLStr, plDestPos, plDestLen) {
        ...
        result := ComCall(6, this, "int", lSrcPos, "int", lSrcLen, "ptr", pUnkOuter, "uint", dwClsContext, "ptr", piid, "ptr*", ppDestMLStr, plDestPosMarshal, plDestPos, plDestLenMarshal, plDestLen, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
