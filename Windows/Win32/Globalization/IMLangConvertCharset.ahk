...
class IMLangConvertCharset extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<UInt32>} puiSrcCodePage 
     * @returns {HRESULT} 
     */
    GetSourceCodePage(puiSrcCodePage) {
        result := ComCall(4, this, "uint*", puiSrcCodePage, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} puiDstCodePage 
     * @returns {HRESULT} 
     */
    GetDestinationCodePage(puiDstCodePage) {
        result := ComCall(5, this, "uint*", puiDstCodePage, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetProperty(pdwProperty) {
        result := ComCall(6, this, "uint*", pdwProperty, "int")
        if(result != 0)
            throw OSError(result)
    ...
    DoConversion(pSrcStr, pcSrcSize, pDstStr, pcDstSize) {
        result := ComCall(7, this, "ptr", pSrcStr, "uint*", pcSrcSize, "ptr", pDstStr, "uint*", pcDstSize, "int")
        if(result != 0)
            throw OSError(result)
    ...
    DoConversionToUnicode(pSrcStr, pcSrcSize, pDstStr, pcDstSize) {
        ...
        result := ComCall(8, this, "ptr", pSrcStr, "uint*", pcSrcSize, "ptr", pDstStr, "uint*", pcDstSize, "int")
        if(result != 0)
            throw OSError(result)
    ...
    DoConversionFromUnicode(pSrcStr, pcSrcSize, pDstStr, pcDstSize) {
        ...
        result := ComCall(9, this, "ptr", pSrcStr, "uint*", pcSrcSize, "ptr", pDstStr, "uint*", pcDstSize, "int")
        if(result != 0)
            throw OSError(result)
