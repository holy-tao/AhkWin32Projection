 ...
 */
export default struct IUserDictionariesRegistrar extends IUnknown {
    /**
     ...
     * used for IUserDictionariesRegistrar interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        RegisterUserDictionary   : IntPtr
        UnregisterUserDictionary : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IUserDictionariesRegistrar.Vtbl()
        }
    ...
    Query(iid) {
        if (IUserDictionariesRegistrar.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.RegisterUserDictionary)
        CallbackFree(this.vtbl.UnregisterUserDictionary)
    }
}
