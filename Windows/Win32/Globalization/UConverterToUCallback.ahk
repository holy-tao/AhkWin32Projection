 ...
 * @version v4.0.30319
 */
class UConverterToUCallback extends IUnknown {
     ...
     */
    Invoke(context_, args, codeUnits, length, reason, pErrorCode) {
        codeUnits := codeUnits is String ? StrPtr(codeUnits) : codeUnits

        context_Marshal := context_ is VarRef ? "ptr" : "ptr"
        pErrorCodeMarshal := pErrorCode is VarRef ? "int*" : "ptr"

        ComCall(3, this, context_Marshal, context_, "ptr", args, "ptr", codeUnits, "int", length, "int", reason, pErrorCodeMarshal, pErrorCode)
    }
}
