 ...
 * @version v4.0.30319
 */
class PFN_WTD_GENERIC_CHAIN_POLICY_CALLBACK extends IUnknown {
     ...
     */
    Invoke(pProvData, dwStepError, dwRegPolicySettings, cSigner, rgpSigner, pvPolicyArg) {
        rgpSignerMarshal := rgpSigner is VarRef ? "ptr*" : "ptr"
        pvPolicyArgMarshal := pvPolicyArg is VarRef ? "ptr" : "ptr"

        result := ComCall(3, this, "ptr", pProvData, "uint", dwStepError, "uint", dwRegPolicySettings, "uint", cSigner, rgpSignerMarshal, rgpSigner, pvPolicyArgMarshal, pvPolicyArg, "int")
        if(result != 0) {
