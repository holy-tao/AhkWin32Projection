...
class ISceSvcAttachmentData extends IUnknown{
     ...
     * @param {Pointer<UInt32>} psceEnumHandle 
     * @returns {HRESULT} 
     */
    GetData(scesvcHandle, sceType, ppvData, psceEnumHandle) {
        result := ComCall(3, this, "ptr", scesvcHandle, "int", sceType, "ptr", ppvData, "uint*", psceEnumHandle, "int")
        if(result != 0)
     ...
     * @param {Pointer<SByte>} lpServiceName 
     * @param {Pointer<SByte>} lpTemplateName 
     * @param {Pointer<ISceSvcAttachmentPersistInfo>} lpSceSvcPersistInfo 
     * @param {Pointer<Void>} pscesvcHandle 
     * @returns {HRESULT} <ul>
    ...
    Initialize(lpServiceName, lpTemplateName, lpSceSvcPersistInfo, pscesvcHandle) {
        result := ComCall(4, this, "char*", lpServiceName, "char*", lpTemplateName, "ptr", lpSceSvcPersistInfo, "ptr", pscesvcHandle, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CloseHandle(scesvcHandle) {
        result := ComCall(6, this, "ptr", scesvcHandle, "int")
        if(result != 0)
            throw OSError(result)
