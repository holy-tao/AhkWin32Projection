 ...
 * @see https://learn.microsoft.com/windows/win32/api/ntsecpkg/nc-ntsecpkg-spinitlsamodecontextfn
 * @namespace Windows.Win32.Security.Authentication.Identity
 */
export default struct SpInitLsaModeContextFn {
    value : IntPtr

    __value {
        set {
            if (value is SpInitLsaModeContextFn) {
     ...
     * @param {Integer} ContextRequirements Flags indicating the context attributes required by the client. The actual context attributes are returned in the <i>ContextAttributes</i> parameter. 
     * 
     * 
     * 
     * 
     ...
     * </tr>
     * </table>
     * @param {Integer} TargetDataRep Flag indicating the data representation, such as byte ordering, on the target. Contains SECURITY_NATIVE_DREP or SECURITY_NETWORK_DREP.
     * @param {Pointer<SecBufferDesc>} InputBuffers Pointer to a 
     ...
     * @param {Pointer<Integer>} ContextAttributes Pointer to flags specifying the attributes of the new context. The client requests a set of attributes using the <i>ContextRequirements</i> parameter. If the <i>ContextRequirements</i> flags do not match the <i>ContextAttributes</i> flags, the client must decide whether to continue or terminate. For a complete list of the valid flags, see 
     ...
     * @param {Pointer<SecBuffer>} ContextData Pointer to a 
     ...
     * @returns {NTSTATUS} If the function succeeds and no more processing is required, return STATUS_SUCCESS. If processing is not complete, the function should return SEC_I_CONTINUE_NEEDED. When this value is returned, the caller must call the 
    ...
    /**
     * A SpInitLsaModeContextFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SpInitLsaModeContextFn {
        /**
         * Creates a SpInitLsaModeContextFn pointer that invokes the given AHK function when called.
         * @param {Func(IntPtr, IntPtr, LSA_UNICODE_STRING, UInt32, UInt32, SecBufferDesc, "ptr*", SecBufferDesc, "uint*", "int64*", BOOLEAN, SecBuffer) => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 12)) {
