...
export default struct PLSA_CREATE_THREAD {
    ...
    Call(SecurityAttributes, StackSize, StartFunction, ThreadParameter, CreationFlags, ThreadId) {
        ...
        result := DllCall(this.value, SECURITY_ATTRIBUTES.Ptr, SecurityAttributes, UInt32, StackSize, "ptr", StartFunction, ThreadParameterMarshal, ThreadParameter, UInt32, CreationFlags, ThreadIdMarshal, ThreadId, HANDLE.Owned)
        return result
    }
    ...
    struct From extends PLSA_CREATE_THREAD {
         ...
         * @param {Func(SECURITY_ATTRIBUTES, UInt32, "ptr", "ptr", UInt32, "uint*") => HANDLE} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [SECURITY_ATTRIBUTES.Ptr, UInt32, "ptr", "ptr", UInt32, "uint*", HANDLE])
        }
