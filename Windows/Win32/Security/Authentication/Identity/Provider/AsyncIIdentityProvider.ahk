...
class AsyncIIdentityProvider extends IUnknown{
    ...
    Finish_GetIdentityEnum(ppIdentityEnum) {
        result := ComCall(4, this, "ptr", ppIdentityEnum, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Finish_Create(ppPropertyStore) {
        result := ComCall(6, this, "ptr", ppPropertyStore, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Finish_FindByUniqueID(ppPropertyStore) {
        result := ComCall(12, this, "ptr", ppPropertyStore, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Finish_GetProviderPropertyStore(ppPropertyStore) {
        result := ComCall(14, this, "ptr", ppPropertyStore, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Finish_Advise(pdwCookie) {
        result := ComCall(16, this, "uint*", pdwCookie, "int")
        if(result != 0)
            throw OSError(result)
