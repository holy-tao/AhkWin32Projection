 ...
 * @charset ANSI
 */
export default struct INITIALIZE_SECURITY_CONTEXT_FN_A {
    value : IntPtr

    __value {
        set {
            if (value is INITIALIZE_SECURITY_CONTEXT_FN_A) {
    ...
    /**
     * A INITIALIZE_SECURITY_CONTEXT_FN_A that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends INITIALIZE_SECURITY_CONTEXT_FN_A {
        /**
         * Creates a INITIALIZE_SECURITY_CONTEXT_FN_A pointer that invokes the given AHK function when called.
         * @param {Func(SecHandle, SecHandle, "char*", UInt32, UInt32, UInt32, SecBufferDesc, UInt32, SecHandle, SecBufferDesc, "uint*", "int64*") => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 12)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 12 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [SecHandle.Ptr, SecHandle.Ptr, "char*", UInt32, UInt32, UInt32, SecBufferDesc.Ptr, UInt32, SecHandle.Ptr, SecBufferDesc.Ptr, "uint*", "int64*", "int"])
        }
