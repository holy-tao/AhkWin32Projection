 ...
 * @see https://learn.microsoft.com/windows/win32/api/ntsecpkg/nc-ntsecpkg-spquerymetadatafn
 * @namespace Windows.Win32.Security.Authentication.Identity
 */
export default struct SpQueryMetaDataFn {
    value : IntPtr

    __value {
        set {
            if (value is SpQueryMetaDataFn) {
     ...
     * </tr>
     * </table>
     * @param {Pointer<Integer>} MetaDataLength The size, in characters, of the <i>MetaData</i> buffer.
     * @param {Pointer<Pointer<Integer>>} MetaData The metadata that the SSP provides.
    ...
    /**
     * A SpQueryMetaDataFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SpQueryMetaDataFn {
        /**
         * Creates a SpQueryMetaDataFn pointer that invokes the given AHK function when called.
         * @param {Func(IntPtr, LSA_UNICODE_STRING, UInt32, "uint*", "ptr*", "ptr*") => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
