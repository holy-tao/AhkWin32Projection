 ...
 */
export default struct PLSA_AP_LOGON_USER_EX3 {
    value : IntPtr

    __value {
        set {
            if (value is PLSA_AP_LOGON_USER_EX3) {
    ...
    /**
     * A PLSA_AP_LOGON_USER_EX3 that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PLSA_AP_LOGON_USER_EX3 {
        /**
         * Creates a PLSA_AP_LOGON_USER_EX3 pointer that invokes the given AHK function when called.
         * @param {Func("ptr*", SECURITY_LOGON_TYPE, IntPtr, "ptr", UInt32, SECPKG_SURROGATE_LOGON, "ptr*", "uint*", LUID, "int*", "int*", "ptr*", "ptr*", "ptr*", "ptr*", SECPKG_PRIMARY_CRED, "ptr*") => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 17)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 17 parameters.", -1, fn)
            }
