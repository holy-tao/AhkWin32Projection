...
export default struct ACQUIRE_CREDENTIALS_HANDLE_FN_A {
    ...
    struct From extends ACQUIRE_CREDENTIALS_HANDLE_FN_A {
         ...
         * @param {Func("char*", "char*", UInt32, "ptr", "ptr", "ptr", "ptr", SecHandle, "int64*") => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 9)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 9 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["char*", "char*", UInt32, "ptr", "ptr", "ptr", "ptr", SecHandle.Ptr, "int64*", "int"])
        }
