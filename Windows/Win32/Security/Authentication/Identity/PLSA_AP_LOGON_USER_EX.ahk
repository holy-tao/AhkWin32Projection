 ...
 */
export default struct PLSA_AP_LOGON_USER_EX {
    value : IntPtr

    __value {
        set {
            if (value is PLSA_AP_LOGON_USER_EX) {
     ...
     * @returns {NTSTATUS} 
     */
    Call(ClientRequest, LogonType, AuthenticationInformation, ClientAuthenticationBase, AuthenticationInformationLength, ProfileBuffer, ProfileBufferLength, LogonId, SubStatus, TokenInformationType, TokenInformation, AccountName, AuthenticatingAuthority, MachineName) {
    ...
    /**
     * A PLSA_AP_LOGON_USER_EX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PLSA_AP_LOGON_USER_EX {
        /**
         * Creates a PLSA_AP_LOGON_USER_EX pointer that invokes the given AHK function when called.
         * @param {Func("ptr*", SECURITY_LOGON_TYPE, IntPtr, "ptr", UInt32, "ptr*", "uint*", LUID, "int*", "int*", "ptr*", "ptr*", "ptr*", "ptr*") => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 14)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 14 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr*", SECURITY_LOGON_TYPE, IntPtr, "ptr", UInt32, "ptr*", "uint*", LUID.Ptr, "int*", "int*", "ptr*", "ptr*", "ptr*", "ptr*", NTSTATUS])
        }
