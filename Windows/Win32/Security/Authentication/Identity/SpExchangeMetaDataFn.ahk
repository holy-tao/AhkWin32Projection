 ...
 * @see https://learn.microsoft.com/windows/win32/api/ntsecpkg/nc-ntsecpkg-spexchangemetadatafn
 * @namespace Windows.Win32.Security.Authentication.Identity
 */
export default struct SpExchangeMetaDataFn {
    value : IntPtr

    __value {
        set {
            if (value is SpExchangeMetaDataFn) {
     ...
     * @param {Pointer<LSA_UNICODE_STRING>} TargetName A pointer to a 
     ...
     * @param {Integer} ContextRequirements Flags that indicate the context attributes required by the client. 
     * 
     * 
     * 
     * 
     ...
     * </tr>
     * </table>
     * @param {Integer} MetaDataLength The size, in characters, of the <i>MetaData</i> buffer.
     * @param {Integer} MetaData The metadata to send.
     * @param {Pointer<Pointer>} ContextHandle A handle to the security handle to use. If this parameter points to <b>NULL</b> on input, this function allocates and initializes a security context by using the values of the <i>CredentialHandle</i> and <i>TargetName</i> parameters.
     * 
    ...
    Call(CredentialHandle, TargetName, ContextRequirements, MetaDataLength, MetaData, ContextHandle) {
        ...
        result := DllCall(this.value, CredentialHandleMarshal, CredentialHandle, TargetNameMarshal, TargetName, UInt32, ContextRequirements, UInt32, MetaDataLength, IntPtr, MetaData, ContextHandleMarshal, ContextHandle, NTSTATUS)
        NTSTATUS.ThrowIfError(result.value)
        return result
    }

    /**
     * A SpExchangeMetaDataFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SpExchangeMetaDataFn {
        /**
         * Creates a SpExchangeMetaDataFn pointer that invokes the given AHK function when called.
         * @param {Func(IntPtr, LSA_UNICODE_STRING, UInt32, UInt32, IntPtr, "ptr*") => NTSTATUS} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [IntPtr, LSA_UNICODE_STRING.Ptr, UInt32, UInt32, IntPtr, "ptr*", NTSTATUS])
        }
