 ...
 */
export default struct PLSA_AUDIT_LOGON_EX {
    value : IntPtr

    __value {
        set {
            if (value is PLSA_AUDIT_LOGON_EX) {
     ...
     */
    Call(_Status, SubStatus, AccountName, AuthenticatingAuthority, WorkstationName, UserSid, LogonType, ImpersonationLevel, TokenSource, LogonId) {
        ...
        DllCall(this.value, NTSTATUS, _Status, NTSTATUS, SubStatus, AccountNameMarshal, AccountName, AuthenticatingAuthorityMarshal, AuthenticatingAuthority, WorkstationNameMarshal, WorkstationName, UserSidMarshal, UserSid, SECURITY_LOGON_TYPE, LogonType, SECURITY_IMPERSONATION_LEVEL, ImpersonationLevel, TOKEN_SOURCE.Ptr, TokenSource, LUID.Ptr, LogonId)
    }

    /**
     * A PLSA_AUDIT_LOGON_EX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PLSA_AUDIT_LOGON_EX {
        /**
         * Creates a PLSA_AUDIT_LOGON_EX pointer that invokes the given AHK function when called.
         * @param {Func(NTSTATUS, NTSTATUS, LSA_UNICODE_STRING, LSA_UNICODE_STRING, LSA_UNICODE_STRING, PSID, SECURITY_LOGON_TYPE, SECURITY_IMPERSONATION_LEVEL, TOKEN_SOURCE, LUID) => IntPtr} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 10)) {
