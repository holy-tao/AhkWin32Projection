...
class ITpmVirtualSmartCardManager extends IUnknown{
     ...
     * @param {Pointer<Byte>} pbAdminKey 
     * @param {Integer} cbAdminKey 
     * @param {Pointer<Byte>} pbAdminKcv 
     * @param {Integer} cbAdminKcv 
     * @param {Pointer<Byte>} pbPuk 
     * @param {Integer} cbPuk 
     * @param {Pointer<Byte>} pbPin 
     * @param {Integer} cbPin 
     * @param {BOOL} fGenerate 
     * @param {Pointer<ITpmVirtualSmartCardManagerStatusCallback>} pStatusCallback 
    ...
    CreateVirtualSmartCard(pszFriendlyName, bAdminAlgId, pbAdminKey, cbAdminKey, pbAdminKcv, cbAdminKcv, pbPuk, cbPuk, pbPin, cbPin, fGenerate, pStatusCallback, ppszInstanceId, pfNeedReboot) {
        ...
        result := ComCall(3, this, "ptr", pszFriendlyName, "char", bAdminAlgId, "char*", pbAdminKey, "uint", cbAdminKey, "char*", pbAdminKcv, "uint", cbAdminKcv, "char*", pbPuk, "uint", cbPuk, "char*", pbPin, "uint", cbPin, "int", fGenerate, "ptr", pStatusCallback, "ptr", ppszInstanceId, "ptr", pfNeedReboot, "int")
        if(result != 0)
    ...
    DestroyVirtualSmartCard(pszInstanceId, pStatusCallback, pfNeedReboot) {
        ...
        result := ComCall(4, this, "ptr", pszInstanceId, "ptr", pStatusCallback, "ptr", pfNeedReboot, "int")
        if(result != 0)
            throw OSError(result)
