...
class IX509SCEPEnrollmentHelper extends IDispatch{
    ...
    InitializeForPending(strServerUrl, strRequestHeaders, Context, strTransactionId) {
        ...
        result := ComCall(8, this, "ptr", strServerUrl, "ptr", strRequestHeaders, "int", Context, "ptr", strTransactionId, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Enroll(ProcessFlags, pDisposition) {
        result := ComCall(9, this, "int", ProcessFlags, "int*", pDisposition, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FetchPending(ProcessFlags, pDisposition) {
        result := ComCall(10, this, "int", ProcessFlags, "int*", pDisposition, "int")
        if(result != 0)
            throw OSError(result)
