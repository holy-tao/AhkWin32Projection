 ...
 * @version v4.0.30319
 */
class FNCERTSRVRESTOREPREPAREW extends IUnknown {
     ...
     */
    Invoke(pwszServerName, dwRestoreFlags, phbc) {
        pwszServerName := pwszServerName is String ? StrPtr(pwszServerName) : pwszServerName

        phbcMarshal := phbc is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, "ptr", pwszServerName, "uint", dwRestoreFlags, phbcMarshal, phbc, "int")
        if(result != 0) {
