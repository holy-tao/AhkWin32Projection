 ...
 * @version v4.0.30319
 */
class FNCERTSRVBACKUPPREPAREW extends IUnknown {
     ...
     */
    Invoke(pwszServerName, grbitJet, dwBackupFlags, phbc) {
        pwszServerName := pwszServerName is String ? StrPtr(pwszServerName) : pwszServerName

        phbcMarshal := phbc is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, "ptr", pwszServerName, "uint", grbitJet, "uint", dwBackupFlags, phbcMarshal, phbc, "int")
        if(result != 0) {
