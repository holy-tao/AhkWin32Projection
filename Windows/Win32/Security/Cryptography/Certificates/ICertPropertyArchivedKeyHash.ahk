 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/certenroll/nn-certenroll-icertpropertyarchivedkeyhash
...
class ICertPropertyArchivedKeyHash extends ICertProperty{
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/certenroll/nf-certenroll-icertpropertyarchivedkeyhash-initialize
     */
    Initialize(Encoding, strArchivedKeyHashValue) {
        if(strArchivedKeyHashValue is String) {
            pin := BSTR.Alloc(strArchivedKeyHashValue)
            strArchivedKeyHashValue := pin.Value
        }

        result := ComCall(14, this, "int", Encoding, "ptr", strArchivedKeyHashValue, "int")
        if(result != 0) {
     ...
     * @returns {BSTR} 
     * @see https://learn.microsoft.com/windows/win32/api//content/certenroll/nf-certenroll-icertpropertyarchivedkeyhash-get_archivedkeyhash
     */
    get_ArchivedKeyHash(Encoding) {
        pValue := BSTR()
        result := ComCall(15, this, "int", Encoding, "ptr", pValue, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
