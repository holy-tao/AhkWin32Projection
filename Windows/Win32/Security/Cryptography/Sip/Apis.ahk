...
export CryptSIPCreateIndirectData(pSubjectInfo, pcbIndirectData, pIndirectData) {
    ...
    result := DllCall("WINTRUST.dll\CryptSIPCreateIndirectData", SIP_SUBJECTINFO.Ptr, pSubjectInfo, pcbIndirectDataMarshal, pcbIndirectData, SIP_INDIRECT_DATA.Ptr, pIndirectData, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CryptSIPVerifyIndirectData(pSubjectInfo, pIndirectData) {
    A_LastError := 0

    result := DllCall("WINTRUST.dll\CryptSIPVerifyIndirectData", SIP_SUBJECTINFO.Ptr, pSubjectInfo, SIP_INDIRECT_DATA.Ptr, pIndirectData, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CryptSIPRetrieveSubjectGuid(FileName, hFileIn, pgSubject) {
    FileName := FileName is String ? StrPtr(FileName) : FileName

    A_LastError := 0

    result := DllCall("CRYPT32.dll\CryptSIPRetrieveSubjectGuid", "ptr", FileName, HANDLE, hFileIn, Guid.Ptr, pgSubject, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CryptSIPRetrieveSubjectGuidForCatalogFile(FileName, hFileIn, pgSubject) {
    FileName := FileName is String ? StrPtr(FileName) : FileName

    A_LastError := 0

    result := DllCall("CRYPT32.dll\CryptSIPRetrieveSubjectGuidForCatalogFile", "ptr", FileName, HANDLE, hFileIn, Guid.Ptr, pgSubject, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CryptSIPAddProvider(psNewProv) {
    A_LastError := 0

    result := DllCall("CRYPT32.dll\CryptSIPAddProvider", SIP_ADD_NEWPROVIDER.Ptr, psNewProv, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CryptSIPRemoveProvider(pgProv) {
    A_LastError := 0

    result := DllCall("CRYPT32.dll\CryptSIPRemoveProvider", Guid.Ptr, pgProv, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows8.0
 */
export CryptSIPGetCaps(pSubjInfo, pCaps) {
    result := DllCall("WINTRUST.dll\CryptSIPGetCaps", SIP_SUBJECTINFO.Ptr, pSubjInfo, SIP_CAP_SET_V3.Ptr, pCaps, BOOL)
    return result
}
