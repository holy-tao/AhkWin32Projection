 ...
 */
export default struct NCryptOpenKeyFn {
    value : IntPtr

    __value {
        set {
            if (value is NCryptOpenKeyFn) {
    ...
    /**
     * A NCryptOpenKeyFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends NCryptOpenKeyFn {
        /**
         * Creates a NCryptOpenKeyFn pointer that invokes the given AHK function when called.
         * @param {Func(NCRYPT_PROV_HANDLE, PWSTR, UInt32, UInt32) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [NCRYPT_PROV_HANDLE, PWSTR, UInt32, UInt32, "int"])
        }
