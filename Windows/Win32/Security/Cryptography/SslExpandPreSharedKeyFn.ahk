 ...
 */
export default struct SslExpandPreSharedKeyFn {
    value : IntPtr

    __value {
        set {
            if (value is SslExpandPreSharedKeyFn) {
    ...
    /**
     * A SslExpandPreSharedKeyFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SslExpandPreSharedKeyFn {
        /**
         * Creates a SslExpandPreSharedKeyFn pointer that invokes the given AHK function when called.
         * @param {Func(NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, IntPtr, UInt32, BCryptBufferDesc, UInt32) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, IntPtr, UInt32, BCryptBufferDesc.Ptr, UInt32, "int"])
        }
