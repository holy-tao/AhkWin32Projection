#Requires AutoHotkey v2.1-alpha.26+ 64-bit
#Import "..\..\Foundation\PSTR.ahk" { PSTR }
#Import ".\NCRYPT_PROV_HANDLE.ahk" { NCRYPT_PROV_HANDLE }
#Import ".\NCRYPT_KEY_HANDLE.ahk" { NCRYPT_KEY_HANDLE }
#Import "..\..\Foundation\HRESULT.ahk" { HRESULT }

/**
 * @namespace Windows.Win32.Security.Cryptography
 */
export default struct SslExportKeyingMaterialFn {
    value : IntPtr

    __value {
        set {
            if (value is SslExportKeyingMaterialFn) {
                this.value := value.value
            }
            else {
                this.value := value
            }
        }
    }

    /**
     * 
     * @param {NCRYPT_PROV_HANDLE} hSslProvider 
     * @param {NCRYPT_KEY_HANDLE} hMasterKey 
     * @param {PSTR} sLabel 
     * @param {Integer} pbRandoms 
     * @param {Integer} cbRandoms 
     * @param {Integer} pbContextValue 
     * @param {Integer} cbContextValue 
     * @param {Integer} pbOutput 
     * @param {Integer} cbOutput 
     * @param {Integer} dwFlags 
     * @returns {HRESULT} 
     */
    Call(hSslProvider, hMasterKey, sLabel, pbRandoms, cbRandoms, pbContextValue, cbContextValue, pbOutput, cbOutput, dwFlags) {
        sLabel := sLabel is String ? StrPtr(sLabel) : sLabel

        result := DllCall(this.value, NCRYPT_PROV_HANDLE, hSslProvider, NCRYPT_KEY_HANDLE, hMasterKey, "ptr", sLabel, IntPtr, pbRandoms, UInt32, cbRandoms, IntPtr, pbContextValue, UInt16, cbContextValue, IntPtr, pbOutput, UInt32, cbOutput, UInt32, dwFlags, "HRESULT")
        return result
    }

    /**
     * A SslExportKeyingMaterialFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SslExportKeyingMaterialFn {
        /**
         * Creates a SslExportKeyingMaterialFn pointer that invokes the given AHK function when called.
         * @param {Func(NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, PSTR, IntPtr, UInt32, IntPtr, UInt16, IntPtr, UInt32, UInt32) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 10)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 10 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, PSTR, IntPtr, UInt32, IntPtr, UInt16, IntPtr, UInt32, UInt32, "int"])
        }

        __Delete() {
            if (this.value) {
                CallbackFree(this.value)
            }
        }
    }
}
