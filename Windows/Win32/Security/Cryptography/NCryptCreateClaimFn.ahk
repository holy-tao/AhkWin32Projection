 ...
 */
export default struct NCryptCreateClaimFn {
    value : IntPtr

    __value {
        set {
            if (value is NCryptCreateClaimFn) {
    ...
    /**
     * A NCryptCreateClaimFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends NCryptCreateClaimFn {
        /**
         * Creates a NCryptCreateClaimFn pointer that invokes the given AHK function when called.
         * @param {Func(NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, NCRYPT_KEY_HANDLE, UInt32, BCryptBufferDesc, IntPtr, UInt32, UInt32) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 8)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 8 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, NCRYPT_KEY_HANDLE, UInt32, BCryptBufferDesc.Ptr, IntPtr, UInt32, UInt32, "int"])
        }
