...
export default struct PFN_AUTHENTICODE_DIGEST_SIGN_EX_WITHFILEHANDLE {
    ...
    Call(pMetadataBlob, digestAlgId, pbToBeSignedDigest, cbToBeSignedDigest, hFile, pSignedDigest, hCertChainStore) {
        ...
        result := DllCall(this.value, pMetadataBlobMarshal, pMetadataBlob, ALG_ID, digestAlgId, pbToBeSignedDigestMarshal, pbToBeSignedDigest, UInt32, cbToBeSignedDigest, HANDLE, hFile, CRYPT_INTEGER_BLOB.Ptr, pSignedDigest, "ptr*", &ppSignerCert := 0, HCERTSTORE, hCertChainStore, "HRESULT")
        return ppSignerCert
    }
