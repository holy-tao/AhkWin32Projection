 ...
 */
export default struct SslExtractEarlyKeyFn {
    value : IntPtr

    __value {
        set {
            if (value is SslExtractEarlyKeyFn) {
    ...
    /**
     * A SslExtractEarlyKeyFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SslExtractEarlyKeyFn {
        /**
         * Creates a SslExtractEarlyKeyFn pointer that invokes the given AHK function when called.
         * @param {Func(NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, UInt32, UInt32, BCryptBufferDesc, UInt32) => "int"} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [NCRYPT_PROV_HANDLE, NCRYPT_KEY_HANDLE, UInt32, UInt32, BCryptBufferDesc.Ptr, UInt32, "int"])
        }
