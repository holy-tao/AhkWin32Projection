 ...
 */
export default struct PFN_MD_IMPORT_SESSION_KEY {
    value : IntPtr

    __value {
        set {
            if (value is PFN_MD_IMPORT_SESSION_KEY) {
     ...
     * @returns {Integer} 
     */
    Call(pCardData, pwszBlobType, pwszAlgId, phKey, pbInput, cbInput) {
    ...
    /**
     * A PFN_MD_IMPORT_SESSION_KEY that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFN_MD_IMPORT_SESSION_KEY {
        /**
         * Creates a PFN_MD_IMPORT_SESSION_KEY pointer that invokes the given AHK function when called.
         * @param {Func(CARD_DATA, PWSTR, PWSTR, "ptr*", IntPtr, UInt32) => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
