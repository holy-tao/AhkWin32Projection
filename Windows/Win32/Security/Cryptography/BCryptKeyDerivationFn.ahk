 ...
 * @version v4.0.30319
 */
class BCryptKeyDerivationFn extends IUnknown {
     ...
     */
    Invoke(hKey_, pParameterList, pbDerivedKey, cbDerivedKey, pcbResult, dwFlags) {
        hKey_ := hKey_ is Win32Handle ? NumGet(hKey_, "ptr") : hKey_

        pcbResultMarshal := pcbResult is VarRef ? "uint*" : "ptr"

        result := ComCall(3, this, "ptr", hKey_, "ptr", pParameterList, "ptr", pbDerivedKey, "uint", cbDerivedKey, pcbResultMarshal, pcbResult, "uint", dwFlags, "int")
        return result
    }
}
