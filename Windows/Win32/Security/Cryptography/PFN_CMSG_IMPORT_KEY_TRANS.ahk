 ...
 * </tr>
 * </table>
 * @see https://learn.microsoft.com/windows/win32/api/wincrypt/nc-wincrypt-pfn_cmsg_import_key_trans
 * @namespace Windows.Win32.Security.Cryptography
 */
export default struct PFN_CMSG_IMPORT_KEY_TRANS {
    value : IntPtr

    __value {
        set {
            if (value is PFN_CMSG_IMPORT_KEY_TRANS) {
     ...
     * @param {Pointer<CMSG_CTRL_KEY_TRANS_DECRYPT_PARA>} pKeyTransDecryptPara A pointer to a <a href="https://docs.microsoft.com/windows/desktop/api/wincrypt/ns-wincrypt-cmsg_ctrl_key_trans_decrypt_para">CMSG_CTRL_KEY_TRANS_DECRYPT_PARA</a> structure that contains information about the key transport recipient.
     ...
     */
    Call(pContentEncryptionAlgorithm, pKeyTransDecryptPara, dwFlags, phContentEncryptKey) {
        static pvReserved := 0 ;Reserved parameters must always be NULL

        phContentEncryptKeyMarshal := phContentEncryptKey is VarRef ? "ptr*" : "ptr"

        result := DllCall(this.value, CRYPT_ALGORITHM_IDENTIFIER.Ptr, pContentEncryptionAlgorithm, CMSG_CTRL_KEY_TRANS_DECRYPT_PARA.Ptr, pKeyTransDecryptPara, UInt32, dwFlags, "ptr", pvReserved, phContentEncryptKeyMarshal, phContentEncryptKey, BOOL)
        return result
    }

    /**
     * A PFN_CMSG_IMPORT_KEY_TRANS that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFN_CMSG_IMPORT_KEY_TRANS {
        /**
         * Creates a PFN_CMSG_IMPORT_KEY_TRANS pointer that invokes the given AHK function when called.
         * @param {Func(CRYPT_ALGORITHM_IDENTIFIER, CMSG_CTRL_KEY_TRANS_DECRYPT_PARA, UInt32, "ptr*") => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [CRYPT_ALGORITHM_IDENTIFIER.Ptr, CMSG_CTRL_KEY_TRANS_DECRYPT_PARA.Ptr, UInt32, "ptr*", BOOL])
        }
