 ...
 */
export default struct SslOpenProviderFn {
    value : IntPtr

    __value {
        set {
            if (value is SslOpenProviderFn) {
    ...
    Call(pszProviderName, dwFlags) {
        ...
        result := DllCall(this.value, NCRYPT_PROV_HANDLE.Ptr, phSslProvider, "ptr", pszProviderName, UInt32, dwFlags, "HRESULT")
        return phSslProvider
    }

    /**
     * A SslOpenProviderFn that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends SslOpenProviderFn {
        /**
         * Creates a SslOpenProviderFn pointer that invokes the given AHK function when called.
