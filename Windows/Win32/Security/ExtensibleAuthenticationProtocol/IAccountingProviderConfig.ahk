...
class IAccountingProviderConfig extends IUnknown{
    ...
    Initialize(pszMachineName) {
        ...
        result := ComCall(3, this, "ptr", pszMachineName, "ptr*", &puConnectionParam := 0, "int")
        if(result != 0) {
    ...
    Uninitialize(uConnectionParam) {
        result := ComCall(4, this, "ptr", uConnectionParam, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    Configure(uConnectionParam, hWnd_, dwFlags, uReserved1, uReserved2) {
        hWnd_ := hWnd_ is Win32Handle ? NumGet(hWnd_, "ptr") : hWnd_

        result := ComCall(5, this, "ptr", uConnectionParam, "ptr", hWnd_, "uint", dwFlags, "ptr", uReserved1, "ptr", uReserved2, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    Activate(uConnectionParam, uReserved1, uReserved2) {
        result := ComCall(6, this, "ptr", uConnectionParam, "ptr", uReserved1, "ptr", uReserved2, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    Deactivate(uConnectionParam, uReserved1, uReserved2) {
        result := ComCall(7, this, "ptr", uConnectionParam, "ptr", uReserved1, "ptr", uReserved2, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
