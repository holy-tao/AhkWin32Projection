 ...
 */
export default struct DispSVGTextElement extends IDispatch {
    /**
     ...
     * used for DispSVGTextElement interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispSVGTextElement.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispSVGTextElement.IID.Equals(iid)) {
            return true
        }
