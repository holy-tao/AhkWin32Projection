...
class IHTMLWindow2 extends IHTMLFramesCollection2{
    ...
    setTimeout(expression, msec, language, timerID) {
        ...
        result := ComCall(14, this, "ptr", expression, "int", msec, "ptr", language, "int*", timerID, "int")
        if(result != 0)
            throw OSError(result)
    ...
    confirm(message, confirmed) {
        ...
        result := ComCall(17, this, "ptr", message, "ptr", confirmed, "int")
        if(result != 0)
            throw OSError(result)
    ...
    open(url, name, features, replace, pomWindowResult) {
        ...
        result := ComCall(29, this, "ptr", url, "ptr", name, "ptr", features, "short", replace, "ptr", pomWindowResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    setInterval(expression, msec, language, timerID) {
        ...
        result := ComCall(64, this, "ptr", expression, "int", msec, "ptr", language, "int*", timerID, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    toString(String) {
        result := ComCall(69, this, "ptr", String, "int")
        if(result != 0)
            throw OSError(result)
