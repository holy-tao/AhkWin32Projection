 ...
 */
export default struct IGetSVGDocument extends IDispatch {
    /**
     ...
     * used for IGetSVGDocument interfaces
    */
    struct Vtbl extends IDispatch.Vtbl {
        getSVGDocument : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IGetSVGDocument.Vtbl()
        }
    ...
    Query(iid) {
        if (IGetSVGDocument.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.getSVGDocument)
    }
}
