...
class IHTMLElement extends IDispatch{
    ...
    scrollIntoView(varargStart) {
        result := ComCall(46, this, "ptr", varargStart, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IHTMLElement>} pChild 
     * @param {Pointer<VARIANT_BOOL>} pfResult 
     * @returns {HRESULT} 
     */
    contains(pChild, pfResult) {
        result := ComCall(47, this, "ptr", pChild, "ptr", pfResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    insertAdjacentHTML(where, html) {
        ...
        result := ComCall(65, this, "ptr", where, "ptr", html, "int")
        if(result != 0)
            throw OSError(result)
    ...
    insertAdjacentText(where, text) {
        ...
        result := ComCall(66, this, "ptr", where, "ptr", text, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    toString(String) {
        result := ComCall(73, this, "ptr", String, "int")
        if(result != 0)
            throw OSError(result)
