...
class IHTMLPainterEventInfo extends IUnknown{
     ...
     */
    GetEventInfoFlags() {
        result := ComCall(3, this, "int*", &plEventInfoFlags := 0, "int")
        if(result != 0) {
    ...
    GetEventTarget(ppElement) {
        result := ComCall(4, this, "ptr*", ppElement, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/winuser/nf-winuser-setcursor
     */
    SetCursor(lPartID) {
        result := ComCall(5, this, "int", lPartID, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    StringFromPartID(lPartID) {
        pbstrPart := BSTR()
        result := ComCall(6, this, "int", lPartID, "ptr", pbstrPart, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
