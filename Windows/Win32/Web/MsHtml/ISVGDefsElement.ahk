 ...
 */
export default struct ISVGDefsElement extends IDispatch {
    /**
     ...
     * used for ISVGDefsElement interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISVGDefsElement.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (ISVGDefsElement.IID.Equals(iid)) {
            return true
        }
