...
class IHTMLWindow7 extends IDispatch{
    ...
    getSelection(ppIHTMLSelection) {
        result := ComCall(7, this, "ptr", ppIHTMLSelection, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<IHTMLDOMNode>} varArgIn 
    ...
    getComputedStyle(varArgIn, bstrPseudoElt, ppComputedStyle) {
        ...
        result := ComCall(8, this, "ptr", varArgIn, "ptr", bstrPseudoElt, "ptr", ppComputedStyle, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_outerWidth(p) {
        result := ComCall(18, this, "int*", p, "int")
        if(result != 0)
            throw OSError(result)
