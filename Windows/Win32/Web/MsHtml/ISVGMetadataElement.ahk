 ...
 */
export default struct ISVGMetadataElement extends IDispatch {
    /**
     ...
     * used for ISVGMetadataElement interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISVGMetadataElement.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (ISVGMetadataElement.IID.Equals(iid)) {
            return true
        }
