#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\Foundation\BSTR.ahk
#Include ..\..\System\Com\IDispatch.ahk

/**
 * @namespace Windows.Win32.Web.MsHtml
 * @version v4.0.30319
 */
class IHTMLStyleSheetRulesAppliedCollection extends IDispatch{
    /**
     * The interface identifier for IHTMLStyleSheetRulesAppliedCollection
     * @type {Guid}
     */
    static IID => Guid("{305104c0-98b5-11cf-bb82-00aa00bdce0b}")

    /**
     * The class identifier for HTMLStyleSheetRulesAppliedCollection
     * @type {Guid}
     */
    static CLSID => Guid("{eb36f845-2395-4719-b85c-d0d80e184bd9}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 7

    /**
     * 
     * @param {Integer} index 
     * @param {Pointer<IHTMLStyleSheetRule>} ppHTMLStyleSheetRule 
     * @returns {HRESULT} 
     */
    item(index, ppHTMLStyleSheetRule) {
        result := ComCall(7, this, "int", index, "ptr", ppHTMLStyleSheetRule, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} p 
     * @returns {HRESULT} 
     */
    get_length(p) {
        result := ComCall(8, this, "int*", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} name 
     * @param {Pointer<IHTMLStyleSheetRule>} ppRule 
     * @returns {HRESULT} 
     */
    propertyAppliedBy(name, ppRule) {
        name := name is String ? BSTR.Alloc(name).Value : name

        result := ComCall(9, this, "ptr", name, "ptr", ppRule, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} name 
     * @param {Integer} index 
     * @param {Pointer<IHTMLStyleSheetRule>} ppRule 
     * @returns {HRESULT} 
     */
    propertyAppliedTrace(name, index, ppRule) {
        name := name is String ? BSTR.Alloc(name).Value : name

        result := ComCall(10, this, "ptr", name, "int", index, "ptr", ppRule, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} name 
     * @param {Pointer<Int32>} pLength 
     * @returns {HRESULT} 
     */
    propertyAppliedTraceLength(name, pLength) {
        name := name is String ? BSTR.Alloc(name).Value : name

        result := ComCall(11, this, "ptr", name, "int*", pLength, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
