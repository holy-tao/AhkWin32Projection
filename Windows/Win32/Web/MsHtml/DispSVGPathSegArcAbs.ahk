 ...
 */
export default struct DispSVGPathSegArcAbs extends IDispatch {
    /**
     ...
     * used for DispSVGPathSegArcAbs interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispSVGPathSegArcAbs.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispSVGPathSegArcAbs.IID.Equals(iid)) {
            return true
        }
