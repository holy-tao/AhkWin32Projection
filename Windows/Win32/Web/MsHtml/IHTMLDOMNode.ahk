...
class IHTMLDOMNode extends IDispatch{
    ...
    hasChildNodes(fChildren) {
        result := ComCall(9, this, "ptr", fChildren, "int")
        if(result != 0)
            throw OSError(result)
    ...
    insertBefore(newChild, refChild, node) {
        result := ComCall(12, this, "ptr", newChild, "ptr", refChild, "ptr", node, "int")
        if(result != 0)
            throw OSError(result)
    ...
    removeChild(oldChild, node) {
        result := ComCall(13, this, "ptr", oldChild, "ptr", node, "int")
        if(result != 0)
            throw OSError(result)
    ...
    replaceChild(newChild, oldChild, node) {
        result := ComCall(14, this, "ptr", newChild, "ptr", oldChild, "ptr", node, "int")
        if(result != 0)
            throw OSError(result)
    ...
    cloneNode(fDeep, clonedNode) {
        result := ComCall(15, this, "short", fDeep, "ptr", clonedNode, "int")
        if(result != 0)
            throw OSError(result)
    ...
    removeNode(fDeep, removed) {
        result := ComCall(16, this, "short", fDeep, "ptr", removed, "int")
        if(result != 0)
            throw OSError(result)
    ...
    swapNode(otherNode, swappedNode) {
        result := ComCall(17, this, "ptr", otherNode, "ptr", swappedNode, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IHTMLDOMNode>} replacement 
     * @param {Pointer<IHTMLDOMNode>} replaced 
     * @returns {HRESULT} 
     */
    replaceNode(replacement, replaced) {
        result := ComCall(18, this, "ptr", replacement, "ptr", replaced, "int")
        if(result != 0)
            throw OSError(result)
    ...
    appendChild(newChild, node) {
        result := ComCall(19, this, "ptr", newChild, "ptr", node, "int")
        if(result != 0)
            throw OSError(result)
