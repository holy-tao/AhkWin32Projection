 ...
 */
export default struct DispHTMLMarqueeElement extends IDispatch {
    /**
     ...
     * used for DispHTMLMarqueeElement interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispHTMLMarqueeElement.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispHTMLMarqueeElement.IID.Equals(iid)) {
            return true
        }
