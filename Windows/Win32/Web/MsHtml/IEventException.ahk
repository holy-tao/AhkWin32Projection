 ...
 */
export default struct IEventException extends IDispatch {
    /**
     ...
     * used for IEventException interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IEventException.Vtbl()
        }
    ...
    Query(iid) {
        if (IEventException.IID.Equals(iid)) {
            return true
        }
