 ...
 */
export default struct DispHTMLWndSelectElement extends IDispatch {
    /**
     ...
     * used for DispHTMLWndSelectElement interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispHTMLWndSelectElement.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispHTMLWndSelectElement.IID.Equals(iid)) {
            return true
        }
