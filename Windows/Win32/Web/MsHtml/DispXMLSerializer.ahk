 ...
 */
export default struct DispXMLSerializer extends IDispatch {
    /**
     ...
     * used for DispXMLSerializer interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispXMLSerializer.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispXMLSerializer.IID.Equals(iid)) {
            return true
        }
