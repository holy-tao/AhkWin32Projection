 ...
 */
export default struct IHTMLChangePlayback extends IUnknown {
    /**
     ...
     * used for IHTMLChangePlayback interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IHTMLChangePlayback.Vtbl()
        }
    ...
    Query(iid) {
        if (IHTMLChangePlayback.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.ExecChange)
    }
}
