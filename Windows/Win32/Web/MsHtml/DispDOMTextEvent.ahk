 ...
 */
export default struct DispDOMTextEvent extends IDispatch {
    /**
     ...
     * used for DispDOMTextEvent interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := DispDOMTextEvent.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (DispDOMTextEvent.IID.Equals(iid)) {
            return true
        }
