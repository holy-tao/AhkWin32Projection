 ...
 */
export default struct INavigatorDoNotTrack extends IDispatch {
    /**
     ...
     * used for INavigatorDoNotTrack interfaces
    */
    struct Vtbl extends IDispatch.Vtbl {
        get_msDoNotTrack : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := INavigatorDoNotTrack.Vtbl()
        }
    ...
    Query(iid) {
        if (INavigatorDoNotTrack.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.get_msDoNotTrack)
    }
}
