 ...
 */
export default struct IViewObjectPrint extends IUnknown {
    /**
     ...
     * used for IViewObjectPrint interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetPrintBitmap : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IViewObjectPrint.Vtbl()
        }
    ...
    Query(iid) {
        if (IViewObjectPrint.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetPrintBitmap)
    }
}
