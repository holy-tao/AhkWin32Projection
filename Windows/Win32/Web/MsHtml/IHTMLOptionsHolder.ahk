#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\Foundation\BSTR.ahk
#Include ..\..\System\Com\IDispatch.ahk

/**
 * @namespace Windows.Win32.Web.MsHtml
 * @version v4.0.30319
 */
class IHTMLOptionsHolder extends IDispatch{
    /**
     * The interface identifier for IHTMLOptionsHolder
     * @type {Guid}
     */
    static IID => Guid("{3050f378-98b5-11cf-bb82-00aa00bdce0b}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 7

    /**
     * 
     * @param {Pointer<IHTMLDocument2>} p 
     * @returns {HRESULT} 
     */
    get_document(p) {
        result := ComCall(7, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IHTMLFontNamesCollection>} p 
     * @returns {HRESULT} 
     */
    get_fonts(p) {
        result := ComCall(8, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT} v 
     * @returns {HRESULT} 
     */
    put_execArg(v) {
        result := ComCall(9, this, "ptr", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<VARIANT>} p 
     * @returns {HRESULT} 
     */
    get_execArg(p) {
        result := ComCall(10, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} v 
     * @returns {HRESULT} 
     */
    put_errorLine(v) {
        result := ComCall(11, this, "int", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} p 
     * @returns {HRESULT} 
     */
    get_errorLine(p) {
        result := ComCall(12, this, "int*", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} v 
     * @returns {HRESULT} 
     */
    put_errorCharacter(v) {
        result := ComCall(13, this, "int", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} p 
     * @returns {HRESULT} 
     */
    get_errorCharacter(p) {
        result := ComCall(14, this, "int*", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} v 
     * @returns {HRESULT} 
     */
    put_errorCode(v) {
        result := ComCall(15, this, "int", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} p 
     * @returns {HRESULT} 
     */
    get_errorCode(p) {
        result := ComCall(16, this, "int*", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} v 
     * @returns {HRESULT} 
     */
    put_errorMessage(v) {
        v := v is String ? BSTR.Alloc(v).Value : v

        result := ComCall(17, this, "ptr", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<BSTR>} p 
     * @returns {HRESULT} 
     */
    get_errorMessage(p) {
        result := ComCall(18, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT_BOOL} v 
     * @returns {HRESULT} 
     */
    put_errorDebug(v) {
        result := ComCall(19, this, "short", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<VARIANT_BOOL>} p 
     * @returns {HRESULT} 
     */
    get_errorDebug(p) {
        result := ComCall(20, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IHTMLWindow2>} p 
     * @returns {HRESULT} 
     */
    get_unsecuredWindowOfDocument(p) {
        result := ComCall(21, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} v 
     * @returns {HRESULT} 
     */
    put_findText(v) {
        v := v is String ? BSTR.Alloc(v).Value : v

        result := ComCall(22, this, "ptr", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<BSTR>} p 
     * @returns {HRESULT} 
     */
    get_findText(p) {
        result := ComCall(23, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT_BOOL} v 
     * @returns {HRESULT} 
     */
    put_anythingAfterFrameset(v) {
        result := ComCall(24, this, "short", v, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<VARIANT_BOOL>} p 
     * @returns {HRESULT} 
     */
    get_anythingAfterFrameset(p) {
        result := ComCall(25, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} fontName 
     * @param {Pointer<IHTMLFontSizesCollection>} pSizesCollection 
     * @returns {HRESULT} 
     */
    sizes(fontName, pSizesCollection) {
        fontName := fontName is String ? BSTR.Alloc(fontName).Value : fontName

        result := ComCall(26, this, "ptr", fontName, "ptr", pSizesCollection, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT} initFile 
     * @param {VARIANT} initDir 
     * @param {VARIANT} filter 
     * @param {VARIANT} title 
     * @param {Pointer<BSTR>} pathName 
     * @returns {HRESULT} 
     */
    openfiledlg(initFile, initDir, filter, title, pathName) {
        result := ComCall(27, this, "ptr", initFile, "ptr", initDir, "ptr", filter, "ptr", title, "ptr", pathName, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT} initFile 
     * @param {VARIANT} initDir 
     * @param {VARIANT} filter 
     * @param {VARIANT} title 
     * @param {Pointer<BSTR>} pathName 
     * @returns {HRESULT} 
     */
    savefiledlg(initFile, initDir, filter, title, pathName) {
        result := ComCall(28, this, "ptr", initFile, "ptr", initDir, "ptr", filter, "ptr", title, "ptr", pathName, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {VARIANT} initColor 
     * @param {Pointer<Int32>} rgbColor 
     * @returns {HRESULT} 
     */
    choosecolordlg(initColor, rgbColor) {
        result := ComCall(29, this, "ptr", initColor, "int*", rgbColor, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @returns {HRESULT} 
     */
    showSecurityInfo() {
        result := ComCall(30, this, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IHTMLObjectElement>} object 
     * @param {Pointer<VARIANT_BOOL>} fApartment 
     * @returns {HRESULT} 
     */
    isApartmentModel(object, fApartment) {
        result := ComCall(31, this, "ptr", object, "ptr", fApartment, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {BSTR} fontName 
     * @param {Pointer<Int32>} charset 
     * @returns {HRESULT} 
     */
    getCharset(fontName, charset) {
        fontName := fontName is String ? BSTR.Alloc(fontName).Value : fontName

        result := ComCall(32, this, "ptr", fontName, "int*", charset, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<BSTR>} p 
     * @returns {HRESULT} 
     */
    get_secureConnectionInfo(p) {
        result := ComCall(33, this, "ptr", p, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
