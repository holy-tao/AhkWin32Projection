...
class IHTMLAttributeCollection4 extends IDispatch{
    ...
    getNamedItemNS(pvarNS, bstrName, ppNodeOut) {
        ...
        result := ComCall(7, this, "ptr", pvarNS, "ptr", bstrName, "ptr", ppNodeOut, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<IHTMLDOMAttribute2>} pNodeIn 
     * @param {Pointer<IHTMLDOMAttribute2>} ppNodeOut 
     * @returns {HRESULT} 
     */
    ...
    removeNamedItemNS(pvarNS, bstrName, ppNodeOut) {
        ...
        result := ComCall(9, this, "ptr", pvarNS, "ptr", bstrName, "ptr", ppNodeOut, "int")
        if(result != 0)
            throw OSError(result)
    ...
    getNamedItem(bstrName, ppNodeOut) {
        ...
        result := ComCall(10, this, "ptr", bstrName, "ptr", ppNodeOut, "int")
        if(result != 0)
            throw OSError(result)
    ...
    setNamedItem(pNodeIn, ppNodeOut) {
        result := ComCall(11, this, "ptr", pNodeIn, "ptr", ppNodeOut, "int")
        if(result != 0)
            throw OSError(result)
    ...
    removeNamedItem(bstrName, ppNodeOut) {
        ...
        result := ComCall(12, this, "ptr", bstrName, "ptr", ppNodeOut, "int")
        if(result != 0)
            throw OSError(result)
    ...
    item(index, ppNodeOut) {
        result := ComCall(13, this, "int", index, "ptr", ppNodeOut, "int")
        if(result != 0)
            throw OSError(result)
