...
class IDedupIterateChunksHash32 extends IUnknown{
    ...
    PushBuffer(pBuffer, ulBufferLength) {
        ...
        result := ComCall(3, this, pBufferMarshal, pBuffer, "uint", ulBufferLength, "int")
        if(result != 0) {
    ...
    Next(ulMaxChunks, pArrChunks, pulFetched) {
        ...
        result := ComCall(4, this, "uint", ulMaxChunks, "ptr", pArrChunks, pulFetchedMarshal, pulFetched, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
