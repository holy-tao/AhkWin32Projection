...
class IXpsOMGradientStop extends IUnknown{
     ...
     */
    GetOffset(offset) {
        result := ComCall(4, this, "float*", offset, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetColor(color, colorProfile) {
        result := ComCall(6, this, "ptr", color, "ptr", colorProfile, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetColor(color, colorProfile) {
        result := ComCall(7, this, "ptr", color, "ptr", colorProfile, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    Clone(gradientStop) {
        result := ComCall(8, this, "ptr", gradientStop, "int")
        if(result != 0)
            throw OSError(result)
