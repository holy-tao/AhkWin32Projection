 ...
 */
export default struct IXpsOMPartUriCollection extends IUnknown {
    /**
     ...
     * used for IXpsOMPartUriCollection interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IXpsOMPartUriCollection.Vtbl()
        }
     ...
     */
    GetAt(index) {
        result := ComCall(4, this, UInt32, index, "ptr*", &partUri := 0, "HRESULT")
    ...
    InsertAt(index, partUri) {
        result := ComCall(5, this, UInt32, index, "ptr", partUri, "HRESULT")
        return result
    }
     ...
     */
    SetAt(index, partUri) {
        result := ComCall(7, this, UInt32, index, "ptr", partUri, "HRESULT")
        return result
    }
    ...
    Query(iid) {
        if (IXpsOMPartUriCollection.IID.Equals(iid)) {
            return true
        }
