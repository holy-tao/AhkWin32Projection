 ...
 * @since windows6.1
 */
export StartXpsPrintJob(printerName, jobName, outputFileName, progressEvent, completionEvent, printablePagesOn, printablePagesOnCount, xpsPrintJob, documentStream, printTicketStream) {
 ...
 * @since windows6.1
 */
export StartXpsPrintJob1(printerName, jobName, outputFileName, progressEvent, completionEvent, xpsPrintJob, printContentReceiver) {
    printerName := printerName is String ? StrPtr(printerName) : printerName
    jobName := jobName is String ? StrPtr(jobName) : jobName
    outputFileName := outputFileName is String ? StrPtr(outputFileName) : outputFileName

    result := DllCall("XPSPRINT.dll\StartXpsPrintJob1", "ptr", printerName, "ptr", jobName, "ptr", outputFileName, HANDLE, progressEvent, HANDLE, completionEvent, IXpsPrintJob.Ptr, xpsPrintJob, IXpsOMPackageTarget.Ptr, printContentReceiver, "HRESULT")
    return result
}
