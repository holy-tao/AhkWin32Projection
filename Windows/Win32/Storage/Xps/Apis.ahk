...
;@region Methods
     ...
     * @param {PRINTER_DEVICE_CAPABILITIES} fwCapability 
     ...
     * @param {Integer} pvIn A pointer to the input structure required for the specified escape.
     ...
     * @since windows5.0
     */
    static Escape(_hdc, iEscape, cjIn, pvIn, pvOut) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        pvOutMarshal := pvOut is VarRef ? "ptr" : "ptr"

        result := DllCall("GDI32.dll\Escape", "ptr", _hdc, "int", iEscape, "int", cjIn, "ptr", pvIn, pvOutMarshal, pvOut, "int")
        return result
    }
     ...
     * @param {Integer} lpInData A pointer to the input structure required for the specified escape. See also Remarks.
     * @param {Integer} cjOutput The number of bytes of data pointed to by the <i>lpszOutData</i> parameter.
     * @param {Integer} lpOutData A pointer to the structure that receives output from this escape. This parameter must not be <b>NULL</b> if <b>ExtEscape</b> is called as a query function. If no data is to be returned in this structure, set <i>cbOutput</i> to 0. See also Remarks.
     ...
     * @since windows5.0
     */
    static ExtEscape(_hdc, iEscape, cjInput, lpInData, cjOutput, lpOutData) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\ExtEscape", "ptr", _hdc, "int", iEscape, "int", cjInput, "ptr", lpInData, "int", cjOutput, "ptr", lpOutData, "int")
        return result
    }
     ...
     * @param {HDC} _hdc A handle to the device context for the print job.
     ...
     * @since windows5.0
     */
    static StartDocA(_hdc, lpdi) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\StartDocA", "ptr", _hdc, "ptr", lpdi, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static StartDocW(_hdc, lpdi) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\StartDocW", "ptr", _hdc, "ptr", lpdi, "int")
        return result
    }
     ...
     * @param {HDC} _hdc Handle to the device context for the print job.
    ...
    static EndDoc(_hdc) {
        ...
        result := DllCall("GDI32.dll\EndDoc", "ptr", _hdc, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static StartPage(_hdc) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\StartPage", "ptr", _hdc, "int")
        return result
    }
    ...
    static EndPage(_hdc) {
        ...
        result := DllCall("GDI32.dll\EndPage", "ptr", _hdc, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static AbortDoc(_hdc) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\AbortDoc", "ptr", _hdc, "int")
        return result
    }
     ...
     * <div> </div>
     * @param {HDC} _hdc Handle to the device context for the print job.
     * @param {Pointer<ABORTPROC>} _proc Pointer to the application-defined abort function. For more information about the callback function, see the <a href="https://docs.microsoft.com/windows/desktop/api/wingdi/nc-wingdi-abortproc">AbortProc</a> callback function.
     * @returns {Integer} If the function succeeds, the return value is greater than zero.
     * 
     * If the function fails, the return value is SP_ERROR.
     ...
     * @since windows5.0
     */
    static SetAbortProc(_hdc, _proc) {
        _hdc := _hdc is Win32Handle ? NumGet(_hdc, "ptr") : _hdc

        result := DllCall("GDI32.dll\SetAbortProc", "ptr", _hdc, "ptr", _proc, "int")
        return result
    }
     ...
     * @param {HWND} _hwnd A handle to the window that will be copied.
     * @param {HDC} hdcBlt A handle to the device context.
     * @param {PRINT_WINDOW_FLAGS} nFlags 
     ...
     */
    static PrintWindow(_hwnd, hdcBlt, nFlags) {
        _hwnd := _hwnd is Win32Handle ? NumGet(_hwnd, "ptr") : _hwnd
        hdcBlt := hdcBlt is Win32Handle ? NumGet(hdcBlt, "ptr") : hdcBlt

        result := DllCall("USER32.dll\PrintWindow", "ptr", _hwnd, "ptr", hdcBlt, "uint", nFlags, "int")
        return result
    }
