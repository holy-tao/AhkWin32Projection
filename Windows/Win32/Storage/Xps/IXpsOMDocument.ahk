...
class IXpsOMDocument extends IXpsOMPart{
    ...
    GetOwner(documentSequence) {
        result := ComCall(5, this, "ptr", documentSequence, "int")
        if(result != 0)
    ...
    GetPageReferences(pageReferences) {
        result := ComCall(6, this, "ptr", pageReferences, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPrintTicketResource(printTicketResource) {
        result := ComCall(7, this, "ptr", printTicketResource, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDocumentStructureResource(documentStructureResource) {
        result := ComCall(9, this, "ptr", documentStructureResource, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetSignatureBlockResources(signatureBlockResources) {
        result := ComCall(11, this, "ptr", signatureBlockResources, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    Clone(document) {
        result := ComCall(12, this, "ptr", document, "int")
        if(result != 0)
            throw OSError(result)
