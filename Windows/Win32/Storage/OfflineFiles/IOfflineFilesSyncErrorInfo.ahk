...
class IOfflineFilesSyncErrorInfo extends IOfflineFilesErrorInfo{
    ...
    /**
     * 
     * @param {Pointer<Int32>} pSyncOp 
     * @returns {HRESULT} 
     */
    GetSyncOperation(pSyncOp) {
        result := ComCall(5, this, "int*", pSyncOp, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pdwItemChangeFlags 
     * @returns {HRESULT} 
     */
    GetItemChangeFlags(pdwItemChangeFlags) {
        result := ComCall(6, this, "uint*", pdwItemChangeFlags, "int")
        if(result != 0)
            throw OSError(result)
    ...
    InfoEnumerated(pbLocalEnumerated, pbRemoteEnumerated, pbOriginalEnumerated) {
        result := ComCall(7, this, "ptr", pbLocalEnumerated, "ptr", pbRemoteEnumerated, "ptr", pbOriginalEnumerated, "int")
        if(result != 0)
            throw OSError(result)
    ...
    InfoAvailable(pbLocalInfo, pbRemoteInfo, pbOriginalInfo) {
        result := ComCall(8, this, "ptr", pbLocalInfo, "ptr", pbRemoteInfo, "ptr", pbOriginalInfo, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetLocalInfo(ppInfo) {
        result := ComCall(9, this, "ptr", ppInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRemoteInfo(ppInfo) {
        result := ComCall(10, this, "ptr", ppInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetOriginalInfo(ppInfo) {
        result := ComCall(11, this, "ptr", ppInfo, "int")
        if(result != 0)
            throw OSError(result)
