...
class IFsrmFileGroupManager extends IDispatch{
    ...
    CreateFileGroup(fileGroup) {
        result := ComCall(7, this, "ptr", fileGroup, "int")
        if(result != 0)
    ...
    GetFileGroup(name, fileGroup) {
        ...
        result := ComCall(8, this, "ptr", name, "ptr", fileGroup, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumFileGroups(options, fileGroups) {
        result := ComCall(9, this, "int", options, "ptr", fileGroups, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ImportFileGroups(serializedFileGroups, fileGroupNamesArray, fileGroups) {
        ...
        result := ComCall(11, this, "ptr", serializedFileGroups, "ptr", fileGroupNamesArray, "ptr", fileGroups, "int")
        if(result != 0)
            throw OSError(result)
