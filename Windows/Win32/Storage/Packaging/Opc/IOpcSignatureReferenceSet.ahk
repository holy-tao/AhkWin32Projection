...
class IOpcSignatureReferenceSet extends IUnknown{
    ...
    Create(referenceUri, referenceId, type, digestMethod, transformMethod, reference) {
        ...
        result := ComCall(3, this, "ptr", referenceUri, "ptr", referenceId, "ptr", type, "ptr", digestMethod, "int", transformMethod, "ptr", reference, "int")
        if(result != 0)
    ...
    GetEnumerator(referenceEnumerator) {
        result := ComCall(5, this, "ptr", referenceEnumerator, "int")
        if(result != 0)
            throw OSError(result)
