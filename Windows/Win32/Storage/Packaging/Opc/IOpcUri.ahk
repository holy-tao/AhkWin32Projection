...
class IOpcUri extends IUri{
    ...
    GetRelationshipsPartUri(relationshipPartUri) {
        result := ComCall(28, this, "ptr", relationshipPartUri, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<IOpcPartUri>} targetPartUri 
     * @param {Pointer<IUri>} relativeUri 
     * @returns {HRESULT} 
     */
    GetRelativeUri(targetPartUri, relativeUri) {
        result := ComCall(29, this, "ptr", targetPartUri, "ptr", relativeUri, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CombinePartUri(relativeUri, combinedUri) {
        result := ComCall(30, this, "ptr", relativeUri, "ptr", combinedUri, "int")
        if(result != 0)
            throw OSError(result)
