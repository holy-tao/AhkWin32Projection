...
class IOpcPartSet extends IUnknown{
     ...
     */
    GetPart(name, part) {
        result := ComCall(3, this, "ptr", name, "ptr", part, "int")
        if(result != 0)
    ...
    CreatePart(name, contentType, compressionOptions, part) {
        ...
        result := ComCall(4, this, "ptr", name, "ptr", contentType, "int", compressionOptions, "ptr", part, "int")
        if(result != 0)
            throw OSError(result)
    ...
    PartExists(name, partExists) {
        result := ComCall(6, this, "ptr", name, "ptr", partExists, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetEnumerator(partEnumerator) {
        result := ComCall(7, this, "ptr", partEnumerator, "int")
        if(result != 0)
            throw OSError(result)
