...
class IOpcSignatureReference extends IUnknown{
     ...
     */
    GetId(referenceId) {
        result := ComCall(3, this, "ptr", referenceId, "int")
        if(result != 0)
     ...
     */
    GetUri(referenceUri) {
        result := ComCall(4, this, "ptr", referenceUri, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetType(type) {
        result := ComCall(5, this, "ptr", type, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDigestValue(digestValue, count) {
        result := ComCall(8, this, "char*", digestValue, "uint*", count, "int")
        if(result != 0)
            throw OSError(result)
