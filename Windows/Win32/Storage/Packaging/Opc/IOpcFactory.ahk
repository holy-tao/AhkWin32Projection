...
class IOpcFactory extends IUnknown{
    ...
    CreatePackageRootUri(rootUri) {
        result := ComCall(3, this, "ptr", rootUri, "int")
        if(result != 0)
    ...
    CreatePartUri(pwzUri, partUri) {
        ...
        result := ComCall(4, this, "ptr", pwzUri, "ptr", partUri, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateStreamOnFile(filename, ioMode, securityAttributes, dwFlagsAndAttributes, stream) {
        ...
        result := ComCall(5, this, "ptr", filename, "int", ioMode, "ptr", securityAttributes, "uint", dwFlagsAndAttributes, "ptr", stream, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreatePackage(package) {
        result := ComCall(6, this, "ptr", package, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ReadPackageFromStream(stream, flags, package) {
        result := ComCall(7, this, "ptr", stream, "int", flags, "ptr", package, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDigitalSignatureManager(package, signatureManager) {
        result := ComCall(9, this, "ptr", package, "ptr", signatureManager, "int")
        if(result != 0)
            throw OSError(result)
