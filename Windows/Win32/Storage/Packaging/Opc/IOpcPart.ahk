...
class IOpcPart extends IUnknown{
    ...
    GetRelationshipSet(relationshipSet) {
        result := ComCall(3, this, "ptr", relationshipSet, "int")
        if(result != 0)
    ...
    GetContentType(contentType) {
        result := ComCall(6, this, "ptr", contentType, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} compressionOptions 
     * @returns {HRESULT} 
     */
    GetCompressionOptions(compressionOptions) {
        result := ComCall(7, this, "int*", compressionOptions, "int")
        if(result != 0)
            throw OSError(result)
