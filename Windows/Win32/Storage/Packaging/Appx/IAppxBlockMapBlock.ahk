 ...
 * 
 * <div class="code"></div>
 * @see https://learn.microsoft.com/windows/win32/api//content/appxpackaging/nn-appxpackaging-iappxblockmapblock
...
class IAppxBlockMapBlock extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/appxpackaging/nf-appxpackaging-iappxblockmapblock-gethash
     */
    GetHash(bufferSize) {
        bufferSizeMarshal := bufferSize is VarRef ? "uint*" : "ptr"

        result := ComCall(3, this, bufferSizeMarshal, bufferSize, "ptr*", &buffer_ := 0, "int")
        if(result != 0) {
     ...
     * @returns {Integer} 
     * @see https://learn.microsoft.com/windows/win32/api//content/appxpackaging/nf-appxpackaging-iappxblockmapblock-getcompressedsize
     */
    GetCompressedSize() {
        result := ComCall(4, this, "uint*", &size_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
