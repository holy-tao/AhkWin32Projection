 ...
 */
export default struct IAppxBlockMapFile extends IUnknown {
    /**
     ...
     * used for IAppxBlockMapFile interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetBlocks              : IntPtr
        GetLocalFileHeaderSize : IntPtr
        GetName                : IntPtr
        GetUncompressedSize    : IntPtr
        ValidateFileHash       : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAppxBlockMapFile.Vtbl()
        }
     ...
     */
    GetName() {
        result := ComCall(5, this, PWSTR.Ptr, &name := 0, "HRESULT")
        return name
    }
    ...
    ValidateFileHash(fileStream) {
        result := ComCall(7, this, "ptr", fileStream, BOOL.Ptr, &isValid := 0, "HRESULT")
        return isValid
    }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetBlocks)
        CallbackFree(this.vtbl.GetLocalFileHeaderSize)
        CallbackFree(this.vtbl.GetName)
        CallbackFree(this.vtbl.GetUncompressedSize)
        CallbackFree(this.vtbl.ValidateFileHash)
    }
}
