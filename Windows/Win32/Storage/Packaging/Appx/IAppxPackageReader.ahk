...
class IAppxPackageReader extends IUnknown{
    ...
    GetBlockMap(blockMapReader) {
        result := ComCall(3, this, "ptr", blockMapReader, "int")
        if(result != 0)
    ...
    GetFootprintFile(type, file) {
        result := ComCall(4, this, "int", type, "ptr", file, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPayloadFile(fileName, file) {
        ...
        result := ComCall(5, this, "ptr", fileName, "ptr", file, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPayloadFiles(filesEnumerator) {
        result := ComCall(6, this, "ptr", filesEnumerator, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetManifest(manifestReader) {
        result := ComCall(7, this, "ptr", manifestReader, "int")
        if(result != 0)
            throw OSError(result)
