#Requires AutoHotkey v2.1-alpha.26+ 64-bit
#Import ".\COMPACT_VIRTUAL_DISK_VERSION.ahk" { COMPACT_VIRTUAL_DISK_VERSION }

/**
 * Contains virtual hard disk (VHD) compacting parameters.
 * @see https://learn.microsoft.com/windows/win32/api/virtdisk/ns-virtdisk-compact_virtual_disk_parameters
 * @namespace Windows.Win32.Storage.Vhd
 */
export default struct COMPACT_VIRTUAL_DISK_PARAMETERS {
    #StructPack 4


    struct _Version1 {
        Reserved : UInt32

    }

    /**
     * A <a href="https://docs.microsoft.com/windows/win32/api/virtdisk/ne-virtdisk-compact_virtual_disk_version">COMPACT_VIRTUAL_DISK_VERSION</a> 
     *      enumeration that specifies the version of the 
     *      <b>COMPACT_VIRTUAL_DISK_PARAMETERS</b> 
     *      structure being passed to or from the VHD functions.
     */
    Version : COMPACT_VIRTUAL_DISK_VERSION

    Version1 : COMPACT_VIRTUAL_DISK_PARAMETERS._Version1

}
