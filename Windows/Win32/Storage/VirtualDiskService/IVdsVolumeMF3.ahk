 ...
 * @see https://learn.microsoft.com/windows/win32/api/vds/nn-vds-ivdsvolumemf3
 * @namespace Windows.Win32.Storage.VirtualDiskService
 */
export default struct IVdsVolumeMF3 extends IUnknown {
    /**
     ...
     * used for IVdsVolumeMF3 interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        QueryVolumeGuidPathnames : IntPtr
        FormatEx2                : IntPtr
        OfflineVolume            : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVdsVolumeMF3.Vtbl()
        }
    ...
    FormatEx2(pwszFileSystemTypeName, usFileSystemRevision, ulDesiredUnitAllocationSize, pwszLabel, Options) {
        ...
        result := ComCall(4, this, "ptr", pwszFileSystemTypeName, UInt16, usFileSystemRevision, UInt32, ulDesiredUnitAllocationSize, "ptr", pwszLabel, UInt32, Options, "ptr*", &ppAsync := 0, "HRESULT")
    ...
    Query(iid) {
        if (IVdsVolumeMF3.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.QueryVolumeGuidPathnames)
        CallbackFree(this.vtbl.FormatEx2)
        CallbackFree(this.vtbl.OfflineVolume)
    }
}
