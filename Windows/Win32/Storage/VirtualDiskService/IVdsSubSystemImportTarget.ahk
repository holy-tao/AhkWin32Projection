 ...
 */
export default struct IVdsSubSystemImportTarget extends IUnknown {
    /**
     ...
     * used for IVdsSubSystemImportTarget interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetImportTarget : IntPtr
        SetImportTarget : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVdsSubSystemImportTarget.Vtbl()
        }
    ...
    Query(iid) {
        if (IVdsSubSystemImportTarget.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetImportTarget)
        CallbackFree(this.vtbl.SetImportTarget)
    }
}
