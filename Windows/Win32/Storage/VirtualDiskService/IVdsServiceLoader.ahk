 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/vds/nn-vds-ivdsserviceloader
...
class IVdsServiceLoader extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/vds/nf-vds-ivdsserviceloader-loadservice
     */
    LoadService(pwszMachineName) {
        pwszMachineName := pwszMachineName is String ? StrPtr(pwszMachineName) : pwszMachineName

        result := ComCall(3, this, "ptr", pwszMachineName, "ptr*", &ppService := 0, "int")
        if(result != 0) {
