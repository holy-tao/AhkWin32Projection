...
class IVdsAdvancedDisk extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<VDS_PARTITION_PROP>} ppPartitionPropArray 
     * @param {Pointer<Int32>} plNumberOfPartitions 
     * @returns {HRESULT} 
     */
    QueryPartitions(ppPartitionPropArray, plNumberOfPartitions) {
        result := ComCall(4, this, "ptr", ppPartitionPropArray, "int*", plNumberOfPartitions, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreatePartition(ullOffset, ullSize, para, ppAsync) {
        result := ComCall(5, this, "uint", ullOffset, "uint", ullSize, "ptr", para, "ptr", ppAsync, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FormatPartition(ullOffset, type, pwszLabel, dwUnitAllocationSize, bForce, bQuickFormat, bEnableCompression, ppAsync) {
        ...
        result := ComCall(11, this, "uint", ullOffset, "int", type, "ptr", pwszLabel, "uint", dwUnitAllocationSize, "int", bForce, "int", bQuickFormat, "int", bEnableCompression, "ptr", ppAsync, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Clean(bForce, bForceOEM, bFullClean, ppAsync) {
        result := ComCall(12, this, "int", bForce, "int", bForceOEM, "int", bFullClean, "ptr", ppAsync, "int")
        if(result != 0)
            throw OSError(result)
