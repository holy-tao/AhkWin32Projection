 ...
 */
export default struct IVdsServiceUninstallDisk extends IUnknown {
    /**
     ...
     * used for IVdsServiceUninstallDisk interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetDiskIdFromLunInfo : IntPtr
        UninstallDisks       : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVdsServiceUninstallDisk.Vtbl()
        }
    ...
    GetDiskIdFromLunInfo(pLunInfo) {
        pDiskId := Guid()
        result := ComCall(3, this, VDS_LUN_INFORMATION.Ptr, pLunInfo, Guid.Ptr, pDiskId, "HRESULT")
        return pDiskId
    }
    ...
    Query(iid) {
        if (IVdsServiceUninstallDisk.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetDiskIdFromLunInfo)
        CallbackFree(this.vtbl.UninstallDisks)
    }
}
