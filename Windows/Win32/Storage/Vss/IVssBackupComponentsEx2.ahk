 ...
 */
export default struct IVssBackupComponentsEx2 extends IVssBackupComponentsEx {
    /**
     ...
     * used for IVssBackupComponentsEx2 interfaces
    */
    struct Vtbl extends IVssBackupComponentsEx.Vtbl {
        UnexposeSnapshot        : IntPtr
        SetAuthoritativeRestore : IntPtr
        SetRollForward          : IntPtr
        SetRestoreName          : IntPtr
        BreakSnapshotSetEx      : IntPtr
        PreFastRecovery         : IntPtr
        FastRecovery            : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVssBackupComponentsEx2.Vtbl()
        }
    ...
    UnexposeSnapshot(snapshotId) {
        result := ComCall(53, this, Guid, snapshotId, "HRESULT")
        return result
    }
    ...
    SetRollForward(writerId, ct, wszLogicalPath, wszComponentName, rollType, wszRollForwardPoint) {
        ...
        result := ComCall(55, this, Guid, writerId, VSS_COMPONENT_TYPE, ct, "ptr", wszLogicalPath, "ptr", wszComponentName, VSS_ROLLFORWARD_TYPE, rollType, "ptr", wszRollForwardPoint, "HRESULT")
        return result
    }
    ...
    SetRestoreName(writerId, ct, wszLogicalPath, wszComponentName, wszRestoreName) {
        ...
        result := ComCall(56, this, Guid, writerId, VSS_COMPONENT_TYPE, ct, "ptr", wszLogicalPath, "ptr", wszComponentName, "ptr", wszRestoreName, "HRESULT")
        return result
    }
    ...
    Query(iid) {
        if (IVssBackupComponentsEx2.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.UnexposeSnapshot)
        CallbackFree(this.vtbl.SetAuthoritativeRestore)
        CallbackFree(this.vtbl.SetRollForward)
        CallbackFree(this.vtbl.SetRestoreName)
        CallbackFree(this.vtbl.BreakSnapshotSetEx)
        CallbackFree(this.vtbl.PreFastRecovery)
        CallbackFree(this.vtbl.FastRecovery)
    }
}
