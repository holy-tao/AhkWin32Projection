...
class IVssSnapshotMgmt extends IUnknown{
    ...
    GetProviderMgmtInterface(ProviderId, InterfaceId, ppItf) {
        result := ComCall(3, this, "ptr", ProviderId, "ptr", InterfaceId, "ptr", ppItf, "int")
        if(result != 0)
    ...
    QueryVolumesSupportedForSnapshots(ProviderId, lContext, ppEnum) {
        result := ComCall(4, this, "ptr", ProviderId, "int", lContext, "ptr", ppEnum, "int")
        if(result != 0)
            throw OSError(result)
    ...
    QuerySnapshotsByVolume(pwszVolumeName, ProviderId, ppEnum) {
        result := ComCall(5, this, "ushort*", pwszVolumeName, "ptr", ProviderId, "ptr", ppEnum, "int")
        if(result != 0)
            throw OSError(result)
