 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/vsmgmt/nn-vsmgmt-ivsssnapshotmgmt
...
class IVssSnapshotMgmt extends IUnknown{
     ...
     *       interface.
     * @returns {Pointer<IUnknown>} Address of an interface pointer that is filled in with the returned interface pointer.
     * @see https://learn.microsoft.com/windows/win32/api//content/vsmgmt/nf-vsmgmt-ivsssnapshotmgmt-getprovidermgmtinterface
     */
    GetProviderMgmtInterface(ProviderId, InterfaceId) {
        result := ComCall(3, this, "ptr", ProviderId, "ptr", InterfaceId, "ptr*", &ppItf := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/vsmgmt/nf-vsmgmt-ivsssnapshotmgmt-queryvolumessupportedforsnapshots
     */
    QueryVolumesSupportedForSnapshots(ProviderId, lContext) {
        result := ComCall(4, this, "ptr", ProviderId, "int", lContext, "ptr*", &ppEnum := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/vsmgmt/nf-vsmgmt-ivsssnapshotmgmt-querysnapshotsbyvolume
     */
    QuerySnapshotsByVolume(pwszVolumeName, ProviderId) {
        pwszVolumeNameMarshal := pwszVolumeName is VarRef ? "ushort*" : "ptr"

        result := ComCall(5, this, pwszVolumeNameMarshal, pwszVolumeName, "ptr", ProviderId, "ptr*", &ppEnum := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
