 ...
 */
export default struct IVssExamineWriterMetadataEx extends IVssExamineWriterMetadata {
    /**
     ...
     * used for IVssExamineWriterMetadataEx interfaces
    */
    struct Vtbl extends IVssExamineWriterMetadata.Vtbl {
        GetIdentityEx : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVssExamineWriterMetadataEx.Vtbl()
        }
    ...
    Query(iid) {
        if (IVssExamineWriterMetadataEx.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetIdentityEx)
    }
}
