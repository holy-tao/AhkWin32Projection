...
;@region Methods
     ...
     * @returns {Integer} Pointer to a caller-allocated variable that receives the number of bytes returned in the buffer that <i>lpBuffer</i> points to if the call to <b>FilterFindFirst</b> succeeds. This parameter is required and cannot be <b>NULL</b>.
     * @see https://docs.microsoft.com/windows/win32/api//fltuser/nf-fltuser-filterfindfirst
     */
    static FilterFindFirst(dwInformationClass, lpBuffer, dwBufferSize, lpFilterFind) {
        result := DllCall("FLTLIB.dll\FilterFindFirst", "int", dwInformationClass, "ptr", lpBuffer, "uint", dwBufferSize, "uint*", &lpBytesReturned := 0, "ptr", lpFilterFind, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {Integer} Pointer to a caller-allocated variable that receives the number of bytes returned in the buffer that <i>lpBuffer</i> points to if the call to <b>FilterVolumeFindFirst</b> succeeds. This parameter is required and cannot be <b>NULL</b>.
     * @see https://docs.microsoft.com/windows/win32/api//fltuser/nf-fltuser-filtervolumefindfirst
     */
    static FilterVolumeFindFirst(dwInformationClass, lpBuffer, dwBufferSize, lpVolumeFind) {
        result := DllCall("FLTLIB.dll\FilterVolumeFindFirst", "int", dwInformationClass, "ptr", lpBuffer, "uint", dwBufferSize, "uint*", &lpBytesReturned := 0, "ptr", lpVolumeFind, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {Integer} Pointer to a caller-allocated variable that receives the number of bytes returned in the buffer that <i>lpBuffer</i> points to, if the call to <b>FilterInstanceFindFirst</b> succeeds. This parameter is required and cannot be <b>NULL</b>.
     * @see https://docs.microsoft.com/windows/win32/api//fltuser/nf-fltuser-filterinstancefindfirst
     */
    static FilterInstanceFindFirst(lpFilterName, dwInformationClass, lpBuffer, dwBufferSize, lpFilterInstanceFind) {
        lpFilterName := lpFilterName is String ? StrPtr(lpFilterName) : lpFilterName

        result := DllCall("FLTLIB.dll\FilterInstanceFindFirst", "ptr", lpFilterName, "int", dwInformationClass, "ptr", lpBuffer, "uint", dwBufferSize, "uint*", &lpBytesReturned := 0, "ptr", lpFilterInstanceFind, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {Integer} Pointer to a caller-allocated variable that receives the number of bytes returned in the buffer that <i>lpBuffer</i> points to if the call to <b>FilterVolumeInstanceFindFirst</b> succeeds. This parameter is required and cannot be <b>NULL</b>.
     * @see https://docs.microsoft.com/windows/win32/api//fltuser/nf-fltuser-filtervolumeinstancefindfirst
     */
    static FilterVolumeInstanceFindFirst(lpVolumeName, dwInformationClass, lpBuffer, dwBufferSize, lpVolumeInstanceFind) {
        lpVolumeName := lpVolumeName is String ? StrPtr(lpVolumeName) : lpVolumeName

        result := DllCall("FLTLIB.dll\FilterVolumeInstanceFindFirst", "ptr", lpVolumeName, "int", dwInformationClass, "ptr", lpBuffer, "uint", dwBufferSize, "uint*", &lpBytesReturned := 0, "ptr", lpVolumeInstanceFind, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
