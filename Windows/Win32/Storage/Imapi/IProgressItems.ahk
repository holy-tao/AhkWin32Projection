...
class IProgressItems extends IDispatch{
    ...
    get__NewEnum(NewEnum) {
        result := ComCall(7, this, "ptr", NewEnum, "int")
        if(result != 0)
    ...
    ProgressItemFromBlock(block, item) {
        result := ComCall(10, this, "uint", block, "ptr", item, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ProgressItemFromDescription(description, item) {
        ...
        result := ComCall(11, this, "ptr", description, "ptr", item, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_EnumProgressItems(NewEnum) {
        result := ComCall(12, this, "ptr", NewEnum, "int")
        if(result != 0)
            throw OSError(result)
