...
;@region Methods
     ...
     * @param {Pointer<IMalloc>} lpMalloc > [in] Pointer to a memory allocator object exposing the OLE [IMalloc](/windows/desktop/api/objidl/nn-objidl-imalloc) interface. MAPI needs to use this allocation method when working with the OLE [IStorage](/windows/desktop/api/objidl/nn-objidl-istorage) interface.
     * @param {Integer} ulFlags > [in] Reserved; must be zero.
     * @param {Pointer<IntPtr>} lppMsgSess > [out] Pointer to a pointer to the returned message session object.
     ...
     * @param {Pointer} lpMsgSess > [in] Pointer to a message session object within which the new **IMessage**-on- **IStorage** object is to be created.
     ...
     * @param {Pointer<IMalloc>} lpMalloc > [in] Pointer to a memory allocator object exposing the OLE **IMalloc** interface. The **IMessage** interface needs to use this allocation method when working with interfaces such as **IStorage** and **IStream**.
     ...
     * @param {Pointer<IStorage>} lpStg > [in, out] Pointer to an OLE **IStorage** object that is open and has read-only or read/write permission. Because **IMessage** does not support write-only access, **OpenIMsgOnIStg** does not accept a storage object opened in write-only mode.
     * @param {Pointer<MSGCALLRELEASE>} lpfMsgCallRelease > [in] Optional pointer to a callback function based on the [MSGCALLRELEASE](msgcallrelease.md) prototype that MAPI is to call following the last release on the **IMessage**-on- **IStorage** object.
