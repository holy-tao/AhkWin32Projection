 ...
 * @charset Unicode
 */
export default struct IDirectInputDeviceW extends IUnknown {
    /**
     * The interface identifier for IDirectInputDeviceW
     * @type {Guid}
     */
    static IID := Guid("{5944e681-c92e-11cf-bfc7-444553540000}")
     ...
     * used for IDirectInputDeviceW interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDirectInputDeviceW.Vtbl()
        }
    ...
    _Query(iid) {
        if (IDirectInputDeviceW.IID.Equals(iid)) {
            return true
        }
