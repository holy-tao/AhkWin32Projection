 ...
 */
export default struct IWSDSignatureProperty extends IUnknown {
    /**
     ...
     * used for IWSDSignatureProperty interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        IsMessageSigned           : IntPtr
        IsMessageSignatureTrusted : IntPtr
        GetKeyInfo                : IntPtr
        GetSignature              : IntPtr
        GetSignedInfoHash         : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWSDSignatureProperty.Vtbl()
        }
     ...
     */
    IsMessageSigned() {
        result := ComCall(3, this, BOOL.Ptr, &pbSigned := 0, "HRESULT")
        return pbSigned
    }
    ...
    IsMessageSignatureTrusted() {
        result := ComCall(4, this, BOOL.Ptr, &pbSignatureTrusted := 0, "HRESULT")
    ...
    Query(iid) {
        if (IWSDSignatureProperty.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.IsMessageSigned)
        CallbackFree(this.vtbl.IsMessageSignatureTrusted)
        CallbackFree(this.vtbl.GetKeyInfo)
        CallbackFree(this.vtbl.GetSignature)
        CallbackFree(this.vtbl.GetSignedInfoHash)
    }
}
