...
class IWSDXMLContext extends IUnknown{
     ...
     * @param {Pointer<WSDXML_NAMESPACE>} ppNamespace 
     * @returns {HRESULT} 
     */
    AddNamespace(pszUri, pszSuggestedPrefix, ppNamespace) {
        ...
        result := ComCall(3, this, "ptr", pszUri, "ptr", pszSuggestedPrefix, "ptr", ppNamespace, "int")
        if(result != 0)
     ...
     * @param {PWSTR} pszUri 
     * @param {PWSTR} pszName 
     * @param {Pointer<WSDXML_NAME>} ppName 
     * @returns {HRESULT} 
     */
    AddNameToNamespace(pszUri, pszName, ppName) {
        ...
        result := ComCall(4, this, "ptr", pszUri, "ptr", pszName, "ptr", ppName, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<WSDXML_NAMESPACE>} pNamespaces 
     * @param {Integer} wNamespacesCount 
     * @param {Integer} bLayerNumber 
     * @returns {HRESULT} 
     */
    SetNamespaces(pNamespaces, wNamespacesCount, bLayerNumber) {
        result := ComCall(5, this, "ptr", pNamespaces, "ushort", wNamespacesCount, "char", bLayerNumber, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<WSDXML_TYPE>} pTypes 
     * @param {Integer} dwTypesCount 
     * @param {Integer} bLayerNumber 
     * @returns {HRESULT} 
     */
    SetTypes(pTypes, dwTypesCount, bLayerNumber) {
        result := ComCall(6, this, "ptr", pTypes, "uint", dwTypesCount, "char", bLayerNumber, "int")
        if(result != 0)
            throw OSError(result)
