 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/wsdxml/nn-wsdxml-iwsdxmlcontext
...
class IWSDXMLContext extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wsdxml/nf-wsdxml-iwsdxmlcontext-addnamespace
     */
    AddNamespace(pszUri, pszSuggestedPrefix) {
        ...
        result := ComCall(3, this, "ptr", pszUri, "ptr", pszSuggestedPrefix, "ptr*", &ppNamespace := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wsdxml/nf-wsdxml-iwsdxmlcontext-addnametonamespace
     */
    AddNameToNamespace(pszUri, pszName) {
        pszUri := pszUri is String ? StrPtr(pszUri) : pszUri
        pszName := pszName is String ? StrPtr(pszName) : pszName

        result := ComCall(4, this, "ptr", pszUri, "ptr", pszName, "ptr*", &ppName := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/wsdxml/nf-wsdxml-iwsdxmlcontext-setnamespaces
     */
    SetNamespaces(pNamespaces, wNamespacesCount, bLayerNumber) {
        pNamespacesMarshal := pNamespaces is VarRef ? "ptr*" : "ptr"

        result := ComCall(5, this, pNamespacesMarshal, pNamespaces, "ushort", wNamespacesCount, "char", bLayerNumber, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/wsdxml/nf-wsdxml-iwsdxmlcontext-settypes
     */
    SetTypes(pTypes, dwTypesCount, bLayerNumber) {
        pTypesMarshal := pTypes is VarRef ? "ptr*" : "ptr"

        result := ComCall(6, this, pTypesMarshal, pTypes, "uint", dwTypesCount, "char", bLayerNumber, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
