...
class IWSDEndpointProxy extends IUnknown{
    ...
    SendOneWayRequest(pBody, pOperation) {
        result := ComCall(3, this, "ptr", pBody, "ptr", pOperation, "int")
        if(result != 0)
    ...
    SendTwoWayRequest(pBody, pOperation, pResponseContext) {
        result := ComCall(4, this, "ptr", pBody, "ptr", pOperation, "ptr", pResponseContext, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SendTwoWayRequestAsync(pBody, pOperation, pAsyncState, pCallback, pResult) {
        result := ComCall(5, this, "ptr", pBody, "ptr", pOperation, "ptr", pAsyncState, "ptr", pCallback, "ptr", pResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetErrorInfo(ppszErrorInfo) {
        result := ComCall(8, this, "ptr", ppszErrorInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<WSD_SOAP_FAULT>} ppFault 
     * @returns {HRESULT} 
     */
    GetFaultInfo(ppFault) {
        result := ComCall(9, this, "ptr", ppFault, "int")
        if(result != 0)
            throw OSError(result)
