 ...
 */
export default struct IWiaTransferCallback extends IUnknown {
    /**
     ...
     * used for IWiaTransferCallback interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        TransferCallback : IntPtr
        GetNextStream    : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWiaTransferCallback.Vtbl()
        }
    ...
    TransferCallback(lFlags, pWiaTransferParams) {
        result := ComCall(3, this, Int32, lFlags, WiaTransferParams.Ptr, pWiaTransferParams, "HRESULT")
        return result
    }
    ...
    GetNextStream(lFlags, bstrItemName, bstrFullItemName) {
        ...
        result := ComCall(4, this, Int32, lFlags, BSTR, bstrItemName, BSTR, bstrFullItemName, "ptr*", &ppDestination := 0, "HRESULT")
        return IStream(ppDestination)
    }

    Query(iid) {
        if (IWiaTransferCallback.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.TransferCallback)
        CallbackFree(this.vtbl.GetNextStream)
    }
}
