...
class IWiaPropertyStorage extends IUnknown{
     ...
     * @param {Integer} cpspec 
     * @param {Pointer<PROPSPEC>} rgpspec 
     * @param {Pointer<UInt32>} rgflags 
     * @param {Pointer<PROPVARIANT>} rgpropvar 
     * @returns {HRESULT} 
     */
    GetPropertyAttributes(cpspec, rgpspec, rgflags, rgpropvar) {
        result := ComCall(15, this, "uint", cpspec, "ptr", rgpspec, "uint*", rgflags, "ptr", rgpropvar, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pulNumProps 
     * @returns {HRESULT} 
     */
    GetCount(pulNumProps) {
        result := ComCall(16, this, "uint*", pulNumProps, "int")
        if(result != 0)
            throw OSError(result)
