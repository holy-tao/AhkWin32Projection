...
class IWiaMiniDrvTransferCallback extends IUnknown{
     ...
     * @returns {HRESULT} 
     */
    GetNextStream(lFlags, bstrItemName, bstrFullItemName, ppIStream) {
        bstrItemName := bstrItemName is String ? BSTR.Alloc(bstrItemName).Value : bstrItemName
        bstrFullItemName := bstrFullItemName is String ? BSTR.Alloc(bstrFullItemName).Value : bstrFullItemName

        result := ComCall(3, this, "int", lFlags, "ptr", bstrItemName, "ptr", bstrFullItemName, "ptr", ppIStream, "int")
        if(result != 0)
    ...
    SendMessage(lFlags, pWiaTransferParams) {
        result := ComCall(4, this, "int", lFlags, "ptr", pWiaTransferParams, "int")
        if(result != 0)
            throw OSError(result)
