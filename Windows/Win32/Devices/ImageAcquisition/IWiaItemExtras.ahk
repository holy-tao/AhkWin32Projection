...
class IWiaItemExtras extends IUnknown{
     ...
     * @param {Pointer<Byte>} lpInData 
     * @param {Integer} cbInDataSize 
     * @param {Pointer<Byte>} pOutData 
     * @param {Integer} dwOutDataSize 
     * @param {Pointer<UInt32>} pdwActualDataSize 
    ...
    Escape(dwEscapeCode, lpInData, cbInDataSize, pOutData, dwOutDataSize, pdwActualDataSize) {
        result := ComCall(4, this, "uint", dwEscapeCode, "char*", lpInData, "uint", cbInDataSize, "char*", pOutData, "uint", dwOutDataSize, "uint*", pdwActualDataSize, "int")
        if(result != 0)
            throw OSError(result)
