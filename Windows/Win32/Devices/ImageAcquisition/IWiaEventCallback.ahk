 ...
 * </tr>
 * </table>
 * @see https://learn.microsoft.com/windows/win32/api//content/wia_xp/nn-wia_xp-iwiaeventcallback
...
class IWiaEventCallback extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wia_xp/nf-wia_xp-iwiaeventcallback-imageeventcallback
     */
    ImageEventCallback(pEventGUID, bstrEventDescription, bstrDeviceID, bstrDeviceDescription, dwDeviceType, bstrFullItemName, pulEventType, ulReserved) {
        if(bstrEventDescription is String) {
            pin := BSTR.Alloc(bstrEventDescription)
            bstrEventDescription := pin.Value
        }
        if(bstrDeviceID is String) {
            pin := BSTR.Alloc(bstrDeviceID)
            bstrDeviceID := pin.Value
        }
        if(bstrDeviceDescription is String) {
            pin := BSTR.Alloc(bstrDeviceDescription)
            bstrDeviceDescription := pin.Value
        }
        ...
        result := ComCall(3, this, "ptr", pEventGUID, "ptr", bstrEventDescription, "ptr", bstrDeviceID, "ptr", bstrDeviceDescription, "uint", dwDeviceType, "ptr", bstrFullItemName, pulEventTypeMarshal, pulEventType, "uint", ulReserved, "int")
        if(result != 0) {
