 ...
 */
export default struct IWiaDataCallback extends IUnknown {
    /**
     ...
     * used for IWiaDataCallback interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        BandedDataCallback : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWiaDataCallback.Vtbl()
        }
    ...
    Query(iid) {
        if (IWiaDataCallback.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.BandedDataCallback)
    }
}
