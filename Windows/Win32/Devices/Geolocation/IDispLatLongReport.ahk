#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\System\Com\IDispatch.ahk

/**
 * @namespace Windows.Win32.Devices.Geolocation
 * @version v4.0.30319
 */
class IDispLatLongReport extends IDispatch{
    /**
     * The interface identifier for IDispLatLongReport
     * @type {Guid}
     */
    static IID => Guid("{8ae32723-389b-4a11-9957-5bdd48fc9617}")

    /**
     * The class identifier for DispLatLongReport
     * @type {Guid}
     */
    static CLSID => Guid("{7a7c3277-8f84-4636-95b2-ebb5507ff77e}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 7

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_Latitude(pVal) {
        result := ComCall(7, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_Longitude(pVal) {
        result := ComCall(8, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_ErrorRadius(pVal) {
        result := ComCall(9, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_Altitude(pVal) {
        result := ComCall(10, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_AltitudeError(pVal) {
        result := ComCall(11, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Double>} pVal 
     * @returns {HRESULT} 
     */
    get_Timestamp(pVal) {
        result := ComCall(12, this, "double*", pVal, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
