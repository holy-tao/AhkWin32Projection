 ...
 */
export default struct ILocationReport extends IUnknown {
    /**
     ...
     * used for ILocationReport interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ILocationReport.Vtbl()
        }
    ...
    GetSensorID() {
        pSensorID := Guid()
        result := ComCall(3, this, Guid.Ptr, pSensorID, "HRESULT")
        return pSensorID
    }
    ...
    GetTimestamp() {
        ...
        result := ComCall(4, this, SYSTEMTIME.Ptr, pCreationTime, "HRESULT")
    ...
    GetValue(pKey) {
        pValue := PROPVARIANT()
        result := ComCall(5, this, PROPERTYKEY.Ptr, pKey, PROPVARIANT.Ptr, pValue, "HRESULT")
        return pValue
    }

    Query(iid) {
        if (ILocationReport.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetSensorID)
