 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nn-upnphost-iupnpregistrar
...
class IUPnPRegistrar extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-registerdevice
     */
    RegisterDevice(bstrXMLDesc, bstrProgIDDeviceControlClass, bstrInitString, bstrContainerId, bstrResourcePath, nLifeTime) {
        ...
        if(bstrProgIDDeviceControlClass is String) {
            pin := BSTR.Alloc(bstrProgIDDeviceControlClass)
            bstrProgIDDeviceControlClass := pin.Value
        }
        if(bstrInitString is String) {
            pin := BSTR.Alloc(bstrInitString)
            bstrInitString := pin.Value
        }
        if(bstrContainerId is String) {
            pin := BSTR.Alloc(bstrContainerId)
            bstrContainerId := pin.Value
        }
        if(bstrResourcePath is String) {
            pin := BSTR.Alloc(bstrResourcePath)
            bstrResourcePath := pin.Value
        }

        pbstrDeviceIdentifier := BSTR()
        result := ComCall(3, this, "ptr", bstrXMLDesc, "ptr", bstrProgIDDeviceControlClass, "ptr", bstrInitString, "ptr", bstrContainerId, "ptr", bstrResourcePath, "int", nLifeTime, "ptr", pbstrDeviceIdentifier, "int")
        if(result != 0) {
     ...
     * @returns {BSTR} Receives the device identifier. Use this identifier when unregistering or re-registering the device. Save this Device ID; you must use it when calling 
     * <a href="https://docs.microsoft.com/windows/desktop/api/upnphost/nf-upnphost-iupnpregistrar-unregisterdevice">UnregisterDevice</a>.
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-registerrunningdevice
     */
    RegisterRunningDevice(bstrXMLDesc, punkDeviceControl, bstrInitString, bstrResourcePath, nLifeTime) {
        ...
        pbstrDeviceIdentifier := BSTR()
        result := ComCall(4, this, "ptr", bstrXMLDesc, "ptr", punkDeviceControl, "ptr", bstrInitString, "ptr", bstrResourcePath, "int", nLifeTime, "ptr", pbstrDeviceIdentifier, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-registerdeviceprovider
     */
    RegisterDeviceProvider(bstrProviderName, bstrProgIDProviderClass, bstrInitString, bstrContainerId) {
        ...
        if(bstrProgIDProviderClass is String) {
            pin := BSTR.Alloc(bstrProgIDProviderClass)
            bstrProgIDProviderClass := pin.Value
        }
        if(bstrInitString is String) {
            pin := BSTR.Alloc(bstrInitString)
            bstrInitString := pin.Value
        ...
        result := ComCall(5, this, "ptr", bstrProviderName, "ptr", bstrProgIDProviderClass, "ptr", bstrInitString, "ptr", bstrContainerId, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-getuniquedevicename
     */
    GetUniqueDeviceName(bstrDeviceIdentifier, bstrTemplateUDN) {
        ...
        if(bstrTemplateUDN is String) {
            pin := BSTR.Alloc(bstrTemplateUDN)
            bstrTemplateUDN := pin.Value
        }

        pbstrUDN := BSTR()
        result := ComCall(6, this, "ptr", bstrDeviceIdentifier, "ptr", bstrTemplateUDN, "ptr", pbstrUDN, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, the return value is S_OK. Otherwise, the method returns one of the COM error codes defined in WinError.h.
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-unregisterdevice
     */
    UnregisterDevice(bstrDeviceIdentifier, fPermanent) {
        ...
        result := ComCall(7, this, "ptr", bstrDeviceIdentifier, "int", fPermanent, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, the return value is S_OK. Otherwise, the method returns one of the COM error codes defined in WinError.h.
     * @see https://learn.microsoft.com/windows/win32/api//content/upnphost/nf-upnphost-iupnpregistrar-unregisterdeviceprovider
     */
    UnregisterDeviceProvider(bstrProviderName) {
        ...
        result := ComCall(8, this, "ptr", bstrProviderName, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
