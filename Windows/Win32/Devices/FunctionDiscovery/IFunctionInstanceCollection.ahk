...
class IFunctionInstanceCollection extends IUnknown{
    ...
    Get(pszInstanceIdentity, pdwIndex, ppIFunctionInstance) {
        ...
        result := ComCall(4, this, "ptr", pszInstanceIdentity, "uint*", pdwIndex, "ptr", ppIFunctionInstance, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Item(dwIndex, ppIFunctionInstance) {
        result := ComCall(5, this, "uint", dwIndex, "ptr", ppIFunctionInstance, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Remove(dwIndex, ppIFunctionInstance) {
        result := ComCall(7, this, "uint", dwIndex, "ptr", ppIFunctionInstance, "int")
        if(result != 0)
            throw OSError(result)
