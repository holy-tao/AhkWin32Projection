 ...
 */
export default struct IPortableDevice extends IUnknown {
    /**
     ...
     * used for IPortableDevice interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        ...
        Advise         : IntPtr
        Unadvise       : IntPtr
        GetPnPDeviceID : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPortableDevice.Vtbl()
        }
     ...
     */
    GetPnPDeviceID() {
        result := ComCall(11, this, PWSTR.Ptr, &ppszPnPDeviceID := 0, "HRESULT")
        return ppszPnPDeviceID
    }

    Query(iid) {
        if (IPortableDevice.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetPnPDeviceID)
    }
}
