...
class IPortableDeviceContent extends IUnknown{
     ...
     * @param {Pointer<IPortableDeviceValues>} pFilter 
    ...
    EnumObjects(dwFlags, pszParentObjectID, pFilter, ppEnum) {
        ...
        result := ComCall(3, this, "uint", dwFlags, "ptr", pszParentObjectID, "ptr", pFilter, "ptr", ppEnum, "int")
        if(result != 0)
    ...
    Properties(ppProperties) {
        result := ComCall(4, this, "ptr", ppProperties, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Transfer(ppResources) {
        result := ComCall(5, this, "ptr", ppResources, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateObjectWithPropertiesOnly(pValues, ppszObjectID) {
        result := ComCall(6, this, "ptr", pValues, "ptr", ppszObjectID, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateObjectWithPropertiesAndData(pValues, ppData, pdwOptimalWriteBufferSize, ppszCookie) {
        result := ComCall(7, this, "ptr", pValues, "ptr", ppData, "uint*", pdwOptimalWriteBufferSize, "ptr", ppszCookie, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IPortableDevicePropVariantCollection>} pPersistentUniqueIDs 
    ...
    GetObjectIDsFromPersistentUniqueIDs(pPersistentUniqueIDs, ppObjectIDs) {
        result := ComCall(9, this, "ptr", pPersistentUniqueIDs, "ptr", ppObjectIDs, "int")
        if(result != 0)
            throw OSError(result)
