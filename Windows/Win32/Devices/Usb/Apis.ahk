...
;@region Methods
     ...
     */
    static WinUsb_GetDescriptor(InterfaceHandle, DescriptorType, Index, LanguageID, _Buffer, BufferLength, LengthTransferred) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_GetDescriptor", "ptr", InterfaceHandle, "char", DescriptorType, "char", Index, "ushort", LanguageID, "ptr", _Buffer, "uint", BufferLength, LengthTransferredMarshal, LengthTransferred, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     */
    static WinUsb_QueryDeviceInformation(InterfaceHandle, InformationType, BufferLength, _Buffer) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_QueryDeviceInformation", "ptr", InterfaceHandle, "uint", InformationType, BufferLengthMarshal, BufferLength, "ptr", _Buffer, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     */
    static WinUsb_ReadPipe(InterfaceHandle, PipeID, _Buffer, BufferLength, LengthTransferred, _Overlapped) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_ReadPipe", "ptr", InterfaceHandle, "char", PipeID, "ptr", _Buffer, "uint", BufferLength, LengthTransferredMarshal, LengthTransferred, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     */
    static WinUsb_WritePipe(InterfaceHandle, PipeID, _Buffer, BufferLength, LengthTransferred, _Overlapped) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_WritePipe", "ptr", InterfaceHandle, "char", PipeID, "ptr", _Buffer, "uint", BufferLength, LengthTransferredMarshal, LengthTransferred, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     */
    static WinUsb_ControlTransfer(InterfaceHandle, SetupPacket, _Buffer, BufferLength, LengthTransferred, _Overlapped) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_ControlTransfer", "ptr", InterfaceHandle, "ptr", SetupPacket, "ptr", _Buffer, "uint", BufferLength, LengthTransferredMarshal, LengthTransferred, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_GetCurrentFrameNumber(InterfaceHandle, CurrentFrameNumber, _TimeStamp) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_GetCurrentFrameNumber", "ptr", InterfaceHandle, CurrentFrameNumberMarshal, CurrentFrameNumber, _TimeStampMarshal, _TimeStamp, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_GetAdjustedFrameNumber(CurrentFrameNumber, _TimeStamp) {
        CurrentFrameNumberMarshal := CurrentFrameNumber is VarRef ? "uint*" : "ptr"

        A_LastError := 0

        result := DllCall("WINUSB.dll\WinUsb_GetAdjustedFrameNumber", CurrentFrameNumberMarshal, CurrentFrameNumber, "int64", _TimeStamp, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_RegisterIsochBuffer(InterfaceHandle, PipeID, _Buffer, BufferLength, IsochBufferHandle) {
        InterfaceHandle := InterfaceHandle is Win32Handle ? NumGet(InterfaceHandle, "ptr") : InterfaceHandle
        ...
        result := DllCall("WINUSB.dll\WinUsb_RegisterIsochBuffer", "ptr", InterfaceHandle, "char", PipeID, "ptr", _Buffer, "uint", BufferLength, IsochBufferHandleMarshal, IsochBufferHandle, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_WriteIsochPipe(BufferHandle, Offset, Length, FrameNumber, _Overlapped) {
        BufferHandleMarshal := BufferHandle is VarRef ? "ptr" : "ptr"
        FrameNumberMarshal := FrameNumber is VarRef ? "uint*" : "ptr"

        A_LastError := 0

        result := DllCall("WINUSB.dll\WinUsb_WriteIsochPipe", BufferHandleMarshal, BufferHandle, "uint", Offset, "uint", Length, FrameNumberMarshal, FrameNumber, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_ReadIsochPipe(BufferHandle, Offset, Length, FrameNumber, NumberOfPackets, IsoPacketDescriptors, _Overlapped) {
        BufferHandleMarshal := BufferHandle is VarRef ? "ptr" : "ptr"
        FrameNumberMarshal := FrameNumber is VarRef ? "uint*" : "ptr"

        A_LastError := 0

        result := DllCall("WINUSB.dll\WinUsb_ReadIsochPipe", BufferHandleMarshal, BufferHandle, "uint", Offset, "uint", Length, FrameNumberMarshal, FrameNumber, "uint", NumberOfPackets, "ptr", IsoPacketDescriptors, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_WriteIsochPipeAsap(BufferHandle, Offset, Length, ContinueStream, _Overlapped) {
        BufferHandleMarshal := BufferHandle is VarRef ? "ptr" : "ptr"

        A_LastError := 0

        result := DllCall("WINUSB.dll\WinUsb_WriteIsochPipeAsap", BufferHandleMarshal, BufferHandle, "uint", Offset, "uint", Length, "int", ContinueStream, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows8.1
     */
    static WinUsb_ReadIsochPipeAsap(BufferHandle, Offset, Length, ContinueStream, NumberOfPackets, IsoPacketDescriptors, _Overlapped) {
        BufferHandleMarshal := BufferHandle is VarRef ? "ptr" : "ptr"

        A_LastError := 0

        result := DllCall("WINUSB.dll\WinUsb_ReadIsochPipeAsap", BufferHandleMarshal, BufferHandle, "uint", Offset, "uint", Length, "int", ContinueStream, "uint", NumberOfPackets, "ptr", IsoPacketDescriptors, "ptr", _Overlapped, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
