#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\System\Com\IDispatch.ahk

/**
 * Gets and sets information concerning a queue. The IEnumQueue::Next and ITACDGroup::get_Queues methods create the ITQueue interface.
 * @see https://docs.microsoft.com/windows/win32/api//tapi3cc/nn-tapi3cc-itqueue
 * @namespace Windows.Win32.Devices.Tapi
 * @version v4.0.30319
 */
class ITQueue extends IDispatch{
    /**
     * The interface identifier for ITQueue
     * @type {Guid}
     */
    static IID => Guid("{5afc3149-4bcc-11d1-bf80-00805fc147d3}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 7

    /**
     * 
     * @param {Integer} lPeriod 
     * @returns {HRESULT} 
     */
    put_MeasurementPeriod(lPeriod) {
        result := ComCall(7, this, "int", lPeriod, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plPeriod 
     * @returns {HRESULT} 
     */
    get_MeasurementPeriod(plPeriod) {
        result := ComCall(8, this, "int*", plPeriod, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_TotalCallsQueued(plCalls) {
        result := ComCall(9, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_CurrentCallsQueued(plCalls) {
        result := ComCall(10, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_TotalCallsAbandoned(plCalls) {
        result := ComCall(11, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_TotalCallsFlowedIn(plCalls) {
        result := ComCall(12, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_TotalCallsFlowedOut(plCalls) {
        result := ComCall(13, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plWaitTime 
     * @returns {HRESULT} 
     */
    get_LongestEverWaitTime(plWaitTime) {
        result := ComCall(14, this, "int*", plWaitTime, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plWaitTime 
     * @returns {HRESULT} 
     */
    get_CurrentLongestWaitTime(plWaitTime) {
        result := ComCall(15, this, "int*", plWaitTime, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plWaitTime 
     * @returns {HRESULT} 
     */
    get_AverageWaitTime(plWaitTime) {
        result := ComCall(16, this, "int*", plWaitTime, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Int32>} plCalls 
     * @returns {HRESULT} 
     */
    get_FinalDisposition(plCalls) {
        result := ComCall(17, this, "int*", plCalls, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<BSTR>} ppName 
     * @returns {HRESULT} 
     */
    get_Name(ppName) {
        result := ComCall(18, this, "ptr", ppName, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
