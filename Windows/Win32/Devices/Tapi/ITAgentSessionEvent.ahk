 ...
 */
export default struct ITAgentSessionEvent extends IDispatch {
    /**
     ...
     * used for ITAgentSessionEvent interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITAgentSessionEvent.Vtbl()
        }
    ...
    Query(iid) {
        if (ITAgentSessionEvent.IID.Equals(iid)) {
            return true
        }
