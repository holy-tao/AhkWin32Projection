 ...
 */
export default struct ITDirectoryObjectUser extends IDispatch {
    /**
     ...
     * used for ITDirectoryObjectUser interfaces
    */
    struct Vtbl extends IDispatch.Vtbl {
        get_IPPhonePrimary : IntPtr
        put_IPPhonePrimary : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITDirectoryObjectUser.Vtbl()
        }
    ...
    put_IPPhonePrimary(pName) {
        ...
        result := ComCall(8, this, BSTR, pName, "HRESULT")
        return result
    }

    Query(iid) {
        if (ITDirectoryObjectUser.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.get_IPPhonePrimary)
        CallbackFree(this.vtbl.put_IPPhonePrimary)
    }
}
