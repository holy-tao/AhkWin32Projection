...
class ITRendezvous extends IDispatch{
    ...
    EnumerateDefaultDirectories(ppEnumDirectory) {
        result := ComCall(8, this, "ptr", ppEnumDirectory, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDirectory(DirectoryType, pName, ppDir) {
        ...
        result := ComCall(9, this, "int", DirectoryType, "ptr", pName, "ptr", ppDir, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDirectoryObject(DirectoryObjectType, pName, ppDirectoryObject) {
        ...
        result := ComCall(10, this, "int", DirectoryObjectType, "ptr", pName, "ptr", ppDirectoryObject, "int")
        if(result != 0)
            throw OSError(result)
