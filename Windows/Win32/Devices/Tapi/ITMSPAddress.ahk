 ...
 */
export default struct ITMSPAddress extends IUnknown {
    /**
     ...
     * used for ITMSPAddress interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        Initialize      : IntPtr
        Shutdown        : IntPtr
        CreateMSPCall   : IntPtr
        ShutdownMSPCall : IntPtr
        ReceiveTSPData  : IntPtr
        GetEvent        : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITMSPAddress.Vtbl()
        }
    ...
    Query(iid) {
        if (ITMSPAddress.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.CreateMSPCall)
        CallbackFree(this.vtbl.ShutdownMSPCall)
        CallbackFree(this.vtbl.ReceiveTSPData)
