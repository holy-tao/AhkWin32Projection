 ...
 * @see https://learn.microsoft.com/windows/win32/api/winbio_adapter/nc-winbio_adapter-pibio_storage_control_unit_privileged_fn
 * @namespace Windows.Win32.Devices.BiometricFramework
 */
export default struct PIBIO_STORAGE_CONTROL_UNIT_PRIVILEGED_FN {
    value : IntPtr

    __value {
        set {
            if (value is PIBIO_STORAGE_CONTROL_UNIT_PRIVILEGED_FN) {
    ...
    /**
     * 
     * @param {Pointer<WINBIO_PIPELINE>} Pipeline A pointer to the <a href="https://docs.microsoft.com/windows/desktop/api/winbio_adapter/ns-winbio_adapter-winbio_pipeline">WINBIO_PIPELINE</a> structure associated with the biometric unit performing the operation.
     * @param {Integer} ControlCode A <b>ULONG</b> value that specifies the vendor-defined operation to perform.
     * @param {Integer} SendBuffer A pointer to a buffer that contains the control information sent to the storage adapter. The format and content of the buffer is vendor defined.
     * @param {Pointer} SendBufferSize The size, in bytes, of the buffer specified by the <i>SendBuffer</i> parameter.
     * @param {Integer} ReceiveBuffer A pointer to a buffer that receives information returned by the storage adapter in response to the control operation. The format of the buffer is vendor defined.
     * @param {Pointer} ReceiveBufferSize The size, in bytes, of the buffer specified by the <i>ReceiveBuffer</i> parameter.
     * @param {Pointer<Pointer>} ReceiveDataSize A pointer to a variable that receives  the size, in bytes, of the data written to the buffer specified by the <i>ReceiveBuffer</i> parameter.
     * @param {Pointer<Integer>} OperationStatus A pointer to a variable that receives a vendor-defined status code that specifies the outcome of the control operation.
    ...
    /**
     * A PIBIO_STORAGE_CONTROL_UNIT_PRIVILEGED_FN that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PIBIO_STORAGE_CONTROL_UNIT_PRIVILEGED_FN {
        /**
         * Creates a PIBIO_STORAGE_CONTROL_UNIT_PRIVILEGED_FN pointer that invokes the given AHK function when called.
