...
/**
 * Performs a vendor-defined control operation that requires elevated privilege.E
 * @remarks
 ...
 * @see https://learn.microsoft.com/windows/win32/api/winbio_adapter/nc-winbio_adapter-pibio_engine_control_unit_privileged_fn
 * @namespace Windows.Win32.Devices.BiometricFramework
 */
export default struct PIBIO_ENGINE_CONTROL_UNIT_PRIVILEGED_FN {
    value : IntPtr

    __value {
        set {
            if (value is PIBIO_ENGINE_CONTROL_UNIT_PRIVILEGED_FN) {
     ...
     * @param {Integer} ReceiveBuffer Pointer to a  buffer that receives information returned by the engine adapter in response to the control operation.  The format of the buffer is vendor-defined.
    ...
    /**
     * A PIBIO_ENGINE_CONTROL_UNIT_PRIVILEGED_FN that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PIBIO_ENGINE_CONTROL_UNIT_PRIVILEGED_FN {
        /**
         * Creates a PIBIO_ENGINE_CONTROL_UNIT_PRIVILEGED_FN pointer that invokes the given AHK function when called.
