 ...
 * @see https://learn.microsoft.com/windows/win32/api/winbio_adapter/nc-winbio_adapter-pibio_engine_query_extended_enrollment_status_fn
 * @namespace Windows.Win32.Devices.BiometricFramework
 */
export default struct PIBIO_ENGINE_QUERY_EXTENDED_ENROLLMENT_STATUS_FN {
    value : IntPtr

    __value {
        set {
            if (value is PIBIO_ENGINE_QUERY_EXTENDED_ENROLLMENT_STATUS_FN) {
     ...
     * </table>
     */
    Call(Pipeline, EnrollmentStatus, EnrollmentStatusSize) {
        result := DllCall(this.value, WINBIO_PIPELINE.Ptr, Pipeline, IntPtr, EnrollmentStatus, IntPtr, EnrollmentStatusSize, "HRESULT")
        return result
    }

    /**
     * A PIBIO_ENGINE_QUERY_EXTENDED_ENROLLMENT_STATUS_FN that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PIBIO_ENGINE_QUERY_EXTENDED_ENROLLMENT_STATUS_FN {
        /**
         * Creates a PIBIO_ENGINE_QUERY_EXTENDED_ENROLLMENT_STATUS_FN pointer that invokes the given AHK function when called.
