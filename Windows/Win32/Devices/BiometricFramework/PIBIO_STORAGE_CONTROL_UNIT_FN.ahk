...
/**
 * The PIBIO_STORAGE_CONTROL_UNIT_FN callback function (winbio_adapter.h) performs a vendor-defined control operation that does not require elevated privilege.
 * @remarks
 ...
 * @see https://learn.microsoft.com/windows/win32/api/winbio_adapter/nc-winbio_adapter-pibio_storage_control_unit_fn
 * @namespace Windows.Win32.Devices.BiometricFramework
 */
export default struct PIBIO_STORAGE_CONTROL_UNIT_FN {
    value : IntPtr

    __value {
        set {
            if (value is PIBIO_STORAGE_CONTROL_UNIT_FN) {
     ...
     * @param {Integer} SendBuffer Pointer to a buffer that contains the control information sent to the storage adapter. The format and content of the buffer is vendor-defined.
     * @param {Pointer} SendBufferSize Size, in bytes, of the buffer specified by the <i>SendBuffer</i> parameter.
     * @param {Integer} ReceiveBuffer Pointer to the buffer that receives information sent by the storage adapter in response to the control operation. The format of the buffer is vendor-defined.
    ...
    /**
     * A PIBIO_STORAGE_CONTROL_UNIT_FN that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PIBIO_STORAGE_CONTROL_UNIT_FN {
        /**
         * Creates a PIBIO_STORAGE_CONTROL_UNIT_FN pointer that invokes the given AHK function when called.
