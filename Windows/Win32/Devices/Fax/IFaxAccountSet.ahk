...
class IFaxAccountSet extends IDispatch{
    ...
    GetAccounts(ppFaxAccounts) {
        result := ComCall(7, this, "ptr", ppFaxAccounts, "int")
        if(result != 0)
    ...
    GetAccount(bstrAccountName, pFaxAccount) {
        ...
        result := ComCall(8, this, "ptr", bstrAccountName, "ptr", pFaxAccount, "int")
        if(result != 0)
            throw OSError(result)
    ...
    AddAccount(bstrAccountName, pFaxAccount) {
        ...
        result := ComCall(9, this, "ptr", bstrAccountName, "ptr", pFaxAccount, "int")
        if(result != 0)
            throw OSError(result)
