 ...
 * @since windows5.0
 */
export FaxConnectFaxServerA(MachineName, FaxHandle) {
    MachineName := MachineName is String ? StrPtr(MachineName) : MachineName

    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxConnectFaxServerA", "ptr", MachineName, HANDLE.Ptr, FaxHandle, BOOL)
 ...
 * @since windows5.0
 */
export FaxConnectFaxServerW(MachineName, FaxHandle) {
    MachineName := MachineName is String ? StrPtr(MachineName) : MachineName

    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxConnectFaxServerW", "ptr", MachineName, HANDLE.Ptr, FaxHandle, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @returns {BOOL} 
 */
export FaxClose(FaxHandle) {
    result := DllCall("WINFAX.dll\FaxClose", HANDLE, FaxHandle, BOOL)
    return result
}
...
export FaxSendDocumentA(FaxHandle, FileName, JobParams, CoverpageInfo, FaxJobId) {
    ...
    result := DllCall("WINFAX.dll\FaxSendDocumentA", HANDLE, FaxHandle, "ptr", FileName, FAX_JOB_PARAMA.Ptr, JobParams, FAX_COVERPAGE_INFOA.Ptr, CoverpageInfo, FaxJobIdMarshal, FaxJobId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxSendDocumentW(FaxHandle, FileName, JobParams, CoverpageInfo, FaxJobId) {
    ...
    result := DllCall("WINFAX.dll\FaxSendDocumentW", HANDLE, FaxHandle, "ptr", FileName, FAX_JOB_PARAMW.Ptr, JobParams, FAX_COVERPAGE_INFOW.Ptr, CoverpageInfo, FaxJobIdMarshal, FaxJobId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumJobsA(FaxHandle, JobEntry, JobsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumJobsA", HANDLE, FaxHandle, JobEntryMarshal, JobEntry, JobsReturnedMarshal, JobsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumJobsW(FaxHandle, JobEntry, JobsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumJobsW", HANDLE, FaxHandle, JobEntryMarshal, JobEntry, JobsReturnedMarshal, JobsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetDeviceStatusA(FaxPortHandle, DeviceStatus) {
    ...
    result := DllCall("WINFAX.dll\FaxGetDeviceStatusA", HANDLE, FaxPortHandle, DeviceStatusMarshal, DeviceStatus, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetDeviceStatusW(FaxPortHandle, DeviceStatus) {
    ...
    result := DllCall("WINFAX.dll\FaxGetDeviceStatusW", HANDLE, FaxPortHandle, DeviceStatusMarshal, DeviceStatus, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetConfigurationA(FaxHandle, FaxConfig) {
    ...
    result := DllCall("WINFAX.dll\FaxGetConfigurationA", HANDLE, FaxHandle, FaxConfigMarshal, FaxConfig, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetConfigurationW(FaxHandle, FaxConfig) {
    ...
    result := DllCall("WINFAX.dll\FaxGetConfigurationW", HANDLE, FaxHandle, FaxConfigMarshal, FaxConfig, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetConfigurationA(FaxHandle, FaxConfig) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetConfigurationA", HANDLE, FaxHandle, FAX_CONFIGURATIONA.Ptr, FaxConfig, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetConfigurationW(FaxHandle, FaxConfig) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetConfigurationW", HANDLE, FaxHandle, FAX_CONFIGURATIONW.Ptr, FaxConfig, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetLoggingCategoriesA(FaxHandle, Categories, NumberCategories) {
    ...
    result := DllCall("WINFAX.dll\FaxGetLoggingCategoriesA", HANDLE, FaxHandle, CategoriesMarshal, Categories, NumberCategoriesMarshal, NumberCategories, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetLoggingCategoriesW(FaxHandle, Categories, NumberCategories) {
    ...
    result := DllCall("WINFAX.dll\FaxGetLoggingCategoriesW", HANDLE, FaxHandle, CategoriesMarshal, Categories, NumberCategoriesMarshal, NumberCategories, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumPortsA(FaxHandle, PortInfo, PortsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumPortsA", HANDLE, FaxHandle, PortInfoMarshal, PortInfo, PortsReturnedMarshal, PortsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumPortsW(FaxHandle, PortInfo, PortsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumPortsW", HANDLE, FaxHandle, PortInfoMarshal, PortInfo, PortsReturnedMarshal, PortsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetPortA(FaxPortHandle, PortInfo) {
    ...
    result := DllCall("WINFAX.dll\FaxGetPortA", HANDLE, FaxPortHandle, PortInfoMarshal, PortInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetPortW(FaxPortHandle, PortInfo) {
    ...
    result := DllCall("WINFAX.dll\FaxGetPortW", HANDLE, FaxPortHandle, PortInfoMarshal, PortInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetPortA(FaxPortHandle, PortInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetPortA", HANDLE, FaxPortHandle, FAX_PORT_INFOA.Ptr, PortInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetPortW(FaxPortHandle, PortInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetPortW", HANDLE, FaxPortHandle, FAX_PORT_INFOW.Ptr, PortInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumRoutingMethodsA(FaxPortHandle, RoutingMethod, MethodsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumRoutingMethodsA", HANDLE, FaxPortHandle, RoutingMethodMarshal, RoutingMethod, MethodsReturnedMarshal, MethodsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumRoutingMethodsW(FaxPortHandle, RoutingMethod, MethodsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumRoutingMethodsW", HANDLE, FaxPortHandle, RoutingMethodMarshal, RoutingMethod, MethodsReturnedMarshal, MethodsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxEnableRoutingMethodA(FaxPortHandle, RoutingGuid, Enabled) {
    RoutingGuid := RoutingGuid is String ? StrPtr(RoutingGuid) : RoutingGuid

    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxEnableRoutingMethodA", HANDLE, FaxPortHandle, "ptr", RoutingGuid, BOOL, Enabled, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxEnableRoutingMethodW(FaxPortHandle, RoutingGuid, Enabled) {
    RoutingGuid := RoutingGuid is String ? StrPtr(RoutingGuid) : RoutingGuid

    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxEnableRoutingMethodW", HANDLE, FaxPortHandle, "ptr", RoutingGuid, BOOL, Enabled, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumGlobalRoutingInfoA(FaxHandle, RoutingInfo, MethodsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumGlobalRoutingInfoA", HANDLE, FaxHandle, RoutingInfoMarshal, RoutingInfo, MethodsReturnedMarshal, MethodsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxEnumGlobalRoutingInfoW(FaxHandle, RoutingInfo, MethodsReturned) {
    ...
    result := DllCall("WINFAX.dll\FaxEnumGlobalRoutingInfoW", HANDLE, FaxHandle, RoutingInfoMarshal, RoutingInfo, MethodsReturnedMarshal, MethodsReturned, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetGlobalRoutingInfoA(FaxHandle, RoutingInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetGlobalRoutingInfoA", HANDLE, FaxHandle, FAX_GLOBAL_ROUTING_INFOA.Ptr, RoutingInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxSetGlobalRoutingInfoW(FaxHandle, RoutingInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxSetGlobalRoutingInfoW", HANDLE, FaxHandle, FAX_GLOBAL_ROUTING_INFOW.Ptr, RoutingInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetRoutingInfoA(FaxPortHandle, RoutingGuid, RoutingInfoBuffer, RoutingInfoBufferSize) {
    ...
    result := DllCall("WINFAX.dll\FaxGetRoutingInfoA", HANDLE, FaxPortHandle, "ptr", RoutingGuid, RoutingInfoBufferMarshal, RoutingInfoBuffer, RoutingInfoBufferSizeMarshal, RoutingInfoBufferSize, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxGetRoutingInfoW(FaxPortHandle, RoutingGuid, RoutingInfoBuffer, RoutingInfoBufferSize) {
    ...
    result := DllCall("WINFAX.dll\FaxGetRoutingInfoW", HANDLE, FaxPortHandle, "ptr", RoutingGuid, RoutingInfoBufferMarshal, RoutingInfoBuffer, RoutingInfoBufferSizeMarshal, RoutingInfoBufferSize, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxStartPrintJobA(PrinterName, PrintInfo, FaxJobId, FaxContextInfo) {
    ...
    result := DllCall("WINFAX.dll\FaxStartPrintJobA", "ptr", PrinterName, FAX_PRINT_INFOA.Ptr, PrintInfo, FaxJobIdMarshal, FaxJobId, FAX_CONTEXT_INFOA.Ptr, FaxContextInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export FaxStartPrintJobW(PrinterName, PrintInfo, FaxJobId, FaxContextInfo) {
    ...
    result := DllCall("WINFAX.dll\FaxStartPrintJobW", "ptr", PrinterName, FAX_PRINT_INFOW.Ptr, PrintInfo, FaxJobIdMarshal, FaxJobId, FAX_CONTEXT_INFOW.Ptr, FaxContextInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxPrintCoverPageA(FaxContextInfo, CoverPageInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxPrintCoverPageA", FAX_CONTEXT_INFOA.Ptr, FaxContextInfo, FAX_COVERPAGE_INFOA.Ptr, CoverPageInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxPrintCoverPageW(FaxContextInfo, CoverPageInfo) {
    A_LastError := 0

    result := DllCall("WINFAX.dll\FaxPrintCoverPageW", FAX_CONTEXT_INFOW.Ptr, FaxContextInfo, FAX_COVERPAGE_INFOW.Ptr, CoverPageInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FaxRegisterServiceProviderW(DeviceProvider, FriendlyName, ImageName, TspName) {
    ...
    result := DllCall("WINFAX.dll\FaxRegisterServiceProviderW", "ptr", DeviceProvider, "ptr", FriendlyName, "ptr", ImageName, "ptr", TspName, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @returns {BOOL} 
 */
export FaxUnregisterServiceProviderW(DeviceProvider) {
    DeviceProvider := DeviceProvider is String ? StrPtr(DeviceProvider) : DeviceProvider

    result := DllCall("WINFAX.dll\FaxUnregisterServiceProviderW", "ptr", DeviceProvider, BOOL)
    return result
}
 ...
 * @since windows5.0
 */
export FaxRegisterRoutingExtensionW(FaxHandle, ExtensionName, FriendlyName, ImageName, CallBack, _Context) {
 ...
 */
export CanSendToFaxRecipient() {
    result := DllCall("fxsutility.dll\CanSendToFaxRecipient", BOOL)
    return result
}
 ...
 * @since windows6.0.6000
 */
export SendToFaxRecipient(sndMode, lpFileName) {
    lpFileName := lpFileName is String ? StrPtr(lpFileName) : lpFileName

    result := DllCall("fxsutility.dll\SendToFaxRecipient", SendToMode, sndMode, "ptr", lpFileName, UInt32)
    return result
}
