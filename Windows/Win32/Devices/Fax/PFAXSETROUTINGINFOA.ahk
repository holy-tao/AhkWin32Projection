 ...
 * @charset ANSI
 */
export default struct PFAXSETROUTINGINFOA {
    value : IntPtr

    __value {
        set {
            if (value is PFAXSETROUTINGINFOA) {
    ...
    /**
     * A PFAXSETROUTINGINFOA that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFAXSETROUTINGINFOA {
        /**
         * Creates a PFAXSETROUTINGINFOA pointer that invokes the given AHK function when called.
         * @param {Func(HANDLE, PSTR, "char*", UInt32) => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [HANDLE, PSTR, "char*", UInt32, BOOL])
        }
