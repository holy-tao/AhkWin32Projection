 ...
 * @charset Unicode
 */
export default struct PFAXGETROUTINGINFOW {
    value : IntPtr

    __value {
        set {
            if (value is PFAXGETROUTINGINFOW) {
    ...
    /**
     * A PFAXGETROUTINGINFOW that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFAXGETROUTINGINFOW {
        /**
         * Creates a PFAXGETROUTINGINFOW pointer that invokes the given AHK function when called.
         * @param {Func(HANDLE, PWSTR, "ptr*", "uint*") => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [HANDLE, PWSTR, "ptr*", "uint*", BOOL])
        }
