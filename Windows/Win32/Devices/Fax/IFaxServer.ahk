...
export default struct IFaxServer extends IDispatch {
    ...
    */
     ...
     * @remarks
     * Before accessing most of the objects of the fax extended Component Object Model (COM), the application must call this method to initiate a connection with an active fax server. A fax server connection is not required for you to access a <a href="https://docs.microsoft.com/previous-versions/windows/desktop/fax/-mfax-faxdocument">FaxDocument</a> object. The method fails if the client is not connected to an active fax server. 
     * 
     * To connect to the local server, set the <i>bstrServerName</i> parameter to <b>NULL</b> or an empty string. For usage examples, see <a href="https://docs.microsoft.com/previous-versions/windows/desktop/fax/-mfax-connecting-to-the-fax-server">Connecting to the Fax Server</a>.
    ...
    Implement(implObj, flags := "") {
        ...
        this.vtbl.GetDeviceProviders := CallbackCreate(ObjBindMethod(implObj, "GetDeviceProviders"), flags, 2)
        this.vtbl.GetDevices := CallbackCreate(ObjBindMethod(implObj, "GetDevices"), flags, 2)
        this.vtbl.get_InboundRouting := CallbackCreate(ObjBindMethod(implObj, "get_InboundRouting"), flags, 2)
        this.vtbl.get_Folders := CallbackCreate(ObjBindMethod(implObj, "get_Folders"), flags, 2)
        this.vtbl.get_LoggingOptions := CallbackCreate(ObjBindMethod(implObj, "get_LoggingOptions"), flags, 2)
        ...
        this.vtbl.get_Activity := CallbackCreate(ObjBindMethod(implObj, "get_Activity"), flags, 2)
        this.vtbl.get_OutboundRouting := CallbackCreate(ObjBindMethod(implObj, "get_OutboundRouting"), flags, 2)
        this.vtbl.get_ReceiptOptions := CallbackCreate(ObjBindMethod(implObj, "get_ReceiptOptions"), flags, 2)
        ...
        this.vtbl.GetExtensionProperty := CallbackCreate(ObjBindMethod(implObj, "GetExtensionProperty"), flags, 3)
        this.vtbl.SetExtensionProperty := CallbackCreate(ObjBindMethod(implObj, "SetExtensionProperty"), flags, 3)
        this.vtbl.ListenToServerEvents := CallbackCreate(ObjBindMethod(implObj, "ListenToServerEvents"), flags, 2)
        this.vtbl.RegisterDeviceProvider := CallbackCreate(ObjBindMethod(implObj, "RegisterDeviceProvider"), flags, 6)
        this.vtbl.UnregisterDeviceProvider := CallbackCreate(ObjBindMethod(implObj, "UnregisterDeviceProvider"), flags, 2)
        this.vtbl.RegisterInboundRoutingExtension := CallbackCreate(ObjBindMethod(implObj, "RegisterInboundRoutingExtension"), flags, 5)
        this.vtbl.UnregisterInboundRoutingExtension := CallbackCreate(ObjBindMethod(implObj, "UnregisterInboundRoutingExtension"), flags, 2)
        this.vtbl.get_RegisteredEvents := CallbackCreate(ObjBindMethod(implObj, "get_RegisteredEvents"), flags, 2)
        this.vtbl.get_APIVersion := CallbackCreate(ObjBindMethod(implObj, "get_APIVersion"), flags, 2)
    }
