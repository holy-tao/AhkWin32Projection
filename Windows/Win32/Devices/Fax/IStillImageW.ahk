...
class IStillImageW extends IUnknown{
     ...
     * @param {Pointer<UInt32>} pdwItemsReturned 
     * @param {Pointer<Void>} ppBuffer 
     * @returns {HRESULT} 
     */
    GetDeviceList(dwType, dwFlags, pdwItemsReturned, ppBuffer) {
        result := ComCall(4, this, "uint", dwType, "uint", dwFlags, "uint*", pdwItemsReturned, "ptr", ppBuffer, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDeviceInfo(pwszDeviceName, ppBuffer) {
        ...
        result := ComCall(5, this, "ptr", pwszDeviceName, "ptr", ppBuffer, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDevice(pwszDeviceName, dwMode, pDevice, punkOuter) {
        ...
        result := ComCall(6, this, "ptr", pwszDeviceName, "uint", dwMode, "ptr", pDevice, "ptr", punkOuter, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDeviceValue(pwszDeviceName, pValueName, pType, pData, cbData) {
        ...
        result := ComCall(7, this, "ptr", pwszDeviceName, "ptr", pValueName, "uint*", pType, "ptr", pData, "uint*", cbData, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * 
     * @param {PWSTR} pwszDeviceName 
     * @param {Pointer<UInt32>} pdwEventCode 
    ...
    GetSTILaunchInformation(pwszDeviceName, pdwEventCode, pwszEventName) {
        ...
        result := ComCall(9, this, "ptr", pwszDeviceName, "uint*", pdwEventCode, "ptr", pwszEventName, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetHwNotificationState(pwszDeviceName, pbCurrentState) {
        ...
        result := ComCall(13, this, "ptr", pwszDeviceName, "ptr", pbCurrentState, "int")
        if(result != 0)
            throw OSError(result)
