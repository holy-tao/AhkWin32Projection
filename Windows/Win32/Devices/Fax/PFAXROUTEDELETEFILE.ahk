 ...
 * <div> </div>
 * @see https://learn.microsoft.com/windows/win32/api/faxroute/nc-faxroute-pfaxroutedeletefile
 * @namespace Windows.Win32.Devices.Fax
 */
export default struct PFAXROUTEDELETEFILE {
    value : IntPtr

    __value {
        set {
            if (value is PFAXROUTEDELETEFILE) {
    ...
    Call(JobId, FileName) {
        ...
        result := DllCall(this.value, UInt32, JobId, "ptr", FileName, Int32)
        return result
    }

    /**
     * A PFAXROUTEDELETEFILE that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFAXROUTEDELETEFILE {
        /**
         * Creates a PFAXROUTEDELETEFILE pointer that invokes the given AHK function when called.
         * @param {Func(UInt32, PWSTR) => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 2)) {
