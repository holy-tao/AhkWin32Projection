 ...
 * @see https://learn.microsoft.com/windows/win32/api/winfax/nc-winfax-pfax_recipient_callbackw
 * @namespace Windows.Win32.Devices.Fax
 * @charset Unicode
 */
export default struct PFAX_RECIPIENT_CALLBACKW {
    value : IntPtr

    __value {
        set {
            if (value is PFAX_RECIPIENT_CALLBACKW) {
    ...
    Call(FaxHandle, RecipientNumber, _Context, JobParams, CoverpageInfo) {
        ...
        result := DllCall(this.value, HANDLE, FaxHandle, UInt32, RecipientNumber, _ContextMarshal, _Context, FAX_JOB_PARAMW.Ptr, JobParams, FAX_COVERPAGE_INFOW.Ptr, CoverpageInfo, BOOL)
        return result
    }

    /**
     * A PFAX_RECIPIENT_CALLBACKW that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PFAX_RECIPIENT_CALLBACKW {
        /**
         * Creates a PFAX_RECIPIENT_CALLBACKW pointer that invokes the given AHK function when called.
         * @param {Func(HANDLE, UInt32, "ptr", FAX_JOB_PARAMW, FAX_COVERPAGE_INFOW) => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 5)) {
