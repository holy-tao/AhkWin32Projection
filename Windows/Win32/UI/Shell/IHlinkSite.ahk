...
class IHlinkSite extends IUnknown{
    ...
    QueryService(dwSiteData, guidService, riid, ppiunk) {
        result := ComCall(3, this, "uint", dwSiteData, "ptr", guidService, "ptr", riid, "ptr", ppiunk, "int")
        if(result != 0)
    ...
    GetMoniker(dwSiteData, dwAssign, dwWhich, ppimk) {
        result := ComCall(4, this, "uint", dwSiteData, "uint", dwAssign, "uint", dwWhich, "ptr", ppimk, "int")
        if(result != 0)
            throw OSError(result)
