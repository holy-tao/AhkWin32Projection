 ...
 */
export default struct IShellItem extends IUnknown {
    /**
     ...
     * used for IShellItem interfaces
    */
    ...
    BindToHandler(pbc, bhid, riid) {
        result := ComCall(3, this, "ptr", pbc, Guid.Ptr, bhid, Guid.Ptr, riid, "ptr*", &ppv := 0, "HRESULT")
        return ppv
    }
    ...
    GetDisplayName(sigdnName) {
        result := ComCall(5, this, SIGDN, sigdnName, PWSTR.Ptr, &ppszName := 0, "HRESULT")
        return ppszName
    }
    ...
    GetAttributes(sfgaoMask) {
        result := ComCall(6, this, SFGAO_FLAGS, sfgaoMask, "uint*", &psfgaoAttribs := 0, "HRESULT")
