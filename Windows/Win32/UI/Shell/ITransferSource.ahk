#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\System\Com\IUnknown.ahk

/**
 * Exposes methods to manipulate IShellItem, including copy, move, recycle, and others. This interface is offered to provide more control over file operations by providing an ITransferSource::Advise method.
 * @see https://docs.microsoft.com/windows/win32/api//shobjidl_core/nn-shobjidl_core-itransfersource
 * @namespace Windows.Win32.UI.Shell
 * @version v4.0.30319
 */
class ITransferSource extends IUnknown{
    /**
     * The interface identifier for ITransferSource
     * @type {Guid}
     */
    static IID => Guid("{00adb003-bde9-45c6-8e29-d09f9353e108}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 3

    /**
     * 
     * @param {Pointer<ITransferAdviseSink>} psink 
     * @param {Pointer<UInt32>} pdwCookie 
     * @returns {HRESULT} 
     */
    Advise(psink, pdwCookie) {
        result := ComCall(3, this, "ptr", psink, "uint*", pdwCookie, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} dwCookie 
     * @returns {HRESULT} 
     */
    Unadvise(dwCookie) {
        result := ComCall(4, this, "uint", dwCookie, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IPropertyChangeArray>} pproparray 
     * @returns {HRESULT} 
     */
    SetProperties(pproparray) {
        result := ComCall(5, this, "ptr", pproparray, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psi 
     * @param {Integer} flags 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    OpenItem(psi, flags, riid, ppv) {
        result := ComCall(6, this, "ptr", psi, "uint", flags, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psi 
     * @param {Pointer<IShellItem>} psiParentDst 
     * @param {PWSTR} pszNameDst 
     * @param {Integer} flags 
     * @param {Pointer<IShellItem>} ppsiNew 
     * @returns {HRESULT} 
     */
    MoveItem(psi, psiParentDst, pszNameDst, flags, ppsiNew) {
        pszNameDst := pszNameDst is String ? StrPtr(pszNameDst) : pszNameDst

        result := ComCall(7, this, "ptr", psi, "ptr", psiParentDst, "ptr", pszNameDst, "uint", flags, "ptr", ppsiNew, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {Pointer<IShellItem>} psiParentDest 
     * @param {Integer} flags 
     * @param {Pointer<IShellItem>} ppsiNewDest 
     * @returns {HRESULT} 
     */
    RecycleItem(psiSource, psiParentDest, flags, ppsiNewDest) {
        result := ComCall(8, this, "ptr", psiSource, "ptr", psiParentDest, "uint", flags, "ptr", ppsiNewDest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {Integer} flags 
     * @returns {HRESULT} 
     */
    RemoveItem(psiSource, flags) {
        result := ComCall(9, this, "ptr", psiSource, "uint", flags, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {PWSTR} pszNewName 
     * @param {Integer} flags 
     * @param {Pointer<IShellItem>} ppsiNewDest 
     * @returns {HRESULT} 
     */
    RenameItem(psiSource, pszNewName, flags, ppsiNewDest) {
        pszNewName := pszNewName is String ? StrPtr(pszNewName) : pszNewName

        result := ComCall(10, this, "ptr", psiSource, "ptr", pszNewName, "uint", flags, "ptr", ppsiNewDest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {Pointer<IShellItem>} psiParentDest 
     * @param {PWSTR} pszNewName 
     * @param {Integer} flags 
     * @param {Pointer<IShellItem>} ppsiNewDest 
     * @returns {HRESULT} 
     */
    LinkItem(psiSource, psiParentDest, pszNewName, flags, ppsiNewDest) {
        pszNewName := pszNewName is String ? StrPtr(pszNewName) : pszNewName

        result := ComCall(11, this, "ptr", psiSource, "ptr", psiParentDest, "ptr", pszNewName, "uint", flags, "ptr", ppsiNewDest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {Pointer<IShellItem>} ppsiNew 
     * @returns {HRESULT} 
     */
    ApplyPropertiesToItem(psiSource, ppsiNew) {
        result := ComCall(12, this, "ptr", psiSource, "ptr", ppsiNew, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiSource 
     * @param {Pointer<IShellItem>} psiParentDest 
     * @param {Pointer<PWSTR>} ppszDestinationName 
     * @returns {HRESULT} 
     */
    GetDefaultDestinationName(psiSource, psiParentDest, ppszDestinationName) {
        result := ComCall(13, this, "ptr", psiSource, "ptr", psiParentDest, "ptr", ppszDestinationName, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiChildFolderDest 
     * @returns {HRESULT} 
     */
    EnterFolder(psiChildFolderDest) {
        result := ComCall(14, this, "ptr", psiChildFolderDest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IShellItem>} psiChildFolderDest 
     * @returns {HRESULT} 
     */
    LeaveFolder(psiChildFolderDest) {
        result := ComCall(15, this, "ptr", psiChildFolderDest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
