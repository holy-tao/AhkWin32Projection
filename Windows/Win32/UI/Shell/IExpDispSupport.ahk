 ...
 */
export default struct IExpDispSupport extends IUnknown {
    /**
     ...
     * used for IExpDispSupport interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IExpDispSupport.Vtbl()
        }
    ...
    Query(iid) {
        if (IExpDispSupport.IID.Equals(iid)) {
            return true
        }
