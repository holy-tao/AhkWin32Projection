 ...
 */
export default struct IEnumerableView extends IUnknown {
    /**
     ...
     * used for IEnumerableView interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        SetEnumReadyCallback         : IntPtr
        CreateEnumIDListFromContents : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IEnumerableView.Vtbl()
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.SetEnumReadyCallback)
        CallbackFree(this.vtbl.CreateEnumIDListFromContents)
    }
}
