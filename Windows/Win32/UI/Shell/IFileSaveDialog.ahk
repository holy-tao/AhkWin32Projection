...
class IFileSaveDialog extends IFileDialog{
    ...
    /**
     * 
     * @param {Pointer<IPropertyDescriptionList>} pList 
    ...
    GetProperties(ppStore) {
        result := ComCall(30, this, "ptr", ppStore, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<IPropertyStore>} pStore 
     * @param {HWND} hwnd 
     * @param {Pointer<IFileOperationProgressSink>} pSink 
     * @returns {HRESULT} 
     */
    ApplyProperties(psi, pStore, hwnd, pSink) {
        hwnd := hwnd is Win32Handle ? NumGet(hwnd, "ptr") : hwnd

        result := ComCall(31, this, "ptr", psi, "ptr", pStore, "ptr", hwnd, "ptr", pSink, "int")
        if(result != 0)
            throw OSError(result)
