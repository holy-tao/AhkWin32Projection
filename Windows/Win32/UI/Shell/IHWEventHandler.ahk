 ...
 */
export default struct IHWEventHandler extends IUnknown {
    /**
     ...
     * used for IHWEventHandler interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        Initialize             : IntPtr
        HandleEvent            : IntPtr
        HandleEventWithContent : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IHWEventHandler.Vtbl()
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.HandleEvent)
        CallbackFree(this.vtbl.HandleEventWithContent)
    }
}
