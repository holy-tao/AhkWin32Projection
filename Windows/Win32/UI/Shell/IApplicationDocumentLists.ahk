...
#Import ".\APPDOCLISTTYPE.ahk" { APPDOCLISTTYPE }
 ...
 */
export default struct IApplicationDocumentLists extends IUnknown {
    /**
     ...
     * used for IApplicationDocumentLists interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IApplicationDocumentLists.Vtbl()
        }
    ...
    GetList(listtype, cItemsDesired, riid) {
        result := ComCall(4, this, APPDOCLISTTYPE, listtype, UInt32, cItemsDesired, Guid.Ptr, riid, "ptr*", &ppv := 0, "HRESULT")
        return ppv
    }

    Query(iid) {
        if (IApplicationDocumentLists.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetList)
    }
}
