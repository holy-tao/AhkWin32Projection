...
class IAttachmentExecute extends IUnknown{
    ...
    Prompt(hwnd, prompt, paction) {
        ...
        result := ComCall(10, this, "ptr", hwnd, "int", prompt, "int*", paction, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Execute(hwnd, pszVerb, phProcess) {
        ...
        result := ComCall(12, this, "ptr", hwnd, "ptr", pszVerb, "ptr", phProcess, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SaveWithUI(hwnd) {
        ...
        result := ComCall(13, this, "ptr", hwnd, "int")
        if(result != 0)
            throw OSError(result)
