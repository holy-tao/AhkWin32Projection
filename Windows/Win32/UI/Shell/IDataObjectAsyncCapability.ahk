...
class IDataObjectAsyncCapability extends IUnknown{
    ...
    GetAsyncMode(pfIsOpAsync) {
        result := ComCall(4, this, "ptr", pfIsOpAsync, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IBindCtx>} pbcReserved 
     * @returns {HRESULT} 
     */
    ...
    InOperation(pfInAsyncOp) {
        result := ComCall(6, this, "ptr", pfInAsyncOp, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EndOperation(hResult, pbcReserved, dwEffects) {
        result := ComCall(7, this, "int", hResult, "ptr", pbcReserved, "uint", dwEffects, "int")
        if(result != 0)
            throw OSError(result)
