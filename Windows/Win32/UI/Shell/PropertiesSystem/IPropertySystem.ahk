...
class IPropertySystem extends IUnknown{
    ...
    GetPropertyDescription(propkey, riid, ppv) {
        result := ComCall(3, this, "ptr", propkey, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
    ...
    GetPropertyDescriptionByName(pszCanonicalName, riid, ppv) {
        ...
        result := ComCall(4, this, "ptr", pszCanonicalName, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPropertyDescriptionListFromString(pszPropList, riid, ppv) {
        ...
        result := ComCall(5, this, "ptr", pszPropList, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumeratePropertyDescriptions(filterOn, riid, ppv) {
        result := ComCall(6, this, "int", filterOn, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FormatForDisplayAlloc(key, propvar, pdff, ppszDisplay) {
        result := ComCall(8, this, "ptr", key, "ptr", propvar, "int", pdff, "ptr", ppszDisplay, "int")
        if(result != 0)
            throw OSError(result)
