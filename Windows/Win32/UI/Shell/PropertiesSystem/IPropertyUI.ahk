...
class IPropertyUI extends IUnknown{
    ...
    ParsePropertyName(pszName, pfmtid, ppid, pchEaten) {
        ...
        result := ComCall(3, this, "ptr", pszName, "ptr", pfmtid, "uint*", ppid, "uint*", pchEaten, "int")
        if(result != 0)
    ...
    GetDefaultWidth(fmtid, pid, pcxChars) {
        result := ComCall(7, this, "ptr", fmtid, "uint", pid, "uint*", pcxChars, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFlags(fmtid, pid, pflags) {
        result := ComCall(8, this, "ptr", fmtid, "uint", pid, "int*", pflags, "int")
        if(result != 0)
            throw OSError(result)
