...
class ISyncMgrConflictStore extends IUnknown{
    ...
    EnumConflicts(pszHandlerID, pszItemID, ppEnum) {
        ...
        result := ComCall(3, this, "ptr", pszHandlerID, "ptr", pszItemID, "ptr", ppEnum, "int")
        if(result != 0)
    ...
    BindToConflict(pConflictIdInfo, riid, ppv) {
        result := ComCall(4, this, "ptr", pConflictIdInfo, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {PWSTR} pszHandlerID 
     * @param {PWSTR} pszItemID 
     * @param {Pointer<UInt32>} pnConflicts 
     * @returns {HRESULT} 
     */
    GetCount(pszHandlerID, pszItemID, pnConflicts) {
        ...
        result := ComCall(6, this, "ptr", pszHandlerID, "ptr", pszItemID, "uint*", pnConflicts, "int")
        if(result != 0)
            throw OSError(result)
