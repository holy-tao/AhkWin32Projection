 ...
 */
export default struct IQueryCancelAutoPlay extends IUnknown {
    /**
     ...
     * used for IQueryCancelAutoPlay interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        AllowAutoPlay : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IQueryCancelAutoPlay.Vtbl()
        }
    ...
    Query(iid) {
        if (IQueryCancelAutoPlay.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.AllowAutoPlay)
    }
}
