...
class ISyncMgrHandler extends IUnknown{
    ...
    GetHandlerInfo(ppHandlerInfo) {
        result := ComCall(4, this, "ptr", ppHandlerInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetObject(rguidObjectID, riid, ppv) {
        result := ComCall(5, this, "ptr", rguidObjectID, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetCapabilities(pmCapabilities) {
        result := ComCall(6, this, "int*", pmCapabilities, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPolicies(pmPolicies) {
        result := ComCall(7, this, "int*", pmPolicies, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<ISyncMgrSessionCreator>} pSessionCreator 
    ...
    Synchronize(ppszItemIDs, cItems, hwndOwner, pSessionCreator, punk) {
        ...
        result := ComCall(10, this, "ptr", ppszItemIDs, "uint", cItems, "ptr", hwndOwner, "ptr", pSessionCreator, "ptr", punk, "int")
        if(result != 0)
            throw OSError(result)
