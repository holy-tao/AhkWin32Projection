 ...
 * </div>
 * <div> </div>
 * @see https://learn.microsoft.com/windows/win32/api//content/storageprovider/nn-storageprovider-istorageproviderhandler
...
class IStorageProviderHandler extends IUnknown{
     ...
     * @param {PWSTR} path_ The path for the relevant file.
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/storageprovider/nf-storageprovider-istorageproviderhandler-getpropertyhandlerfrompath
     */
    GetPropertyHandlerFromPath(path_) {
        path_ := path_ is String ? StrPtr(path_) : path_

        result := ComCall(3, this, "ptr", path_, "ptr*", &propertyHandler := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/storageprovider/nf-storageprovider-istorageproviderhandler-getpropertyhandlerfromuri
     */
    GetPropertyHandlerFromUri(uri) {
        uri := uri is String ? StrPtr(uri) : uri

        result := ComCall(4, this, "ptr", uri, "ptr*", &propertyHandler := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/storageprovider/nf-storageprovider-istorageproviderhandler-getpropertyhandlerfromfileid
     */
    GetPropertyHandlerFromFileId(fileId) {
        fileId := fileId is String ? StrPtr(fileId) : fileId

        result := ComCall(5, this, "ptr", fileId, "ptr*", &propertyHandler := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
