#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\System\Com\IUnknown.ahk

/**
 * Retrieves the IStorageProviderPropertyHandler associated with a specific file or folder.
 * @remarks
 * 
  * <div class="alert"><b>Caution</b>  <p class="note">You should only implement this interface if you have a specific need to do so.  
  * 
  * </div>
  * <div> </div>
  * 
 * @see https://docs.microsoft.com/windows/win32/api//storageprovider/nn-storageprovider-istorageproviderhandler
 * @namespace Windows.Win32.UI.Shell
 * @version v4.0.30319
 */
class IStorageProviderHandler extends IUnknown{
    /**
     * The interface identifier for IStorageProviderHandler
     * @type {Guid}
     */
    static IID => Guid("{162c6fb5-44d3-435b-903d-e613fa093fb5}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 3

    /**
     * 
     * @param {PWSTR} path 
     * @param {Pointer<IStorageProviderPropertyHandler>} propertyHandler 
     * @returns {HRESULT} 
     */
    GetPropertyHandlerFromPath(path, propertyHandler) {
        path := path is String ? StrPtr(path) : path

        result := ComCall(3, this, "ptr", path, "ptr", propertyHandler, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} uri 
     * @param {Pointer<IStorageProviderPropertyHandler>} propertyHandler 
     * @returns {HRESULT} 
     */
    GetPropertyHandlerFromUri(uri, propertyHandler) {
        uri := uri is String ? StrPtr(uri) : uri

        result := ComCall(4, this, "ptr", uri, "ptr", propertyHandler, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} fileId 
     * @param {Pointer<IStorageProviderPropertyHandler>} propertyHandler 
     * @returns {HRESULT} 
     */
    GetPropertyHandlerFromFileId(fileId, propertyHandler) {
        fileId := fileId is String ? StrPtr(fileId) : fileId

        result := ComCall(5, this, "ptr", fileId, "ptr", propertyHandler, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
