 ...
 * @see https://learn.microsoft.com/windows/win32/api/syncmgr/nn-syncmgr-isyncmgreventlinkuioperation
 * @namespace Windows.Win32.UI.Shell
 */
export default struct ISyncMgrEventLinkUIOperation extends ISyncMgrUIOperation {
    /**
     ...
     * used for ISyncMgrEventLinkUIOperation interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISyncMgrEventLinkUIOperation.Vtbl()
        }
    ...
    Init(rguidEventID, pEvent) {
        result := ComCall(4, this, Guid.Ptr, rguidEventID, "ptr", pEvent, "HRESULT")
        return result
    }

    Query(iid) {
        if (ISyncMgrEventLinkUIOperation.IID.Equals(iid)) {
            return true
        }
