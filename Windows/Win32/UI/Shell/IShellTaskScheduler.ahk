...
class IShellTaskScheduler extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<IRunnableTask>} prt 
    ...
    AddTask(prt, rtoid, lParam, dwPriority) {
        result := ComCall(3, this, "ptr", prt, "ptr", rtoid, "ptr", lParam, "uint", dwPriority, "int")
        if(result != 0)
    ...
    RemoveTasks(rtoid, lParam, bWaitIfRunning) {
        result := ComCall(4, this, "ptr", rtoid, "ptr", lParam, "int", bWaitIfRunning, "int")
        if(result != 0)
            throw OSError(result)
