...
class IHlinkTarget extends IUnknown{
    ...
    Navigate(grfHLNF, pwzJumpLocation) {
        ...
        result := ComCall(5, this, "uint", grfHLNF, "ptr", pwzJumpLocation, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetMoniker(pwzLocation, dwAssign) {
        ...
        result := ComCall(6, this, "ptr", pwzLocation, "uint", dwAssign, "ptr*", &ppimkLocation := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetFriendlyName(pwzLocation) {
        ...
        result := ComCall(7, this, "ptr", pwzLocation, "ptr*", &ppwzFriendlyName := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
