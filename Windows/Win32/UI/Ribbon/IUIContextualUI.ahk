 ...
 */
export default struct IUIContextualUI extends IUnknown {
    /**
     ...
     * used for IUIContextualUI interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        ShowAtLocation : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IUIContextualUI.Vtbl()
        }
    ...
    Query(iid) {
        if (IUIContextualUI.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.ShowAtLocation)
    }
}
