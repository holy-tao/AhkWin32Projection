...
class IInkCursor extends IDispatch{
     ...
     */
    get_Inverted(Status) {
        result := ComCall(9, this, "ptr", Status, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_DrawingAttributes(Attributes) {
        result := ComCall(10, this, "ptr", Attributes, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_Tablet(Tablet) {
        result := ComCall(12, this, "ptr", Tablet, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_Buttons(Buttons) {
        result := ComCall(13, this, "ptr", Buttons, "int")
        if(result != 0)
            throw OSError(result)
