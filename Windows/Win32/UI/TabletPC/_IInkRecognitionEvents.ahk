 ...
 */
export default struct _IInkRecognitionEvents extends IDispatch {
    /**
     ...
     * used for _IInkRecognitionEvents interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := _IInkRecognitionEvents.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (_IInkRecognitionEvents.IID.Equals(iid)) {
            return true
        }
