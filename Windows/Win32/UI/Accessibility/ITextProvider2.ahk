 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/uiautomationcore/nn-uiautomationcore-itextprovider2
...
class ITextProvider2 extends ITextProvider{
    ...
    RangeFromAnnotation(annotationElement) {
        result := ComCall(9, this, "ptr", annotationElement, "ptr*", &pRetVal := 0, "int")
        if(result != 0) {
    ...
    GetCaretRange(isActive) {
        ...
        result := ComCall(10, this, isActiveMarshal, isActive, "ptr*", &pRetVal := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
