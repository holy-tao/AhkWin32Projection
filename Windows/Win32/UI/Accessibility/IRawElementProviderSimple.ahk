...
#Import ".\ProviderOptions.ahk" { ProviderOptions }
 ...
 */
export default struct IRawElementProviderSimple extends IUnknown {
    /**
     ...
     * used for IRawElementProviderSimple interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        get_ProviderOptions        : IntPtr
        GetPatternProvider         : IntPtr
        GetPropertyValue           : IntPtr
        get_HostRawElementProvider : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IRawElementProviderSimple.Vtbl()
        }
    ...
    GetPatternProvider(patternId) {
        result := ComCall(4, this, UIA_PATTERN_ID, patternId, "ptr*", &pRetVal := 0, "HRESULT")
    ...
    GetPropertyValue(propertyId) {
        pRetVal := VARIANT()
        result := ComCall(5, this, UIA_PROPERTY_ID, propertyId, VARIANT.Ptr, pRetVal, "HRESULT")
        return pRetVal
    }
    ...
    Query(iid) {
        if (IRawElementProviderSimple.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.get_ProviderOptions)
        CallbackFree(this.vtbl.GetPatternProvider)
        CallbackFree(this.vtbl.GetPropertyValue)
        CallbackFree(this.vtbl.get_HostRawElementProvider)
    }
}
