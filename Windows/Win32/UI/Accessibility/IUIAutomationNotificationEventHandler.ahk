 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/uiautomationclient/nn-uiautomationclient-iuiautomationnotificationeventhandler
...
class IUIAutomationNotificationEventHandler extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/uiautomationclient/nf-uiautomationclient-iuiautomationnotificationeventhandler-handlenotificationevent
     */
    HandleNotificationEvent(sender, notificationKind_, notificationProcessing_, displayString, activityId) {
        if(displayString is String) {
            pin := BSTR.Alloc(displayString)
            displayString := pin.Value
        }
        if(activityId is String) {
            pin := BSTR.Alloc(activityId)
            activityId := pin.Value
        }

        result := ComCall(3, this, "ptr", sender, "int", notificationKind_, "int", notificationProcessing_, "ptr", displayString, "ptr", activityId, "int")
        if(result != 0) {
