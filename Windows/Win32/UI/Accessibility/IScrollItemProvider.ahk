 ...
 */
export default struct IScrollItemProvider extends IUnknown {
    /**
     ...
     * used for IScrollItemProvider interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IScrollItemProvider.Vtbl()
        }
    ...
    Query(iid) {
        if (IScrollItemProvider.IID.Equals(iid)) {
            return true
        }
