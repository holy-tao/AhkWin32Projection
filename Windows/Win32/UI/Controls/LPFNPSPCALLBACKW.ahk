 ...
 * @see https://learn.microsoft.com/windows/win32/api/prsht/nc-prsht-lpfnpspcallbackw
 * @namespace Windows.Win32.UI.Controls
 * @charset Unicode
 */
export default struct LPFNPSPCALLBACKW {
    value : IntPtr

    __value {
        set {
            if (value is LPFNPSPCALLBACKW) {
     ...
     * @param {Pointer<PROPSHEETPAGEW>} ppsp Type: <b>LPPROPSHEETPAGE</b>
     * 
    ...
    Call(_hwnd, uMsg, ppsp) {
        result := DllCall(this.value, HWND, _hwnd, PSPCB_MESSAGE, uMsg, PROPSHEETPAGEW.Ptr, ppsp, UInt32)
        return result
    }

    /**
     * A LPFNPSPCALLBACKW that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends LPFNPSPCALLBACKW {
        /**
         * Creates a LPFNPSPCALLBACKW pointer that invokes the given AHK function when called.
         * @param {Func(HWND, PSPCB_MESSAGE, PROPSHEETPAGEW) => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 3)) {
