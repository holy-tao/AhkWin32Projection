...
class IAccStore extends IUnknown{
    ...
    GetDocuments(enumUnknown) {
        result := ComCall(5, this, "ptr", enumUnknown, "int")
        if(result != 0)
            throw OSError(result)
    ...
    LookupByHWND(hWnd, riid, ppunk) {
        ...
        result := ComCall(6, this, "ptr", hWnd, "ptr", riid, "ptr", ppunk, "int")
        if(result != 0)
            throw OSError(result)
    ...
    LookupByPoint(pt, riid, ppunk) {
        result := ComCall(7, this, "ptr", pt, "ptr", riid, "ptr", ppunk, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFocused(riid, ppunk) {
        result := ComCall(9, this, "ptr", riid, "ptr", ppunk, "int")
        if(result != 0)
            throw OSError(result)
