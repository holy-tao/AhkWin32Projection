 ...
 */
export default struct ITfEditSession extends IUnknown {
    /**
     ...
     * used for ITfEditSession interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        DoEditSession : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITfEditSession.Vtbl()
        }
     ...
     */
    DoEditSession(ec) {
        result := ComCall(3, this, UInt32, ec, "HRESULT")
        return result
    }

    Query(iid) {
        if (ITfEditSession.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.DoEditSession)
    }
}
