 ...
 */
export default struct IEnumITfCompositionView extends IUnknown {
    /**
     ...
     * used for IEnumITfCompositionView interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IEnumITfCompositionView.Vtbl()
        }
    ...
    Next(ulCount, rgCompositionView, pcFetched) {
        ...
        result := ComCall(4, this, UInt32, ulCount, ITfCompositionView.Ptr, rgCompositionView, pcFetchedMarshal, pcFetched, "HRESULT")
        return result
    }
    ...
    _Query(iid) {
        if (IEnumITfCompositionView.IID.Equals(iid)) {
            return true
        }
