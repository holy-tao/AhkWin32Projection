 ...
 */
export default struct ITfContextOwner extends IUnknown {
    /**
     ...
     * used for ITfContextOwner interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITfContextOwner.Vtbl()
        }
    ...
    GetWnd() {
        phwnd := HWND()
        result := ComCall(7, this, HWND.Ptr, phwnd, "HRESULT")
        return phwnd
    }
    ...
    GetAttribute(rguidAttribute) {
        pvarValue := VARIANT()
        result := ComCall(8, this, Guid.Ptr, rguidAttribute, VARIANT.Ptr, pvarValue, "HRESULT")
        return pvarValue
    }
