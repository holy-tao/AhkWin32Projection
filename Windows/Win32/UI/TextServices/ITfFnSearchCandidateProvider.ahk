 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/ctffunc/nn-ctffunc-itffnsearchcandidateprovider
...
class ITfFnSearchCandidateProvider extends ITfFunction{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/ctffunc/nf-ctffunc-itffnsearchcandidateprovider-getsearchcandidates
     */
    GetSearchCandidates(bstrQuery, bstrApplicationId) {
        if(bstrQuery is String) {
            pin := BSTR.Alloc(bstrQuery)
            bstrQuery := pin.Value
        }
        if(bstrApplicationId is String) {
            pin := BSTR.Alloc(bstrApplicationId)
            bstrApplicationId := pin.Value
        }

        result := ComCall(4, this, "ptr", bstrQuery, "ptr", bstrApplicationId, "ptr*", &pplist := 0, "int")
        if(result != 0) {
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/ctffunc/nf-ctffunc-itffnsearchcandidateprovider-setresult
     */
    SetResult(bstrQuery, bstrApplicationID, bstrResult) {
        ...
        result := ComCall(5, this, "ptr", bstrQuery, "ptr", bstrApplicationID, "ptr", bstrResult, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
