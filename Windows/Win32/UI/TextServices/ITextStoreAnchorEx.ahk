 ...
 */
export default struct ITextStoreAnchorEx extends IUnknown {
    /**
     ...
     * used for ITextStoreAnchorEx interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ITextStoreAnchorEx.Vtbl()
        }
    ...
    ScrollToRect(pStart, pEnd, rc, dwPosition) {
        result := ComCall(3, this, "ptr", pStart, "ptr", pEnd, RECT, rc, UInt32, dwPosition, "HRESULT")
        return result
    }

    _Query(iid) {
        if (ITextStoreAnchorEx.IID.Equals(iid)) {
            return true
        }
