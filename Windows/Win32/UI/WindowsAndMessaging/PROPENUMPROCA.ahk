 ...
 * @see https://learn.microsoft.com/windows/win32/api/winuser/nc-winuser-propenumproca
 * @namespace Windows.Win32.UI.WindowsAndMessaging
 * @charset ANSI
 */
export default struct PROPENUMPROCA {
    value : IntPtr

    __value {
        set {
            if (value is PROPENUMPROCA) {
    ...
    /**
     * A PROPENUMPROCA that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PROPENUMPROCA {
        /**
         * Creates a PROPENUMPROCA pointer that invokes the given AHK function when called.
         * @param {Func(HWND, PSTR, HANDLE) => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 3)) {
