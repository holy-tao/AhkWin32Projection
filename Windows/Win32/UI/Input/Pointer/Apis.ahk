 ...
 * @since windows8.0
 */
export GetUnpredictedMessagePos() {
    result := DllCall("USER32.dll\GetUnpredictedMessagePos", UInt32)
    return result
}
 ...
 * @since windows10.0.17763
 */
export DestroySyntheticPointerDevice(device) {
    DllCall("USER32.dll\DestroySyntheticPointerDevice", HSYNTHETICPOINTERDEVICE, device)
}
 ...
 * @since windows8.0
 */
export EnableMouseInPointer(fEnable) {
    A_LastError := 0

    result := DllCall("USER32.dll\EnableMouseInPointer", BOOL, fEnable, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export IsMouseInPointerEnabled() {
    result := DllCall("USER32.dll\IsMouseInPointerEnabled", BOOL)
    return result
}
 ...
 * @since windows8.0
 */
export GetPointerDevice(device, pointerDevice) {
    A_LastError := 0

    result := DllCall("USER32.dll\GetPointerDevice", HANDLE, device, POINTER_DEVICE_INFO.Ptr, pointerDevice, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows8.0
 */
export GetPointerDeviceRects(device, pointerDeviceRect, displayRect) {
    A_LastError := 0

    result := DllCall("USER32.dll\GetPointerDeviceRects", HANDLE, device, RECT.Ptr, pointerDeviceRect, RECT.Ptr, displayRect, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
