 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nn-imepad-iimepadapplet
...
class IImePadApplet extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nf-imepad-iimepadapplet-initialize
     */
    Initialize(lpIImePad) {
        result := ComCall(3, this, "ptr", lpIImePad, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} <b>S_OK</b> if successful, otherwise <b>E_FAIL</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nf-imepad-iimepadapplet-terminate
     */
    Terminate() {
    ...
    GetAppletConfig(lpAppletCfg) {
        result := ComCall(5, this, "ptr", lpAppletCfg, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} <b>S_OK</b> if successful, otherwise <b>E_FAIL</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nf-imepad-iimepadapplet-createui
     */
    CreateUI(hwndParent, lpImeAppletUI) {
        hwndParent := hwndParent is Win32Handle ? NumGet(hwndParent, "ptr") : hwndParent

        result := ComCall(6, this, "ptr", hwndParent, "ptr", lpImeAppletUI, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @param {WPARAM} wParam_ Additional information specific to <i>notify</i>.
     * @param {LPARAM} lParam_ Additional information specific to <i>notify</i>.
     * @returns {HRESULT} <b>S_OK</b> if successful, otherwise <b>E_FAIL</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nf-imepad-iimepadapplet-notify
     */
    Notify(lpImePad, notify, wParam_, lParam_) {
        wParam_ := wParam_ is Win32Handle ? NumGet(wParam_, "ptr") : wParam_
        lParam_ := lParam_ is Win32Handle ? NumGet(lParam_, "ptr") : lParam_

        result := ComCall(7, this, "ptr", lpImePad, "int", notify, "ptr", wParam_, "ptr", lParam_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
