 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nn-imepad-iimepad
...
class IImePad extends IUnknown{
     ...
     * @param {Integer} reqId 
     * @param {WPARAM} wParam_ Additional information specific to <i>reqId</i>.
     * @param {LPARAM} lParam_ Additional information specific to <i>reqId</i>.
     * @returns {HRESULT} <b>S_OK</b> if successful, otherwise <b>E_FAIL</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/imepad/nf-imepad-iimepad-request
     */
    Request(pIImePadApplet, reqId, wParam_, lParam_) {
        wParam_ := wParam_ is Win32Handle ? NumGet(wParam_, "ptr") : wParam_
        lParam_ := lParam_ is Win32Handle ? NumGet(lParam_, "ptr") : lParam_

        result := ComCall(3, this, "ptr", pIImePadApplet, "int", reqId, "ptr", wParam_, "ptr", lParam_, "int")
        if(result != 0) {
