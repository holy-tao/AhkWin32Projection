...
;@region Methods
     ...
     * @since windows5.0
     */
    static ActivateKeyboardLayout(_hkl, Flags) {
        _hkl := _hkl is Win32Handle ? NumGet(_hkl, "ptr") : _hkl

        A_LastError := 0

        result := DllCall("USER32.dll\ActivateKeyboardLayout", "ptr", _hkl, "uint", Flags, "ptr")
        if(A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static UnloadKeyboardLayout(_hkl) {
        _hkl := _hkl is Win32Handle ? NumGet(_hkl, "ptr") : _hkl

        A_LastError := 0

        result := DllCall("USER32.dll\UnloadKeyboardLayout", "ptr", _hkl, "int")
     ...
     */
    static RegisterHotKey(_hWnd, id, fsModifiers, vk) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        A_LastError := 0

        result := DllCall("USER32.dll\RegisterHotKey", "ptr", _hWnd, "int", id, "uint", fsModifiers, "uint", vk, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static UnregisterHotKey(_hWnd, id) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        A_LastError := 0

        result := DllCall("USER32.dll\UnregisterHotKey", "ptr", _hWnd, "int", id, "int")
        if(!result && A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static SetFocus(_hWnd) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        A_LastError := 0

        result := DllCall("USER32.dll\SetFocus", "ptr", _hWnd, "ptr")
        if(A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static GetKeyNameTextA(_lParam, lpString, cchSize) {
        lpString := lpString is String ? StrPtr(lpString) : lpString

        A_LastError := 0

        result := DllCall("USER32.dll\GetKeyNameTextA", "int", _lParam, "ptr", lpString, "int", cchSize, "int")
        if(A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static GetKeyNameTextW(_lParam, lpString, cchSize) {
        lpString := lpString is String ? StrPtr(lpString) : lpString

        A_LastError := 0

        result := DllCall("USER32.dll\GetKeyNameTextW", "int", _lParam, "ptr", lpString, "int", cchSize, "int")
        if(A_LastError) {
            throw OSError(A_LastError)
        }
     ...
     * @since windows5.0
     */
    static SetCapture(_hWnd) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        result := DllCall("USER32.dll\SetCapture", "ptr", _hWnd, "ptr")
        resultHandle := HWND({Value: result}, True)
        return resultHandle
    }
     ...
     * @since windows5.0
     */
    static EnableWindow(_hWnd, bEnable) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        result := DllCall("USER32.dll\EnableWindow", "ptr", _hWnd, "int", bEnable, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static IsWindowEnabled(_hWnd) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        result := DllCall("USER32.dll\IsWindowEnabled", "ptr", _hWnd, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static DragDetect(_hwnd, pt) {
        _hwnd := _hwnd is Win32Handle ? NumGet(_hwnd, "ptr") : _hwnd

        result := DllCall("USER32.dll\DragDetect", "ptr", _hwnd, "ptr", pt, "int")
        return result
    }
     ...
     * @since windows5.0
     */
    static SetActiveWindow(_hWnd) {
        _hWnd := _hWnd is Win32Handle ? NumGet(_hWnd, "ptr") : _hWnd

        A_LastError := 0

        result := DllCall("USER32.dll\SetActiveWindow", "ptr", _hWnd, "ptr")
        if(A_LastError) {
            throw OSError(A_LastError)
        }
