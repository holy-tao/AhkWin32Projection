 ...
 * @since windows6.1
 */
export CloseTouchInputHandle(_hTouchInput) {
    A_LastError := 0

    result := DllCall("USER32.dll\CloseTouchInputHandle", HTOUCHINPUT, _hTouchInput, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.1
 */
export RegisterTouchWindow(_hwnd, ulFlags) {
    A_LastError := 0

    result := DllCall("USER32.dll\RegisterTouchWindow", HWND, _hwnd, REGISTER_TOUCH_WINDOW_FLAGS, ulFlags, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.1
 */
export UnregisterTouchWindow(_hwnd) {
    A_LastError := 0

    result := DllCall("USER32.dll\UnregisterTouchWindow", HWND, _hwnd, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.1
 */
export GetGestureInfo(_hGestureInfo, pGestureInfo) {
    A_LastError := 0

    result := DllCall("USER32.dll\GetGestureInfo", HGESTUREINFO, _hGestureInfo, GESTUREINFO.Ptr, pGestureInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.1
 */
export CloseGestureInfoHandle(_hGestureInfo) {
    A_LastError := 0

    result := DllCall("USER32.dll\CloseGestureInfoHandle", HGESTUREINFO, _hGestureInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
