...
class IGameInput extends IUnknown{
    ...
    GetCurrentReading(inputKind, device, reading) {
        result := ComCall(4, this, "int", inputKind, "ptr", device, "ptr", reading, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<IGameInputReading>} referenceReading 
     * @param {Integer} inputKind 
     * @param {Pointer<IGameInputDevice>} device 
     * @param {Pointer<IGameInputReading>} reading 
     * @returns {HRESULT} 
    ...
    GetNextReading(referenceReading, inputKind, device, reading) {
        result := ComCall(5, this, "ptr", referenceReading, "int", inputKind, "ptr", device, "ptr", reading, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPreviousReading(referenceReading, inputKind, device, reading) {
        result := ComCall(6, this, "ptr", referenceReading, "int", inputKind, "ptr", device, "ptr", reading, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetTemporalReading(timestamp, device, reading) {
        result := ComCall(7, this, "uint", timestamp, "ptr", device, "ptr", reading, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt64>} callbackToken 
     * @returns {HRESULT} 
     */
    RegisterReadingCallback(device, inputKind, analogThreshold, context, callbackFunc, callbackToken) {
        result := ComCall(8, this, "ptr", device, "int", inputKind, "float", analogThreshold, "ptr", context, "ptr", callbackFunc, "uint*", callbackToken, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RegisterDeviceCallback(device, inputKind, statusFilter, enumerationKind, context, callbackFunc, callbackToken) {
        result := ComCall(9, this, "ptr", device, "int", inputKind, "int", statusFilter, "int", enumerationKind, "ptr", context, "ptr", callbackFunc, "uint*", callbackToken, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RegisterSystemButtonCallback(device, buttonFilter, context, callbackFunc, callbackToken) {
        result := ComCall(10, this, "ptr", device, "int", buttonFilter, "ptr", context, "ptr", callbackFunc, "uint*", callbackToken, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RegisterKeyboardLayoutCallback(device, context, callbackFunc, callbackToken) {
        result := ComCall(11, this, "ptr", device, "ptr", context, "ptr", callbackFunc, "uint*", callbackToken, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDispatcher(dispatcher) {
        result := ComCall(14, this, "ptr", dispatcher, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateAggregateDevice(inputKind, device) {
        result := ComCall(15, this, "int", inputKind, "ptr", device, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindDeviceFromId(value, device) {
        result := ComCall(16, this, "ptr", value, "ptr", device, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindDeviceFromObject(value, device) {
        result := ComCall(17, this, "ptr", value, "ptr", device, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindDeviceFromPlatformHandle(value, device) {
        ...
        result := ComCall(18, this, "ptr", value, "ptr", device, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindDeviceFromPlatformString(value, device) {
        ...
        result := ComCall(19, this, "ptr", value, "ptr", device, "int")
        if(result != 0)
            throw OSError(result)
