...
class IGameInputRawDeviceReport extends IUnknown{
     ...
     * @returns {Pointer} 
     */
    GetRawData(bufferSize, buffer_) {
        buffer_Marshal := buffer_ is VarRef ? "ptr" : "ptr"

        result := ComCall(6, this, "ptr", bufferSize, buffer_Marshal, buffer_, "ptr")
        return result
    }
     ...
     * @returns {Boolean} 
     */
    SetRawData(bufferSize, buffer_) {
        buffer_Marshal := buffer_ is VarRef ? "ptr" : "ptr"

        result := ComCall(7, this, "ptr", bufferSize, buffer_Marshal, buffer_, "int")
        return result
    }
