...
class IDxcContainerReflection extends IUnknown{
     ...
     */
    GetPartCount() {
        result := ComCall(4, this, "uint*", &pResult := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetPartKind(idx) {
        result := ComCall(5, this, "uint", idx, "uint*", &pResult := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetPartContent(idx) {
        result := ComCall(6, this, "uint", idx, "ptr*", &ppResult := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    FindFirstPartKind(kind) {
        result := ComCall(7, this, "uint", kind, "uint*", &pResult := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetPartReflection(idx, iid, ppvObject) {
        ...
        result := ComCall(8, this, "uint", idx, "ptr", iid, ppvObjectMarshal, ppvObject, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
