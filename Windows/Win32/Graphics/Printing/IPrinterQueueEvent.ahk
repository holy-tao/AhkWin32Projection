 ...
 */
export default struct IPrinterQueueEvent extends IDispatch {
    /**
     ...
     * used for IPrinterQueueEvent interfaces
    */
    struct Vtbl extends IDispatch.Vtbl {
        OnBidiResponseReceived : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPrinterQueueEvent.Vtbl()
        }
    ...
    OnBidiResponseReceived(bstrResponse, hrStatus) {
        ...
        result := ComCall(7, this, BSTR, bstrResponse, "int", hrStatus, "HRESULT")
        return result
    }

    Query(iid) {
        if (IPrinterQueueEvent.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.OnBidiResponseReceived)
    }
}
