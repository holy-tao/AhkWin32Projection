...
class IPrinterPropertyBag extends IDispatch{
    ...
    GetBytes(bstrName, pcbValue, ppValue) {
        ...
        result := ComCall(13, this, "ptr", bstrName, "uint*", pcbValue, "char*", ppValue, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetBytes(bstrName, cbValue, pValue) {
        ...
        result := ComCall(14, this, "ptr", bstrName, "uint", cbValue, "char*", pValue, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetReadStream(bstrName, ppValue) {
        ...
        result := ComCall(15, this, "ptr", bstrName, "ptr", ppValue, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetWriteStream(bstrName, ppValue) {
        ...
        result := ComCall(16, this, "ptr", bstrName, "ptr", ppValue, "int")
        if(result != 0)
            throw OSError(result)
