 ...
 */
export default struct IPartPrintTicket extends IPartBase {
    /**
     ...
     * used for IPartPrintTicket interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPartPrintTicket.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (IPartPrintTicket.IID.Equals(iid)) {
            return true
        }
