 ...
 */
export default struct IPrintAsyncNotifyChannel extends IUnknown {
    /**
     ...
     * used for IPrintAsyncNotifyChannel interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        SendNotification : IntPtr
        CloseChannel     : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPrintAsyncNotifyChannel.Vtbl()
        }
    ...
    Implement(implObj, flags := "") {
        ...
        this.vtbl.SendNotification := CallbackCreate(ObjBindMethod(implObj, "SendNotification"), flags, 2)
        this.vtbl.CloseChannel := CallbackCreate(ObjBindMethod(implObj, "CloseChannel"), flags, 2)
    }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.SendNotification)
        CallbackFree(this.vtbl.CloseChannel)
    }
}
