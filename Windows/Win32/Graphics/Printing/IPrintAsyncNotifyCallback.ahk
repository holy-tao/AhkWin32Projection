 ...
 */
export default struct IPrintAsyncNotifyCallback extends IUnknown {
    /**
     ...
     * used for IPrintAsyncNotifyCallback interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        OnEventNotify : IntPtr
        ChannelClosed : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPrintAsyncNotifyCallback.Vtbl()
        }
    ...
    Query(iid) {
        if (IPrintAsyncNotifyCallback.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.ChannelClosed)
    }
}
