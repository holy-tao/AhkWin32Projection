...
class IPrintReadStream extends IUnknown{
    ...
    Seek(dlibMove, dwOrigin, plibNewPosition) {
        result := ComCall(3, this, "int64", dlibMove, "uint", dwOrigin, "uint*", plibNewPosition, "int")
        if(result != 0)
    ...
    ReadBytes(pvBuffer, cbRequested, pcbRead, pbEndOfFile) {
        result := ComCall(4, this, "ptr", pvBuffer, "uint", cbRequested, "uint*", pcbRead, "ptr", pbEndOfFile, "int")
        if(result != 0)
            throw OSError(result)
