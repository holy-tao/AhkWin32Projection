 ...
 */
export default struct ID3D12Resource1 extends ID3D12Resource {
    /**
     ...
     * used for ID3D12Resource1 interfaces
    */
    struct Vtbl extends ID3D12Resource.Vtbl {
        GetProtectedResourceSession : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ID3D12Resource1.Vtbl()
        }
    ...
    GetProtectedResourceSession(riid) {
        result := ComCall(15, this, Guid.Ptr, riid, "ptr*", &ppProtectedSession := 0, "HRESULT")
        return ppProtectedSession
    }

    Query(iid) {
        if (ID3D12Resource1.IID.Equals(iid)) {
            return true
        }
