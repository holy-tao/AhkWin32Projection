...
class ID3D12Device7 extends ID3D12Device6{
     ...
     * @param {Pointer<ID3D12StateObject>} pStateObjectToGrowFrom 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppNewStateObject 
     * @returns {HRESULT} 
     */
    AddToStateObject(pAddition, pStateObjectToGrowFrom, riid, ppNewStateObject) {
        result := ComCall(66, this, "ptr", pAddition, "ptr", pStateObjectToGrowFrom, "ptr", riid, "ptr", ppNewStateObject, "int")
        if(result != 0)
    ...
    CreateProtectedResourceSession1(pDesc, riid, ppSession) {
        result := ComCall(67, this, "ptr", pDesc, "ptr", riid, "ptr", ppSession, "int")
        if(result != 0)
            throw OSError(result)
