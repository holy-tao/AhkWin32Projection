...
class ID3D12CompilerCacheSession extends ID3D12CompilerFactoryChild{
     ...
     * @param {Pointer<UInt32>} pGroupVersion 
     * @returns {HRESULT} 
     */
    FindGroup(pGroupKey, pGroupVersion) {
        result := ComCall(4, this, "ptr", pGroupKey, "uint*", pGroupVersion, "int")
        if(result != 0)
     ...
     * 
     * @param {Pointer<D3D12_COMPILER_CACHE_GROUP_KEY>} pGroupKey 
     * @param {Pointer<UInt32>} pExpectedGroupVersion 
    ...
    FindGroupValueKeys(pGroupKey, pExpectedGroupVersion, CallbackFunc, pContext) {
        result := ComCall(5, this, "ptr", pGroupKey, "uint*", pExpectedGroupVersion, "ptr", CallbackFunc, "ptr", pContext, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindGroupValues(pGroupKey, pExpectedGroupVersion, ValueTypeFlags, CallbackFunc, pContext) {
        result := ComCall(6, this, "ptr", pGroupKey, "uint*", pExpectedGroupVersion, "int", ValueTypeFlags, "ptr", CallbackFunc, "ptr", pContext, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindValue(pValueKey, pTypedValues, NumTypedValues, pCallbackFunc, pContext) {
        result := ComCall(7, this, "ptr", pValueKey, "ptr", pTypedValues, "uint", NumTypedValues, "ptr", pCallbackFunc, "ptr", pContext, "int")
        if(result != 0)
            throw OSError(result)
