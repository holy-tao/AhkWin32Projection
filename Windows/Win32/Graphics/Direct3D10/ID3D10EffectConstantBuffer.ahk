...
class ID3D10EffectConstantBuffer extends ID3D10EffectVariable{
    ...
    /**
     * 
     * @param {Pointer<ID3D10Buffer>} pConstantBuffer 
     * @returns {HRESULT} 
     */
    ...
    GetConstantBuffer(ppConstantBuffer) {
        result := ComCall(26, this, "ptr", ppConstantBuffer, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<ID3D10ShaderResourceView>} pTextureBuffer 
     * @returns {HRESULT} 
     */
    ...
    GetTextureBuffer(ppTextureBuffer) {
        result := ComCall(28, this, "ptr", ppTextureBuffer, "int")
        if(result != 0)
            throw OSError(result)
