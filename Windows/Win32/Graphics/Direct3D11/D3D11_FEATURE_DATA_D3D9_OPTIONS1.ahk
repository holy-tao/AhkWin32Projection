...
class D3D11_FEATURE_DATA_D3D9_OPTIONS1 extends Win32Struct
{
    ...
    FullNonPow2TextureSupported{
        get {
            if(!this.HasProp("__FullNonPow2TextureSupported"))
                this.__FullNonPow2TextureSupported := BOOL(this.ptr + 0)
            return this.__FullNonPow2TextureSupported
        }
    }
    ...
    DepthAsTextureWithLessEqualComparisonFilterSupported{
        get {
            if(!this.HasProp("__DepthAsTextureWithLessEqualComparisonFilterSupported"))
                this.__DepthAsTextureWithLessEqualComparisonFilterSupported := BOOL(this.ptr + 4)
            return this.__DepthAsTextureWithLessEqualComparisonFilterSupported
        }
    }
    ...
    SimpleInstancingSupported{
        get {
            if(!this.HasProp("__SimpleInstancingSupported"))
                this.__SimpleInstancingSupported := BOOL(this.ptr + 8)
    ...
    TextureCubeFaceRenderTargetWithNonCubeDepthStencilSupported{
        get {
            if(!this.HasProp("__TextureCubeFaceRenderTargetWithNonCubeDepthStencilSupported"))
                this.__TextureCubeFaceRenderTargetWithNonCubeDepthStencilSupported := BOOL(this.ptr + 12)
            return this.__TextureCubeFaceRenderTargetWithNonCubeDepthStencilSupported
        }
    }
}
