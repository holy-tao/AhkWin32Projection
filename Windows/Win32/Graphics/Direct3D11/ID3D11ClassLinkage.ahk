...
class ID3D11ClassLinkage extends ID3D11DeviceChild{
    ...
    GetClassInstance(pClassInstanceName, InstanceIndex, ppInstance) {
        result := ComCall(7, this, "ptr", pClassInstanceName, "uint", InstanceIndex, "ptr", ppInstance, "int")
        if(result != 0)
    ...
    CreateClassInstance(pClassTypeName, ConstantBufferOffset, ConstantVectorOffset, TextureOffset, SamplerOffset, ppInstance) {
        result := ComCall(8, this, "ptr", pClassTypeName, "uint", ConstantBufferOffset, "uint", ConstantVectorOffset, "uint", TextureOffset, "uint", SamplerOffset, "ptr", ppInstance, "int")
        if(result != 0)
            throw OSError(result)
