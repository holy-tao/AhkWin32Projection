...
class ID3D11FunctionLinkingGraph extends IUnknown{
    ...
    SetInputSignature(pInputParameters, cInputParameters, ppInputNode) {
        result := ComCall(4, this, "ptr", pInputParameters, "uint", cInputParameters, "ptr", ppInputNode, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetOutputSignature(pOutputParameters, cOutputParameters, ppOutputNode) {
        result := ComCall(5, this, "ptr", pOutputParameters, "uint", cOutputParameters, "ptr", ppOutputNode, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<ID3D11Module>} pModuleWithFunctionPrototype 
    ...
    CallFunction(pModuleInstanceNamespace, pModuleWithFunctionPrototype, pFunctionName, ppCallNode) {
        result := ComCall(6, this, "ptr", pModuleInstanceNamespace, "ptr", pModuleWithFunctionPrototype, "ptr", pFunctionName, "ptr", ppCallNode, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<ID3D11LinkingNode>} pSrcNode 
     * @param {Integer} SrcParameterIndex 
     * @param {Pointer<ID3D11LinkingNode>} pDstNode 
    ...
    GetLastError(ppErrorBuffer) {
        result := ComCall(9, this, "ptr", ppErrorBuffer, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GenerateHlsl(uFlags, ppBuffer) {
        result := ComCall(10, this, "uint", uFlags, "ptr", ppBuffer, "int")
        if(result != 0)
            throw OSError(result)
