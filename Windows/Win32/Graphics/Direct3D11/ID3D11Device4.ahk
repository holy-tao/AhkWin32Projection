 ...
 */
export default struct ID3D11Device4 extends ID3D11Device3 {
    /**
     ...
     * used for ID3D11Device4 interfaces
    */
    struct Vtbl extends ID3D11Device3.Vtbl {
        RegisterDeviceRemovedEvent : IntPtr
        UnregisterDeviceRemoved    : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ID3D11Device4.Vtbl()
        }
    ...
    RegisterDeviceRemovedEvent(hEvent) {
        result := ComCall(65, this, HANDLE, hEvent, "uint*", &pdwCookie := 0, "HRESULT")
        return pdwCookie
    }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.RegisterDeviceRemovedEvent)
        CallbackFree(this.vtbl.UnregisterDeviceRemoved)
    }
}
