 ...
 */
export default struct ID3D11GeometryShader extends ID3D11DeviceChild {
    /**
     ...
     * used for ID3D11GeometryShader interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ID3D11GeometryShader.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (ID3D11GeometryShader.IID.Equals(iid)) {
            return true
        }
