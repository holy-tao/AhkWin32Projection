...
export GdipCreateEffect(guid, _effect) {
    _effectMarshal := _effect is VarRef ? "ptr*" : IntPtr
...
export GdipCreatePath(brushMode, _path) {
    _pathMarshal := _path is VarRef ? "ptr*" : IntPtr
...
export GdipClonePath(_path, clonePath) {
    clonePathMarshal := clonePath is VarRef ? "ptr*" : IntPtr
...
export GdipGetPathFillMode(_path, _fillmode) {
    _fillmodeMarshal := _fillmode is VarRef ? "int*" : IntPtr
...
export GdipGetPathData(_path, _pathData) {
    _pathDataMarshal := _pathData is VarRef ? "ptr*" : IntPtr
...
export GdipFlattenPath(_path, _matrix, flatness) {
    _matrixMarshal := _matrix is VarRef ? "ptr*" : IntPtr
...
export GdipCreatePathIter(iterator, _path) {
    iteratorMarshal := iterator is VarRef ? "ptr*" : IntPtr
...
export GdipPathIterNextSubpath(iterator, resultCount, startIndex, endIndex, isClosed) {
    resultCountMarshal := resultCount is VarRef ? "int*" : IntPtr
    startIndexMarshal := startIndex is VarRef ? "int*" : IntPtr
    endIndexMarshal := endIndex is VarRef ? "int*" : IntPtr
    isClosedMarshal := isClosed is VarRef ? "int*" : IntPtr
...
export GdipPathIterNextPathType(iterator, resultCount, pathType, startIndex, endIndex) {
    resultCountMarshal := resultCount is VarRef ? "int*" : IntPtr
    pathTypeMarshal := pathType is VarRef ? "char*" : IntPtr
    startIndexMarshal := startIndex is VarRef ? "int*" : IntPtr
    endIndexMarshal := endIndex is VarRef ? "int*" : IntPtr
...
export GdipPathIterIsValid(iterator, valid) {
    validMarshal := valid is VarRef ? "int*" : IntPtr
...
export GdipPathIterHasCurve(iterator, hasCurve) {
    hasCurveMarshal := hasCurve is VarRef ? "int*" : IntPtr
...
export GdipPathIterEnumerate(iterator, resultCount, _points, types, count) {
    resultCountMarshal := resultCount is VarRef ? "int*" : IntPtr
    typesMarshal := types is VarRef ? "char*" : IntPtr
...
export GdipCloneMatrix(_matrix, cloneMatrix) {
    _matrixMarshal := _matrix is VarRef ? "ptr*" : IntPtr
    cloneMatrixMarshal := cloneMatrix is VarRef ? "ptr*" : IntPtr
...
export GdipMultiplyMatrix(_matrix, matrix2, order) {
    _matrixMarshal := _matrix is VarRef ? "ptr*" : IntPtr
    matrix2Marshal := matrix2 is VarRef ? "ptr*" : IntPtr
...
export GdipGetMatrixElements(_matrix, matrixOut) {
    _matrixMarshal := _matrix is VarRef ? "ptr*" : IntPtr
    matrixOutMarshal := matrixOut is VarRef ? "float*" : IntPtr
...
export GdipCreateRegion(_region) {
    _regionMarshal := _region is VarRef ? "ptr*" : IntPtr
...
export GdipCreateRegionRgnData(regionData, _size, _region) {
    regionDataMarshal := regionData is VarRef ? "char*" : IntPtr
    _regionMarshal := _region is VarRef ? "ptr*" : IntPtr
...
export GdipCloneRegion(_region, cloneRegion) {
    cloneRegionMarshal := cloneRegion is VarRef ? "ptr*" : IntPtr
...
export GdipGetRegionData(_region, _buffer, bufferSize, sizeFilled) {
    ...
    sizeFilledMarshal := sizeFilled is VarRef ? "uint*" : IntPtr
    sizeFilledMarshal := sizeFilled == 0 ? IntPtr : "uint*"
...
export GdipCloneBrush(brush, cloneBrush) {
    cloneBrushMarshal := cloneBrush is VarRef ? "ptr*" : IntPtr
...
export GdipGetBrushType(brush, type) {
    typeMarshal := type is VarRef ? "int*" : IntPtr
...
export GdipCreateHatchBrush(_hatchstyle, forecol, backcol, brush) {
    brushMarshal := brush is VarRef ? "ptr*" : IntPtr
...
export GdipGetHatchStyle(brush, _hatchstyle) {
    _hatchstyleMarshal := _hatchstyle is VarRef ? "int*" : IntPtr
...
export GdipGetHatchForegroundColor(brush, forecol) {
    forecolMarshal := forecol is VarRef ? "uint*" : IntPtr
...
export GdipGetHatchBackgroundColor(brush, backcol) {
    backcolMarshal := backcol is VarRef ? "uint*" : IntPtr
...
export GdipCreateTexture(_image, _wrapmode, texture) {
    textureMarshal := texture is VarRef ? "ptr*" : IntPtr
...
export GdipGetTextureWrapMode(brush, _wrapmode) {
    _wrapmodeMarshal := _wrapmode is VarRef ? "int*" : IntPtr
...
export GdipGetTextureImage(brush, _image) {
    _imageMarshal := _image is VarRef ? "ptr*" : IntPtr
...
export GdipGetSolidFillColor(brush, _color) {
    _colorMarshal := _color is VarRef ? "uint*" : IntPtr
...
export GdipCreateLineBrush(point1, point2, color1, color2, _wrapMode, lineGradient) {
    lineGradientMarshal := lineGradient is VarRef ? "ptr*" : IntPtr
...
export GdipGetLineColors(brush, colors) {
    colorsMarshal := colors is VarRef ? "uint*" : IntPtr
...
export GdipGetLineGammaCorrection(brush, useGammaCorrection) {
    useGammaCorrectionMarshal := useGammaCorrection is VarRef ? "int*" : IntPtr
...
export GdipGetLineBlend(brush, blend, positions, count) {
    blendMarshal := blend is VarRef ? "float*" : IntPtr
    positionsMarshal := positions is VarRef ? "float*" : IntPtr
...
export GdipGetLinePresetBlend(brush, blend, positions, count) {
    blendMarshal := blend is VarRef ? "uint*" : IntPtr
    positionsMarshal := positions is VarRef ? "float*" : IntPtr
...
export GdipCreatePathGradient(_points, count, _wrapMode, polyGradient) {
    polyGradientMarshal := polyGradient is VarRef ? "ptr*" : IntPtr
...
export GdipGetPathGradientFocusScales(brush, xScale, yScale) {
    xScaleMarshal := xScale is VarRef ? "float*" : IntPtr
    yScaleMarshal := yScale is VarRef ? "float*" : IntPtr
...
export GdipCreatePen1(_color, width, _unit, pen) {
    penMarshal := pen is VarRef ? "ptr*" : IntPtr
...
export GdipClonePen(pen, clonepen) {
    clonepenMarshal := clonepen is VarRef ? "ptr*" : IntPtr
...
export GdipGetPenUnit(pen, _unit) {
    _unitMarshal := _unit is VarRef ? "int*" : IntPtr
...
export GdipGetPenStartCap(pen, startCap) {
    startCapMarshal := startCap is VarRef ? "int*" : IntPtr
...
export GdipGetPenEndCap(pen, endCap) {
    endCapMarshal := endCap is VarRef ? "int*" : IntPtr
...
export GdipGetPenDashCap197819(pen, _dashCap) {
    _dashCapMarshal := _dashCap is VarRef ? "int*" : IntPtr
...
export GdipGetPenLineJoin(pen, _lineJoin) {
    _lineJoinMarshal := _lineJoin is VarRef ? "int*" : IntPtr
...
export GdipGetPenCustomStartCap(pen, customCap) {
    customCapMarshal := customCap is VarRef ? "ptr*" : IntPtr
...
export GdipGetPenMiterLimit(pen, miterLimit) {
    miterLimitMarshal := miterLimit is VarRef ? "float*" : IntPtr
...
export GdipGetPenMode(pen, penMode) {
    penModeMarshal := penMode is VarRef ? "int*" : IntPtr
...
export GdipGetPenColor(pen, argb) {
    argbMarshal := argb is VarRef ? "uint*" : IntPtr
...
export GdipGetPenDashStyle(pen, _dashstyle) {
    _dashstyleMarshal := _dashstyle is VarRef ? "int*" : IntPtr
...
export GdipGetPenDashOffset(pen, offset) {
    offsetMarshal := offset is VarRef ? "float*" : IntPtr
...
export GdipSetPenDashArray(pen, dash, count) {
    dashMarshal := dash is VarRef ? "float*" : IntPtr
...
export GdipCloneCustomLineCap(customCap, clonedCap) {
    clonedCapMarshal := clonedCap is VarRef ? "ptr*" : IntPtr
...
export GdipGetCustomLineCapType(customCap, capType) {
    capTypeMarshal := capType is VarRef ? "int*" : IntPtr
...
export GdipGetCustomLineCapBaseCap(customCap, baseCap) {
    baseCapMarshal := baseCap is VarRef ? "int*" : IntPtr
...
export GdipGetCustomLineCapBaseInset(customCap, inset) {
    insetMarshal := inset is VarRef ? "float*" : IntPtr
...
export GdipGetCustomLineCapWidthScale(customCap, widthScale) {
    widthScaleMarshal := widthScale is VarRef ? "float*" : IntPtr
...
export GdipCreateAdjustableArrowCap(height, width, isFilled, cap) {
    capMarshal := cap is VarRef ? "ptr*" : IntPtr
...
export GdipGetAdjustableArrowCapMiddleInset(cap, middleInset) {
    middleInsetMarshal := middleInset is VarRef ? "float*" : IntPtr
...
export GdipGetAdjustableArrowCapFillState(cap, fillState) {
    fillStateMarshal := fillState is VarRef ? "int*" : IntPtr
...
export GdipCloneImage(_image, cloneImage) {
    cloneImageMarshal := cloneImage is VarRef ? "ptr*" : IntPtr
...
export GdipGetImageGraphicsContext(_image, graphics) {
    graphicsMarshal := graphics is VarRef ? "ptr*" : IntPtr
...
export GdipGetImageBounds(_image, srcRect, srcUnit) {
    srcUnitMarshal := srcUnit is VarRef ? "int*" : IntPtr
...
export GdipGetImageWidth(_image, width) {
    widthMarshal := width is VarRef ? "uint*" : IntPtr
...
export GdipGetImageHeight(_image, height) {
    heightMarshal := height is VarRef ? "uint*" : IntPtr
...
export GdipGetImageThumbnail(_image, thumbWidth, thumbHeight, thumbImage, callback, callbackData) {
    thumbImageMarshal := thumbImage is VarRef ? "ptr*" : IntPtr
...
export GdipGetImagePaletteSize(_image, _size) {
    _sizeMarshal := _size is VarRef ? "int*" : IntPtr
...
export GdipGetPropertyCount(_image, numOfProperty) {
    numOfPropertyMarshal := numOfProperty is VarRef ? "uint*" : IntPtr
...
export GdipGetPropertyIdList(_image, numOfProperty, list) {
    listMarshal := list is VarRef ? "uint*" : IntPtr
...
export GdipGetPropertySize(_image, totalBufferSize, numProperties) {
    totalBufferSizeMarshal := totalBufferSize is VarRef ? "uint*" : IntPtr
    numPropertiesMarshal := numProperties is VarRef ? "uint*" : IntPtr
...
export GdipCreateBitmapFromStream(stream, _bitmap) {
    _bitmapMarshal := _bitmap is VarRef ? "ptr*" : IntPtr
...
export GdipCreateBitmapFromScan0(width, height, stride, format, scan0, _bitmap) {
    scan0Marshal := scan0 is VarRef ? "char*" : IntPtr
    scan0Marshal := scan0 == 0 ? IntPtr : "char*"
    _bitmapMarshal := _bitmap is VarRef ? "ptr*" : IntPtr
...
export GdipCreateBitmapFromGdiDib(gdiBitmapInfo, gdiBitmapData, _bitmap) {
    gdiBitmapDataMarshal := gdiBitmapData is VarRef ? "ptr" : IntPtr
    _bitmapMarshal := _bitmap is VarRef ? "ptr*" : IntPtr
...
export GdipCloneBitmapArea(x, y, width, height, format, srcBitmap, dstBitmap) {
    dstBitmapMarshal := dstBitmap is VarRef ? "ptr*" : IntPtr
...
export GdipBitmapApplyEffect(_bitmap, _effect, roi, useAuxData, auxData, auxDataSize) {
    _effectMarshal := _effect is VarRef ? "ptr*" : IntPtr
    auxDataMarshal := auxData is VarRef ? "ptr*" : IntPtr
    auxDataSizeMarshal := auxDataSize is VarRef ? "int*" : IntPtr
...
export GdipBitmapCreateApplyEffect(inputBitmaps, numInputs, _effect, roi, outputRect, outputBitmap, useAuxData, auxData, auxDataSize) {
    inputBitmapsMarshal := inputBitmaps is VarRef ? "ptr*" : IntPtr
    _effectMarshal := _effect is VarRef ? "ptr*" : IntPtr
    outputBitmapMarshal := outputBitmap is VarRef ? "ptr*" : IntPtr
    auxDataMarshal := auxData is VarRef ? "ptr*" : IntPtr
    auxDataSizeMarshal := auxDataSize is VarRef ? "int*" : IntPtr
...
export GdipBitmapGetHistogram(_bitmap, format, NumberOfEntries, channel0, channel1, channel2, channel3) {
    channel0Marshal := channel0 is VarRef ? "uint*" : IntPtr
    channel1Marshal := channel1 is VarRef ? "uint*" : IntPtr
    channel2Marshal := channel2 is VarRef ? "uint*" : IntPtr
    channel3Marshal := channel3 is VarRef ? "uint*" : IntPtr
...
export GdipBitmapGetHistogramSize(format, NumberOfEntries) {
    NumberOfEntriesMarshal := NumberOfEntries is VarRef ? "uint*" : IntPtr
...
export GdipCreateImageAttributes(imageattr) {
    imageattrMarshal := imageattr is VarRef ? "ptr*" : IntPtr
...
export GdipCloneImageAttributes(imageattr, cloneImageattr) {
    cloneImageattrMarshal := cloneImageattr is VarRef ? "ptr*" : IntPtr
...
export GdipGetCompositingMode(graphics, _compositingMode) {
    _compositingModeMarshal := _compositingMode is VarRef ? "int*" : IntPtr
...
export GdipGetCompositingQuality(graphics, _compositingQuality) {
    _compositingQualityMarshal := _compositingQuality is VarRef ? "int*" : IntPtr
...
export GdipGetSmoothingMode(graphics, _smoothingMode) {
    _smoothingModeMarshal := _smoothingMode is VarRef ? "int*" : IntPtr
...
export GdipGetPixelOffsetMode(graphics, _pixelOffsetMode) {
    _pixelOffsetModeMarshal := _pixelOffsetMode is VarRef ? "int*" : IntPtr
...
export GdipGetTextRenderingHint(graphics, _mode) {
    _modeMarshal := _mode is VarRef ? "int*" : IntPtr
...
export GdipGetTextContrast(graphics, contrast) {
    contrastMarshal := contrast is VarRef ? "uint*" : IntPtr
...
export GdipGetInterpolationMode(graphics, _interpolationMode) {
    _interpolationModeMarshal := _interpolationMode is VarRef ? "int*" : IntPtr
...
export GdipGetDpiX(graphics, dpi) {
    dpiMarshal := dpi is VarRef ? "float*" : IntPtr
...
export GdipDrawImageFX(graphics, _image, source, _xForm, _effect, imageAttributes, srcUnit) {
    _xFormMarshal := _xForm is VarRef ? "ptr*" : IntPtr
    _effectMarshal := _effect is VarRef ? "ptr*" : IntPtr
...
export GdipCreateMetafileFromWmf(hWmf, deleteWmf, _wmfPlaceableFileHeader, _metafile) {
    _metafileMarshal := _metafile is VarRef ? "ptr*" : IntPtr
...
export GdipGetMetafileDownLevelRasterizationLimit(_metafile, metafileRasterizationLimitDpi) {
    metafileRasterizationLimitDpiMarshal := metafileRasterizationLimitDpi is VarRef ? "uint*" : IntPtr
...
export GdipGetImageDecodersSize(numDecoders, _size) {
    numDecodersMarshal := numDecoders is VarRef ? "uint*" : IntPtr
    _sizeMarshal := _size is VarRef ? "uint*" : IntPtr
...
export GdipGetImageEncodersSize(numEncoders, _size) {
    numEncodersMarshal := numEncoders is VarRef ? "uint*" : IntPtr
    _sizeMarshal := _size is VarRef ? "uint*" : IntPtr
...
export GdipCreateFontFamilyFromName(name, _fontCollection, _fontFamily) {
    ...
    _fontFamilyMarshal := _fontFamily is VarRef ? "ptr*" : IntPtr
...
export GdipCloneFontFamily(_fontFamily, clonedFontFamily) {
    clonedFontFamilyMarshal := clonedFontFamily is VarRef ? "ptr*" : IntPtr
...
export GdipGetGenericFontFamilySansSerif(nativeFamily) {
    nativeFamilyMarshal := nativeFamily is VarRef ? "ptr*" : IntPtr
...
export GdipIsStyleAvailable(family, style, IsStyleAvailable) {
    IsStyleAvailableMarshal := IsStyleAvailable is VarRef ? "int*" : IntPtr
...
export GdipGetEmHeight(family, style, EmHeight) {
    EmHeightMarshal := EmHeight is VarRef ? "ushort*" : IntPtr
...
export GdipGetCellAscent(family, style, CellAscent) {
    CellAscentMarshal := CellAscent is VarRef ? "ushort*" : IntPtr
...
export GdipGetCellDescent(family, style, CellDescent) {
    CellDescentMarshal := CellDescent is VarRef ? "ushort*" : IntPtr
...
export GdipGetLineSpacing(family, style, LineSpacing) {
    LineSpacingMarshal := LineSpacing is VarRef ? "ushort*" : IntPtr
...
export GdipCreateFontFromDC(_hdc, _font) {
    _fontMarshal := _font is VarRef ? "ptr*" : IntPtr
...
export GdipCloneFont(_font, cloneFont) {
    cloneFontMarshal := cloneFont is VarRef ? "ptr*" : IntPtr
...
export GdipGetFamily(_font, family) {
    familyMarshal := family is VarRef ? "ptr*" : IntPtr
...
export GdipGetFontStyle(_font, style) {
    styleMarshal := style is VarRef ? "int*" : IntPtr
...
export GdipGetFontSize(_font, _size) {
    _sizeMarshal := _size is VarRef ? "float*" : IntPtr
...
export GdipNewInstalledFontCollection(_fontCollection) {
    _fontCollectionMarshal := _fontCollection is VarRef ? "ptr*" : IntPtr
...
export GdipGetFontCollectionFamilyCount(_fontCollection, numFound) {
    numFoundMarshal := numFound is VarRef ? "int*" : IntPtr
...
export GdipGetFontCollectionFamilyList(_fontCollection, numSought, gpfamilies, numFound) {
    gpfamiliesMarshal := gpfamilies is VarRef ? "ptr*" : IntPtr
    numFoundMarshal := numFound is VarRef ? "int*" : IntPtr
...
export GdipMeasureString(graphics, _string, length, _font, layoutRect, stringFormat, boundingBox, codepointsFitted, linesFilled) {
    ...
    codepointsFittedMarshal := codepointsFitted is VarRef ? "int*" : IntPtr
    linesFilledMarshal := linesFilled is VarRef ? "int*" : IntPtr
...
export GdipMeasureCharacterRanges(graphics, _string, length, _font, layoutRect, stringFormat, regionCount, regions) {
    ...
    regionsMarshal := regions is VarRef ? "ptr*" : IntPtr
...
export GdipCloneStringFormat(format, newFormat) {
    newFormatMarshal := newFormat is VarRef ? "ptr*" : IntPtr
...
export GdipGetStringFormatAlign(format, align) {
    alignMarshal := align is VarRef ? "int*" : IntPtr
...
export GdipGetStringFormatTrimming(format, trimming) {
    trimmingMarshal := trimming is VarRef ? "int*" : IntPtr
...
export GdipGetStringFormatHotkeyPrefix(format, _hotkeyPrefix) {
    _hotkeyPrefixMarshal := _hotkeyPrefix is VarRef ? "int*" : IntPtr
...
export GdipSetStringFormatTabStops(format, firstTabOffset, count, tabStops) {
    tabStopsMarshal := tabStops is VarRef ? "float*" : IntPtr
...
export GdipGetStringFormatTabStops(format, count, firstTabOffset, tabStops) {
    firstTabOffsetMarshal := firstTabOffset is VarRef ? "float*" : IntPtr
    tabStopsMarshal := tabStops is VarRef ? "float*" : IntPtr
...
export GdipGetStringFormatDigitSubstitution(format, language, substitute) {
    ...
    substituteMarshal := substitute is VarRef ? "int*" : IntPtr
...
export GdipCreateCachedBitmap(_bitmap, graphics, _cachedBitmap) {
    _cachedBitmapMarshal := _cachedBitmap is VarRef ? "ptr*" : IntPtr
...
export GdipEmfToWmfBits(hemf, cbData16, pData16, iMapMode, eFlags) {
    pData16Marshal := pData16 is VarRef ? "char*" : IntPtr
    pData16Marshal := pData16 == 0 ? IntPtr : "char*"
...
export GdipConvertToEmfPlus(refGraphics, _metafile, conversionFailureFlag, _emfType, description, out_metafile) {
    ...
    conversionFailureFlagMarshal := conversionFailureFlag is VarRef ? "int*" : IntPtr
    out_metafileMarshal := out_metafile is VarRef ? "ptr*" : IntPtr
