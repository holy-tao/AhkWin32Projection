 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/d3d9/nn-d3d9-idirect3dswapchain9ex
...
class IDirect3DSwapChain9Ex extends IDirect3DSwapChain9{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/d3d9/nf-d3d9-idirect3dswapchain9ex-getlastpresentcount
     */
    GetLastPresentCount(pLastPresentCount) {
        pLastPresentCountMarshal := pLastPresentCount is VarRef ? "uint*" : "ptr"

        result := ComCall(10, this, pLastPresentCountMarshal, pLastPresentCount, "int")
        if(result != 0) {
    ...
    GetPresentStats(pPresentationStatistics) {
        result := ComCall(11, this, "ptr", pPresentationStatistics, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/d3d9/nf-d3d9-idirect3dswapchain9ex-getdisplaymodeex
     */
    GetDisplayModeEx(pMode, pRotation) {
        pRotationMarshal := pRotation is VarRef ? "int*" : "ptr"

        result := ComCall(12, this, "ptr", pMode, pRotationMarshal, pRotation, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
