...
class IDirect3D9Ex extends IDirect3D9{
    ...
    EnumAdapterModesEx(Adapter, pFilter, Mode, pMode) {
        result := ComCall(18, this, "uint", Adapter, "ptr", pFilter, "uint", Mode, "ptr", pMode, "int")
        if(result != 0)
    ...
    GetAdapterDisplayModeEx(Adapter, pMode, pRotation) {
        result := ComCall(19, this, "uint", Adapter, "ptr", pMode, "int*", pRotation, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateDeviceEx(Adapter, DeviceType, hFocusWindow, BehaviorFlags, pPresentationParameters, pFullscreenDisplayMode, ppReturnedDeviceInterface) {
        ...
        result := ComCall(20, this, "uint", Adapter, "int", DeviceType, "ptr", hFocusWindow, "uint", BehaviorFlags, "ptr", pPresentationParameters, "ptr", pFullscreenDisplayMode, "ptr", ppReturnedDeviceInterface, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetAdapterLUID(Adapter, pLUID) {
        result := ComCall(21, this, "uint", Adapter, "ptr", pLUID, "int")
        if(result != 0)
            throw OSError(result)
