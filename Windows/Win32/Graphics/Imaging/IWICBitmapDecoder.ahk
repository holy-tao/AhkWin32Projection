...
class IWICBitmapDecoder extends IUnknown{
     ...
     * @param {Pointer<UInt32>} pdwCapability 
     * @returns {HRESULT} 
     */
    QueryCapability(pIStream, pdwCapability) {
        result := ComCall(3, this, "ptr", pIStream, "uint*", pdwCapability, "int")
        if(result != 0)
    ...
    GetDecoderInfo(ppIDecoderInfo) {
        result := ComCall(6, this, "ptr", ppIDecoderInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPreview(ppIBitmapSource) {
        result := ComCall(9, this, "ptr", ppIBitmapSource, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetColorContexts(cCount, ppIColorContexts, pcActualCount) {
        result := ComCall(10, this, "uint", cCount, "ptr", ppIColorContexts, "uint*", pcActualCount, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetThumbnail(ppIThumbnail) {
        result := ComCall(11, this, "ptr", ppIThumbnail, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFrame(index, ppIBitmapFrame) {
        result := ComCall(13, this, "uint", index, "ptr", ppIBitmapFrame, "int")
        if(result != 0)
            throw OSError(result)
