...
class IWICBitmapEncoder extends IUnknown{
    ...
    GetEncoderInfo(ppIEncoderInfo) {
        result := ComCall(5, this, "ptr", ppIEncoderInfo, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IWICBitmapSource>} pIPreview 
     * @returns {HRESULT} 
     */
    ...
    CreateNewFrame(ppIFrameEncode, ppIEncoderOptions) {
        result := ComCall(10, this, "ptr", ppIFrameEncode, "ptr", ppIEncoderOptions, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetMetadataQueryWriter(ppIMetadataQueryWriter) {
        result := ComCall(12, this, "ptr", ppIMetadataQueryWriter, "int")
        if(result != 0)
            throw OSError(result)
