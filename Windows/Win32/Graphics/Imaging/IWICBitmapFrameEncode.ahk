...
class IWICBitmapFrameEncode extends IUnknown{
     ...
     * @param {Pointer<IPropertyBag2>} pIEncoderOptions 
     * @returns {HRESULT} <ul>
    ...
    /**
     * 
     * @param {Pointer<IWICBitmapSource>} pIThumbnail 
     * @returns {HRESULT} 
     */
     ...
     * @param {Pointer<Byte>} pbPixels 
     * @returns {HRESULT} 
     */
    WritePixels(lineCount, cbStride, cbBufferSize, pbPixels) {
        result := ComCall(10, this, "uint", lineCount, "uint", cbStride, "uint", cbBufferSize, "char*", pbPixels, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetMetadataQueryWriter(ppIMetadataQueryWriter) {
        result := ComCall(13, this, "ptr", ppIMetadataQueryWriter, "int")
        if(result != 0)
            throw OSError(result)
