 ...
 */
export default struct IWICFormatConverter extends IWICBitmapSource {
    /**
     ...
     * used for IWICFormatConverter interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWICFormatConverter.Vtbl()
        }
    ...
    CanConvert(srcPixelFormat, dstPixelFormat) {
        result := ComCall(9, this, Guid.Ptr, srcPixelFormat, Guid.Ptr, dstPixelFormat, BOOL.Ptr, &pfCanConvert := 0, "HRESULT")
        return pfCanConvert
    }

    Query(iid) {
        if (IWICFormatConverter.IID.Equals(iid)) {
            return true
        }
