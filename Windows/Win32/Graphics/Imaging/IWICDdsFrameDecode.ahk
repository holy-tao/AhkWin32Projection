...
class IWICDdsFrameDecode extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pWidthInBlocks 
     * @param {Pointer<UInt32>} pHeightInBlocks 
     * @returns {HRESULT} 
     */
    GetSizeInBlocks(pWidthInBlocks, pHeightInBlocks) {
        result := ComCall(3, this, "uint*", pWidthInBlocks, "uint*", pHeightInBlocks, "int")
        if(result != 0)
    ...
    CopyBlocks(prcBoundsInBlocks, cbStride, cbBufferSize, pbBuffer) {
        result := ComCall(5, this, "ptr", prcBoundsInBlocks, "uint", cbStride, "uint", cbBufferSize, "char*", pbBuffer, "int")
        if(result != 0)
            throw OSError(result)
