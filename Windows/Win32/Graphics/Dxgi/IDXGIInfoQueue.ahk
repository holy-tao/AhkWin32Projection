...
class IDXGIInfoQueue extends IUnknown{
    ...
    GetMessage(Producer, MessageIndex, pMessage, pMessageByteLength) {
        result := ComCall(5, this, "ptr", Producer, "uint", MessageIndex, "ptr", pMessage, "ptr*", pMessageByteLength, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetStorageFilter(Producer, pFilter, pFilterByteLength) {
        result := ComCall(13, this, "ptr", Producer, "ptr", pFilter, "ptr*", pFilterByteLength, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRetrievalFilter(Producer, pFilter, pFilterByteLength) {
        result := ComCall(22, this, "ptr", Producer, "ptr", pFilter, "ptr*", pFilterByteLength, "int")
        if(result != 0)
            throw OSError(result)
