...
class IDWriteTextFormat extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<Int32>} lineSpacingMethod 
     * @param {Pointer<Single>} lineSpacing 
     * @param {Pointer<Single>} baseline 
     * @returns {HRESULT} 
     */
    GetLineSpacing(lineSpacingMethod, lineSpacing, baseline) {
        result := ComCall(18, this, "int*", lineSpacingMethod, "float*", lineSpacing, "float*", baseline, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFontCollection(fontCollection) {
        result := ComCall(19, this, "ptr", fontCollection, "int")
        if(result != 0)
            throw OSError(result)
