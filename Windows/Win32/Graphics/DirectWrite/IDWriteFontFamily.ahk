...
class IDWriteFontFamily extends IDWriteFontList{
    ...
    GetFamilyNames(names) {
        result := ComCall(6, this, "ptr", names, "int")
        if(result != 0)
    ...
    GetFirstMatchingFont(weight, stretch, style, matchingFont) {
        result := ComCall(7, this, "int", weight, "int", stretch, "int", style, "ptr", matchingFont, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetMatchingFonts(weight, stretch, style, matchingFonts) {
        result := ComCall(8, this, "int", weight, "int", stretch, "int", style, "ptr", matchingFonts, "int")
        if(result != 0)
            throw OSError(result)
