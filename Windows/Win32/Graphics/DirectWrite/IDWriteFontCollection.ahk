 ...
 */
export default struct IDWriteFontCollection extends IUnknown {
    /**
     ...
     * used for IDWriteFontCollection interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetFontFamilyCount  : IntPtr
        GetFontFamily       : IntPtr
        FindFamilyName      : IntPtr
        GetFontFromFontFace : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDWriteFontCollection.Vtbl()
        }
     ...
     */
    GetFontFamily(index) {
        result := ComCall(4, this, UInt32, index, "ptr*", &_fontFamily := 0, "HRESULT")
    ...
    Query(iid) {
        if (IDWriteFontCollection.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetFontFamilyCount)
        CallbackFree(this.vtbl.GetFontFamily)
        CallbackFree(this.vtbl.FindFamilyName)
        CallbackFree(this.vtbl.GetFontFromFontFace)
    }
}
