...
class IDWriteFontCollection2 extends IDWriteFontCollection1{
    ...
    GetFontFamily(index, fontFamily) {
        result := ComCall(9, this, "uint", index, "ptr", fontFamily, "int")
        if(result != 0)
    ...
    GetMatchingFonts(familyName, fontAxisValues, fontAxisValueCount, fontList) {
        ...
        result := ComCall(10, this, "ptr", familyName, "ptr", fontAxisValues, "uint", fontAxisValueCount, "ptr", fontList, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetFontSet(fontSet) {
        result := ComCall(12, this, "ptr", fontSet, "int")
        if(result != 0)
            throw OSError(result)
