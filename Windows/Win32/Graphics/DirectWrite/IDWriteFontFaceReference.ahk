...
class IDWriteFontFaceReference extends IUnknown{
    ...
    CreateFontFace(fontFace) {
        result := ComCall(3, this, "ptr", fontFace, "int")
        if(result != 0)
    ...
    CreateFontFaceWithSimulations(fontFaceSimulationFlags, fontFace) {
        result := ComCall(4, this, "int", fontFaceSimulationFlags, "ptr", fontFace, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFontFile(fontFile) {
        result := ComCall(8, this, "ptr", fontFile, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnqueueGlyphDownloadRequest(glyphIndices, glyphCount) {
        result := ComCall(15, this, "ushort*", glyphIndices, "uint", glyphCount, "int")
        if(result != 0)
            throw OSError(result)
