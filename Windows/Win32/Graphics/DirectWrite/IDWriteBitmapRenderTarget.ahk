...
class IDWriteBitmapRenderTarget extends IUnknown{
    ...
    DrawGlyphRun(baselineOriginX, baselineOriginY, measuringMode, glyphRun, renderingParams, textColor, blackBoxRect) {
        result := ComCall(3, this, "float", baselineOriginX, "float", baselineOriginY, "int", measuringMode, "ptr", glyphRun, "ptr", renderingParams, "uint", textColor, "ptr", blackBoxRect, "int")
        if(result != 0)
     ...
     */
    GetSize(size) {
        result := ComCall(9, this, "ptr", size, "int")
        if(result != 0)
            throw OSError(result)
