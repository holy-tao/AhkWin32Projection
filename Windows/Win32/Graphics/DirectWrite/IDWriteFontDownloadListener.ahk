 ...
 */
export default struct IDWriteFontDownloadListener extends IUnknown {
    /**
     ...
     * used for IDWriteFontDownloadListener interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        DownloadCompleted : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDWriteFontDownloadListener.Vtbl()
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.DownloadCompleted)
    }
}
