...
class IDWriteFactory1 extends IDWriteFactory{
    ...
    GetEudcFontCollection(fontCollection, checkForUpdates) {
        result := ComCall(24, this, "ptr", fontCollection, "int", checkForUpdates, "int")
        if(result != 0)
    ...
    CreateCustomRenderingParams(gamma, enhancedContrast, enhancedContrastGrayscale, clearTypeLevel, pixelGeometry, renderingMode, renderingParams) {
        result := ComCall(25, this, "float", gamma, "float", enhancedContrast, "float", enhancedContrastGrayscale, "float", clearTypeLevel, "int", pixelGeometry, "int", renderingMode, "ptr", renderingParams, "int")
        if(result != 0)
            throw OSError(result)
