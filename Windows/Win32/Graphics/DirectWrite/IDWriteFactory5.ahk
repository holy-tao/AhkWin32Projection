...
class IDWriteFactory5 extends IDWriteFactory4{
    ...
    CreateFontSetBuilder(fontSetBuilder) {
        result := ComCall(43, this, "ptr", fontSetBuilder, "int")
        if(result != 0)
    ...
    CreateInMemoryFontFileLoader(newLoader) {
        result := ComCall(44, this, "ptr", newLoader, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateHttpFontFileLoader(referrerUrl, extraHeaders, newLoader) {
        ...
        result := ComCall(45, this, "ptr", referrerUrl, "ptr", extraHeaders, "ptr", newLoader, "int")
        if(result != 0)
            throw OSError(result)
    ...
    UnpackFontFile(containerType, fileData, fileDataSize, unpackedFontStream) {
        result := ComCall(47, this, "int", containerType, "ptr", fileData, "uint", fileDataSize, "ptr", unpackedFontStream, "int")
        if(result != 0)
            throw OSError(result)
