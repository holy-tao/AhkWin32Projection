...
#Import ".\DD_CREATESURFACEEXDATA.ahk" { DD_CREATESURFACEEXDATA }
 ...
 * }
 * ```
 * @see https://learn.microsoft.com/windows/win32/api/ddrawint/nc-ddrawint-pdd_createsurfaceex
 * @namespace Windows.Win32.Graphics.DirectDraw
 */
export default struct PDD_CREATESURFACEEX {
    value : IntPtr

    __value {
        set {
            if (value is PDD_CREATESURFACEEX) {
     ...
     */
    Call(param0) {
        result := DllCall(this.value, DD_CREATESURFACEEXDATA.Ptr, param0, UInt32)
        return result
    }

    /**
     * A PDD_CREATESURFACEEX that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PDD_CREATESURFACEEX {
        /**
         * Creates a PDD_CREATESURFACEEX pointer that invokes the given AHK function when called.
         * @param {Func(DD_CREATESURFACEEXDATA) => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 1)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 1 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [DD_CREATESURFACEEXDATA.Ptr, UInt32])
        }
