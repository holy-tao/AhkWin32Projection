...
#Import ".\DD_LOCKDATA.ahk" { DD_LOCKDATA }
 ...
 * @see https://learn.microsoft.com/windows/win32/api/ddrawint/nc-ddrawint-pdd_surfcb_lock
 * @namespace Windows.Win32.Graphics.DirectDraw
 */
export default struct PDD_SURFCB_LOCK {
    value : IntPtr

    __value {
        set {
            if (value is PDD_SURFCB_LOCK) {
     ...
     */
    Call(param0) {
        result := DllCall(this.value, DD_LOCKDATA.Ptr, param0, UInt32)
        return result
    }

    /**
     * A PDD_SURFCB_LOCK that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PDD_SURFCB_LOCK {
        /**
         * Creates a PDD_SURFCB_LOCK pointer that invokes the given AHK function when called.
         * @param {Func(DD_LOCKDATA) => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 1)) {
