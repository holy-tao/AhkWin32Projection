 ...
 */
export default struct IDCompositionClip extends IUnknown {
    /**
     ...
     * used for IDCompositionClip interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDCompositionClip.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (IDCompositionClip.IID.Equals(iid)) {
            return true
        }
