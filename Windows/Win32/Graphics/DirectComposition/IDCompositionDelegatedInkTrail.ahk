...
class IDCompositionDelegatedInkTrail extends IUnknown{
     ...
     * @param {Pointer<UInt32>} generationId 
     * @returns {HRESULT} 
     */
    AddTrailPoints(inkPoints, inkPointsCount, generationId) {
        result := ComCall(3, this, "ptr", inkPoints, "uint", inkPointsCount, "uint*", generationId, "int")
        if(result != 0)
    ...
    AddTrailPointsWithPrediction(inkPoints, inkPointsCount, predictedInkPoints, predictedInkPointsCount, generationId) {
        result := ComCall(4, this, "ptr", inkPoints, "uint", inkPointsCount, "ptr", predictedInkPoints, "uint", predictedInkPointsCount, "uint*", generationId, "int")
        if(result != 0)
            throw OSError(result)
