 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/dcomp/nn-dcomp-idcompositiondesktopdevice
...
class IDCompositionDesktopDevice extends IDCompositionDevice2{
     ...
     * @param {HWND} hwnd_ The window to which the composition target object should be bound. This parameter must not be NULL.
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dcomp/nf-dcomp-idcompositiondesktopdevice-createtargetforhwnd
     */
    CreateTargetForHwnd(hwnd_, topmost) {
        hwnd_ := hwnd_ is Win32Handle ? NumGet(hwnd_, "ptr") : hwnd_

        result := ComCall(24, this, "ptr", hwnd_, "int", topmost, "ptr*", &target := 0, "int")
        if(result != 0) {
     ...
     * @param {HANDLE} handle_ The handle of an existing composition surface that was created by a call to the <a href="https://docs.microsoft.com/windows/desktop/api/dcomp/nf-dcomp-dcompositioncreatesurfacehandle">DCompositionCreateSurfaceHandle</a> function.
     * @returns {IUnknown} The new composition surface object. This parameter must not be NULL.
     * @see https://learn.microsoft.com/windows/win32/api//content/dcomp/nf-dcomp-idcompositiondesktopdevice-createsurfacefromhandle
     */
    CreateSurfaceFromHandle(handle_) {
        handle_ := handle_ is Win32Handle ? NumGet(handle_, "ptr") : handle_

        result := ComCall(25, this, "ptr", handle_, "ptr*", &surface := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
