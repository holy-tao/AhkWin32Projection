...
class IPhotoProgressDialog extends IUnknown{
    ...
    IsCheckboxChecked(nCheckboxId, pfChecked) {
        result := ComCall(11, this, "int", nCheckboxId, "ptr", pfChecked, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetImage(nImageType, hIcon, hBitmap) {
        ...
        result := ComCall(13, this, "int", nImageType, "ptr", hIcon, "ptr", hBitmap, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IPhotoProgressActionCB>} pPhotoProgressActionCB 
     * @returns {HRESULT} 
     */
    ...
    IsCancelled(pfCancelled) {
        result := ComCall(19, this, "ptr", pfCancelled, "int")
        if(result != 0)
            throw OSError(result)
