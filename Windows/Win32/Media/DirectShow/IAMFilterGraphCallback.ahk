 ...
 */
export default struct IAMFilterGraphCallback extends IUnknown {
    /**
     ...
     * used for IAMFilterGraphCallback interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        UnableToRender : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAMFilterGraphCallback.Vtbl()
        }
    ...
    Query(iid) {
        if (IAMFilterGraphCallback.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.UnableToRender)
    }
}
