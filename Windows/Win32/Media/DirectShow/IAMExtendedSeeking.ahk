 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nn-qnetwork-iamextendedseeking
...
class IAMExtendedSeeking extends IDispatch{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-get_exseekcapabilities
     */
    get_ExSeekCapabilities(pExCapabilities) {
        pExCapabilitiesMarshal := pExCapabilities is VarRef ? "int*" : "ptr"

        result := ComCall(7, this, pExCapabilitiesMarshal, pExCapabilities, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-get_markercount
     */
    get_MarkerCount(pMarkerCount) {
        pMarkerCountMarshal := pMarkerCount is VarRef ? "int*" : "ptr"

        result := ComCall(8, this, pMarkerCountMarshal, pMarkerCount, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-get_currentmarker
     */
    get_CurrentMarker(pCurrentMarker) {
        pCurrentMarkerMarshal := pCurrentMarker is VarRef ? "int*" : "ptr"

        result := ComCall(9, this, pCurrentMarkerMarshal, pCurrentMarker, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-getmarkertime
     */
    GetMarkerTime(MarkerNum, pMarkerTime) {
        pMarkerTimeMarshal := pMarkerTime is VarRef ? "double*" : "ptr"

        result := ComCall(10, this, "int", MarkerNum, pMarkerTimeMarshal, pMarkerTime, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-getmarkername
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-put_playbackspeed
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/qnetwork/nf-qnetwork-iamextendedseeking-get_playbackspeed
     */
    get_PlaybackSpeed(pSpeed) {
        pSpeedMarshal := pSpeed is VarRef ? "double*" : "ptr"

        result := ComCall(13, this, pSpeedMarshal, pSpeed, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
