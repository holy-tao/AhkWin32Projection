...
class IAMMultiMediaStream extends IMultiMediaStream{
     ...
     * @param {Pointer<IGraphBuilder>} pFilterGraph 
     * @returns {HRESULT} <ul>
    ...
    GetFilterGraph(ppGraphBuilder) {
        result := ComCall(13, this, "ptr", ppGraphBuilder, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetFilter(ppFilter) {
        result := ComCall(14, this, "ptr", ppFilter, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IUnknown>} pStreamObject 
    ...
    AddMediaStream(pStreamObject, PurposeId, dwFlags, ppNewStream) {
        result := ComCall(15, this, "ptr", pStreamObject, "ptr", PurposeId, "uint", dwFlags, "ptr", ppNewStream, "int")
        if(result != 0)
            throw OSError(result)
