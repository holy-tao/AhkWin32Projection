 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nn-bdaiface-ibda_pincontrol
...
class IBDA_PinControl extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_pincontrol-getpinid
     */
    GetPinID(pulPinID) {
        pulPinIDMarshal := pulPinID is VarRef ? "uint*" : "ptr"

        result := ComCall(3, this, pulPinIDMarshal, pulPinID, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_pincontrol-getpintype
     */
    GetPinType(pulPinType) {
        pulPinTypeMarshal := pulPinType is VarRef ? "uint*" : "ptr"

        result := ComCall(4, this, pulPinTypeMarshal, pulPinType, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_pincontrol-registrationcontext
     */
    RegistrationContext(pulRegistrationCtx) {
        pulRegistrationCtxMarshal := pulRegistrationCtx is VarRef ? "uint*" : "ptr"

        result := ComCall(5, this, pulRegistrationCtxMarshal, pulRegistrationCtx, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
