 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nn-bdaiface-ibda_frequencyfilter
...
class IBDA_FrequencyFilter extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_autotune
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_autotune
     */
    get_Autotune(pulTransponder) {
        pulTransponderMarshal := pulTransponder is VarRef ? "uint*" : "ptr"

        result := ComCall(4, this, pulTransponderMarshal, pulTransponder, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_frequency
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_frequency
     */
    get_Frequency(pulFrequency) {
        pulFrequencyMarshal := pulFrequency is VarRef ? "uint*" : "ptr"

        result := ComCall(6, this, pulFrequencyMarshal, pulFrequency, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_polarity
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_polarity
     */
    get_Polarity(pPolarity) {
        pPolarityMarshal := pPolarity is VarRef ? "int*" : "ptr"

        result := ComCall(8, this, pPolarityMarshal, pPolarity, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_range
     */
    put_Range(ulRange) {
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_range
     */
    get_Range(pulRange) {
        pulRangeMarshal := pulRange is VarRef ? "uint*" : "ptr"

        result := ComCall(10, this, pulRangeMarshal, pulRange, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_bandwidth
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_bandwidth
     */
    get_Bandwidth(pulBandwidth) {
        pulBandwidthMarshal := pulBandwidth is VarRef ? "uint*" : "ptr"

        result := ComCall(12, this, pulBandwidthMarshal, pulBandwidth, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-put_frequencymultiplier
     */
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ibda_frequencyfilter-get_frequencymultiplier
     */
    get_FrequencyMultiplier(pulMultiplier) {
        pulMultiplierMarshal := pulMultiplier is VarRef ? "uint*" : "ptr"

        result := ComCall(14, this, pulMultiplierMarshal, pulMultiplier, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
