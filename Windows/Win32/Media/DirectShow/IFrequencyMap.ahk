 ...
 */
export default struct IFrequencyMap extends IUnknown {
    /**
     ...
     * used for IFrequencyMap interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        ...
        get_DefaultFrequencyMapping : IntPtr
        get_CountryCodeList         : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IFrequencyMap.Vtbl()
        }
    ...
    Query(iid) {
        if (IFrequencyMap.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.get_DefaultFrequencyMapping)
        CallbackFree(this.vtbl.get_CountryCodeList)
    }
}
