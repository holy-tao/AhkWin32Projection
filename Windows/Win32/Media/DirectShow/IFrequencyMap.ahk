 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nn-bdaiface-ifrequencymap
...
class IFrequencyMap extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-get_frequencymapping
     */
    get_FrequencyMapping(ulCount, ppulList) {
        ulCountMarshal := ulCount is VarRef ? "uint*" : "ptr"
        ppulListMarshal := ppulList is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, ulCountMarshal, ulCount, ppulListMarshal, ppulList, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-put_frequencymapping
     */
    put_FrequencyMapping(ulCount, pList) {
        pListMarshal := pList is VarRef ? "uint*" : "ptr"

        result := ComCall(4, this, "uint", ulCount, pListMarshal, pList, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-get_countrycode
     */
    get_CountryCode() {
        result := ComCall(5, this, "uint*", &pulCountryCode := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-put_countrycode
     */
     ...
     * @param {Pointer<Pointer<Integer>>} ppulList Pointer to a variable that receives the address of the frequency table. The frequency table is an array of size <i>pulCount</i>, allocated by the method. The caller must free the array by calling <b>CoTaskMemFree</b>.
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-get_defaultfrequencymapping
     */
    get_DefaultFrequencyMapping(ulCountryCode, pulCount, ppulList) {
        pulCountMarshal := pulCount is VarRef ? "uint*" : "ptr"
        ppulListMarshal := ppulList is VarRef ? "ptr*" : "ptr"

        result := ComCall(7, this, "uint", ulCountryCode, pulCountMarshal, pulCount, ppulListMarshal, ppulList, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If it fails, it returns an error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-ifrequencymap-get_countrycodelist
     */
    get_CountryCodeList(pulCount, ppulList) {
        pulCountMarshal := pulCount is VarRef ? "uint*" : "ptr"
        ppulListMarshal := ppulList is VarRef ? "ptr*" : "ptr"

        result := ComCall(8, this, pulCountMarshal, pulCount, ppulListMarshal, ppulList, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
