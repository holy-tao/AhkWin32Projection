 ...
 */
export default struct IVideoFrameStep extends IUnknown {
    /**
     ...
     * used for IVideoFrameStep interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IVideoFrameStep.Vtbl()
        }
    ...
    Query(iid) {
        if (IVideoFrameStep.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.CanStep)
        CallbackFree(this.vtbl.CancelStep)
    }
}
