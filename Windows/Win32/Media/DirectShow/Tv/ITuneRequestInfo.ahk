#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\..\Guid.ahk
#Include ..\..\..\System\Com\IUnknown.ahk

/**
 * The ITuneRequestInfo interface is implemented on the BDA MPEG2 Transport Information Filter (TIF) and is used by the Network Provider.
 * @remarks
 * 
  * To declare the interface identifier (IID) for this interface, use the <b>__uuidof</b> operator: <c>__uuidof(ITuneRequestInfo)</c>.
  * 
  * 
 * @see https://docs.microsoft.com/windows/win32/api//bdatif/nn-bdatif-itunerequestinfo
 * @namespace Windows.Win32.Media.DirectShow.Tv
 * @version v4.0.30319
 */
class ITuneRequestInfo extends IUnknown{
    /**
     * The interface identifier for ITuneRequestInfo
     * @type {Guid}
     */
    static IID => Guid("{a3b152df-7a90-4218-ac54-9830bee8c0b6}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 3

    /**
     * 
     * @param {Pointer<ITuneRequest>} Request 
     * @returns {HRESULT} 
     */
    GetLocatorData(Request) {
        result := ComCall(3, this, "ptr", Request, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @returns {HRESULT} 
     */
    GetComponentData(CurrentRequest) {
        result := ComCall(4, this, "ptr", CurrentRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @returns {HRESULT} 
     */
    CreateComponentList(CurrentRequest) {
        result := ComCall(5, this, "ptr", CurrentRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @param {Pointer<ITuneRequest>} TuneRequest 
     * @returns {HRESULT} 
     */
    GetNextProgram(CurrentRequest, TuneRequest) {
        result := ComCall(6, this, "ptr", CurrentRequest, "ptr", TuneRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @param {Pointer<ITuneRequest>} TuneRequest 
     * @returns {HRESULT} 
     */
    GetPreviousProgram(CurrentRequest, TuneRequest) {
        result := ComCall(7, this, "ptr", CurrentRequest, "ptr", TuneRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @param {Pointer<ITuneRequest>} TuneRequest 
     * @returns {HRESULT} 
     */
    GetNextLocator(CurrentRequest, TuneRequest) {
        result := ComCall(8, this, "ptr", CurrentRequest, "ptr", TuneRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<ITuneRequest>} CurrentRequest 
     * @param {Pointer<ITuneRequest>} TuneRequest 
     * @returns {HRESULT} 
     */
    GetPreviousLocator(CurrentRequest, TuneRequest) {
        result := ComCall(9, this, "ptr", CurrentRequest, "ptr", TuneRequest, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
