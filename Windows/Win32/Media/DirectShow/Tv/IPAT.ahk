...
class IPAT extends IUnknown{
    ...
    GetRecordProgramNumber(dwIndex, pwVal) {
        result := ComCall(7, this, "uint", dwIndex, "ushort*", pwVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecordProgramMapPid(dwIndex, pwVal) {
        result := ComCall(8, this, "uint", dwIndex, "ushort*", pwVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindRecordProgramMapPid(wProgramNumber, pwVal) {
        result := ComCall(9, this, "ushort", wProgramNumber, "ushort*", pwVal, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetNextTable(ppPAT) {
        result := ComCall(11, this, "ptr", ppPAT, "int")
        if(result != 0)
            throw OSError(result)
