...
class IDVB_SDT extends IUnknown{
    ...
    GetRecordEITScheduleFlag(dwRecordIndex, pfVal) {
        result := ComCall(9, this, "uint", dwRecordIndex, "ptr", pfVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecordEITPresentFollowingFlag(dwRecordIndex, pfVal) {
        result := ComCall(10, this, "uint", dwRecordIndex, "ptr", pfVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecordFreeCAMode(dwRecordIndex, pfVal) {
        result := ComCall(12, this, "uint", dwRecordIndex, "ptr", pfVal, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetNextTable(ppSDT) {
        result := ComCall(17, this, "ptr", ppSDT, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetVersionHash(pdwVersionHash) {
        result := ComCall(20, this, "uint*", pdwVersionHash, "int")
        if(result != 0)
            throw OSError(result)
