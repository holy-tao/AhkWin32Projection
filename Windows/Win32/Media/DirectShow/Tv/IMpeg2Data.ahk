...
class IMpeg2Data extends IUnknown{
    ...
    GetSection(pid, tid, pFilter, dwTimeout, ppSectionList) {
        result := ComCall(3, this, "ushort", pid, "char", tid, "ptr", pFilter, "uint", dwTimeout, "ptr", ppSectionList, "int")
        if(result != 0)
    ...
    GetTable(pid, tid, pFilter, dwTimeout, ppSectionList) {
        result := ComCall(4, this, "ushort", pid, "char", tid, "ptr", pFilter, "uint", dwTimeout, "ptr", ppSectionList, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetStreamOfSections(pid, tid, pFilter, hDataReadyEvent, ppMpegStream) {
        ...
        result := ComCall(5, this, "ushort", pid, "char", tid, "ptr", pFilter, "ptr", hDataReadyEvent, "ptr", ppMpegStream, "int")
        if(result != 0)
            throw OSError(result)
