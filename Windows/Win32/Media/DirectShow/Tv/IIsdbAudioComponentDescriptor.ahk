 ...
 * @see https://learn.microsoft.com/windows/win32/api/dvbsiparser/nn-dvbsiparser-iisdbaudiocomponentdescriptor
 * @namespace Windows.Win32.Media.DirectShow.Tv
 */
export default struct IIsdbAudioComponentDescriptor extends IUnknown {
    /**
     ...
     * used for IIsdbAudioComponentDescriptor interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        ...
        GetSimulcastGroupTag  : IntPtr
        GetESMultiLingualFlag : IntPtr
        GetMainComponentFlag  : IntPtr
        GetQualityIndicator   : IntPtr
        GetSamplingRate       : IntPtr
        GetLanguageCode       : IntPtr
        GetLanguageCode2      : IntPtr
        GetTextW              : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IIsdbAudioComponentDescriptor.Vtbl()
        }
    ...
    GetESMultiLingualFlag() {
        result := ComCall(10, this, BOOL.Ptr, &pfVal := 0, "HRESULT")
        return pfVal
    }
    ...
    GetMainComponentFlag() {
        result := ComCall(11, this, BOOL.Ptr, &pfVal := 0, "HRESULT")
        return pfVal
    }
    ...
    GetTextW(convMode) {
        ...
        result := ComCall(16, this, DVB_STRCONV_MODE, convMode, BSTR.Ptr, pbstrText, "HRESULT")
        return pbstrText
    }
    ...
    Implement(implObj, flags := "") {
        ...
        this.vtbl.GetStreamContent := CallbackCreate(ObjBindMethod(implObj, "GetStreamContent"), flags, 2)
        ...
        this.vtbl.GetSimulcastGroupTag := CallbackCreate(ObjBindMethod(implObj, "GetSimulcastGroupTag"), flags, 2)
        this.vtbl.GetESMultiLingualFlag := CallbackCreate(ObjBindMethod(implObj, "GetESMultiLingualFlag"), flags, 2)
        this.vtbl.GetMainComponentFlag := CallbackCreate(ObjBindMethod(implObj, "GetMainComponentFlag"), flags, 2)
        this.vtbl.GetQualityIndicator := CallbackCreate(ObjBindMethod(implObj, "GetQualityIndicator"), flags, 2)
        this.vtbl.GetSamplingRate := CallbackCreate(ObjBindMethod(implObj, "GetSamplingRate"), flags, 2)
        this.vtbl.GetLanguageCode := CallbackCreate(ObjBindMethod(implObj, "GetLanguageCode"), flags, 2)
        this.vtbl.GetLanguageCode2 := CallbackCreate(ObjBindMethod(implObj, "GetLanguageCode2"), flags, 2)
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetSimulcastGroupTag)
        CallbackFree(this.vtbl.GetESMultiLingualFlag)
        CallbackFree(this.vtbl.GetMainComponentFlag)
        CallbackFree(this.vtbl.GetQualityIndicator)
        CallbackFree(this.vtbl.GetSamplingRate)
        CallbackFree(this.vtbl.GetLanguageCode)
        CallbackFree(this.vtbl.GetLanguageCode2)
