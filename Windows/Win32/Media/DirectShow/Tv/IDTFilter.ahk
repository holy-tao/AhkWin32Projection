...
class IDTFilter extends IUnknown{
    ...
    get_BlockedRatingAttributes(enSystem, enLevel, plbfEnAttr) {
        result := ComCall(5, this, "int", enSystem, "int", enLevel, "int*", plbfEnAttr, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_BlockUnRated(pfBlockUnRatedShows) {
        result := ComCall(7, this, "ptr", pfBlockUnRatedShows, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} pmsecsDelayBeforeBlock 
     * @returns {HRESULT} 
     */
    get_BlockUnRatedDelay(pmsecsDelayBeforeBlock) {
        result := ComCall(9, this, "int*", pmsecsDelayBeforeBlock, "int")
        if(result != 0)
            throw OSError(result)
