 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nn-dvbsiparser-iisdbsiparser2
...
class IIsdbSiParser2 extends IDvbSiParser2{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getsdt
     */
    GetSDT(tableId, pwTransportStreamId) {
        pwTransportStreamIdMarshal := pwTransportStreamId is VarRef ? "ushort*" : "ptr"

        result := ComCall(19, this, "char", tableId, pwTransportStreamIdMarshal, pwTransportStreamId, "ptr*", &ppSDT := 0, "int")
        if(result != 0) {
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getbit
     */
    GetBIT(tableId, pwOriginalNetworkId) {
        pwOriginalNetworkIdMarshal := pwOriginalNetworkId is VarRef ? "ushort*" : "ptr"

        result := ComCall(20, this, "char", tableId, pwOriginalNetworkIdMarshal, pwOriginalNetworkId, "ptr*", &ppBIT := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getnbit
     */
    GetNBIT(tableId, pwOriginalNetworkId) {
        pwOriginalNetworkIdMarshal := pwOriginalNetworkId is VarRef ? "ushort*" : "ptr"

        result := ComCall(21, this, "char", tableId, pwOriginalNetworkIdMarshal, pwOriginalNetworkId, "ptr*", &ppNBIT := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getldt
     */
    GetLDT(tableId, pwOriginalServiceId) {
        pwOriginalServiceIdMarshal := pwOriginalServiceId is VarRef ? "ushort*" : "ptr"

        result := ComCall(22, this, "char", tableId, pwOriginalServiceIdMarshal, pwOriginalServiceId, "ptr*", &ppLDT := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getsdtt
     */
    GetSDTT(tableId, pwTableIdExt) {
        pwTableIdExtMarshal := pwTableIdExt is VarRef ? "ushort*" : "ptr"

        result := ComCall(23, this, "char", tableId, pwTableIdExtMarshal, pwTableIdExt, "ptr*", &ppSDTT := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getcdt
     */
    GetCDT(tableId, bSectionNumber, pwDownloadDataId) {
        pwDownloadDataIdMarshal := pwDownloadDataId is VarRef ? "ushort*" : "ptr"

        result := ComCall(24, this, "char", tableId, "char", bSectionNumber, pwDownloadDataIdMarshal, pwDownloadDataId, "ptr*", &ppCDT := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * The caller must release the interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/dvbsiparser/nf-dvbsiparser-iisdbsiparser2-getemm
     */
    GetEMM(pid, wTableIdExt) {
        result := ComCall(25, this, "ushort", pid, "ushort", wTableIdExt, "ptr*", &ppEMM := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
