 ...
 */
export default struct IDVB_DIT extends IUnknown {
    /**
     ...
     * used for IDVB_DIT interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        Initialize        : IntPtr
        GetTransitionFlag : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDVB_DIT.Vtbl()
        }
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api/dvbsiparser/nf-dvbsiparser-idvb_dit-initialize
     */
     ...
     */
    GetTransitionFlag() {
        result := ComCall(4, this, BOOL.Ptr, &pfVal := 0, "HRESULT")
        return pfVal
    }

    Query(iid) {
        if (IDVB_DIT.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetTransitionFlag)
    }
}
