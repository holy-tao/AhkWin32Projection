 ...
 * @see https://learn.microsoft.com/windows/win32/api/encdec/nn-encdec-ixdscodecconfig
 * @namespace Windows.Win32.Media.DirectShow.Tv
 */
export default struct IXDSCodecConfig extends IUnknown {
    /**
     ...
     * used for IXDSCodecConfig interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetSecureChannelObject : IntPtr
        SetPauseBufferTime     : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IXDSCodecConfig.Vtbl()
        }
    ...
    Query(iid) {
        if (IXDSCodecConfig.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.SetPauseBufferTime)
    }
}
