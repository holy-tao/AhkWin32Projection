 ...
 */
export default struct IDvbContentDescriptor extends IUnknown {
    /**
     ...
     * used for IDvbContentDescriptor interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetTag                  : IntPtr
        GetLength               : IntPtr
        GetCountOfRecords       : IntPtr
        GetRecordContentNibbles : IntPtr
        GetRecordUserNibbles    : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDvbContentDescriptor.Vtbl()
        }
    ...
    Query(iid) {
        if (IDvbContentDescriptor.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetRecordContentNibbles)
        CallbackFree(this.vtbl.GetRecordUserNibbles)
    }
}
