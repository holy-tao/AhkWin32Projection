...
class IATSC_MGT extends IUnknown{
    ...
    GetRecordTypePid(dwRecordIndex, ppidVal) {
        result := ComCall(8, this, "uint", dwRecordIndex, "ushort*", ppidVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetCountOfTableDescriptors(pdwVal) {
        result := ComCall(13, this, "uint*", pdwVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetTableDescriptorByIndex(dwIndex, ppDescriptor) {
        result := ComCall(14, this, "uint", dwIndex, "ptr", ppDescriptor, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetTableDescriptorByTag(bTag, pdwCookie, ppDescriptor) {
        result := ComCall(15, this, "char", bTag, "uint*", pdwCookie, "ptr", ppDescriptor, "int")
        if(result != 0)
            throw OSError(result)
