...
class IDVB_RST extends IUnknown{
    ...
    GetCountOfRecords(pdwVal) {
        result := ComCall(4, this, "uint*", pdwVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecordTransportStreamId(dwRecordIndex, pwVal) {
        result := ComCall(5, this, "uint", dwRecordIndex, "ushort*", pwVal, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecordOriginalNetworkId(dwRecordIndex, pwVal) {
        result := ComCall(6, this, "uint", dwRecordIndex, "ushort*", pwVal, "int")
        if(result != 0)
            throw OSError(result)
