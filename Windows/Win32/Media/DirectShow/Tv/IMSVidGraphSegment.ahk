...
class IMSVidGraphSegment extends IPersist{
     ...
     */
    get_Init(pInit) {
        result := ComCall(4, this, "ptr", pInit, "int")
        if(result != 0)
    ...
    EnumFilters(pNewEnum) {
        result := ComCall(6, this, "ptr", pNewEnum, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_Container(ppCtl) {
        result := ComCall(7, this, "ptr", ppCtl, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IMSVidGraphSegmentContainer>} pCtl 
     * @returns {HRESULT} 
     */
    put_Container(pCtl) {
