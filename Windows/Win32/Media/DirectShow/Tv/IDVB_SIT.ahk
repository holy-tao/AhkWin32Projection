...
class IDVB_SIT extends IUnknown{
    ...
    GetTableDescriptorByIndex(dwIndex, ppDescriptor) {
        result := ComCall(6, this, "uint", dwIndex, "ptr", ppDescriptor, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetTableDescriptorByTag(bTag, pdwCookie, ppDescriptor) {
        result := ComCall(7, this, "char", bTag, "uint*", pdwCookie, "ptr", ppDescriptor, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetNextTable(dwTimeout, ppSIT) {
        result := ComCall(15, this, "uint", dwTimeout, "ptr", ppSIT, "int")
        if(result != 0)
            throw OSError(result)
