 ...
 */
export default struct IDTFilterLicenseRenewal extends IUnknown {
    /**
     ...
     * used for IDTFilterLicenseRenewal interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetLicenseRenewalData : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDTFilterLicenseRenewal.Vtbl()
        }
    ...
    Query(iid) {
        if (IDTFilterLicenseRenewal.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetLicenseRenewalData)
    }
}
