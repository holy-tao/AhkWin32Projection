...
class IMSEventBinder extends IDispatch{
    ...
    Bind(pEventObject, EventName, EventHandler) {
        ...
        if(EventHandler is String) {
            pin := BSTR.Alloc(EventHandler)
            EventHandler := pin.Value
        }

        result := ComCall(7, this, "ptr", pEventObject, "ptr", EventName, "ptr", EventHandler, "int*", &CancelID := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/ktop-src/printdocs/unbindptproviderthunk
     */
    Unbind(CancelCookie) {
        result := ComCall(8, this, "uint", CancelCookie, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
