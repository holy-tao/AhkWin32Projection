...
class ISectionList extends IUnknown{
     ...
     * @param {Pointer<IMpeg2Data>} pMpeg2Data 
    ...
    GetNumberOfSections(pCount) {
        result := ComCall(6, this, "ushort*", pCount, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} pdwRawPacketLength 
     * @param {Pointer<SECTION>} ppSection 
     * @returns {HRESULT} 
     */
    GetSectionData(sectionNumber, pdwRawPacketLength, ppSection) {
        result := ComCall(7, this, "ushort", sectionNumber, "uint*", pdwRawPacketLength, "ptr", ppSection, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt16>} pPid 
     * @returns {HRESULT} 
     */
    GetProgramIdentifier(pPid) {
        result := ComCall(8, this, "ushort*", pPid, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Byte>} pTableId 
     * @returns {HRESULT} 
     */
    GetTableIdentifier(pTableId) {
        result := ComCall(9, this, "char*", pTableId, "int")
        if(result != 0)
            throw OSError(result)
