 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nn-bdaiface-impeg2pidmap
...
class IMPEG2PIDMap extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-impeg2pidmap-mappid
     */
    MapPID(culPID, pulPID, MediaSampleContent) {
        pulPIDMarshal := pulPID is VarRef ? "uint*" : "ptr"

        result := ComCall(3, this, "uint", culPID, pulPIDMarshal, pulPID, "int", MediaSampleContent, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} Returns S_OK if successful. If the method fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-impeg2pidmap-unmappid
     */
    UnmapPID(culPID, pulPID) {
        pulPIDMarshal := pulPID is VarRef ? "uint*" : "ptr"

        result := ComCall(4, this, "uint", culPID, pulPIDMarshal, pulPID, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/bdaiface/nf-bdaiface-impeg2pidmap-enumpidmap
     */
    EnumPIDMap() {
        result := ComCall(5, this, "ptr*", &pIEnumPIDMap := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
