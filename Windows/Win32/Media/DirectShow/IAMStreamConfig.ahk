 ...
 */
export default struct IAMStreamConfig extends IUnknown {
    /**
     ...
     * used for IAMStreamConfig interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        SetFormat               : IntPtr
        GetFormat               : IntPtr
        GetNumberOfCapabilities : IntPtr
        GetStreamCaps           : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAMStreamConfig.Vtbl()
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetNumberOfCapabilities)
        CallbackFree(this.vtbl.GetStreamCaps)
    }
}
