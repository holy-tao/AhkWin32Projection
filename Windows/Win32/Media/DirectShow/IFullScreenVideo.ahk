...
class IFullScreenVideo extends IUnknown{
     ...
     */
    CountModes() {
        result := ComCall(3, this, "int*", &pModes := 0, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} 
     */
    GetModeInfo(Mode_, pWidth, pHeight, pDepth) {
        pWidthMarshal := pWidth is VarRef ? "int*" : "ptr"
        pHeightMarshal := pHeight is VarRef ? "int*" : "ptr"
        pDepthMarshal := pDepth is VarRef ? "int*" : "ptr"

        result := ComCall(4, this, "int", Mode_, pWidthMarshal, pWidth, pHeightMarshal, pHeight, pDepthMarshal, pDepth, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetCurrentMode() {
        result := ComCall(5, this, "int*", &pMode := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    SetEnabled(Mode_, bEnabled) {
        result := ComCall(8, this, "int", Mode_, "int", bEnabled, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetClipFactor() {
        result := ComCall(9, this, "int*", &pClipFactor := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} 
     */
    SetMessageDrain(hwnd_) {
        hwnd_ := hwnd_ is Win32Handle ? NumGet(hwnd_, "ptr") : hwnd_

        result := ComCall(11, this, "ptr", hwnd_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetMessageDrain() {
        hwnd_ := HWND()
        result := ComCall(12, this, "ptr", hwnd_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    SetMonitor(Monitor_) {
        result := ComCall(13, this, "int", Monitor_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetMonitor() {
        result := ComCall(14, this, "int*", &Monitor_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
