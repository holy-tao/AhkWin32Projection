...
class IWMLanguageList extends IUnknown{
    ...
    GetLanguageCount(pwCount) {
        result := ComCall(3, this, "ushort*", pwCount, "int")
        if(result != 0)
    ...
    GetLanguageDetails(wIndex, pwszLanguageString, pcchLanguageStringLength) {
        ...
        result := ComCall(4, this, "ushort", wIndex, "ptr", pwszLanguageString, "ushort*", pcchLanguageStringLength, "int")
        if(result != 0)
            throw OSError(result)
    ...
    AddLanguageByRFC1766String(pwszLanguageString, pwIndex) {
        ...
        result := ComCall(5, this, "ptr", pwszLanguageString, "ushort*", pwIndex, "int")
        if(result != 0)
            throw OSError(result)
