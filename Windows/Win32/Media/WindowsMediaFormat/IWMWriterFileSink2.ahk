...
class IWMWriterFileSink2 extends IWMWriterFileSink{
     ...
     */
    IsStopped(pfStopped) {
        result := ComCall(11, this, "ptr", pfStopped, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFileDuration(pcnsDuration) {
        result := ComCall(12, this, "uint*", pcnsDuration, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetFileSize(pcbFile) {
        result := ComCall(13, this, "uint*", pcbFile, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    IsClosed(pfClosed) {
        result := ComCall(15, this, "ptr", pfClosed, "int")
        if(result != 0)
            throw OSError(result)
