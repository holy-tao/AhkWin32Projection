...
class IWMReaderAllocatorEx extends IUnknown{
    ...
    AllocateForStreamEx(wStreamNum, cbBuffer, ppBuffer, dwFlags, cnsSampleTime, cnsSampleDuration, pvContext) {
        result := ComCall(3, this, "ushort", wStreamNum, "uint", cbBuffer, "ptr", ppBuffer, "uint", dwFlags, "uint", cnsSampleTime, "uint", cnsSampleDuration, "ptr", pvContext, "int")
        if(result != 0)
    ...
    AllocateForOutputEx(dwOutputNum, cbBuffer, ppBuffer, dwFlags, cnsSampleTime, cnsSampleDuration, pvContext) {
        result := ComCall(4, this, "uint", dwOutputNum, "uint", cbBuffer, "ptr", ppBuffer, "uint", dwFlags, "uint", cnsSampleTime, "uint", cnsSampleDuration, "ptr", pvContext, "int")
        if(result != 0)
            throw OSError(result)
