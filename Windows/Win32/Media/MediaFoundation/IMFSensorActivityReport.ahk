 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/mfidl/nn-mfidl-imfsensoractivityreport
...
class IMFSensorActivityReport extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mfidl/nf-mfidl-imfsensoractivityreport-getfriendlyname
     */
    GetFriendlyName(FriendlyName, cchFriendlyName) {
        FriendlyName := FriendlyName is String ? StrPtr(FriendlyName) : FriendlyName

        result := ComCall(3, this, "ptr", FriendlyName, "uint", cchFriendlyName, "uint*", &pcchWritten := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mfidl/nf-mfidl-imfsensoractivityreport-getsymboliclink
     */
    GetSymbolicLink(SymbolicLink, cchSymbolicLink) {
        SymbolicLink := SymbolicLink is String ? StrPtr(SymbolicLink) : SymbolicLink

        result := ComCall(4, this, "ptr", SymbolicLink, "uint", cchSymbolicLink, "uint*", &pcchWritten := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mfidl/nf-mfidl-imfsensoractivityreport-getprocesscount
     */
    GetProcessCount() {
     ...
     * @returns {Pointer<IMFSensorProcessActivity>} A pointer to the  <a href="https://docs.microsoft.com/windows/desktop/api/mfidl/nn-mfidl-imfsensorprocessactivity">IMFSensorProcessActivity</a> associated with the specified index.
     * @see https://learn.microsoft.com/windows/win32/api//content/mfidl/nf-mfidl-imfsensoractivityreport-getprocessactivity
     */
    GetProcessActivity(Index) {
        result := ComCall(6, this, "uint", Index, "ptr*", &ppProcessActivity := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return ppProcessActivity
    }
}
