...
class IMFRateSupport extends IUnknown{
    ...
    GetSlowestRate(eDirection, fThin, pflRate) {
        result := ComCall(3, this, "int", eDirection, "int", fThin, "float*", pflRate, "int")
        if(result != 0)
    ...
    GetFastestRate(eDirection, fThin, pflRate) {
        result := ComCall(4, this, "int", eDirection, "int", fThin, "float*", pflRate, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {BOOL} fThin 
     * @param {Float} flRate 
     * @param {Pointer<Single>} pflNearestSupportedRate 
     * @returns {HRESULT} 
     */
    IsRateSupported(fThin, flRate, pflNearestSupportedRate) {
        result := ComCall(5, this, "int", fThin, "float", flRate, "float*", pflNearestSupportedRate, "int")
        if(result != 0)
            throw OSError(result)
