 ...
 */
export default struct IMFRateSupport extends IUnknown {
    /**
     ...
     * used for IMFRateSupport interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetSlowestRate  : IntPtr
        GetFastestRate  : IntPtr
        IsRateSupported : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMFRateSupport.Vtbl()
        }
    ...
    GetSlowestRate(eDirection, fThin) {
        result := ComCall(3, this, MFRATE_DIRECTION, eDirection, BOOL, fThin, "float*", &pflRate := 0, "HRESULT")
        return pflRate
    }
    ...
    GetFastestRate(eDirection, fThin) {
        result := ComCall(4, this, MFRATE_DIRECTION, eDirection, BOOL, fThin, "float*", &pflRate := 0, "HRESULT")
        return pflRate
    }
    ...
    IsRateSupported(fThin, flRate, pflNearestSupportedRate) {
        ...
        result := ComCall(5, this, BOOL, fThin, "float", flRate, pflNearestSupportedRateMarshal, pflNearestSupportedRate, "HRESULT")
        return result
    }

    Query(iid) {
        if (IMFRateSupport.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetSlowestRate)
        CallbackFree(this.vtbl.GetFastestRate)
        CallbackFree(this.vtbl.IsRateSupported)
    }
}
