 ...
 */
export default struct IMFInputTrustAuthority extends IUnknown {
    /**
     ...
     * used for IMFInputTrustAuthority interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetDecrypter  : IntPtr
        RequestAccess : IntPtr
        GetPolicy     : IntPtr
        BindAccess    : IntPtr
        UpdateAccess  : IntPtr
        Reset         : IntPtr
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMFInputTrustAuthority.Vtbl()
        }
    ...
    RequestAccess(Action) {
        result := ComCall(4, this, MFPOLICYMANAGER_ACTION, Action, "ptr*", &ppContentEnablerActivate := 0, "HRESULT")
     ...
     */
    GetPolicy(Action) {
        result := ComCall(5, this, MFPOLICYMANAGER_ACTION, Action, "ptr*", &ppPolicy := 0, "HRESULT")
     ...
     */
    BindAccess(pParam) {
        result := ComCall(6, this, MFINPUTTRUSTAUTHORITY_ACCESS_PARAMS.Ptr, pParam, "HRESULT")
        return result
    }
     ...
     */
    UpdateAccess(pParam) {
        result := ComCall(7, this, MFINPUTTRUSTAUTHORITY_ACCESS_PARAMS.Ptr, pParam, "HRESULT")
        return result
    }
    ...
    Query(iid) {
        if (IMFInputTrustAuthority.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetDecrypter)
        CallbackFree(this.vtbl.RequestAccess)
        CallbackFree(this.vtbl.GetPolicy)
        CallbackFree(this.vtbl.BindAccess)
        CallbackFree(this.vtbl.UpdateAccess)
