...
class IMFSensorActivitiesReport extends IUnknown{
     ...
     */
    GetCount(pcCount) {
        result := ComCall(3, this, "uint*", pcCount, "int")
        if(result != 0)
    ...
    GetActivityReport(Index, sensorActivityReport) {
        result := ComCall(4, this, "uint", Index, "ptr", sensorActivityReport, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetActivityReportByDeviceName(SymbolicName, sensorActivityReport) {
        ...
        result := ComCall(5, this, "ptr", SymbolicName, "ptr", sensorActivityReport, "int")
        if(result != 0)
            throw OSError(result)
