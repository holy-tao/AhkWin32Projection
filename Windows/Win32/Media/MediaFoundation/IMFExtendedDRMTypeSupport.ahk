 ...
 */
export default struct IMFExtendedDRMTypeSupport extends IUnknown {
    /**
     ...
     * used for IMFExtendedDRMTypeSupport interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        IsTypeSupportedEx : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMFExtendedDRMTypeSupport.Vtbl()
        }
    ...
    Query(iid) {
        if (IMFExtendedDRMTypeSupport.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.IsTypeSupportedEx)
    }
}
