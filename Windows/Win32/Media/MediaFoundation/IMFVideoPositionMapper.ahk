...
class IMFVideoPositionMapper extends IUnknown{
     ...
     * @param {Pointer<Single>} pxIn 
     * @param {Pointer<Single>} pyIn 
     * @returns {HRESULT} 
     */
    MapOutputCoordinateToInputStream(xOut, yOut, dwOutputStreamIndex, dwInputStreamIndex, pxIn, pyIn) {
        result := ComCall(3, this, "float", xOut, "float", yOut, "uint", dwOutputStreamIndex, "uint", dwInputStreamIndex, "float*", pxIn, "float*", pyIn, "int")
        if(result != 0)
