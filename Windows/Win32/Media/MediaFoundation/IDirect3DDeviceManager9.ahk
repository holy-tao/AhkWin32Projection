...
class IDirect3DDeviceManager9 extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<IDirect3DDevice9>} pDevice 
    ...
    LockDevice(hDevice, ppDevice, fBlock) {
        ...
        result := ComCall(7, this, "ptr", hDevice, "ptr", ppDevice, "int", fBlock, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetVideoService(hDevice, riid, ppService) {
        ...
        result := ComCall(9, this, "ptr", hDevice, "ptr", riid, "ptr", ppService, "int")
        if(result != 0)
            throw OSError(result)
