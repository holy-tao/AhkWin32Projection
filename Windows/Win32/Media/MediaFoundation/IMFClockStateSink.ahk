 ...
 */
export default struct IMFClockStateSink extends IUnknown {
    /**
     ...
     * used for IMFClockStateSink interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        OnClockStart   : IntPtr
        OnClockStop    : IntPtr
        OnClockPause   : IntPtr
        OnClockRestart : IntPtr
        OnClockSetRate : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMFClockStateSink.Vtbl()
        }
    ...
    Query(iid) {
        if (IMFClockStateSink.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.OnClockStart)
        CallbackFree(this.vtbl.OnClockStop)
        CallbackFree(this.vtbl.OnClockPause)
        CallbackFree(this.vtbl.OnClockRestart)
        CallbackFree(this.vtbl.OnClockSetRate)
    }
}
