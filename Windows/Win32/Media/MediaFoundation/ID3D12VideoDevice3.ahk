 ...
 * @remarks
 * 
 * @see https://learn.microsoft.com/windows/win32/api//content/d3d12video/nn-d3d12video-id3d12videodevice3
...
class ID3D12VideoDevice3 extends ID3D12VideoDevice2{
     ...
     * @returns {Pointer<Pointer<Void>>} A pointer to a memory block that receives a pointer to the video encoder interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/d3d12video/nf-d3d12video-id3d12videodevice3-createvideoencoder
     */
    CreateVideoEncoder(pDesc, riid) {
        result := ComCall(14, this, "ptr", pDesc, "ptr", riid, "ptr*", &ppVideoEncoder := 0, "int")
        if(result != 0) {
     ...
     * @returns {Pointer<Pointer<Void>>} A pointer to a memory block that receives a pointer to the video encoder heap interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/d3d12video/nf-d3d12video-id3d12videodevice3-createvideoencoderheap
     */
    CreateVideoEncoderHeap(pDesc, riid) {
        result := ComCall(15, this, "ptr", pDesc, "ptr", riid, "ptr*", &ppVideoEncoderHeap := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
