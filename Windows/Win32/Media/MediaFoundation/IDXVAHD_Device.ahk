...
class IDXVAHD_Device extends IUnknown{
    ...
    CreateVideoSurface(Width, Height, Format, Pool, Usage, Type, NumSurfaces, ppSurfaces, pSharedHandle) {
        result := ComCall(3, this, "uint", Width, "uint", Height, "uint", Format, "int", Pool, "uint", Usage, "int", Type, "uint", NumSurfaces, "ptr", ppSurfaces, "ptr", pSharedHandle, "int")
        if(result != 0)
    ...
    GetVideoProcessorOutputFormats(Count, pFormats) {
        result := ComCall(5, this, "uint", Count, "uint*", pFormats, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetVideoProcessorInputFormats(Count, pFormats) {
        result := ComCall(6, this, "uint", Count, "uint*", pFormats, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateVideoProcessor(pVPGuid, ppVideoProcessor) {
        result := ComCall(10, this, "ptr", pVPGuid, "ptr", ppVideoProcessor, "int")
        if(result != 0)
            throw OSError(result)
