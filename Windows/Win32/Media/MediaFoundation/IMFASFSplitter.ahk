...
class IMFASFSplitter extends IUnknown{
    ...
    SelectStreams(pwStreamNumbers, wNumStreams) {
        result := ComCall(6, this, "ushort*", pwStreamNumbers, "ushort", wNumStreams, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * 
     * @param {Pointer<UInt16>} pwStreamNumbers 
     * @param {Pointer<UInt16>} pwNumStreams 
     * @returns {HRESULT} 
     */
    GetSelectedStreams(pwStreamNumbers, pwNumStreams) {
        result := ComCall(7, this, "ushort*", pwStreamNumbers, "ushort*", pwNumStreams, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IMFMediaBuffer>} pIBuffer 
    ...
    /**
     * 
     * @param {Pointer<Int32>} pdwStatusFlags 
    ...
    GetNextSample(pdwStatusFlags, pwStreamNumber, ppISample) {
        result := ComCall(9, this, "int*", pdwStatusFlags, "ushort*", pwStreamNumber, "ptr", ppISample, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pdwLastSendTime 
     * @returns {HRESULT} 
     */
    GetLastSendTime(pdwLastSendTime) {
        result := ComCall(11, this, "uint*", pdwLastSendTime, "int")
        if(result != 0)
            throw OSError(result)
