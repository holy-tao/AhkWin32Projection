...
class IMF2DBuffer extends IUnknown{
    ...
    Lock2D(ppbScanline0, plPitch) {
        result := ComCall(3, this, "char*", ppbScanline0, "int*", plPitch, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<Byte>} pbScanline0 
     * @param {Pointer<Int32>} plPitch 
     * @returns {HRESULT} 
     */
    GetScanline0AndPitch(pbScanline0, plPitch) {
        result := ComCall(5, this, "char*", pbScanline0, "int*", plPitch, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsContiguousFormat(pfIsContiguous) {
        result := ComCall(6, this, "ptr", pfIsContiguous, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetContiguousLength(pcbLength) {
        result := ComCall(7, this, "uint*", pcbLength, "int")
        if(result != 0)
            throw OSError(result)
