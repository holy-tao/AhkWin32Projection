...
class IMFMediaKeySystemAccess extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<IPropertyStore>} pCdmCustomConfig 
     * @param {Pointer<IMFMediaKeys2>} ppKeys 
     * @returns {HRESULT} 
     */
    CreateMediaKeys(pCdmCustomConfig, ppKeys) {
        result := ComCall(3, this, "ptr", pCdmCustomConfig, "ptr", ppKeys, "int")
        if(result != 0)
    ...
    get_SupportedConfiguration(ppSupportedConfiguration) {
        result := ComCall(4, this, "ptr", ppSupportedConfiguration, "int")
        if(result != 0)
            throw OSError(result)
