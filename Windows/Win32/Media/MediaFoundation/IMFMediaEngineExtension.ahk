...
class IMFMediaEngineExtension extends IUnknown{
    ...
    CanPlayType(AudioOnly, MimeType, pAnswer) {
        ...
        result := ComCall(3, this, "int", AudioOnly, "ptr", MimeType, "int*", pAnswer, "int")
        if(result != 0)
    ...
    BeginCreateObject(bstrURL, pByteStream, type, ppIUnknownCancelCookie, pCallback, punkState) {
        ...
        result := ComCall(4, this, "ptr", bstrURL, "ptr", pByteStream, "int", type, "ptr", ppIUnknownCancelCookie, "ptr", pCallback, "ptr", punkState, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EndCreateObject(pResult, ppObject) {
        result := ComCall(6, this, "ptr", pResult, "ptr", ppObject, "int")
        if(result != 0)
            throw OSError(result)
