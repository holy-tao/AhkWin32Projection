 ...
 */
export default struct IMFContentProtectionManager extends IUnknown {
    /**
     ...
     * used for IMFContentProtectionManager interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        BeginEnableContent : IntPtr
        EndEnableContent   : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMFContentProtectionManager.Vtbl()
        }
    ...
    Query(iid) {
        if (IMFContentProtectionManager.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.BeginEnableContent)
        CallbackFree(this.vtbl.EndEnableContent)
    }
}
