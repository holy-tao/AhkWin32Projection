...
class IMFMediaEngineClassFactoryEx extends IMFMediaEngineClassFactory{
    ...
    CreateMediaSourceExtension(dwFlags, pAttr, ppMSE) {
        result := ComCall(6, this, "uint", dwFlags, "ptr", pAttr, "ptr", ppMSE, "int")
        if(result != 0)
    ...
    CreateMediaKeys(keySystem, cdmStorePath, ppKeys) {
        ...
        result := ComCall(7, this, "ptr", keySystem, "ptr", cdmStorePath, "ptr", ppKeys, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsTypeSupported(type, keySystem, isSupported) {
        ...
        result := ComCall(8, this, "ptr", type, "ptr", keySystem, "ptr", isSupported, "int")
        if(result != 0)
            throw OSError(result)
