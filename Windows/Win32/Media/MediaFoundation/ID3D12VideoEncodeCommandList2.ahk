...
#Import ".\D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS.ahk" { D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS }
#Import ".\ID3D12VideoEncoder.ahk" { ID3D12VideoEncoder }
#Import ".\D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS.ahk" { D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS }
#Import ".\ID3D12VideoEncodeCommandList1.ahk" { ID3D12VideoEncodeCommandList1 }
#Import ".\ID3D12VideoEncoderHeap.ahk" { ID3D12VideoEncoderHeap }
#Import ".\D3D12_VIDEO_ENCODER_ENCODEFRAME_INPUT_ARGUMENTS.ahk" { D3D12_VIDEO_ENCODER_ENCODEFRAME_INPUT_ARGUMENTS }
#Import ".\D3D12_VIDEO_ENCODER_RESOLVE_METADATA_INPUT_ARGUMENTS.ahk" { D3D12_VIDEO_ENCODER_RESOLVE_METADATA_INPUT_ARGUMENTS }
 ...
 */
export default struct ID3D12VideoEncodeCommandList2 extends ID3D12VideoEncodeCommandList1 {
    /**
     ...
     * used for ID3D12VideoEncodeCommandList2 interfaces
    */
    struct Vtbl extends ID3D12VideoEncodeCommandList1.Vtbl {
        EncodeFrame                  : IntPtr
        ResolveEncoderOutputMetadata : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ID3D12VideoEncodeCommandList2.Vtbl()
        }
    ...
    EncodeFrame(pEncoder, pHeap, pInputArguments, pOutputArguments) {
        ComCall(27, this, "ptr", pEncoder, "ptr", pHeap, D3D12_VIDEO_ENCODER_ENCODEFRAME_INPUT_ARGUMENTS.Ptr, pInputArguments, D3D12_VIDEO_ENCODER_ENCODEFRAME_OUTPUT_ARGUMENTS.Ptr, pOutputArguments)
    }
    ...
    ResolveEncoderOutputMetadata(pInputArguments, pOutputArguments) {
        ComCall(28, this, D3D12_VIDEO_ENCODER_RESOLVE_METADATA_INPUT_ARGUMENTS.Ptr, pInputArguments, D3D12_VIDEO_ENCODER_RESOLVE_METADATA_OUTPUT_ARGUMENTS.Ptr, pOutputArguments)
    }

    Query(iid) {
        if (ID3D12VideoEncodeCommandList2.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.EncodeFrame)
        CallbackFree(this.vtbl.ResolveEncoderOutputMetadata)
    }
}
