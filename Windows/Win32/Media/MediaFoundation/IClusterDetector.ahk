...
class IClusterDetector extends IUnknown{
    ...
    Initialize(wBaseEntryLevel, wClusterEntryLevel) {
        result := ComCall(3, this, "ushort", wBaseEntryLevel, "ushort", wClusterEntryLevel, "int")
        if(result != 0)
     ...
     * @param {Pointer<IToc>} pSrcToc 
     * @param {Pointer<IToc>} ppDstToc 
     * @returns {HRESULT} 
     */
    Detect(dwMaxNumClusters, fMinClusterDuration, fMaxClusterDuration, pSrcToc, ppDstToc) {
        result := ComCall(4, this, "uint", dwMaxNumClusters, "float", fMinClusterDuration, "float", fMaxClusterDuration, "ptr", pSrcToc, "ptr", ppDstToc, "int")
        if(result != 0)
            throw OSError(result)
