...
/**
 * The MCI_DGV_SETAUDIO_PARMSA (ANSI) structure (digitalv.h) contains parameters for the MCI_SETAUDIO command for digital-video devices.
 * @remarks
 ...
 * @see https://learn.microsoft.com/windows/win32/api/digitalv/ns-digitalv-mci_dgv_setaudio_parmsa
    ...
    lpstrAlgorithm{
        get {
            if(!this.HasProp("__lpstrAlgorithm"))
                this.__lpstrAlgorithm := PSTR(this.ptr + 20)
    ...
    lpstrQuality{
        get {
            if(!this.HasProp("__lpstrQuality"))
                this.__lpstrQuality := PSTR(this.ptr + 28)
