 ...
 */
export default struct ISpeechObjectTokens extends IDispatch {
    /**
     ...
     * used for ISpeechObjectTokens interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISpeechObjectTokens.Vtbl()
        }
     ...
     */
    Item(Index) {
        result := ComCall(8, this, Int32, Index, "ptr*", &Token := 0, "HRESULT")
    ...
    Query(iid) {
        if (ISpeechObjectTokens.IID.Equals(iid)) {
            return true
        }
