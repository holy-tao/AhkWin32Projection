...
class ISpPhrase extends IUnknown{
     ...
     */
    GetPhrase() {
        result := ComCall(3, this, "ptr*", &ppCoMemPhrase := 0, "int")
        if(result != 0) {
     ...
     */
    GetSerializedPhrase() {
        result := ComCall(4, this, "ptr*", &ppCoMemPhrase := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetText(ulStart, ulCount, fUseTextReplacements, ppszCoMemText, pbDisplayAttributes) {
        ...
        result := ComCall(5, this, "uint", ulStart, "uint", ulCount, "int", fUseTextReplacements, ppszCoMemTextMarshal, ppszCoMemText, pbDisplayAttributesMarshal, pbDisplayAttributes, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
