 ...
 */
export default struct ISpeechPhraseRules extends IDispatch {
    /**
     ...
     * used for ISpeechPhraseRules interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISpeechPhraseRules.Vtbl()
        }
     ...
     */
    Item(Index) {
        result := ComCall(8, this, Int32, Index, "ptr*", &Rule := 0, "HRESULT")
    ...
    _Query(iid) {
        if (ISpeechPhraseRules.IID.Equals(iid)) {
            return true
        }
