...
class ISpThreadTask extends Win32ComInterface{
    ...
    InitThread(pvTaskData, hwnd) {
        ...
        result := ComCall(0, this, "ptr", pvTaskData, "ptr", hwnd, "int")
        if(result != 0)
    ...
    ThreadProc(pvTaskData, hExitThreadEvent, hNotifyEvent, hwndWorker, pfContinueProcessing) {
        ...
        result := ComCall(1, this, "ptr", pvTaskData, "ptr", hExitThreadEvent, "ptr", hNotifyEvent, "ptr", hwndWorker, "int*", pfContinueProcessing, "int")
        if(result != 0)
            throw OSError(result)
