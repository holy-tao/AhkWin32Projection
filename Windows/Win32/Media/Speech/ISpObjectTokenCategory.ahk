...
class ISpObjectTokenCategory extends ISpDataKey{
    ...
    GetId(ppszCoMemCategoryId) {
        result := ComCall(16, this, "ptr", ppszCoMemCategoryId, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDataKey(spdkl, ppDataKey) {
        result := ComCall(17, this, "int", spdkl, "ptr", ppDataKey, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumTokens(pzsReqAttribs, pszOptAttribs, ppEnum) {
        ...
        result := ComCall(18, this, "ptr", pzsReqAttribs, "ptr", pszOptAttribs, "ptr", ppEnum, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDefaultTokenId(ppszCoMemTokenId) {
        result := ComCall(20, this, "ptr", ppszCoMemTokenId, "int")
        if(result != 0)
            throw OSError(result)
