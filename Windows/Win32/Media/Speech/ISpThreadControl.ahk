...
class ISpThreadControl extends ISpNotifySink{
     ...
     * @returns {HWND} 
     * @see https://learn.microsoft.com/windows/win32/api//content/threadpoolapiset/nf-threadpoolapiset-startthreadpoolio
     */
    StartThread(dwFlags) {
        phwnd := HWND()
        result := ComCall(4, this, "uint", dwFlags, "ptr", phwnd, "int")
        if(result != 0) {
    ...
    WaitForThreadDone(fForceStop, msTimeOut) {
        result := ComCall(5, this, "int", fForceStop, "int*", &phrThreadResult := 0, "uint", msTimeOut, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/processthreadsapi/nf-processthreadsapi-terminatethread
     */
    TerminateThread() {
