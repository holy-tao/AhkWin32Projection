...
class ISpeechObjectTokenCategory extends IDispatch{
     ...
     */
    GetDataKey(Location_) {
        result := ComCall(11, this, "int", Location_, "ptr*", &DataKey := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    EnumerateTokens(RequiredAttributes, OptionalAttributes) {
        ...
        result := ComCall(12, this, "ptr", RequiredAttributes, "ptr", OptionalAttributes, "ptr*", &Tokens := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
