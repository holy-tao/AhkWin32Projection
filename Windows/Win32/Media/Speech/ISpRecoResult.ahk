...
class ISpRecoResult extends ISpPhrase{
    ...
    GetAlternates(ulStartElement, cElements, ulRequestCount, ppPhrases, pcPhrasesReturned) {
        result := ComCall(8, this, "uint", ulStartElement, "uint", cElements, "uint", ulRequestCount, "ptr", ppPhrases, "uint*", pcPhrasesReturned, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SpeakAudio(ulStartElement, cElements, dwFlags, pulStreamNumber) {
        result := ComCall(10, this, "uint", ulStartElement, "uint", cElements, "uint", dwFlags, "uint*", pulStreamNumber, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<SPSERIALIZEDRESULT>} ppCoMemSerializedResult 
     * @returns {HRESULT} 
     */
    Serialize(ppCoMemSerializedResult) {
        result := ComCall(11, this, "ptr", ppCoMemSerializedResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetRecoContext(ppRecoContext) {
        result := ComCall(13, this, "ptr", ppRecoContext, "int")
        if(result != 0)
            throw OSError(result)
