...
class ISpPrivateEngineCallEx extends IUnknown{
     ...
     * @param {Pointer<Void>} ppCoMemOutFrame 
     * @param {Pointer<UInt32>} pulOutFrameSize 
     * @returns {HRESULT} 
     */
    CallEngineSynchronize(pInFrame, ulInFrameSize, ppCoMemOutFrame, pulOutFrameSize) {
        result := ComCall(3, this, "ptr", pInFrame, "uint", ulInFrameSize, "ptr", ppCoMemOutFrame, "uint*", pulOutFrameSize, "int")
        if(result != 0)
    ...
    CallEngineImmediate(pInFrame, ulInFrameSize, ppCoMemOutFrame, pulOutFrameSize) {
        result := ComCall(4, this, "ptr", pInFrame, "uint", ulInFrameSize, "ptr", ppCoMemOutFrame, "uint*", pulOutFrameSize, "int")
        if(result != 0)
            throw OSError(result)
