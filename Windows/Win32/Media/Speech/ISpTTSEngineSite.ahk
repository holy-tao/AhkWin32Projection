...
class ISpTTSEngineSite extends ISpEventSink{
    ...
    Write(pBuff, cb) {
        ...
        result := ComCall(6, this, pBuffMarshal, pBuff, "uint", cb, "uint*", &pcbWritten := 0, "int")
        if(result != 0) {
     ...
     */
    GetRate() {
        result := ComCall(7, this, "int*", &pRateAdjust := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetVolume() {
        result := ComCall(8, this, "ushort*", &pusVolume := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetSkipInfo(peType, plNumItems) {
        ...
        result := ComCall(9, this, peTypeMarshal, peType, plNumItemsMarshal, plNumItems, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
