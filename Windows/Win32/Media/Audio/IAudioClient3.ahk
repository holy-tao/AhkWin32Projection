 ...
 */
export default struct IAudioClient3 extends IAudioClient2 {
    /**
     ...
     * used for IAudioClient3 interfaces
    */
    struct Vtbl extends IAudioClient2.Vtbl {
        GetSharedModeEnginePeriod        : IntPtr
        GetCurrentSharedModeEnginePeriod : IntPtr
        InitializeSharedAudioStream      : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAudioClient3.Vtbl()
        }
    ...
    Query(iid) {
        if (IAudioClient3.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetSharedModeEnginePeriod)
        CallbackFree(this.vtbl.GetCurrentSharedModeEnginePeriod)
        CallbackFree(this.vtbl.InitializeSharedAudioStream)
    }
}
