 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/mmdeviceapi/nn-mmdeviceapi-immdevice
...
class IMMDevice extends IUnknown{
     ...
     * @returns {Pointer<Pointer<Void>>} Pointer to a pointer variable into which the method writes the address of the interface specified by parameter <i>iid</i>. Through this method, the caller obtains a counted reference to the interface. The caller is responsible for releasing the interface, when it is no longer needed, by calling the interface's <b>Release</b> method. If the <b>Activate</b> call fails,  <i>*ppInterface</i> is <b>NULL</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/mmdeviceapi/nf-mmdeviceapi-immdevice-activate
     */
    Activate(iid, dwClsCtx, pActivationParams) {
        result := ComCall(3, this, "ptr", iid, "uint", dwClsCtx, "ptr", pActivationParams, "ptr*", &ppInterface := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mmdeviceapi/nf-mmdeviceapi-immdevice-openpropertystore
     */
    OpenPropertyStore(stgmAccess) {
        result := ComCall(4, this, "uint", stgmAccess, "ptr*", &ppProperties := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mmdeviceapi/nf-mmdeviceapi-immdevice-getid
     */
    GetId() {
        result := ComCall(5, this, "ptr*", &ppstrId := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mmdeviceapi/nf-mmdeviceapi-immdevice-getstate
     */
    GetState() {
        result := ComCall(6, this, "uint*", &pdwState := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
