...
class IDirectSound extends IUnknown{
    ...
    CreateSoundBuffer(pcDSBufferDesc, ppDSBuffer, pUnkOuter) {
        result := ComCall(3, this, "ptr", pcDSBufferDesc, "ptr", ppDSBuffer, "ptr", pUnkOuter, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<IDirectSoundBuffer>} pDSBufferOriginal 
     * @param {Pointer<IDirectSoundBuffer>} ppDSBufferDuplicate 
     * @returns {HRESULT} 
     */
    DuplicateSoundBuffer(pDSBufferOriginal, ppDSBufferDuplicate) {
        result := ComCall(5, this, "ptr", pDSBufferOriginal, "ptr", ppDSBufferDuplicate, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetCooperativeLevel(hwnd, dwLevel) {
        ...
        result := ComCall(6, this, "ptr", hwnd, "uint", dwLevel, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pdwSpeakerConfig 
     * @returns {HRESULT} 
     */
    GetSpeakerConfig(pdwSpeakerConfig) {
        result := ComCall(8, this, "uint*", pdwSpeakerConfig, "int")
        if(result != 0)
            throw OSError(result)
