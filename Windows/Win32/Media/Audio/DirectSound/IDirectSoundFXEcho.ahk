...
#Import ".\DSFXEcho.ahk" { DSFXEcho }

/**
 * @namespace Windows.Win32.Media.Audio.DirectSound
 */
export default struct IDirectSoundFXEcho extends IUnknown {
    /**
     ...
     * used for IDirectSoundFXEcho interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDirectSoundFXEcho.Vtbl()
        }
    ...
    SetAllParameters(pcDsFxEcho) {
        result := ComCall(3, this, DSFXEcho.Ptr, pcDsFxEcho, "HRESULT")
        return result
    }
    ...
    GetAllParameters() {
        pDsFxEcho := DSFXEcho()
        result := ComCall(4, this, DSFXEcho.Ptr, pDsFxEcho, "HRESULT")
        return pDsFxEcho
    }

    Query(iid) {
        if (IDirectSoundFXEcho.IID.Equals(iid)) {
            return true
        }
