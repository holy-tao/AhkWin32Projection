 ...
 */
export default struct IAudioSessionManager extends IUnknown {
    /**
     ...
     * used for IAudioSessionManager interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetAudioSessionControl : IntPtr
        GetSimpleAudioVolume   : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAudioSessionManager.Vtbl()
        }
    ...
    Query(iid) {
        if (IAudioSessionManager.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetAudioSessionControl)
        CallbackFree(this.vtbl.GetSimpleAudioVolume)
    }
}
