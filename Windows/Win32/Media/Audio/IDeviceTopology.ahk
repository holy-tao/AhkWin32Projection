...
class IDeviceTopology extends IUnknown{
    ...
    GetConnector(nIndex, ppConnector) {
        result := ComCall(4, this, "uint", nIndex, "ptr", ppConnector, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetSubunit(nIndex, ppSubunit) {
        result := ComCall(6, this, "uint", nIndex, "ptr", ppSubunit, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetPartById(nId, ppPart) {
        result := ComCall(7, this, "uint", nId, "ptr", ppPart, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDeviceId(ppwstrDeviceId) {
        result := ComCall(8, this, "ptr", ppwstrDeviceId, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IPart>} pIPartFrom 
     * @param {Pointer<IPart>} pIPartTo 
     * @param {BOOL} bRejectMixedPaths 
     * @param {Pointer<IPartsList>} ppParts 
     * @returns {HRESULT} 
     */
    GetSignalPath(pIPartFrom, pIPartTo, bRejectMixedPaths, ppParts) {
        result := ComCall(9, this, "ptr", pIPartFrom, "ptr", pIPartTo, "int", bRejectMixedPaths, "ptr", ppParts, "int")
        if(result != 0)
            throw OSError(result)
