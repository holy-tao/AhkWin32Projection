 ...
 */
export default struct IAudioCaptureClient extends IUnknown {
    /**
     ...
     * used for IAudioCaptureClient interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetBuffer         : IntPtr
        ReleaseBuffer     : IntPtr
        GetNextPacketSize : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAudioCaptureClient.Vtbl()
        }
    ...
    ReleaseBuffer(NumFramesRead) {
        result := ComCall(4, this, UInt32, NumFramesRead, "HRESULT")
        return result
    }
    ...
    _Query(iid) {
        if (IAudioCaptureClient.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.GetNextPacketSize)
    }
}
