 ...
 */
export default struct IAcousticEchoCancellationControl extends IUnknown {
    /**
     ...
     * used for IAcousticEchoCancellationControl interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        SetEchoCancellationRenderEndpoint : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAcousticEchoCancellationControl.Vtbl()
        }
    ...
    Query(iid) {
        if (IAcousticEchoCancellationControl.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.SetEchoCancellationRenderEndpoint)
    }
}
