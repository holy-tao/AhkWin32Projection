 ...
 */
export default struct IAudioClock2 extends IUnknown {
    /**
     ...
     * used for IAudioClock2 interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetDevicePosition : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IAudioClock2.Vtbl()
        }
    ...
    Query(iid) {
        if (IAudioClock2.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetDevicePosition)
    }
}
