 ...
 */
export default struct IDeviceSpecificProperty extends IUnknown {
    /**
     ...
     * used for IDeviceSpecificProperty interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDeviceSpecificProperty.Vtbl()
        }
    ...
    Query(iid) {
        if (IDeviceSpecificProperty.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.Get4BRange)
    }
}
