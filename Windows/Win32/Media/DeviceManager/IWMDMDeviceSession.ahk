 ...
 */
export default struct IWMDMDeviceSession extends IUnknown {
    /**
     ...
     * used for IWMDMDeviceSession interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWMDMDeviceSession.Vtbl()
        }
    ...
    Query(iid) {
        if (IWMDMDeviceSession.IID.Equals(iid)) {
            return true
        }
