 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nn-mswmdm-imdspobject2
...
class IMDSPObject2 extends IMDSPObject{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nf-mswmdm-imdspobject2-readonclearchannel
     */
    ReadOnClearChannel(pdwSize) {
        pdwSizeMarshal := pdwSize is VarRef ? "uint*" : "ptr"

        result := ComCall(11, this, "char*", &pData := 0, pdwSizeMarshal, pdwSize, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nf-mswmdm-imdspobject2-writeonclearchannel
     */
    WriteOnClearChannel(pData, pdwSize) {
        pDataMarshal := pData is VarRef ? "char*" : "ptr"
        pdwSizeMarshal := pdwSize is VarRef ? "uint*" : "ptr"

        result := ComCall(12, this, pDataMarshal, pData, pdwSizeMarshal, pdwSize, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
