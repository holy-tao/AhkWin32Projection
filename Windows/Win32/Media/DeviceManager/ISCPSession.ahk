 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nn-mswmdm-iscpsession
...
class ISCPSession extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nf-mswmdm-iscpsession-beginsession
     */
    BeginSession(pIDevice, pCtx, dwSizeCtx) {
        pCtxMarshal := pCtx is VarRef ? "char*" : "ptr"

        result := ComCall(3, this, "ptr", pIDevice, pCtxMarshal, pCtx, "uint", dwSizeCtx, "int")
        if(result != 0) {
     ...
     * @param {Integer} dwSizeCtx <b>DWORD</b> containing the size of context.
     * @returns {HRESULT} If the method succeeds, it returns S_OK. If the method fails, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nf-mswmdm-iscpsession-endsession
     */
    EndSession(pCtx, dwSizeCtx) {
        pCtxMarshal := pCtx is VarRef ? "char*" : "ptr"

        result := ComCall(4, this, pCtxMarshal, pCtx, "uint", dwSizeCtx, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mswmdm/nf-mswmdm-iscpsession-getsecurequery
     */
    GetSecureQuery() {
        result := ComCall(5, this, "ptr*", &ppSecureQuery := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
