 ...
 */
export default struct IWMDMObjectInfo extends IUnknown {
    /**
     ...
     * used for IWMDMObjectInfo interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IWMDMObjectInfo.Vtbl()
        }
    ...
    Query(iid) {
        if (IWMDMObjectInfo.IID.Equals(iid)) {
            return true
        }
