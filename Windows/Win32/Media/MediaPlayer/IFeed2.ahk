...
class IFeed2 extends IFeed{
    ...
    GetItemByEffectiveId(itemEffectiveId, disp) {
        result := ComCall(51, this, "int", itemEffectiveId, "ptr", disp, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<Double>} lastItemDownloadTime 
     * @returns {HRESULT} 
     */
    get_LastItemDownloadTime(lastItemDownloadTime) {
        result := ComCall(52, this, "double*", lastItemDownloadTime, "int")
        if(result != 0)
            throw OSError(result)
