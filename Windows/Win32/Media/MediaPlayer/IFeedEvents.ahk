...
class IFeedEvents extends IDispatch{
    ...
    FeedRenamed(path, oldPath) {
        ...
        result := ComCall(9, this, "ptr", path, "ptr", oldPath, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FeedUrlChanged(path) {
        ...
        result := ComCall(10, this, "ptr", path, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FeedDownloadCompleted(path, error) {
        ...
        result := ComCall(13, this, "ptr", path, "int", error, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FeedItemCountChanged(path, itemCountType) {
        ...
        result := ComCall(14, this, "ptr", path, "int", itemCountType, "int")
        if(result != 0)
            throw OSError(result)
