...
class IFeedsManager extends IDispatch{
    ...
    IsSubscribed(feedUrl, subscribed) {
        ...
        result := ComCall(8, this, "ptr", feedUrl, "ptr", subscribed, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ExistsFeed(feedPath, exists) {
        ...
        result := ComCall(9, this, "ptr", feedPath, "ptr", exists, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFeed(feedPath, disp) {
        ...
        result := ComCall(10, this, "ptr", feedPath, "ptr", disp, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFeedByUrl(feedUrl, disp) {
        ...
        result := ComCall(11, this, "ptr", feedUrl, "ptr", disp, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ExistsFolder(folderPath, exists) {
        ...
        result := ComCall(12, this, "ptr", folderPath, "ptr", exists, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetFolder(folderPath, disp) {
        ...
        result := ComCall(13, this, "ptr", folderPath, "ptr", disp, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_DefaultInterval(minutes) {
        result := ComCall(18, this, "int*", minutes, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} itemCountLimit 
     * @returns {HRESULT} 
     */
    get_ItemCountLimit(itemCountLimit) {
        result := ComCall(22, this, "int*", itemCountLimit, "int")
        if(result != 0)
            throw OSError(result)
