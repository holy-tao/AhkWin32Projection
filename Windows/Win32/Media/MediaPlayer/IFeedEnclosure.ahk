...
class IFeedEnclosure extends IDispatch{
     ...
     */
    get_Length() {
        result := ComCall(9, this, "int*", &length := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    get_DownloadStatus() {
        result := ComCall(12, this, "int*", &status_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    get_LastDownloadError() {
        result := ComCall(13, this, "int*", &error := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    SetFile(downloadUrl, downloadFilePath, downloadMimeType, enclosureFilename) {
        if(downloadUrl is String) {
            pin := BSTR.Alloc(downloadUrl)
            downloadUrl := pin.Value
        }
        if(downloadFilePath is String) {
            pin := BSTR.Alloc(downloadFilePath)
            downloadFilePath := pin.Value
        }
        if(downloadMimeType is String) {
            pin := BSTR.Alloc(downloadMimeType)
            downloadMimeType := pin.Value
        }
        if(enclosureFilename is String) {
            pin := BSTR.Alloc(enclosureFilename)
            enclosureFilename := pin.Value
        }
