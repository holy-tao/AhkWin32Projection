...
class ISdoMachine extends IDispatch{
    ...
    GetDictionarySDO(ppDictionarySDO) {
        result := ComCall(8, this, "ptr", ppDictionarySDO, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetServiceSDO(eDataStore, bstrServiceName, ppServiceSDO) {
        ...
        result := ComCall(9, this, "int", eDataStore, "ptr", bstrServiceName, "ptr", ppServiceSDO, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetUserSDO(eDataStore, bstrUserName, ppUserSDO) {
        ...
        result := ComCall(10, this, "int", eDataStore, "ptr", bstrUserName, "ptr", ppUserSDO, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} eOSType 
     * @returns {HRESULT} 
     */
    GetOSType(eOSType) {
        result := ComCall(11, this, "int*", eOSType, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} eDomainType 
     * @returns {HRESULT} 
     */
    GetDomainType(eDomainType) {
        result := ComCall(12, this, "int*", eDomainType, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsDirectoryAvailable(boolDirectoryAvailable) {
        result := ComCall(13, this, "ptr", boolDirectoryAvailable, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetSDOSchema(ppSDOSchema) {
        result := ComCall(15, this, "ptr", ppSDOSchema, "int")
        if(result != 0)
            throw OSError(result)
