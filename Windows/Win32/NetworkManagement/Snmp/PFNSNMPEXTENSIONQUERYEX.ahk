 ...
 * @version v4.0.30319
 */
class PFNSNMPEXTENSIONQUERYEX extends IUnknown {
     ...
     */
    Invoke(nRequestType, nTransactionId, pVarBindList, pContextInfo, pErrorStatus, pErrorIndex) {
        pErrorStatusMarshal := pErrorStatus is VarRef ? "int*" : "ptr"
        pErrorIndexMarshal := pErrorIndex is VarRef ? "int*" : "ptr"

        result := ComCall(3, this, "uint", nRequestType, "uint", nTransactionId, "ptr", pVarBindList, "ptr", pContextInfo, pErrorStatusMarshal, pErrorStatus, pErrorIndexMarshal, pErrorIndex, "int")
        return result
    }
}
