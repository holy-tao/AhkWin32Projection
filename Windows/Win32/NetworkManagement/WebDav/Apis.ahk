 ...
 */
export DavAddConnection(ConnectionHandle, RemoteName, UserName, Password, ClientCert, CertSize) {
 ...
 * @since windows6.0.6000
 */
export DavDeleteConnection(ConnectionHandle) {
    result := DllCall("NETAPI32.dll\DavDeleteConnection", HANDLE, ConnectionHandle, UInt32)
    return result
}
 ...
 * @since windows6.0.6000
 */
export DavGetUNCFromHTTPPath(Url, UncPath, lpSize) {
 ...
 * @since windows6.0.6000
 */
export DavGetHTTPFromUNCPath(UncPath, Url, lpSize) {
    UncPath := UncPath is String ? StrPtr(UncPath) : UncPath
    Url := Url is String ? StrPtr(Url) : Url
 ...
 * @since windows6.0.6000
 */
export DavFlushFile(hFile) {
    result := DllCall("NETAPI32.dll\DavFlushFile", HANDLE, hFile, UInt32)
    return result
}
 ...
 * @since windows6.0.6000
 */
export DavInvalidateCache(URLName) {
    URLName := URLName is String ? StrPtr(URLName) : URLName

    result := DllCall("davclnt.dll\DavInvalidateCache", "ptr", URLName, UInt32)
    return result
}
 ...
 * @since windows6.0.6000
 */
export DavCancelConnectionsToServer(lpName, fForce) {
    lpName := lpName is String ? StrPtr(lpName) : lpName

    result := DllCall("davclnt.dll\DavCancelConnectionsToServer", "ptr", lpName, BOOL, fForce, UInt32)
    return result
}
