...
export default struct DOT11EXTIHV_PERFORM_CAPABILITY_MATCH {
    ...
    Call(hIhvExtAdapter, pIhvProfileParams, pIhvConnProfile, pIhvSecProfile, pConnectableBssid, pdwReasonCode) {
        ...
        pConnectableBssidMarshal := pConnectableBssid == 0 ? IntPtr : DOT11_BSS_LIST.Ptr
        pdwReasonCodeMarshal := pdwReasonCode is VarRef ? "uint*" : IntPtr

        result := DllCall(this.value, hIhvExtAdapterMarshal, hIhvExtAdapter, pIhvProfileParamsMarshal, pIhvProfileParams, DOT11EXT_IHV_CONNECTIVITY_PROFILE.Ptr, pIhvConnProfile, DOT11EXT_IHV_SECURITY_PROFILE.Ptr, pIhvSecProfile, pConnectableBssidMarshal, pConnectableBssid, pdwReasonCodeMarshal, pdwReasonCode, UInt32)
        return result
    }
