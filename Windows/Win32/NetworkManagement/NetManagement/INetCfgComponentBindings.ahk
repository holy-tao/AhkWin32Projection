...
class INetCfgComponentBindings extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<INetCfgComponent>} pnccItem 
     * @returns {HRESULT} 
     */
    BindTo(pnccItem) {
    ...
    EnumBindingPaths(dwFlags, ppIEnum) {
        result := ComCall(8, this, "uint", dwFlags, "ptr", ppIEnum, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<INetCfgBindingPath>} pncbItemSrc 
     * @param {Pointer<INetCfgBindingPath>} pncbItemDest 
     * @returns {HRESULT} 
     */
