...
class INetCfg extends IUnknown{
    ...
    EnumComponents(pguidClass, ppenumComponent) {
        result := ComCall(7, this, "ptr", pguidClass, "ptr", ppenumComponent, "int")
        if(result != 0)
            throw OSError(result)
    ...
    FindComponent(pszwInfId, pComponent) {
        ...
        result := ComCall(8, this, "ptr", pszwInfId, "ptr", pComponent, "int")
        if(result != 0)
            throw OSError(result)
    ...
    QueryNetCfgClass(pguidClass, riid, ppvObject) {
        result := ComCall(9, this, "ptr", pguidClass, "ptr", riid, "ptr", ppvObject, "int")
        if(result != 0)
            throw OSError(result)
