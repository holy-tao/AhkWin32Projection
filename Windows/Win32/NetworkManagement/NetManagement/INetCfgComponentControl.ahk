...
class INetCfgComponentControl extends IUnknown{
     ...
     * @param {Pointer<INetCfg>} pINetCfg 
    ...
    Initialize(pIComp, pINetCfg, fInstalling) {
        result := ComCall(3, this, "ptr", pIComp, "ptr", pINetCfg, "int", fInstalling, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<INetCfgPnpReconfigCallback>} pICallback 
     * @returns {HRESULT} 
     */
    ApplyPnpChanges(pICallback) {
        result := ComCall(5, this, "ptr", pICallback, "int")
        if(result != 0)
            throw OSError(result)
