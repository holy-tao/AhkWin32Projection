...
class INetCfgBindingPath extends IUnknown{
     ...
     */
    GetPathToken() {
        result := ComCall(7, this, "ptr*", &ppszwPathToken := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetOwner() {
        result := ComCall(8, this, "ptr*", &ppComponent := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetDepth() {
        result := ComCall(9, this, "uint*", &pcInterfaces := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    EnumBindingInterfaces() {
        result := ComCall(10, this, "ptr*", &ppenumInterface := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
