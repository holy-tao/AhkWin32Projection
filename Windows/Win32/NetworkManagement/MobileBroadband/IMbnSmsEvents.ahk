...
class IMbnSmsEvents extends IUnknown{
    ...
    OnSetSmsConfigurationComplete(sms, requestID, status) {
        result := ComCall(4, this, "ptr", sms, "uint", requestID, "int", status, "int")
        if(result != 0)
            throw OSError(result)
    ...
    OnSmsSendComplete(sms, requestID, status) {
        result := ComCall(5, this, "ptr", sms, "uint", requestID, "int", status, "int")
        if(result != 0)
            throw OSError(result)
    ...
    OnSmsReadComplete(sms, smsFormat, readMsgs, moreMsgs, requestID, status) {
        result := ComCall(6, this, "ptr", sms, "int", smsFormat, "ptr", readMsgs, "short", moreMsgs, "uint", requestID, "int", status, "int")
        if(result != 0)
            throw OSError(result)
    ...
    OnSmsDeleteComplete(sms, requestID, status) {
        result := ComCall(8, this, "ptr", sms, "uint", requestID, "int", status, "int")
        if(result != 0)
            throw OSError(result)
