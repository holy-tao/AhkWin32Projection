 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/mbnapi/nn-mbnapi-imbndeviceservicesmanager
...
class IMbnDeviceServicesManager extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/mbnapi/nf-mbnapi-imbndeviceservicesmanager-getdeviceservicescontext
     */
    GetDeviceServicesContext(networkInterfaceID) {
        if(networkInterfaceID is String) {
            pin := BSTR.Alloc(networkInterfaceID)
            networkInterfaceID := pin.Value
        }

        result := ComCall(3, this, "ptr", networkInterfaceID, "ptr*", &mbnDevicesContext := 0, "int")
        if(result != 0) {
