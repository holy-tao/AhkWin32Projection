...
class IMbnConnectionEvents extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<IMbnConnection>} newConnection 
     * @param {Integer} requestID 
     * @param {HRESULT} status 
     * @returns {HRESULT} 
     */
    OnConnectComplete(newConnection, requestID, status) {
        result := ComCall(3, this, "ptr", newConnection, "uint", requestID, "int", status, "int")
        if(result != 0)
    ...
    OnDisconnectComplete(newConnection, requestID, status) {
        result := ComCall(4, this, "ptr", newConnection, "uint", requestID, "int", status, "int")
        if(result != 0)
            throw OSError(result)
