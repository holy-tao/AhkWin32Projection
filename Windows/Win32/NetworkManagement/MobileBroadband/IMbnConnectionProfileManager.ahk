...
class IMbnConnectionProfileManager extends IUnknown{
     ...
     * @param {Pointer<SAFEARRAY>} connectionProfiles 
     * @returns {HRESULT} 
     */
    GetConnectionProfiles(mbnInterface, connectionProfiles) {
        result := ComCall(3, this, "ptr", mbnInterface, "ptr", connectionProfiles, "int")
        if(result != 0)
    ...
    GetConnectionProfile(mbnInterface, profileName, connectionProfile) {
        ...
        result := ComCall(4, this, "ptr", mbnInterface, "ptr", profileName, "ptr", connectionProfile, "int")
        if(result != 0)
            throw OSError(result)
