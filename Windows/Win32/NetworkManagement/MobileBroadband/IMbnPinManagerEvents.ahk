 ...
 */
export default struct IMbnPinManagerEvents extends IUnknown {
    /**
     ...
     * used for IMbnPinManagerEvents interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        OnPinListAvailable    : IntPtr
        OnGetPinStateComplete : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMbnPinManagerEvents.Vtbl()
        }
    ...
    Query(iid) {
        if (IMbnPinManagerEvents.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.OnPinListAvailable)
        CallbackFree(this.vtbl.OnGetPinStateComplete)
    }
}
