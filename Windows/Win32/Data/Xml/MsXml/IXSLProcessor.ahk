...
class IXSLProcessor extends IDispatch{
    ...
    get_ownerTemplate(ppTemplate) {
        result := ComCall(9, this, "ptr", ppTemplate, "int")
        if(result != 0)
            throw OSError(result)
    ...
    setStartMode(mode, namespaceURI) {
        ...
        result := ComCall(10, this, "ptr", mode, "ptr", namespaceURI, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_startMode(mode) {
        result := ComCall(11, this, "ptr", mode, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    transform(pDone) {
        result := ComCall(15, this, "ptr", pDone, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} pReadyState 
     * @returns {HRESULT} 
     */
    get_readyState(pReadyState) {
        result := ComCall(17, this, "int*", pReadyState, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_stylesheet(stylesheet) {
        result := ComCall(20, this, "ptr", stylesheet, "int")
        if(result != 0)
            throw OSError(result)
