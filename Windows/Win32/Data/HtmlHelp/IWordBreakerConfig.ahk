...
class IWordBreakerConfig extends IUnknown{
    ...
    GetBreakWordType(pdwBreakWordType) {
        ...
        result := ComCall(6, this, pdwBreakWordTypeMarshal, pdwBreakWordType, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetControlInfo(pgrfBreakFlags, pdwReserved) {
        ...
        result := ComCall(8, this, pgrfBreakFlagsMarshal, pgrfBreakFlags, pdwReservedMarshal, pdwReserved, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetWordStemmer() {
        result := ComCall(11, this, "ptr*", &ppStemmer := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
