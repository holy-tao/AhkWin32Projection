 ...
 */
export IsDestinationReachableA(lpszDestination, lpQOCInfo) {
    lpszDestination := lpszDestination is String ? StrPtr(lpszDestination) : lpszDestination

    A_LastError := 0

    result := DllCall("SensApi.dll\IsDestinationReachableA", "ptr", lpszDestination, QOCINFO.Ptr, lpQOCInfo, BOOL)
 ...
 * @since windows5.1.2600
 */
export IsDestinationReachableW(lpszDestination, lpQOCInfo) {
    lpszDestination := lpszDestination is String ? StrPtr(lpszDestination) : lpszDestination

    A_LastError := 0

    result := DllCall("SensApi.dll\IsDestinationReachableW", "ptr", lpszDestination, QOCINFO.Ptr, lpQOCInfo, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export IsNetworkAlive(lpdwFlags) {
    ...
    result := DllCall("SensApi.dll\IsNetworkAlive", lpdwFlagsMarshal, lpdwFlags, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
