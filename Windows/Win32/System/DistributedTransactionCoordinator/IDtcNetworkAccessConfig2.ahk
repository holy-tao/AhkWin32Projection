...
class IDtcNetworkAccessConfig2 extends IDtcNetworkAccessConfig{
    ...
    GetNetworkInboundAccess(pbInbound) {
        result := ComCall(16, this, "ptr", pbInbound, "int")
        if(result != 0)
    ...
    GetNetworkOutboundAccess(pbOutbound) {
        result := ComCall(17, this, "ptr", pbOutbound, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} pAuthLevel 
     * @returns {HRESULT} 
     */
    GetAuthenticationLevel(pAuthLevel) {
        result := ComCall(20, this, "int*", pAuthLevel, "int")
        if(result != 0)
            throw OSError(result)
