 ...
 */
export default struct IDtcLuRmEnlistmentFactory extends IUnknown {
    /**
     ...
     * used for IDtcLuRmEnlistmentFactory interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDtcLuRmEnlistmentFactory.Vtbl()
        }
    ...
    Query(iid) {
        if (IDtcLuRmEnlistmentFactory.IID.Equals(iid)) {
            return true
        }
