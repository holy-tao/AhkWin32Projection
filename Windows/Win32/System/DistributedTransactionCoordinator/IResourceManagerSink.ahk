 ...
 */
export default struct IResourceManagerSink extends IUnknown {
    /**
     ...
     * used for IResourceManagerSink interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IResourceManagerSink.Vtbl()
        }
    ...
    Query(iid) {
        if (IResourceManagerSink.IID.Equals(iid)) {
            return true
        }
