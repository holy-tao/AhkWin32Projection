...
class ITransactionReceiver extends IUnknown{
    ...
    UnmarshalPropagationToken(cbToken, rgbToken) {
        ...
        result := ComCall(3, this, "uint", cbToken, rgbTokenMarshal, rgbToken, "ptr*", &ppTransaction := 0, "int")
        if(result != 0) {
     ...
     */
    GetReturnTokenSize() {
        result := ComCall(4, this, "uint*", &pcbReturnToken := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    MarshalReturnToken(cbReturnToken, rgbReturnToken, pcbUsed) {
        ...
        result := ComCall(5, this, "uint", cbReturnToken, rgbReturnTokenMarshal, rgbReturnToken, pcbUsedMarshal, pcbUsed, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
