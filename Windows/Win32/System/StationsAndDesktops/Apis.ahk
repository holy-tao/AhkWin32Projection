 ...
 * @since windows5.0
 */
export CreateDesktopA(lpszDesktop, dwFlags, dwDesiredAccess, lpsa) {
 ...
 * @since windows5.0
 */
export CreateDesktopW(lpszDesktop, dwFlags, dwDesiredAccess, lpsa) {
    static lpszDevice := 0, pDevmode := 0 ;Reserved parameters must always be NULL
 ...
 */
export CreateDesktopExA(lpszDesktop, dwFlags, dwDesiredAccess, lpsa, ulHeapSize) {
 ...
 * @since windows6.0.6000
 */
export CreateDesktopExW(lpszDesktop, dwFlags, dwDesiredAccess, lpsa, ulHeapSize) {
    static lpszDevice := 0, pDevmode := 0, pvoid := 0 ;Reserved parameters must always be NULL
 ...
 * @since windows5.0
 */
export OpenDesktopA(lpszDesktop, dwFlags, fInherit, dwDesiredAccess) {
    lpszDesktop := lpszDesktop is String ? StrPtr(lpszDesktop) : lpszDesktop
 ...
 * @since windows5.0
 */
export OpenDesktopW(lpszDesktop, dwFlags, fInherit, dwDesiredAccess) {
    lpszDesktop := lpszDesktop is String ? StrPtr(lpszDesktop) : lpszDesktop
 ...
 * @since windows5.0
 */
export OpenInputDesktop(dwFlags, fInherit, dwDesiredAccess) {
    A_LastError := 0

    result := DllCall("USER32.dll\OpenInputDesktop", DESKTOP_CONTROL_FLAGS, dwFlags, BOOL, fInherit, DESKTOP_ACCESS_FLAGS, dwDesiredAccess, HDESK.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export SwitchDesktop(hDesktop) {
    A_LastError := 0

    result := DllCall("USER32.dll\SwitchDesktop", HDESK, hDesktop, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export SetThreadDesktop(hDesktop) {
    A_LastError := 0

    result := DllCall("USER32.dll\SetThreadDesktop", HDESK, hDesktop, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export CloseDesktop(hDesktop) {
    A_LastError := 0

    result := DllCall("USER32.dll\CloseDesktop", HDESK, hDesktop, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export OpenWindowStationA(lpszWinSta, fInherit, dwDesiredAccess) {
 ...
 * @since windows5.0
 */
export OpenWindowStationW(lpszWinSta, fInherit, dwDesiredAccess) {
    lpszWinSta := lpszWinSta is String ? StrPtr(lpszWinSta) : lpszWinSta
 ...
 * @since windows5.0
 */
export CloseWindowStation(_hWinSta) {
    A_LastError := 0

    result := DllCall("USER32.dll\CloseWindowStation", HWINSTA, _hWinSta, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export SetProcessWindowStation(_hWinSta) {
    A_LastError := 0

    result := DllCall("USER32.dll\SetProcessWindowStation", HWINSTA, _hWinSta, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetProcessWindowStation() {
    ...
    result := DllCall("USER32.dll\GetProcessWindowStation", HWINSTA.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
