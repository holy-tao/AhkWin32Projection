...
class IPerformanceCounterDataCollector extends IDataCollector{
    ...
    get_PerformanceCounters(counters) {
        result := ComCall(34, this, "ptr", counters, "int")
        if(result != 0)
            throw OSError(result)
    ...
    put_PerformanceCounters(counters) {
        result := ComCall(35, this, "ptr", counters, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_SampleInterval(interval) {
        result := ComCall(38, this, "uint*", interval, "int")
        if(result != 0)
            throw OSError(result)
    ...
    put_SampleInterval(interval) {
        result := ComCall(39, this, "uint", interval, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} records 
     * @returns {HRESULT} 
     */
    get_SegmentMaxRecords(records) {
        result := ComCall(40, this, "uint*", records, "int")
        if(result != 0)
            throw OSError(result)
