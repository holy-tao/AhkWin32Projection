...
class ISWbemQualifier extends IDispatch{
    ...
    get_PropagatesToSubclass(bPropagatesToSubclass) {
        result := ComCall(11, this, "ptr", bPropagatesToSubclass, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_PropagatesToInstance(bPropagatesToInstance) {
        result := ComCall(13, this, "ptr", bPropagatesToInstance, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_IsOverridable(bIsOverridable) {
        result := ComCall(15, this, "ptr", bIsOverridable, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_IsAmended(bIsAmended) {
        result := ComCall(17, this, "ptr", bIsAmended, "int")
        if(result != 0)
            throw OSError(result)
