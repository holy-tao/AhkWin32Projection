...
class ISWbemMethod extends IDispatch{
    ...
    get_InParameters(objWbemInParameters) {
        result := ComCall(9, this, "ptr", objWbemInParameters, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_OutParameters(objWbemOutParameters) {
        result := ComCall(10, this, "ptr", objWbemOutParameters, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_Qualifiers_(objWbemQualifierSet) {
        result := ComCall(11, this, "ptr", objWbemQualifierSet, "int")
        if(result != 0)
            throw OSError(result)
