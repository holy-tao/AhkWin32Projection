 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/wbemcli/nn-wbemcli-iwbemobjectsink
...
class IWbemObjectSink extends IUnknown{
    ...
    Indicate(lObjectCount, apObjArray) {
        result := ComCall(3, this, "int", lObjectCount, "ptr*", apObjArray, "int")
        if(result != 0) {
     ...
     * @param {HRESULT} hResult_ This parameter is set to the <b>HRESULT</b> of the asynchronous operation or notification. This is either an error code, if an error occurred, or the amount of progress that has been made on an asynchronous call.
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemcli/nf-wbemcli-iwbemobjectsink-setstatus
     */
    SetStatus(lFlags, hResult_, strParam, pObjParam) {
        if(strParam is String) {
            pin := BSTR.Alloc(strParam)
            strParam := pin.Value
        }

        result := ComCall(4, this, "int", lFlags, "int", hResult_, "ptr", strParam, "ptr", pObjParam, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
