 ...
 */
export default struct MI_ProviderFT_Unsubscribe {
    value : IntPtr

    __value {
        set {
            if (value is MI_ProviderFT_Unsubscribe) {
     ...
     * @param {Pointer<Void>} subscriptionSelf 
    ...
    /**
     * A MI_ProviderFT_Unsubscribe that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends MI_ProviderFT_Unsubscribe {
        /**
         * Creates a MI_ProviderFT_Unsubscribe pointer that invokes the given AHK function when called.
         * @param {Func("ptr", MI_Context, "ushort*", "ushort*", Int64, "ptr") => IntPtr} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 6)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 6 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr", MI_Context.Ptr, "ushort*", "ushort*", Int64, "ptr", IntPtr])
        }
