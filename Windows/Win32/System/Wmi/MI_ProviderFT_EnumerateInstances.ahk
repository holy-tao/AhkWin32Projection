 ...
 */
export default struct MI_ProviderFT_EnumerateInstances {
    value : IntPtr

    __value {
        set {
            if (value is MI_ProviderFT_EnumerateInstances) {
    ...
    /**
     * A MI_ProviderFT_EnumerateInstances that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends MI_ProviderFT_EnumerateInstances {
        /**
         * Creates a MI_ProviderFT_EnumerateInstances pointer that invokes the given AHK function when called.
         * @param {Func("ptr", MI_Context, "ushort*", "ushort*", MI_PropertySet, Int8, MI_Filter) => IntPtr} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 7)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 7 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr", MI_Context.Ptr, "ushort*", "ushort*", MI_PropertySet.Ptr, Int8, MI_Filter.Ptr, IntPtr])
        }
