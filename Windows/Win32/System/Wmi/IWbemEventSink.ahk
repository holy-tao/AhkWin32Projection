 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nn-wbemprov-iwbemeventsink
...
class IWbemEventSink extends IWbemObjectSink{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbemeventsink-setsinksecurity
     */
    SetSinkSecurity(lSDLength, pSD) {
        pSDMarshal := pSD is VarRef ? "char*" : "ptr"

        result := ComCall(5, this, "int", lSDLength, pSDMarshal, pSD, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} This method returns an <b>HRESULT</b> indicating the status of the method call. The following list lists the value contained within an <b>HRESULT</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbemeventsink-isactive
     */
    IsActive() {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbemeventsink-getrestrictedsink
     */
    GetRestrictedSink(lNumQueries, awszQueries, pCallback) {
        awszQueriesMarshal := awszQueries is VarRef ? "ptr*" : "ptr"

        result := ComCall(7, this, "int", lNumQueries, awszQueriesMarshal, awszQueries, "ptr", pCallback, "ptr*", &ppSink := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} This method returns an <b>HRESULT</b> indicating the status of the method call. The following list lists the value contained within an <b>HRESULT</b>.
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbemeventsink-setbatchingparameters
     */
    SetBatchingParameters(lFlags, dwMaxBufferSize, dwMaxSendLatency) {
        result := ComCall(8, this, "int", lFlags, "uint", dwMaxBufferSize, "uint", dwMaxSendLatency, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
