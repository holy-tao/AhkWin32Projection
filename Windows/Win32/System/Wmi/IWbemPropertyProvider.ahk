 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nn-wbemprov-iwbempropertyprovider
...
class IWbemPropertyProvider extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbempropertyprovider-getproperty
     */
    GetProperty(lFlags, strLocale, strClassMapping, strInstMapping, strPropMapping) {
        ...
        if(strClassMapping is String) {
            pin := BSTR.Alloc(strClassMapping)
            strClassMapping := pin.Value
        }
        if(strInstMapping is String) {
            pin := BSTR.Alloc(strInstMapping)
            strInstMapping := pin.Value
        }
        if(strPropMapping is String) {
            pin := BSTR.Alloc(strPropMapping)
            strPropMapping := pin.Value
        }

        pvValue := VARIANT()
        result := ComCall(3, this, "int", lFlags, "ptr", strLocale, "ptr", strClassMapping, "ptr", strInstMapping, "ptr", strPropMapping, "ptr", pvValue, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wbemprov/nf-wbemprov-iwbempropertyprovider-putproperty
     */
    PutProperty(lFlags, strLocale, strClassMapping, strInstMapping, strPropMapping, pvValue) {
        ...
        result := ComCall(4, this, "int", lFlags, "ptr", strLocale, "ptr", strClassMapping, "ptr", strInstMapping, "ptr", strPropMapping, "ptr", pvValue, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
