 ...
 * @see https://learn.microsoft.com/windows/win32/api/msi/nc-msi-installui_handlerw
 * @namespace Windows.Win32.System.ApplicationInstallationAndServicing
 * @charset Unicode
 */
export default struct INSTALLUI_HANDLERW {
    value : IntPtr

    __value {
        set {
            if (value is INSTALLUI_HANDLERW) {
     ...
     * @param {PWSTR} szMessage Specifies the message text.
     * @returns {Integer} The following return values map to the buttons specified by the message box style:
     * 
    ...
    /**
     * A INSTALLUI_HANDLERW that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends INSTALLUI_HANDLERW {
        /**
         * Creates a INSTALLUI_HANDLERW pointer that invokes the given AHK function when called.
         * @param {Func("ptr", UInt32, PWSTR) => Int32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 3)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 3 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr", UInt32, PWSTR, Int32])
        }
