 ...
 */
export default struct IPMExtensionShareTargetInfo extends IUnknown {
    /**
     ...
     * used for IPMExtensionShareTargetInfo interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        get_AllFileTypes         : IntPtr
        get_AllDataFormats       : IntPtr
        get_SupportsAllFileTypes : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPMExtensionShareTargetInfo.Vtbl()
        }
    ...
    get_SupportsAllFileTypes() {
        result := ComCall(5, this, BOOL.Ptr, &pSupportsAllTypes := 0, "HRESULT")
        return pSupportsAllTypes
    }

    Query(iid) {
        if (IPMExtensionShareTargetInfo.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.get_AllDataFormats)
