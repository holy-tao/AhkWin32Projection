...
class IEnumMsmString extends IUnknown{
    ...
    Next(cFetch, rgbstrStrings, pcFetched) {
        ...
        result := ComCall(3, this, "uint", cFetch, "ptr", rgbstrStrings, pcFetchedMarshal, pcFetched, "int")
        if(result != 0) {
     ...
     */
    Clone() {
        result := ComCall(6, this, "ptr*", &pemsmStrings := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
