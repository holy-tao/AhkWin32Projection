 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/rometadataapi/nn-rometadataapi-imetadatadispenser
...
class IMetaDataDispenser extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/rometadataapi/nf-rometadataapi-imetadatadispenser-definescope
     */
    DefineScope(rclsid, dwCreateFlags, riid) {
        result := ComCall(3, this, "ptr", rclsid, "uint", dwCreateFlags, "ptr", riid, "ptr*", &ppIUnk := 0, "int")
        if(result != 0) {
     ...
     * @returns {IUnknown} The pointer to the returned interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/rometadataapi/nf-rometadataapi-imetadatadispenser-openscope
     */
    OpenScope(szScope, dwOpenFlags, riid) {
        szScope := szScope is String ? StrPtr(szScope) : szScope

        result := ComCall(4, this, "ptr", szScope, "uint", dwOpenFlags, "ptr", riid, "ptr*", &ppIUnk := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {IUnknown} The pointer to the returned interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/rometadataapi/nf-rometadataapi-imetadatadispenser-openscopeonmemory
     */
    OpenScopeOnMemory(pData, cbData, dwOpenFlags, riid) {
        pDataMarshal := pData is VarRef ? "ptr" : "ptr"

        result := ComCall(5, this, pDataMarshal, pData, "uint", cbData, "uint", dwOpenFlags, "ptr", riid, "ptr*", &ppIUnk := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
