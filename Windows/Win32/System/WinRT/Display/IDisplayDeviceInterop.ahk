...
class IDisplayDeviceInterop extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/windows.devices.display.core.interop/nf-windows-devices-display-core-interop-idisplaydeviceinterop-opensharedhandle
     */
    OpenSharedHandle(NTHandle, riid) {
        NTHandle := NTHandle is Win32Handle ? NumGet(NTHandle, "ptr") : NTHandle

        result := ComCall(4, this, "ptr", NTHandle, "ptr", riid, "ptr*", &ppvObj := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
