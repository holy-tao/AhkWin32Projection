...
#Import ".\TrustLevel.ahk" { TrustLevel }
 ...
 */
export default struct IInspectable extends IUnknown {
    /**
     ...
     * used for IInspectable interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetIids             : IntPtr
        GetRuntimeClassName : IntPtr
        GetTrustLevel       : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IInspectable.Vtbl()
        }
     ...
     */
    GetRuntimeClassName() {
        className := HSTRING.Owned()
        result := ComCall(4, this, HSTRING.Ptr, className, "HRESULT")
        return className
    }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetIids)
        CallbackFree(this.vtbl.GetRuntimeClassName)
        CallbackFree(this.vtbl.GetTrustLevel)
    }
}
