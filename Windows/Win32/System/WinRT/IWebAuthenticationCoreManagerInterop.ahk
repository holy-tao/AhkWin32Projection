...
class IWebAuthenticationCoreManagerInterop extends IInspectable{
     ...
     * @param {Pointer<IInspectable>} request 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} asyncInfo 
     * @returns {HRESULT} 
     */
    RequestTokenForWindowAsync(appWindow, request, riid, asyncInfo) {
        appWindow := appWindow is Win32Handle ? NumGet(appWindow, "ptr") : appWindow

        result := ComCall(6, this, "ptr", appWindow, "ptr", request, "ptr", riid, "ptr", asyncInfo, "int")
        if(result != 0)
    ...
    RequestTokenWithWebAccountForWindowAsync(appWindow, request, webAccount, riid, asyncInfo) {
        ...
        result := ComCall(7, this, "ptr", appWindow, "ptr", request, "ptr", webAccount, "ptr", riid, "ptr", asyncInfo, "int")
        if(result != 0)
            throw OSError(result)
