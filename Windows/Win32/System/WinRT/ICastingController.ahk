...
class ICastingController extends IUnknown{
     ...
     * @param {Pointer<IUnknown>} castingEngine 
     * @param {Pointer<IUnknown>} castingSource 
     * @returns {HRESULT} <ul>
    ...
    Initialize(castingEngine, castingSource) {
        result := ComCall(3, this, "ptr", castingEngine, "ptr", castingSource, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<ICastingEventHandler>} eventHandler 
     * @param {Pointer<UInt32>} cookie 
     * @returns {HRESULT} 
     */
    Advise(eventHandler, cookie) {
        result := ComCall(6, this, "ptr", eventHandler, "uint*", cookie, "int")
        if(result != 0)
            throw OSError(result)
