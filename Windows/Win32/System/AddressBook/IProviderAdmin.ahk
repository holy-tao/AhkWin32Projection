...
class IProviderAdmin extends IUnknown{
    ...
    GetProviderTable(ulFlags, lppTable) {
        result := ComCall(4, this, "uint", ulFlags, "ptr", lppTable, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateProvider(lpszProvider, cValues, lpProps, ulUIParam, ulFlags, lpUID) {
        result := ComCall(5, this, "char*", lpszProvider, "uint", cValues, "ptr", lpProps, "ptr", ulUIParam, "uint", ulFlags, "ptr", lpUID, "int")
        if(result != 0)
            throw OSError(result)
    ...
    OpenProfileSection(lpUID, lpInterface, ulFlags, lppProfSect) {
        result := ComCall(7, this, "ptr", lpUID, "ptr", lpInterface, "uint", ulFlags, "ptr", lppProfSect, "int")
        if(result != 0)
            throw OSError(result)
