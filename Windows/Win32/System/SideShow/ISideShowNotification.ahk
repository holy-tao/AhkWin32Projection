...
class ISideShowNotification extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<UInt32>} out_pNotificationId 
     * @returns {HRESULT} 
     */
    get_NotificationId(out_pNotificationId) {
        result := ComCall(3, this, "uint*", out_pNotificationId, "int")
        if(result != 0)
    ...
    get_Title(out_ppwszTitle) {
        result := ComCall(5, this, "ptr", out_ppwszTitle, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_Message(out_ppwszMessage) {
        result := ComCall(7, this, "ptr", out_ppwszMessage, "int")
        if(result != 0)
            throw OSError(result)
