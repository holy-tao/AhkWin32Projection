 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/wdstptmgmt/nn-wdstptmgmt-iwdstransportmanager
...
class IWdsTransportManager extends IDispatch{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/wdstptmgmt/nf-wdstptmgmt-iwdstransportmanager-getwdstransportserver
     */
    GetWdsTransportServer(bszServerName) {
        if(bszServerName is String) {
            pin := BSTR.Alloc(bszServerName)
            bszServerName := pin.Value
        }

        result := ComCall(7, this, "ptr", bszServerName, "ptr*", &ppWdsTransportServer := 0, "int")
        if(result != 0) {
