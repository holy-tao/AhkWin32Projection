...
class ICorProfilerInfo12 extends ICorProfilerInfo11{
     ...
     * @param {Pointer<UInt64>} pSession 
     * @returns {HRESULT} 
     */
    EventPipeStartSession(cProviderConfigs, pProviderConfigs, requestRundown, pSession) {
        result := ComCall(101, this, "uint", cProviderConfigs, "ptr", pProviderConfigs, "int", requestRundown, "uint*", pSession, "int")
        if(result != 0)
    ...
    EventPipeCreateProvider(providerName, pProvider) {
        ...
        result := ComCall(104, this, "ptr", providerName, "ptr*", pProvider, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EventPipeGetProviderInfo(provider, cchName, pcchName, providerName) {
        ...
        result := ComCall(105, this, "ptr", provider, "uint", cchName, "uint*", pcchName, "ptr", providerName, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EventPipeDefineEvent(provider, eventName, eventID, keywords, eventVersion, level, opcode, needStack, cParamDescs, pParamDescs, pEvent) {
        ...
        result := ComCall(106, this, "ptr", provider, "ptr", eventName, "uint", eventID, "uint", keywords, "uint", eventVersion, "uint", level, "char", opcode, "int", needStack, "uint", cParamDescs, "ptr", pParamDescs, "ptr*", pEvent, "int")
        if(result != 0)
            throw OSError(result)
