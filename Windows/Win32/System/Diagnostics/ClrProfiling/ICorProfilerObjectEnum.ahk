 ...
 */
export default struct ICorProfilerObjectEnum extends IUnknown {
    /**
     ...
     * used for ICorProfilerObjectEnum interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ICorProfilerObjectEnum.Vtbl()
        }
    ...
    Next(celt, objects, pceltFetched) {
        objectsMarshal := objects is VarRef ? "ptr*" : IntPtr
        pceltFetchedMarshal := pceltFetched is VarRef ? "uint*" : IntPtr

        result := ComCall(7, this, UInt32, celt, objectsMarshal, objects, pceltFetchedMarshal, pceltFetched, "HRESULT")
        return result
    }

    _Query(iid) {
        if (ICorProfilerObjectEnum.IID.Equals(iid)) {
            return true
        }
