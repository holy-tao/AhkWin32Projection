...
class ICorProfilerCallback extends IUnknown{
     ...
     * @param {Pointer<IUnknown>} pICorProfilerInfoUnk 
     * @returns {HRESULT} <ul>
    ...
    JITCachedFunctionSearchStarted(functionId, pbUseCachedFunction) {
        result := ComCall(25, this, "ptr", functionId, "ptr", pbUseCachedFunction, "int")
        if(result != 0)
            throw OSError(result)
    ...
    JITCachedFunctionSearchFinished(functionId, result) {
        result := ComCall(26, this, "ptr", functionId, "int", result, "int")
        if(result != 0)
            throw OSError(result)
    ...
    JITInlining(callerId, calleeId, pfShouldInline) {
        result := ComCall(28, this, "ptr", callerId, "ptr", calleeId, "ptr", pfShouldInline, "int")
        if(result != 0)
            throw OSError(result)
    ...
    MovedReferences(cMovedObjectIDRanges, oldObjectIDRangeStart, newObjectIDRangeStart, cObjectIDRangeLength) {
        result := ComCall(49, this, "uint", cMovedObjectIDRanges, "ptr*", oldObjectIDRangeStart, "ptr*", newObjectIDRangeStart, "uint*", cObjectIDRangeLength, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ObjectAllocated(objectId, classId) {
        result := ComCall(50, this, "ptr", objectId, "ptr", classId, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UIntPtr>} classIds 
     * @param {Pointer<UInt32>} cObjects 
     * @returns {HRESULT} 
     */
    ObjectsAllocatedByClass(cClassCount, classIds, cObjects) {
        result := ComCall(51, this, "uint", cClassCount, "ptr*", classIds, "uint*", cObjects, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UIntPtr>} objectRefIds 
     * @returns {HRESULT} 
     */
    ObjectReferences(objectId, classId, cObjectRefs, objectRefIds) {
        result := ComCall(52, this, "ptr", objectId, "ptr", classId, "uint", cObjectRefs, "ptr*", objectRefIds, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RootReferences(cRootRefs, rootRefIds) {
        result := ComCall(53, this, "uint", cRootRefs, "ptr*", rootRefIds, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ExceptionCatcherEnter(functionId, objectId) {
        result := ComCall(66, this, "ptr", functionId, "ptr", objectId, "int")
        if(result != 0)
            throw OSError(result)
    ...
    COMClassicVTableCreated(wrappedClassId, implementedIID, pVTable, cSlots) {
        result := ComCall(68, this, "ptr", wrappedClassId, "ptr", implementedIID, "ptr", pVTable, "uint", cSlots, "int")
        if(result != 0)
            throw OSError(result)
    ...
    COMClassicVTableDestroyed(wrappedClassId, implementedIID, pVTable) {
        result := ComCall(69, this, "ptr", wrappedClassId, "ptr", implementedIID, "ptr", pVTable, "int")
        if(result != 0)
            throw OSError(result)
