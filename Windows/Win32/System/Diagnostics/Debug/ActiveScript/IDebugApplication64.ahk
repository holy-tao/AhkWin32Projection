...
class IDebugApplication64 extends IRemoteDebugApplication{
    ...
    DebugOutput(pstr_) {
        ...
        result := ComCall(16, this, "ptr", pstr_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    HandleBreakPoint(br) {
        result := ComCall(18, this, "int", br, "int*", &pbra := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetBreakFlags(pabf, pprdatSteppingThread) {
        ...
        result := ComCall(20, this, pabfMarshal, pabf, "ptr*", pprdatSteppingThread, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/processthreadsapi/nf-processthreadsapi-getcurrentthread
     */
    GetCurrentThread() {
        result := ComCall(21, this, "ptr*", &pat := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    CreateAsyncDebugOperation(psdo) {
        result := ComCall(22, this, "ptr", psdo, "ptr*", &ppado := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    AddStackFrameSniffer(pdsfs) {
        result := ComCall(23, this, "ptr", pdsfs, "uint*", &pdwCookie := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    CreateApplicationNode() {
        result := ComCall(27, this, "ptr*", &ppdanNew := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    HandleRuntimeError(pErrorDebug, pScriptSite, pbra, perra, pfCallOnScriptError) {
        ...
        result := ComCall(29, this, "ptr", pErrorDebug, "ptr", pScriptSite, pbraMarshal, pbra, perraMarshal, perra, pfCallOnScriptErrorMarshal, pfCallOnScriptError, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    AddGlobalExpressionContextProvider(pdsfs) {
        result := ComCall(32, this, "ptr", pdsfs, "uint*", &pdwCookie := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
