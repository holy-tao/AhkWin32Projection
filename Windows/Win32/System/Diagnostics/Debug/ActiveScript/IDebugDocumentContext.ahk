 ...
 */
export default struct IDebugDocumentContext extends IUnknown {
    /**
     ...
     * used for IDebugDocumentContext interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetDocument      : IntPtr
        EnumCodeContexts : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDebugDocumentContext.Vtbl()
        }
    ...
    Query(iid) {
        if (IDebugDocumentContext.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.EnumCodeContexts)
    }
}
