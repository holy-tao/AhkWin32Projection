...
class IDebugHelper extends IUnknown{
     ...
     * @param {Pointer<IDebugApplicationThread>} pdat 
    ...
    CreatePropertyBrowser(pvar, bstrName, pdat, ppdob) {
        ...
        result := ComCall(3, this, "ptr", pvar, "ptr", bstrName, "ptr", pdat, "ptr", ppdob, "int")
        if(result != 0)
     ...
     * @param {PWSTR} bstrName 
     * @param {Pointer<IDebugApplicationThread>} pdat 
     * @param {Pointer<IDebugFormatter>} pdf 
     * @param {Pointer<IDebugProperty>} ppdob 
     * @returns {HRESULT} 
     */
    CreatePropertyBrowserEx(pvar, bstrName, pdat, pdf, ppdob) {
        ...
        result := ComCall(4, this, "ptr", pvar, "ptr", bstrName, "ptr", pdat, "ptr", pdf, "ptr", ppdob, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CreateSimpleConnectionPoint(pdisp, ppscp) {
        result := ComCall(5, this, "ptr", pdisp, "ptr", ppscp, "int")
        if(result != 0)
            throw OSError(result)
