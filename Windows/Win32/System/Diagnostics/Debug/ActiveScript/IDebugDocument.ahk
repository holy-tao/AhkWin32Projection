 ...
 */
export default struct IDebugDocument extends IDebugDocumentInfo {
    /**
     ...
     * used for IDebugDocument interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDebugDocument.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (IDebugDocument.IID.Equals(iid)) {
            return true
        }
