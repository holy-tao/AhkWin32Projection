...
class IDebugApplicationThread11032 extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<UInt32>} puiThreadRequests 
     * @returns {HRESULT} 
     */
    GetActiveThreadRequestCount(puiThreadRequests) {
        result := ComCall(3, this, "uint*", puiThreadRequests, "int")
        if(result != 0)
    ...
    IsSuspendedForBreakPoint(pfIsSuspended) {
        result := ComCall(4, this, "ptr", pfIsSuspended, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsThreadCallable(pfIsCallable) {
        result := ComCall(5, this, "ptr", pfIsCallable, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IDebugThreadCall32>} pptc 
