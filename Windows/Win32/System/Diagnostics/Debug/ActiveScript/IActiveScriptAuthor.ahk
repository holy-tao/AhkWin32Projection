...
class IActiveScriptAuthor extends IUnknown{
    ...
    GetScriptTextAttributes(pszCode, cch, pszDelimiter, dwFlags, pattr) {
        ...
        result := ComCall(6, this, "ptr", pszCode, "uint", cch, "ptr", pszDelimiter, "uint", dwFlags, "ushort*", pattr, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetScriptletTextAttributes(pszCode, cch, pszDelimiter, dwFlags, pattr) {
        ...
        result := ComCall(7, this, "ptr", pszCode, "uint", cch, "ptr", pszDelimiter, "uint", dwFlags, "ushort*", pattr, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetRoot(ppsp) {
        result := ComCall(8, this, "ptr", ppsp, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pgrfasa 
     * @returns {HRESULT} 
     */
    GetLanguageFlags(pgrfasa) {
        result := ComCall(9, this, "uint*", pgrfasa, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetEventHandler(pdisp, pszItem, pszSubItem, pszEvent, ppse) {
        ...
        result := ComCall(10, this, "ptr", pdisp, "ptr", pszItem, "ptr", pszSubItem, "ptr", pszEvent, "ptr", ppse, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} pdwListTypesProvided 
     * @param {Pointer<UInt32>} pichListAnchorPosition 
     * @param {Pointer<UInt32>} pichFuncAnchorPosition 
     * @param {Pointer<Int32>} pmemid 
     * @param {Pointer<Int32>} piCurrentParameter 
    ...
    GetInfoFromContext(pszCode, cchCode, ichCurrentPosition, dwListTypesRequested, pdwListTypesProvided, pichListAnchorPosition, pichFuncAnchorPosition, pmemid, piCurrentParameter, ppunk) {
        ...
        result := ComCall(15, this, "ptr", pszCode, "uint", cchCode, "uint", ichCurrentPosition, "uint", dwListTypesRequested, "uint*", pdwListTypesProvided, "uint*", pichListAnchorPosition, "uint*", pichFuncAnchorPosition, "int*", pmemid, "int*", piCurrentParameter, "ptr", ppunk, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsCommitChar(ch, pfcommit) {
        result := ComCall(16, this, "char", ch, "ptr", pfcommit, "int")
        if(result != 0)
            throw OSError(result)
