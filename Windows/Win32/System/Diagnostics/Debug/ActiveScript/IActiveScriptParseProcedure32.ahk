 ...
 */
export default struct IActiveScriptParseProcedure32 extends IUnknown {
    /**
     ...
     * used for IActiveScriptParseProcedure32 interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IActiveScriptParseProcedure32.Vtbl()
        }
    ...
    ParseProcedureText(pstrCode, pstrFormalParams, pstrProcedureName, pstrItemName, punkContext, pstrDelimiter, dwSourceContextCookie, ulStartingLineNumber, dwFlags) {
        ...
        result := ComCall(3, this, "ptr", pstrCode, "ptr", pstrFormalParams, "ptr", pstrProcedureName, "ptr", pstrItemName, "ptr", punkContext, "ptr", pstrDelimiter, UInt32, dwSourceContextCookie, UInt32, ulStartingLineNumber, UInt32, dwFlags, "ptr*", &ppdisp := 0, "HRESULT")
        return IDispatch(ppdisp)
    }

    _Query(iid) {
        if (IActiveScriptParseProcedure32.IID.Equals(iid)) {
            return true
        }
    ...
    Implement(implObj, flags := "") {
        ...
        this.vtbl.ParseProcedureText := CallbackCreate(ObjBindMethod(implObj, "ParseProcedureText"), flags, 11)
    }
