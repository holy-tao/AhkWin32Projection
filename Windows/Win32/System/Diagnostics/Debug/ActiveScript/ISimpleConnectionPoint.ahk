...
class ISimpleConnectionPoint extends IUnknown{
     ...
     */
    GetEventCount() {
        result := ComCall(3, this, "uint*", &pulCount := 0, "int")
        if(result != 0) {
    ...
    DescribeEvents(iEvent, cEvents, prgid, prgbstr, pcEventsFetched) {
        ...
        result := ComCall(4, this, "uint", iEvent, "uint", cEvents, prgidMarshal, prgid, "ptr", prgbstr, pcEventsFetchedMarshal, pcEventsFetched, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    Advise(pdisp) {
        result := ComCall(5, this, "ptr", pdisp, "uint*", &pdwCookie := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
