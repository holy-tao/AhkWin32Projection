 ...
 */
export default struct IDebugStackFrameSniffer extends IUnknown {
    /**
     * The interface identifier for IDebugStackFrameSniffer
     * @type {Guid}
     */
    static IID := Guid("{51973c18-cb0c-11d0-b5c9-00a0244a0e7a}")
     ...
     * used for IDebugStackFrameSniffer interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDebugStackFrameSniffer.Vtbl()
        }
    ...
    Query(iid) {
        if (IDebugStackFrameSniffer.IID.Equals(iid)) {
            return true
        }
