...
class IDebugHostContextExtensibility extends IUnknown{
    ...
    ReadExtensionData(blobId, bufferSize, buffer) {
        result := ComCall(4, this, "uint", blobId, "uint", bufferSize, "ptr", buffer, "int")
        if(result != 0)
    ...
    CloneContextForModification(extensionHandle) {
        result := ComCall(5, this, "ptr", extensionHandle, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CloneContextWithModification(blobId, dataSize, data, clonedContext) {
        result := ComCall(6, this, "uint", blobId, "uint", dataSize, "ptr", data, "ptr", clonedContext, "int")
        if(result != 0)
            throw OSError(result)
