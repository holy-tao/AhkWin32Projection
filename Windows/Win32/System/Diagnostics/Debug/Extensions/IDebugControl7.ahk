...
class IDebugControl7 extends IUnknown{
    ...
    GetStackTraceEx(FrameOffset, StackOffset, InstructionOffset, Frames, FramesSize, FramesFilled) {
        ...
        result := ComCall(169, this, "uint", FrameOffset, "uint", StackOffset, "uint", InstructionOffset, "ptr", Frames, "uint", FramesSize, FramesFilledMarshal, FramesFilled, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetContextStackTraceEx(StartContext, StartContextSize, Frames, FramesSize, FrameContexts, FrameContextsSize, FrameContextsEntrySize, FramesFilled) {
        ...
        result := ComCall(171, this, "ptr", StartContext, "uint", StartContextSize, "ptr", Frames, "uint", FramesSize, "ptr", FrameContexts, "uint", FrameContextsSize, "uint", FrameContextsEntrySize, FramesFilledMarshal, FramesFilled, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetBreakpointByGuid(Guid) {
        result := ComCall(173, this, "ptr", Guid, "ptr*", &Bp := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetExecutionStatusEx() {
        result := ComCall(174, this, "uint*", &Status_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetSynchronizationStatus(SendsAttempted, SecondsSinceLastResponse) {
        ...
        result := ComCall(175, this, SendsAttemptedMarshal, SendsAttempted, SecondsSinceLastResponseMarshal, SecondsSinceLastResponse, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} 
     */
    GetDebuggeeType2(Flags, Class_, Qualifier) {
        Class_Marshal := Class_ is VarRef ? "uint*" : "ptr"
        QualifierMarshal := Qualifier is VarRef ? "uint*" : "ptr"

        result := ComCall(176, this, "uint", Flags, Class_Marshal, Class_, QualifierMarshal, Qualifier, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
