...
class IDebugSystemObjects4 extends IUnknown{
     ...
     * @returns {Integer} 
     */
    GetCurrentProcessExecutableNameWide(Buffer_, BufferSize) {
        Buffer_ := Buffer_ is String ? StrPtr(Buffer_) : Buffer_

        result := ComCall(46, this, "ptr", Buffer_, "uint", BufferSize, "uint*", &ExeSize := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {Integer} 
     */
    GetCurrentSystemServerNameWide(Buffer_, BufferSize) {
        Buffer_ := Buffer_ is String ? StrPtr(Buffer_) : Buffer_

        result := ComCall(47, this, "ptr", Buffer_, "uint", BufferSize, "uint*", &NameSize := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
