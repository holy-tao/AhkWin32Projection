 ...
 */
export default struct IDebugHostEvaluator extends IUnknown {
    /**
     ...
     * used for IDebugHostEvaluator interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        EvaluateExpression         : IntPtr
        EvaluateExtendedExpression : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDebugHostEvaluator.Vtbl()
        }
    ...
    Query(iid) {
        if (IDebugHostEvaluator.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.EvaluateExpression)
        CallbackFree(this.vtbl.EvaluateExtendedExpression)
    }
}
