...
class IDebugAdvanced4 extends IUnknown{
     ...
     * @returns {Integer} 
     */
    GetSourceFileInformationWide(Which, SourceFile, Arg64, Arg32, Buffer_R, BufferSize) {
        SourceFile := SourceFile is String ? StrPtr(SourceFile) : SourceFile

        result := ComCall(10, this, "uint", Which, "ptr", SourceFile, "uint", Arg64, "uint", Arg32, "ptr", Buffer_R, "uint", BufferSize, "uint*", &InfoSize := 0, "HRESULT")
        return InfoSize
    }
     ...
     * @returns {HRESULT} 
     */
    FindSourceFileAndTokenWide(StartElement, ModAddr, File, Flags, FileToken, FileTokenSize, FoundElement, Buffer_R, BufferSize, FoundSize) {
        File := File is String ? StrPtr(File) : File
        Buffer_R := Buffer_R is String ? StrPtr(Buffer_R) : Buffer_R
        ...
        result := ComCall(11, this, "uint", StartElement, "uint", ModAddr, "ptr", File, "uint", Flags, "ptr", FileToken, "uint", FileTokenSize, FoundElementMarshal, FoundElement, "ptr", Buffer_R, "uint", BufferSize, FoundSizeMarshal, FoundSize, "HRESULT")
        return result
    }
     ...
     * @returns {HRESULT} 
     */
    GetSymbolInformationWide(Which, Arg64, Arg32, Buffer_R, BufferSize, InfoSize, StringBuffer, StringBufferSize, StringSize) {
        StringBuffer := StringBuffer is String ? StrPtr(StringBuffer) : StringBuffer

        InfoSizeMarshal := InfoSize is VarRef ? "uint*" : "ptr"
        StringSizeMarshal := StringSize is VarRef ? "uint*" : "ptr"

        result := ComCall(12, this, "uint", Which, "uint", Arg64, "uint", Arg32, "ptr", Buffer_R, "uint", BufferSize, InfoSizeMarshal, InfoSize, "ptr", StringBuffer, "uint", StringBufferSize, StringSizeMarshal, StringSize, "HRESULT")
        return result
    }
     ...
     * @returns {HRESULT} 
     */
    GetSymbolInformationWideEx(Which, Arg64, Arg32, Buffer_R, BufferSize, InfoSize, StringBuffer, StringBufferSize, StringSize, pInfoEx) {
        StringBuffer := StringBuffer is String ? StrPtr(StringBuffer) : StringBuffer

        InfoSizeMarshal := InfoSize is VarRef ? "uint*" : "ptr"
        StringSizeMarshal := StringSize is VarRef ? "uint*" : "ptr"

        result := ComCall(13, this, "uint", Which, "uint", Arg64, "uint", Arg32, "ptr", Buffer_R, "uint", BufferSize, InfoSizeMarshal, InfoSize, "ptr", StringBuffer, "uint", StringBufferSize, StringSizeMarshal, StringSize, "ptr", pInfoEx, "HRESULT")
        return result
    }
}
