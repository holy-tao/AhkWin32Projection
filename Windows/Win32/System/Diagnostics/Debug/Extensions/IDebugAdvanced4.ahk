...
class IDebugAdvanced4 extends IUnknown{
     ...
     * @returns {HRESULT} 
     */
    GetSymbolInformationWideEx(Which, Arg64, Arg32, Buffer_, BufferSize, InfoSize, StringBuffer, StringBufferSize, StringSize, pInfoEx) {
        StringBuffer := StringBuffer is String ? StrPtr(StringBuffer) : StringBuffer

        InfoSizeMarshal := InfoSize is VarRef ? "uint*" : "ptr"
        StringSizeMarshal := StringSize is VarRef ? "uint*" : "ptr"

        result := ComCall(13, this, "uint", Which, "uint", Arg64, "uint", Arg32, "ptr", Buffer_, "uint", BufferSize, InfoSizeMarshal, InfoSize, "ptr", StringBuffer, "uint", StringBufferSize, StringSizeMarshal, StringSize, "ptr", pInfoEx, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
