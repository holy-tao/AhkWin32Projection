...
class IDynamicKeyProviderConcept extends IUnknown{
     ...
     * @param {PWSTR} key 
     * @param {Pointer<Pointer<IModelObject>>} keyValue 
    ...
    GetKey(contextObject, key, keyValue, metadata, hasKey) {
        ...
        result := ComCall(3, this, "ptr", contextObject, "ptr", key, "ptr*", keyValue, "ptr*", metadata, hasKeyMarshal, hasKey, "int")
        if(result != 0) {
    ...
    EnumerateKeys(contextObject) {
        result := ComCall(5, this, "ptr", contextObject, "ptr*", &ppEnumerator := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
