...
class IKeyStore extends IUnknown{
    ...
    GetKey(key, object_, metadata) {
        ...
        result := ComCall(3, this, "ptr", key, "ptr*", object_, "ptr*", metadata, "int")
        if(result != 0) {
    ...
    SetKey(key, object_, metadata) {
        ...
        result := ComCall(4, this, "ptr", key, "ptr", object_, "ptr", metadata, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetKeyValue(key, object_, metadata) {
        ...
        result := ComCall(5, this, "ptr", key, "ptr*", object_, "ptr*", metadata, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    SetKeyValue(key, object_) {
        ...
        result := ComCall(6, this, "ptr", key, "ptr", object_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
