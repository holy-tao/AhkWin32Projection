...
class IDebugBreakpoint2 extends IUnknown{
     ...
     * @returns {Integer} 
     */
    GetCommandWide(Buffer_R, BufferSize) {
        Buffer_R := Buffer_R is String ? StrPtr(Buffer_R) : Buffer_R

        result := ComCall(24, this, "ptr", Buffer_R, "uint", BufferSize, "uint*", &CommandSize := 0, "HRESULT")
        return CommandSize
    }
     ...
     * @returns {Integer} 
     */
    GetOffsetExpressionWide(Buffer_R, BufferSize) {
        Buffer_R := Buffer_R is String ? StrPtr(Buffer_R) : Buffer_R

        result := ComCall(26, this, "ptr", Buffer_R, "uint", BufferSize, "uint*", &ExpressionSize := 0, "HRESULT")
        return ExpressionSize
    }
