...
#Import ".\SOURCEFILE.ahk" { SOURCEFILE }

/**
 * PSYM_ENUMSOURCEFILES_CALLBACK (dbghelp.h) is an application-defined callback function used with the SymEnumSourceFiles function.
 * @see https://learn.microsoft.com/windows/win32/api/dbghelp/nc-dbghelp-psym_enumsourcefiles_callback
 * @namespace Windows.Win32.System.Diagnostics.Debug
 * @charset ANSI
 */
export default struct PSYM_ENUMSOURCEFILES_CALLBACK {
    value : IntPtr

    __value {
        set {
            if (value is PSYM_ENUMSOURCEFILES_CALLBACK) {
    ...
    /**
     * A PSYM_ENUMSOURCEFILES_CALLBACK that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PSYM_ENUMSOURCEFILES_CALLBACK {
        /**
         * Creates a PSYM_ENUMSOURCEFILES_CALLBACK pointer that invokes the given AHK function when called.
         * @param {Func(SOURCEFILE, "ptr") => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 2)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 2 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [SOURCEFILE.Ptr, "ptr", BOOL])
        }
