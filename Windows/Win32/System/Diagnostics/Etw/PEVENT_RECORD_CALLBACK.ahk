 ...
 * @see https://learn.microsoft.com/windows/win32/api/evntrace/nc-evntrace-pevent_record_callback
 * @namespace Windows.Win32.System.Diagnostics.Etw
 */
export default struct PEVENT_RECORD_CALLBACK {
    value : IntPtr

    __value {
        set {
            if (value is PEVENT_RECORD_CALLBACK) {
     ...
     */
    Call(EventRecord) {
        DllCall(this.value, EVENT_RECORD.Ptr, EventRecord)
    }

    /**
     * A PEVENT_RECORD_CALLBACK that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends PEVENT_RECORD_CALLBACK {
        /**
         * Creates a PEVENT_RECORD_CALLBACK pointer that invokes the given AHK function when called.
         * @param {Func(EVENT_RECORD) => IntPtr} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 1)) {
