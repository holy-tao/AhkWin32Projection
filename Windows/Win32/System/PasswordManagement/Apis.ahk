 ...
 */
export MSChapSrvChangePassword(ServerName, UserName, LmOldPresent, LmOldOwfPassword, LmNewOwfPassword, NtOldOwfPassword, NtNewOwfPassword) {
    ServerName := ServerName is String ? StrPtr(ServerName) : ServerName
    UserName := UserName is String ? StrPtr(UserName) : UserName

    result := DllCall("ADVAPI32.dll\MSChapSrvChangePassword", "ptr", ServerName, "ptr", UserName, BOOLEAN, LmOldPresent, LM_OWF_PASSWORD.Ptr, LmOldOwfPassword, LM_OWF_PASSWORD.Ptr, LmNewOwfPassword, LM_OWF_PASSWORD.Ptr, NtOldOwfPassword, LM_OWF_PASSWORD.Ptr, NtNewOwfPassword, UInt32)
    return result
}
 ...
 * @since windows5.1.2600
 */
export MSChapSrvChangePassword2(ServerName, UserName, NewPasswordEncryptedWithOldNt, OldNtOwfPasswordEncryptedWithNewNt, LmPresent, NewPasswordEncryptedWithOldLm, OldLmOwfPasswordEncryptedWithNewLmOrNt) {
    ServerName := ServerName is String ? StrPtr(ServerName) : ServerName
    UserName := UserName is String ? StrPtr(UserName) : UserName

    result := DllCall("ADVAPI32.dll\MSChapSrvChangePassword2", "ptr", ServerName, "ptr", UserName, SAMPR_ENCRYPTED_USER_PASSWORD.Ptr, NewPasswordEncryptedWithOldNt, ENCRYPTED_LM_OWF_PASSWORD.Ptr, OldNtOwfPasswordEncryptedWithNewNt, BOOLEAN, LmPresent, SAMPR_ENCRYPTED_USER_PASSWORD.Ptr, NewPasswordEncryptedWithOldLm, ENCRYPTED_LM_OWF_PASSWORD.Ptr, OldLmOwfPasswordEncryptedWithNewLmOrNt, UInt32)
    return result
}
