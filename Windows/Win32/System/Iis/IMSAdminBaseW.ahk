...
class IMSAdminBaseW extends IUnknown{
     ...
     * @param {PWSTR} pszMDPath 
     * @param {Pointer<METADATA_RECORD>} pmdrMDData 
     * @param {Pointer<UInt32>} pdwMDRequiredDataLen 
     * @returns {HRESULT} 
     */
    GetData(hMDHandle, pszMDPath, pmdrMDData, pdwMDRequiredDataLen) {
        pszMDPath := pszMDPath is String ? StrPtr(pszMDPath) : pszMDPath

        result := ComCall(10, this, "uint", hMDHandle, "ptr", pszMDPath, "ptr", pmdrMDData, "uint*", pdwMDRequiredDataLen, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumData(hMDHandle, pszMDPath, pmdrMDData, dwMDEnumDataIndex, pdwMDRequiredDataLen) {
        ...
        result := ComCall(12, this, "uint", hMDHandle, "ptr", pszMDPath, "ptr", pmdrMDData, "uint", dwMDEnumDataIndex, "uint*", pdwMDRequiredDataLen, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Integer} dwMDDataType 
     * @param {Pointer<UInt32>} pdwMDNumDataEntries 
     * @param {Pointer<UInt32>} pdwMDDataSetNumber 
     * @param {Integer} dwMDBufferSize 
     * @param {Pointer<Byte>} pbMDBuffer 
     * @param {Pointer<UInt32>} pdwMDRequiredBufferSize 
     * @returns {HRESULT} 
     */
    GetAllData(hMDHandle, pszMDPath, dwMDAttributes, dwMDUserType, dwMDDataType, pdwMDNumDataEntries, pdwMDDataSetNumber, dwMDBufferSize, pbMDBuffer, pdwMDRequiredBufferSize) {
        ...
        result := ComCall(13, this, "uint", hMDHandle, "ptr", pszMDPath, "uint", dwMDAttributes, "uint", dwMDUserType, "uint", dwMDDataType, "uint*", pdwMDNumDataEntries, "uint*", pdwMDDataSetNumber, "uint", dwMDBufferSize, "char*", pbMDBuffer, "uint*", pdwMDRequiredBufferSize, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDataPaths(hMDHandle, pszMDPath, dwMDIdentifier, dwMDDataType, dwMDBufferSize, pszBuffer, pdwMDRequiredBufferSize) {
        ...
        result := ComCall(16, this, "uint", hMDHandle, "ptr", pszMDPath, "uint", dwMDIdentifier, "uint", dwMDDataType, "uint", dwMDBufferSize, "ptr", pszBuffer, "uint*", pdwMDRequiredBufferSize, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} phMDNewHandle 
     * @returns {HRESULT} 
     */
    OpenKey(hMDHandle, pszMDPath, dwMDAccessRequested, dwMDTimeOut, phMDNewHandle) {
        pszMDPath := pszMDPath is String ? StrPtr(pszMDPath) : pszMDPath

        result := ComCall(17, this, "uint", hMDHandle, "ptr", pszMDPath, "uint", dwMDAccessRequested, "uint", dwMDTimeOut, "uint*", phMDNewHandle, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pdwSystemChangeNumber 
     * @returns {HRESULT} 
     */
    GetSystemChangeNumber(pdwSystemChangeNumber) {
        result := ComCall(22, this, "uint*", pdwSystemChangeNumber, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetDataSetNumber(hMDHandle, pszMDPath, pdwMDDataSetNumber) {
        ...
        result := ComCall(23, this, "uint", hMDHandle, "ptr", pszMDPath, "uint*", pdwMDDataSetNumber, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * 
     * @param {PWSTR} pszMDBackupLocation 
     * @param {Pointer<UInt32>} pdwMDVersion 
    ...
    EnumBackups(pszMDBackupLocation, pdwMDVersion, pftMDBackupTime, dwMDEnumIndex) {
        ...
        result := ComCall(30, this, "ptr", pszMDBackupLocation, "uint*", pdwMDVersion, "ptr", pftMDBackupTime, "uint", dwMDEnumIndex, "int")
        if(result != 0)
            throw OSError(result)
    ...
    UnmarshalInterface(piadmbwInterface) {
        result := ComCall(32, this, "ptr", piadmbwInterface, "int")
        if(result != 0)
            throw OSError(result)
