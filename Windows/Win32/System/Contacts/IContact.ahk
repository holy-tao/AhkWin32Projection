...
class IContact extends IUnknown{
     ...
     * @param {Pointer<UInt32>} pdwcchContactIDRequired 
     * @returns {HRESULT} 
     */
    GetContactID(pszContactID, cchContactID, pdwcchContactIDRequired) {
        ...
        result := ComCall(3, this, "ptr", pszContactID, "uint", cchContactID, "uint*", pdwcchContactIDRequired, "int")
        if(result != 0)
     ...
     * @param {Pointer<UInt32>} pdwcchPathRequired 
    ...
    GetPath(pszPath, cchPath, pdwcchPathRequired) {
        ...
        result := ComCall(4, this, "ptr", pszPath, "uint", cchPath, "uint*", pdwcchPathRequired, "int")
        if(result != 0)
            throw OSError(result)
