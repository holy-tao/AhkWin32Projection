...
class IContactManager extends IUnknown{
    ...
    Load(pszContactID, ppContact) {
        ...
        result := ComCall(4, this, "ptr", pszContactID, "ptr", ppContact, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetMeContact(ppMeContact) {
        result := ComCall(6, this, "ptr", ppMeContact, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IContact>} pMeContact 
     * @returns {HRESULT} 
     */
    ...
    GetContactCollection(ppContactCollection) {
        result := ComCall(8, this, "ptr", ppContactCollection, "int")
        if(result != 0)
            throw OSError(result)
