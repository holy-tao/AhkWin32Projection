...
class IUpdateServiceManager extends IDispatch{
    ...
    AddService(serviceID, authorizationCabPath, retval) {
        ...
        result := ComCall(8, this, "ptr", serviceID, "ptr", authorizationCabPath, "ptr", retval, "int")
        if(result != 0)
            throw OSError(result)
    ...
    AddScanPackageService(serviceName, scanFileLocation, flags, ppService) {
        ...
        result := ComCall(12, this, "ptr", serviceName, "ptr", scanFileLocation, "int", flags, "ptr", ppService, "int")
        if(result != 0)
            throw OSError(result)
