...
export OOBEComplete(isOOBEComplete) {
    ...
    result := DllCall("KERNEL32.dll\OOBEComplete", isOOBECompleteMarshal, isOOBEComplete, BOOL)
...
export UnregisterWaitUntilOOBECompleted(WaitHandle) {
    ...
    result := DllCall("KERNEL32.dll\UnregisterWaitUntilOOBECompleted", WaitHandleMarshal, WaitHandle, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
