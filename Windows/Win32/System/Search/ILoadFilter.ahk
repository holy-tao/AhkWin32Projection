 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/filtereg/nn-filtereg-iloadfilter
...
class ILoadFilter extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/filtereg/nf-filtereg-iloadfilter-loadifilter
     */
    LoadIFilter(pwcsPath, pFilteredSources, pUnkOuter, fUseDefault, pFilterClsid, SearchDecSize, pwcsSearchDesc, ppIFilt) {
        ...
        result := ComCall(3, this, "ptr", pwcsPath, "ptr", pFilteredSources, "ptr", pUnkOuter, "int", fUseDefault, "ptr", pFilterClsid, SearchDecSizeMarshal, SearchDecSize, pwcsSearchDescMarshal, pwcsSearchDesc, "ptr*", ppIFilt, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/filtereg/nf-filtereg-iloadfilter-loadifilterfromstorage
     */
    LoadIFilterFromStorage(pStg, pUnkOuter, pwcsOverride, fUseDefault, pFilterClsid, SearchDecSize, pwcsSearchDesc, ppIFilt) {
        ...
        result := ComCall(4, this, "ptr", pStg, "ptr", pUnkOuter, "ptr", pwcsOverride, "int", fUseDefault, "ptr", pFilterClsid, SearchDecSizeMarshal, SearchDecSize, pwcsSearchDescMarshal, pwcsSearchDesc, "ptr*", ppIFilt, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @param {Pointer<IFilter>} ppIFilt 
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/filtereg/nf-filtereg-iloadfilter-loadifilterfromstream
     */
    LoadIFilterFromStream(pStm, pFilteredSources, pUnkOuter, fUseDefault, pFilterClsid, SearchDecSize, pwcsSearchDesc, ppIFilt) {
        SearchDecSizeMarshal := SearchDecSize is VarRef ? "int*" : "ptr"
        pwcsSearchDescMarshal := pwcsSearchDesc is VarRef ? "ptr*" : "ptr"

        result := ComCall(5, this, "ptr", pStm, "ptr", pFilteredSources, "ptr", pUnkOuter, "int", fUseDefault, "ptr", pFilterClsid, SearchDecSizeMarshal, SearchDecSize, pwcsSearchDescMarshal, pwcsSearchDesc, "ptr*", ppIFilt, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
