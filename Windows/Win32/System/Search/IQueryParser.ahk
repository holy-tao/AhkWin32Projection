...
class IQueryParser extends IUnknown{
     ...
     * @param {Pointer<IEnumUnknown>} pCustomProperties 
    ...
    Parse(pszInputString, pCustomProperties, ppSolution) {
        ...
        result := ComCall(3, this, "ptr", pszInputString, "ptr", pCustomProperties, "ptr", ppSolution, "int")
        if(result != 0)
    ...
    GetSchemaProvider(ppSchemaProvider) {
        result := ComCall(7, this, "ptr", ppSchemaProvider, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RestateToString(pCondition, fUseEnglish, ppszQueryString) {
        result := ComCall(8, this, "ptr", pCondition, "int", fUseEnglish, "ptr", ppszQueryString, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ParsePropertyValue(pszPropertyName, pszInputString, ppSolution) {
        ...
        result := ComCall(9, this, "ptr", pszPropertyName, "ptr", pszInputString, "ptr", ppSolution, "int")
        if(result != 0)
            throw OSError(result)
    ...
    RestatePropertyValueToString(pCondition, fUseEnglish, ppszPropertyName, ppszQueryString) {
        result := ComCall(10, this, "ptr", pCondition, "int", fUseEnglish, "ptr", ppszPropertyName, "ptr", ppszQueryString, "int")
        if(result != 0)
            throw OSError(result)
