 ...
 */
export default struct ISearchItemsChangedSink extends IUnknown {
    /**
     ...
     * used for ISearchItemsChangedSink interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISearchItemsChangedSink.Vtbl()
        }
    ...
    Query(iid) {
        if (ISearchItemsChangedSink.IID.Equals(iid)) {
            return true
        }
