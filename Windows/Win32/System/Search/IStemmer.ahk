 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/indexsrv/nn-indexsrv-istemmer
...
class IStemmer extends IUnknown{
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/indexsrv/nf-indexsrv-istemmer-init
     */
    Init(ulMaxTokenSize, pfLicense) {
        pfLicenseMarshal := pfLicense is VarRef ? "int*" : "ptr"

        result := ComCall(3, this, "uint", ulMaxTokenSize, pfLicenseMarshal, pfLicense, "int")
        if(result != 0) {
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/indexsrv/nf-indexsrv-istemmer-generatewordforms
     */
    GenerateWordForms(pwcInBuf, cwc, pStemSink) {
        pwcInBuf := pwcInBuf is String ? StrPtr(pwcInBuf) : pwcInBuf

        result := ComCall(4, this, "ptr", pwcInBuf, "uint", cwc, "ptr", pStemSink, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/indexsrv/nf-indexsrv-istemmer-getlicensetouse
     */
    GetLicenseToUse(ppwcsLicense) {
        ppwcsLicenseMarshal := ppwcsLicense is VarRef ? "ptr*" : "ptr"

        result := ComCall(5, this, ppwcsLicenseMarshal, ppwcsLicense, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
