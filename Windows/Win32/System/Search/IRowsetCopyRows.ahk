 ...
 */
export default struct IRowsetCopyRows extends IUnknown {
    /**
     ...
     * used for IRowsetCopyRows interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IRowsetCopyRows.Vtbl()
        }
    ...
    Query(iid) {
        if (IRowsetCopyRows.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.CloseSource)
        CallbackFree(this.vtbl.CopyByHROWS)
        CallbackFree(this.vtbl.CopyRows)
        CallbackFree(this.vtbl.DefineSource)
    }
}
