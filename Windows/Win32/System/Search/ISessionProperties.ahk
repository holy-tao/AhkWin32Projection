 ...
 */
export default struct ISessionProperties extends IUnknown {
    /**
     ...
     * used for ISessionProperties interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISessionProperties.Vtbl()
        }
    ...
    SetProperties(cPropertySets, rgPropertySets) {
        ...
        result := ComCall(4, this, UInt32, cPropertySets, rgPropertySetsMarshal, rgPropertySets, "HRESULT")
        return result
    }

    _Query(iid) {
        if (ISessionProperties.IID.Equals(iid)) {
            return true
        }
