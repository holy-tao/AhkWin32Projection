...
class IConditionFactory extends IUnknown{
    ...
    MakeNot(pcSub, fSimplify, ppcResult) {
        result := ComCall(3, this, "ptr", pcSub, "int", fSimplify, "ptr", ppcResult, "int")
        if(result != 0)
     ...
     * 
     * @param {Integer} ct 
     * @param {Pointer<IEnumUnknown>} peuSubs 
     * @param {BOOL} fSimplify 
     * @param {Pointer<ICondition>} ppcResult 
     * @returns {HRESULT} 
     */
    MakeAndOr(ct, peuSubs, fSimplify, ppcResult) {
        result := ComCall(4, this, "int", ct, "ptr", peuSubs, "int", fSimplify, "ptr", ppcResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    MakeLeaf(pszPropertyName, cop, pszValueType, ppropvar, pPropertyNameTerm, pOperationTerm, pValueTerm, fExpand, ppcResult) {
        ...
        result := ComCall(5, this, "ptr", pszPropertyName, "int", cop, "ptr", pszValueType, "ptr", ppropvar, "ptr", pPropertyNameTerm, "ptr", pOperationTerm, "ptr", pValueTerm, "int", fExpand, "ptr", ppcResult, "int")
        if(result != 0)
            throw OSError(result)
    ...
    Resolve(pc, sqro, pstReferenceTime, ppcResolved) {
        result := ComCall(6, this, "ptr", pc, "int", sqro, "ptr", pstReferenceTime, "ptr", ppcResolved, "int")
        if(result != 0)
            throw OSError(result)
