 ...
 */
export default struct ICommandStream extends IUnknown {
    /**
     ...
     * used for ICommandStream interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetCommandStream : IntPtr
        SetCommandStream : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ICommandStream.Vtbl()
        }
    ...
    Query(iid) {
        if (ICommandStream.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetCommandStream)
        CallbackFree(this.vtbl.SetCommandStream)
    }
}
