 ...
 * @charset ANSI
 */
export default struct IOleUILinkContainerA extends IUnknown {
     ...
     * used for IOleUILinkContainerA interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IOleUILinkContainerA.Vtbl()
        }
    ...
    Query(iid) {
        if (IOleUILinkContainerA.IID.Equals(iid)) {
            return true
        }
