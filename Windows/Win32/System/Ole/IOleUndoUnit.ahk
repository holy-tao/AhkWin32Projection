 ...
 */
export default struct IOleUndoUnit extends IUnknown {
    /**
     ...
     * used for IOleUndoUnit interfaces
    */
    ...
    GetDescription() {
        pBstr := BSTR.Owned()
        result := ComCall(4, this, BSTR.Ptr, pBstr, "HRESULT")
        return pBstr
    }
    ...
    Query(iid) {
        if (IOleUndoUnit.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.Do)
        CallbackFree(this.vtbl.GetDescription)
        CallbackFree(this.vtbl.GetUnitType)
        CallbackFree(this.vtbl.OnNextAdd)
    }
}
