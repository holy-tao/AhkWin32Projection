 ...
 */
export default struct IDropSource extends IUnknown {
    /**
     ...
     * used for IDropSource interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        QueryContinueDrag : IntPtr
        GiveFeedback      : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IDropSource.Vtbl()
        }
    ...
    QueryContinueDrag(fEscapePressed, grfKeyState) {
        result := ComCall(3, this, BOOL, fEscapePressed, MODIFIERKEYS_FLAGS, grfKeyState, Int32)
        return result
    }
    ...
    GiveFeedback(dwEffect) {
        result := ComCall(4, this, DROPEFFECT, dwEffect, Int32)
        return result
    }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.QueryContinueDrag)
        CallbackFree(this.vtbl.GiveFeedback)
    }
}
