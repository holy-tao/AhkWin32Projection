 ...
 */
export default struct IParseDisplayName extends IUnknown {
    /**
     ...
     * used for IParseDisplayName interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IParseDisplayName.Vtbl()
        }
    ...
    Query(iid) {
        if (IParseDisplayName.IID.Equals(iid)) {
            return true
        }
