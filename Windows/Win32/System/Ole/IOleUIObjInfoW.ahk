 ...
 * @charset Unicode
 */
export default struct IOleUIObjInfoW extends IUnknown {
     ...
     * used for IOleUIObjInfoW interfaces
    */
    ...
    GetObjectInfo(dwObject, lpdwObjSize, lplpszLabel, lplpszType, lplpszShortType, lplpszLocation) {
        ...
        lplpszLabelMarshal := lplpszLabel == 0 ? IntPtr : PWSTR.Ptr
        lplpszTypeMarshal := lplpszType is VarRef ? "ptr*" : IntPtr
        lplpszTypeMarshal := lplpszType == 0 ? IntPtr : PWSTR.Ptr
        lplpszShortTypeMarshal := lplpszShortType is VarRef ? "ptr*" : IntPtr
        lplpszShortTypeMarshal := lplpszShortType == 0 ? IntPtr : PWSTR.Ptr
        lplpszLocationMarshal := lplpszLocation is VarRef ? "ptr*" : IntPtr
        lplpszLocationMarshal := lplpszLocation == 0 ? IntPtr : PWSTR.Ptr
    ...
    _Query(iid) {
        if (IOleUIObjInfoW.IID.Equals(iid)) {
            return true
        }
