 ...
 */
export default struct IPropertyNotifySink extends IUnknown {
    /**
     ...
     * used for IPropertyNotifySink interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        OnChanged     : IntPtr
        OnRequestEdit : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IPropertyNotifySink.Vtbl()
        }
    ...
    Query(iid) {
        if (IPropertyNotifySink.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.OnRequestEdit)
    }
}
