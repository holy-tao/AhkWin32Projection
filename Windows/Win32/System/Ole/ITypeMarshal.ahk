...
class ITypeMarshal extends IUnknown{
     ...
     * @returns {Integer} 
     * @see https://learn.microsoft.com/windows/win32/api//content/windows.foundation/ns-windows-foundation-size
     */
    Size(pvType, dwDestContext, pvDestContext) {
        ...
        result := ComCall(3, this, pvTypeMarshal, pvType, "uint", dwDestContext, pvDestContextMarshal, pvDestContext, "uint*", &pSize := 0, "int")
        if(result != 0) {
     ...
     * @returns {Integer} 
     * @see https://learn.microsoft.com/uwp/api/windows.foundation.metadata.marshalingbehaviorattribute.#ctor
     */
    Marshal(pvType, dwDestContext, pvDestContext, cbBufferLength, pBuffer) {
        pvTypeMarshal := pvType is VarRef ? "ptr" : "ptr"
        pvDestContextMarshal := pvDestContext is VarRef ? "ptr" : "ptr"

        result := ComCall(4, this, pvTypeMarshal, pvType, "uint", dwDestContext, pvDestContextMarshal, pvDestContext, "uint", cbBufferLength, "ptr", pBuffer, "uint*", &pcbWritten := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    Unmarshal(pvType, dwFlags, cbBufferLength, pBuffer, pcbRead) {
        ...
        result := ComCall(5, this, pvTypeMarshal, pvType, "uint", dwFlags, "uint", cbBufferLength, pBufferMarshal, pBuffer, pcbReadMarshal, pcbRead, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    Free(pvType) {
        ...
        result := ComCall(6, this, pvTypeMarshal, pvType, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
