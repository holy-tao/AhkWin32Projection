#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\Com\IUnknown.ahk

/**
 * @namespace Windows.Win32.System.Ole
 * @version v4.0.30319
 */
class ITypeMarshal extends IUnknown{
    /**
     * The interface identifier for ITypeMarshal
     * @type {Guid}
     */
    static IID => Guid("{0000002d-0000-0000-c000-000000000046}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 3

    /**
     * The Size class encapsulates a Width and Height dimension in a 2-D coordinate system.
     * @param {Pointer<Void>} pvType 
     * @param {Integer} dwDestContext 
     * @param {Pointer<Void>} pvDestContext 
     * @param {Pointer<UInt32>} pSize 
     * @returns {HRESULT} 
     * @see https://docs.microsoft.com/windows/win32/api//gdiplustypes/nl-gdiplustypes-size
     */
    Size(pvType, dwDestContext, pvDestContext, pSize) {
        result := ComCall(3, this, "ptr", pvType, "uint", dwDestContext, "ptr", pvDestContext, "uint*", pSize, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Void>} pvType 
     * @param {Integer} dwDestContext 
     * @param {Pointer<Void>} pvDestContext 
     * @param {Integer} cbBufferLength 
     * @param {Pointer} pBuffer 
     * @param {Pointer<UInt32>} pcbWritten 
     * @returns {HRESULT} 
     */
    Marshal(pvType, dwDestContext, pvDestContext, cbBufferLength, pBuffer, pcbWritten) {
        result := ComCall(4, this, "ptr", pvType, "uint", dwDestContext, "ptr", pvDestContext, "uint", cbBufferLength, "ptr", pBuffer, "uint*", pcbWritten, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Void>} pvType 
     * @param {Integer} dwFlags 
     * @param {Integer} cbBufferLength 
     * @param {Pointer<Byte>} pBuffer 
     * @param {Pointer<UInt32>} pcbRead 
     * @returns {HRESULT} 
     */
    Unmarshal(pvType, dwFlags, cbBufferLength, pBuffer, pcbRead) {
        result := ComCall(5, this, "ptr", pvType, "uint", dwFlags, "uint", cbBufferLength, "char*", pBuffer, "uint*", pcbRead, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Void>} pvType 
     * @returns {HRESULT} 
     */
    Free(pvType) {
        result := ComCall(6, this, "ptr", pvType, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
