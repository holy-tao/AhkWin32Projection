...
class IDispError extends IUnknown{
    ...
    QueryErrorInfo(guidErrorType) {
        result := ComCall(3, this, "ptr", guidErrorType, "ptr*", &ppde := 0, "int")
        if(result != 0) {
     ...
     */
    GetNext() {
        result := ComCall(4, this, "ptr*", &ppde := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetHelpInfo(pbstrFileName, pdwContext) {
        ...
        result := ComCall(7, this, "ptr", pbstrFileName, pdwContextMarshal, pdwContext, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
