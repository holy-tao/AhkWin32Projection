...
class IOleItemContainer extends IOleContainer{
    ...
    GetObject(pszItem, dwSpeedNeeded, pbc, riid, ppvObject) {
        ...
        result := ComCall(6, this, "ptr", pszItem, "uint", dwSpeedNeeded, "ptr", pbc, "ptr", riid, "ptr", ppvObject, "int")
        if(result != 0)
     ...
     * @param {Pointer<IBindCtx>} pbc 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppvStorage 
     * @returns {HRESULT} 
     */
    GetObjectStorage(pszItem, pbc, riid, ppvStorage) {
        pszItem := pszItem is String ? StrPtr(pszItem) : pszItem

        result := ComCall(7, this, "ptr", pszItem, "ptr", pbc, "ptr", riid, "ptr", ppvStorage, "int")
        if(result != 0)
            throw OSError(result)
