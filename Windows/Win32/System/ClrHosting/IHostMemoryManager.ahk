...
class IHostMemoryManager extends IUnknown{
    ...
    CreateMalloc(dwMallocType, ppMalloc) {
        result := ComCall(3, this, "uint", dwMallocType, "ptr", ppMalloc, "int")
        if(result != 0)
    ...
    VirtualAlloc(pAddress, dwSize, flAllocationType, flProtect, eCriticalLevel, ppMem) {
        result := ComCall(4, this, "ptr", pAddress, "ptr", dwSize, "uint", flAllocationType, "uint", flProtect, "int", eCriticalLevel, "ptr", ppMem, "int")
        if(result != 0)
            throw OSError(result)
    ...
    VirtualFree(lpAddress, dwSize, dwFreeType) {
        result := ComCall(5, this, "ptr", lpAddress, "ptr", dwSize, "uint", dwFreeType, "int")
        if(result != 0)
            throw OSError(result)
    ...
    VirtualQuery(lpAddress, lpBuffer, dwLength, pResult) {
        result := ComCall(6, this, "ptr", lpAddress, "ptr", lpBuffer, "ptr", dwLength, "ptr*", pResult, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} pflOldProtect 
     * @returns {HRESULT} If the function succeeds, the return value is nonzero.
     * 
    ...
    VirtualProtect(lpAddress, dwSize, flNewProtect, pflOldProtect) {
        result := ComCall(7, this, "ptr", lpAddress, "ptr", dwSize, "uint", flNewProtect, "uint*", pflOldProtect, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} pMemoryLoad 
     * @param {Pointer<UIntPtr>} pAvailableBytes 
     * @returns {HRESULT} 
     */
    GetMemoryLoad(pMemoryLoad, pAvailableBytes) {
        result := ComCall(8, this, "uint*", pMemoryLoad, "ptr*", pAvailableBytes, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<ICLRMemoryNotificationCallback>} pCallback 
     * @returns {HRESULT} 
     */
    ...
    NeedsVirtualAddressSpace(startAddress, size) {
        result := ComCall(10, this, "ptr", startAddress, "ptr", size, "int")
        if(result != 0)
            throw OSError(result)
    ...
    AcquiredVirtualAddressSpace(startAddress, size) {
        result := ComCall(11, this, "ptr", startAddress, "ptr", size, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ReleasedVirtualAddressSpace(startAddress) {
        result := ComCall(12, this, "ptr", startAddress, "int")
        if(result != 0)
            throw OSError(result)
