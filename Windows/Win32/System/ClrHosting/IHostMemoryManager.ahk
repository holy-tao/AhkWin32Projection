...
class IHostMemoryManager extends IUnknown{
    ...
    CreateMalloc(dwMallocType) {
        result := ComCall(3, this, "uint", dwMallocType, "ptr*", &ppMalloc := 0, "int")
        if(result != 0) {
    ...
    VirtualAlloc(pAddress, dwSize, flAllocationType, flProtect, eCriticalLevel) {
        ...
        result := ComCall(4, this, pAddressMarshal, pAddress, "ptr", dwSize, "uint", flAllocationType, "uint", flProtect, "int", eCriticalLevel, "ptr*", &ppMem := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    VirtualFree(lpAddress, dwSize, dwFreeType) {
        ...
        result := ComCall(5, this, lpAddressMarshal, lpAddress, "ptr", dwSize, "uint", dwFreeType, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    VirtualQuery(lpAddress, lpBuffer, dwLength, pResult) {
        ...
        result := ComCall(6, this, lpAddressMarshal, lpAddress, lpBufferMarshal, lpBuffer, "ptr", dwLength, pResultMarshal, pResult, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    VirtualProtect(lpAddress, dwSize, flNewProtect) {
        ...
        result := ComCall(7, this, lpAddressMarshal, lpAddress, "ptr", dwSize, "uint", flNewProtect, "uint*", &pflOldProtect := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetMemoryLoad(pMemoryLoad, pAvailableBytes) {
        ...
        result := ComCall(8, this, pMemoryLoadMarshal, pMemoryLoad, pAvailableBytesMarshal, pAvailableBytes, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} 
     */
    NeedsVirtualAddressSpace(startAddress, size_) {
        startAddressMarshal := startAddress is VarRef ? "ptr" : "ptr"

        result := ComCall(10, this, startAddressMarshal, startAddress, "ptr", size_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} 
     */
    AcquiredVirtualAddressSpace(startAddress, size_) {
        startAddressMarshal := startAddress is VarRef ? "ptr" : "ptr"

        result := ComCall(11, this, startAddressMarshal, startAddress, "ptr", size_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    ReleasedVirtualAddressSpace(startAddress) {
        ...
        result := ComCall(12, this, startAddressMarshal, startAddress, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
