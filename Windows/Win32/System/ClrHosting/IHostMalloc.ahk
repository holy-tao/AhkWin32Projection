...
class IHostMalloc extends IUnknown{
    ...
    Alloc(cbSize, eCriticalLevel) {
        result := ComCall(3, this, "ptr", cbSize, "int", eCriticalLevel, "ptr*", &ppMem := 0, "int")
        if(result != 0) {
    ...
    DebugAlloc(cbSize, eCriticalLevel, pszFileName, iLineNo) {
        ...
        result := ComCall(4, this, "ptr", cbSize, "int", eCriticalLevel, pszFileNameMarshal, pszFileName, "int", iLineNo, "ptr*", &ppMem := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    Free(pMem) {
        ...
        result := ComCall(5, this, pMemMarshal, pMem, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
