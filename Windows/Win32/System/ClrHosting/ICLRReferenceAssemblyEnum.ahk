 ...
 */
export default struct ICLRReferenceAssemblyEnum extends IUnknown {
    /**
     ...
     * used for ICLRReferenceAssemblyEnum interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ICLRReferenceAssemblyEnum.Vtbl()
        }
    ...
    Query(iid) {
        if (ICLRReferenceAssemblyEnum.IID.Equals(iid)) {
            return true
        }
