...
class IHostSecurityManager extends IUnknown{
    ...
    GetSecurityContext(eContextType, ppSecurityContext) {
        result := ComCall(7, this, "int", eContextType, "ptr", ppSecurityContext, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * 
     * @param {Integer} eContextType 
     * @param {Pointer<IHostSecurityContext>} pSecurityContext 
     * @returns {HRESULT} 
     */
    SetSecurityContext(eContextType, pSecurityContext) {
        result := ComCall(8, this, "int", eContextType, "ptr", pSecurityContext, "int")
        if(result != 0)
            throw OSError(result)
