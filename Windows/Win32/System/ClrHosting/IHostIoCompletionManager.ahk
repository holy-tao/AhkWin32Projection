...
class IHostIoCompletionManager extends IUnknown{
     ...
     */
    GetMaxThreads() {
        result := ComCall(6, this, "uint*", &pdwMaxIOCompletionThreads := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetAvailableThreads() {
        result := ComCall(7, this, "uint*", &pdwAvailableIOCompletionThreads := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    InitializeHostOverlapped(pvOverlapped) {
        ...
        result := ComCall(10, this, pvOverlappedMarshal, pvOverlapped, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    GetMinThreads() {
        result := ComCall(13, this, "uint*", &pdwMinIOCompletionThreads := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
