...
class ICLRMetaHost extends IUnknown{
    ...
    GetRuntime(pwzVersion, riid, ppRuntime) {
        ...
        result := ComCall(3, this, "ptr", pwzVersion, "ptr", riid, "ptr", ppRuntime, "int")
        if(result != 0)
    ...
    GetVersionFromFile(pwzFilePath, pwzBuffer, pcchBuffer) {
        ...
        result := ComCall(4, this, "ptr", pwzFilePath, "ptr", pwzBuffer, "uint*", pcchBuffer, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumerateInstalledRuntimes(ppEnumerator) {
        result := ComCall(5, this, "ptr", ppEnumerator, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumerateLoadedRuntimes(hndProcess, ppEnumerator) {
        ...
        result := ComCall(6, this, "ptr", hndProcess, "ptr", ppEnumerator, "int")
        if(result != 0)
            throw OSError(result)
