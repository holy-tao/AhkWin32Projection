 ...
 */
export default struct IMSMQTransactionDispenser extends IDispatch {
    /**
     ...
     * used for IMSMQTransactionDispenser interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IMSMQTransactionDispenser.Vtbl()
        }
    ...
    Query(iid) {
        if (IMSMQTransactionDispenser.IID.Equals(iid)) {
            return true
        }
