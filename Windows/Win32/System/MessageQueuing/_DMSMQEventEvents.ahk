 ...
 */
export default struct _DMSMQEventEvents extends IDispatch {
    /**
     ...
     * used for _DMSMQEventEvents interfaces
    */
    ...
    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := _DMSMQEventEvents.Vtbl()
        }
        super.__New(implObj, flags)
    }

    Query(iid) {
        if (_DMSMQEventEvents.IID.Equals(iid)) {
            return true
        }
