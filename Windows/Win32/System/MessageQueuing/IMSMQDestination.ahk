...
class IMSMQDestination extends IDispatch{
     ...
     */
    get_IsOpen(pfIsOpen) {
        result := ComCall(9, this, "ptr", pfIsOpen, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_IADs(ppIADs) {
        result := ComCall(10, this, "ptr", ppIADs, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_Destinations(ppDestinations) {
        result := ComCall(18, this, "ptr", ppDestinations, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IDispatch>} pDestinations 
     * @returns {HRESULT} 
     */
    ...
    get_Properties(ppcolProperties) {
        result := ComCall(20, this, "ptr", ppcolProperties, "int")
        if(result != 0)
            throw OSError(result)
