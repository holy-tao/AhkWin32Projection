...
class IMSMQQueueInfo4 extends IDispatch{
    ...
    get_IsTransactional2() {
        result := ComCall(41, this, "short*", &pisTransactional := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    get_IsWorldReadable2() {
        result := ComCall(42, this, "short*", &pisWorldReadable := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    put_MulticastAddress(bstrMulticastAddress) {
        if(bstrMulticastAddress is String) {
            pin := BSTR.Alloc(bstrMulticastAddress)
            bstrMulticastAddress := pin.Value
        }
