...
class ISupportLastWriteTime extends IUnknown{
     ...
     * @param {Pointer<UInt64>} pullTimestamp 
     * @returns {HRESULT} 
     */
    GetItemChangeTime(pbItemId, pullTimestamp) {
        result := ComCall(3, this, "char*", pbItemId, "uint*", pullTimestamp, "int")
        if(result != 0)
    ...
    GetChangeUnitChangeTime(pbItemId, pbChangeUnitId, pullTimestamp) {
        result := ComCall(4, this, "char*", pbItemId, "char*", pbChangeUnitId, "uint*", pullTimestamp, "int")
        if(result != 0)
            throw OSError(result)
