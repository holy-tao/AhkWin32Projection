 ...
 */
export default struct ISupportLastWriteTime extends IUnknown {
    /**
     ...
     * used for ISupportLastWriteTime interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        GetItemChangeTime       : IntPtr
        GetChangeUnitChangeTime : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ISupportLastWriteTime.Vtbl()
        }
    ...
    Query(iid) {
        if (ISupportLastWriteTime.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.GetItemChangeTime)
        CallbackFree(this.vtbl.GetChangeUnitChangeTime)
    }
}
