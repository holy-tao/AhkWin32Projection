...
class IRecoverableErrorData extends IUnknown{
     ...
     * @param {Pointer<UInt32>} pcchItemDisplayName 
     * @returns {HRESULT} 
     */
    GetItemDisplayName(pszItemDisplayName, pcchItemDisplayName) {
        ...
        result := ComCall(4, this, "ptr", pszItemDisplayName, "uint*", pcchItemDisplayName, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} pcchErrorDescription 
     * @returns {HRESULT} 
     */
    GetErrorDescription(pszErrorDescription, pcchErrorDescription) {
        ...
        result := ComCall(5, this, "ptr", pszErrorDescription, "uint*", pcchErrorDescription, "int")
        if(result != 0)
            throw OSError(result)
