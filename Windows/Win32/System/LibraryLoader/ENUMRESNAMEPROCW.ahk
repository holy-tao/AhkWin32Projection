...
/**
 * An application-defined callback function used with the EnumResourceNames and EnumResourceNamesEx functions. (Unicode)
 * @remarks
 ...
 * @see https://learn.microsoft.com/windows/win32/api/libloaderapi/nc-libloaderapi-enumresnameprocw
 * @namespace Windows.Win32.System.LibraryLoader
 * @charset Unicode
 */
export default struct ENUMRESNAMEPROCW {
    value : IntPtr

    __value {
        set {
            if (value is ENUMRESNAMEPROCW) {
     ...
     * @param {PWSTR} lpName Type: <b>LPTSTR</b>
     * 
    ...
    /**
     * A ENUMRESNAMEPROCW that invokes the given AHK function when called.
     * This callback is owned by the script and cleaned up automatically.
     */
    struct From extends ENUMRESNAMEPROCW {
        /**
         * Creates a ENUMRESNAMEPROCW pointer that invokes the given AHK function when called.
         * @param {Func(HMODULE, PWSTR, PWSTR, IntPtr) => BOOL} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 4)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 4 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , [HMODULE, PWSTR, PWSTR, IntPtr, BOOL])
        }
