...
#Import ".\LOAD_LIBRARY_FLAGS.ahk" { LOAD_LIBRARY_FLAGS }
 ...
 */
export DisableThreadLibraryCalls(hLibModule) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\DisableThreadLibraryCalls", HMODULE, hLibModule, BOOL)
 ...
 */
export FindResourceExW(_hModule, lpType, lpName, wLanguage) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    result := DllCall("KERNEL32.dll\FindResourceExW", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ushort", wLanguage, HRSRC)
    return result
}
 ...
 * @since windows5.1.2600
 */
export FreeLibraryAndExitThread(hLibModule, dwExitCode) {
    DllCall("KERNEL32.dll\FreeLibraryAndExitThread", HMODULE, hLibModule, "uint", dwExitCode)
}
 ...
 * @since windows5.0
 */
export FreeResource(hResData) {
    result := DllCall("KERNEL32.dll\FreeResource", HGLOBAL, hResData, BOOL)
    return result
}
 ...
 * @param {HMODULE} _hModule A handle to the loaded module whose path is being requested. If this parameter is <b>NULL</b>, 
...
export GetModuleFileNameA(_hModule, lpFilename, nSize) {
    ...
    result := DllCall("KERNEL32.dll\GetModuleFileNameA", HMODULE, _hModule, "ptr", lpFilename, "uint", nSize, UInt32)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetModuleFileNameW(_hModule, lpFilename, nSize) {
    ...
    result := DllCall("KERNEL32.dll\GetModuleFileNameW", HMODULE, _hModule, "ptr", lpFilename, "uint", nSize, UInt32)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetModuleHandleExA(dwFlags, lpModuleName, phModule) {
    ...
    result := DllCall("KERNEL32.dll\GetModuleHandleExA", "uint", dwFlags, "ptr", lpModuleName, HMODULE.Ptr, phModule, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetModuleHandleExW(dwFlags, lpModuleName, phModule) {
    ...
    result := DllCall("KERNEL32.dll\GetModuleHandleExW", "uint", dwFlags, "ptr", lpModuleName, HMODULE.Ptr, phModule, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @param {HMODULE} _hModule A handle to the DLL module that contains the function or variable. The 
 ...
 * @since windows5.1.2600
 */
export GetProcAddress(_hModule, lpProcName) {
    lpProcName := lpProcName is String ? StrPtr(lpProcName) : lpProcName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\GetProcAddress", HMODULE, _hModule, "ptr", lpProcName, IntPtr)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @param {LOAD_LIBRARY_FLAGS} dwFlags The action to be taken when loading the module. If no flags are specified, the behavior of this function is
 ...
 * @since windows5.1.2600
 */
export LoadLibraryExA(lpLibFileName, dwFlags) {
    ...
    result := DllCall("KERNEL32.dll\LoadLibraryExA", "ptr", lpLibFileName, HANDLE, hFile, LOAD_LIBRARY_FLAGS, dwFlags, HMODULE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export LoadLibraryExW(lpLibFileName, dwFlags) {
    static hFile := 0 ;Reserved parameters must always be NULL
    ...
    result := DllCall("KERNEL32.dll\LoadLibraryExW", "ptr", lpLibFileName, HANDLE, hFile, LOAD_LIBRARY_FLAGS, dwFlags, HMODULE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export LockResource(hResData) {
    result := DllCall("KERNEL32.dll\LockResource", HGLOBAL, hResData, IntPtr)
    return result
}
 ...
 * @since windows8.0
 */
export AddDllDirectory(NewDirectory) {
    NewDirectory := NewDirectory is String ? StrPtr(NewDirectory) : NewDirectory

    A_LastError := 0

    result := DllCall("KERNEL32.dll\AddDllDirectory", "ptr", NewDirectory, IntPtr)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
...
export RemoveDllDirectory(Cookie) {
    ...
    result := DllCall("KERNEL32.dll\RemoveDllDirectory", CookieMarshal, Cookie, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 *       <a href="https://support.microsoft.com/kb/2533623">KB2533623</a> must be 
 *       installed on the target platform.
 * @param {LOAD_LIBRARY_FLAGS} DirectoryFlags 
 * @returns {BOOL} If the function succeeds, the return value is nonzero.
 * 
 ...
 * @since windows8.0
 */
export SetDefaultDllDirectories(DirectoryFlags) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\SetDefaultDllDirectories", LOAD_LIBRARY_FLAGS, DirectoryFlags, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 */
export EnumResourceLanguagesExA(_hModule, lpType, lpName, lpEnumFunc, _lParam, dwFlags, LangId) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\EnumResourceLanguagesExA", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export EnumResourceLanguagesExW(_hModule, lpType, lpName, lpEnumFunc, _lParam, dwFlags, LangId) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\EnumResourceLanguagesExW", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceNamesExA(_hModule, lpType, lpEnumFunc, _lParam, dwFlags, LangId) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceNamesExA", HMODULE, _hModule, "ptr", lpType, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceNamesExW(_hModule, lpType, lpEnumFunc, _lParam, dwFlags, LangId) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceNamesExW", HMODULE, _hModule, "ptr", lpType, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceTypesExA(_hModule, lpEnumFunc, _lParam, dwFlags, LangId) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceTypesExA", HMODULE, _hModule, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceTypesExW(_hModule, lpEnumFunc, _lParam, dwFlags, LangId) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceTypesExW", HMODULE, _hModule, "ptr", lpEnumFunc, "ptr", _lParam, "uint", dwFlags, "ushort", LangId, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 */
export FindResourceW(_hModule, lpName, lpType) {
    lpName := lpName is String ? StrPtr(lpName) : lpName
    lpType := lpType is String ? StrPtr(lpType) : lpType
 ...
 * @since windows5.1.2600
 */
export LoadLibraryA(lpLibFileName) {
    lpLibFileName := lpLibFileName is String ? StrPtr(lpLibFileName) : lpLibFileName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\LoadLibraryA", "ptr", lpLibFileName, HMODULE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export LoadLibraryW(lpLibFileName) {
    lpLibFileName := lpLibFileName is String ? StrPtr(lpLibFileName) : lpLibFileName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\LoadLibraryW", "ptr", lpLibFileName, HMODULE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @param {HMODULE} _hModule Type: **HMODULE**
 * 
 ...
 * @param {Pointer} _lParam Type: **LONG_PTR**
 * 
 * An application-defined value passed to the callback function. This parameter can be used in error checking.
...
export EnumResourceNamesW(_hModule, lpType, lpEnumFunc, _lParam) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceNamesW", HMODULE, _hModule, "ptr", lpType, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    return result
}
...
export EnumResourceNamesA(_hModule, lpType, lpEnumFunc, _lParam) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceNamesA", HMODULE, _hModule, "ptr", lpType, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows8.0
 */
export LoadPackagedLibrary(lpwLibFileName) {
    ...
    result := DllCall("KERNEL32.dll\LoadPackagedLibrary", "ptr", lpwLibFileName, "uint", Reserved, HMODULE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 */
export QueryOptionalDelayLoadedAPI(hParentModule, lpDllName, lpProcName) {
    static Reserved := 0 ;Reserved parameters must always be NULL
    ...
    result := DllCall("api-ms-win-core-libraryloader-l2-1-0.dll\QueryOptionalDelayLoadedAPI", HMODULE, hParentModule, "ptr", lpDllName, "ptr", lpProcName, "uint", Reserved, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export FindResourceA(_hModule, lpName, lpType) {
    lpName := lpName is String ? StrPtr(lpName) : lpName
    lpType := lpType is String ? StrPtr(lpType) : lpType
 ...
 * @since windows5.0
 */
export FindResourceExA(_hModule, lpType, lpName, wLanguage) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\FindResourceExA", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ushort", wLanguage, HRSRC)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceTypesA(_hModule, lpEnumFunc, _lParam) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceTypesA", HMODULE, _hModule, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export EnumResourceTypesW(_hModule, lpEnumFunc, _lParam) {
    ...
    result := DllCall("KERNEL32.dll\EnumResourceTypesW", HMODULE, _hModule, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export EnumResourceLanguagesA(_hModule, lpType, lpName, lpEnumFunc, _lParam) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\EnumResourceLanguagesA", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export EnumResourceLanguagesW(_hModule, lpType, lpName, lpEnumFunc, _lParam) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\EnumResourceLanguagesW", HMODULE, _hModule, "ptr", lpType, "ptr", lpName, "ptr", lpEnumFunc, "ptr", _lParam, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export BeginUpdateResourceA(pFileName, bDeleteExistingResources) {
    pFileName := pFileName is String ? StrPtr(pFileName) : pFileName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\BeginUpdateResourceA", "ptr", pFileName, BOOL, bDeleteExistingResources, HANDLE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export BeginUpdateResourceW(pFileName, bDeleteExistingResources) {
    pFileName := pFileName is String ? StrPtr(pFileName) : pFileName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\BeginUpdateResourceW", "ptr", pFileName, BOOL, bDeleteExistingResources, HANDLE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export UpdateResourceA(hUpdate, lpType, lpName, wLanguage, lpData, cb) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\UpdateResourceA", HANDLE, hUpdate, "ptr", lpType, "ptr", lpName, "ushort", wLanguage, "ptr", lpData, "uint", cb, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export UpdateResourceW(hUpdate, lpType, lpName, wLanguage, lpData, cb) {
    lpType := lpType is String ? StrPtr(lpType) : lpType
    lpName := lpName is String ? StrPtr(lpName) : lpName

    A_LastError := 0

    result := DllCall("KERNEL32.dll\UpdateResourceW", HANDLE, hUpdate, "ptr", lpType, "ptr", lpName, "ushort", wLanguage, "ptr", lpData, "uint", cb, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export EndUpdateResourceA(hUpdate, fDiscard) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\EndUpdateResourceA", HANDLE, hUpdate, BOOL, fDiscard, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.0
 */
export EndUpdateResourceW(hUpdate, fDiscard) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\EndUpdateResourceW", HANDLE, hUpdate, BOOL, fDiscard, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetDllDirectoryA(nBufferLength, lpBuffer) {
    ...
    result := DllCall("KERNEL32.dll\GetDllDirectoryA", "uint", nBufferLength, "ptr", lpBuffer, UInt32)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
...
export GetDllDirectoryW(nBufferLength, lpBuffer) {
    ...
    result := DllCall("KERNEL32.dll\GetDllDirectoryW", "uint", nBufferLength, "ptr", lpBuffer, UInt32)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
