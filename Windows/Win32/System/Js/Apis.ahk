...
export JsCreateContext(runtime, debugApplication, newContext) {
    runtimeMarshal := runtime is VarRef ? "ptr" : IntPtr
...
export JsCreateRuntime(attributes, runtimeVersion, threadService, runtime) {
    threadServiceMarshal := threadService == 0 ? IntPtr : JsThreadServiceCallback
    runtimeMarshal := runtime is VarRef ? "ptr*" : IntPtr

    result := DllCall("chakra.dll\JsCreateRuntime", JsRuntimeAttributes, attributes, JsRuntimeVersion, runtimeVersion, threadServiceMarshal, threadService, runtimeMarshal, runtime, JsErrorCode)
    return result
}
...
export JsGetRuntimeMemoryUsage(runtime, memoryUsage) {
    runtimeMarshal := runtime is VarRef ? "ptr" : IntPtr
    memoryUsageMarshal := memoryUsage is VarRef ? "ptr*" : IntPtr
...
export JsGetRuntimeMemoryLimit(runtime, memoryLimit) {
    runtimeMarshal := runtime is VarRef ? "ptr" : IntPtr
    memoryLimitMarshal := memoryLimit is VarRef ? "ptr*" : IntPtr
...
export JsAddRef(ref, count) {
    refMarshal := ref is VarRef ? "ptr" : IntPtr
...
export JsGetCurrentContext(currentContext) {
    currentContextMarshal := currentContext is VarRef ? "ptr*" : IntPtr
...
export JsIdle(nextIdleTick) {
    nextIdleTickMarshal := nextIdleTick is VarRef ? "uint*" : IntPtr
    nextIdleTickMarshal := nextIdleTick == 0 ? IntPtr : "uint*"
...
export JsGetPropertyIdFromName(name, propertyId) {
    ...
    propertyIdMarshal := propertyId is VarRef ? "ptr*" : IntPtr
...
export JsGetPropertyNameFromId(propertyId, name) {
    propertyIdMarshal := propertyId is VarRef ? "ptr" : IntPtr
    nameMarshal := name is VarRef ? "ptr*" : IntPtr
...
export JsGetUndefinedValue(undefinedValue) {
    undefinedValueMarshal := undefinedValue is VarRef ? "ptr*" : IntPtr
...
export JsGetNullValue(nullValue) {
    nullValueMarshal := nullValue is VarRef ? "ptr*" : IntPtr
...
export JsGetTrueValue(trueValue) {
    trueValueMarshal := trueValue is VarRef ? "ptr*" : IntPtr
...
export JsGetFalseValue(falseValue) {
    falseValueMarshal := falseValue is VarRef ? "ptr*" : IntPtr
...
export JsBoolToBoolean(value, booleanValue) {
    booleanValueMarshal := booleanValue is VarRef ? "ptr*" : IntPtr
...
export JsBooleanToBool(value, boolValue) {
    ...
    boolValueMarshal := boolValue is VarRef ? "int*" : IntPtr
...
export JsNumberToDouble(value, doubleValue) {
    valueMarshal := value is VarRef ? "ptr" : IntPtr
    doubleValueMarshal := doubleValue is VarRef ? "double*" : IntPtr
...
export JsConvertValueToNumber(value, numberValue) {
    valueMarshal := value is VarRef ? "ptr" : IntPtr
    numberValueMarshal := numberValue is VarRef ? "ptr*" : IntPtr
...
export JsGetStringLength(stringValue, length) {
    stringValueMarshal := stringValue is VarRef ? "ptr" : IntPtr
...
export JsStringToPointer(value, stringValue, stringLength) {
    valueMarshal := value is VarRef ? "ptr" : IntPtr
    stringValueMarshal := stringValue is VarRef ? "ptr*" : IntPtr
    stringLengthMarshal := stringLength is VarRef ? "ptr*" : IntPtr
...
export JsGetGlobalObject(globalObject) {
    globalObjectMarshal := globalObject is VarRef ? "ptr*" : IntPtr
...
export JsCreateExternalObject(data, finalizeCallback, _object) {
    ...
    finalizeCallbackMarshal := finalizeCallback == 0 ? IntPtr : JsFinalizeCallback
    _objectMarshal := _object is VarRef ? "ptr*" : IntPtr

    result := DllCall("chakra.dll\JsCreateExternalObject", dataMarshal, data, finalizeCallbackMarshal, finalizeCallback, _objectMarshal, _object, JsErrorCode)
    return result
}
...
export JsGetPrototype(_object, prototypeObject) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    prototypeObjectMarshal := prototypeObject is VarRef ? "ptr*" : IntPtr
...
export JsGetOwnPropertyDescriptor(_object, propertyId, propertyDescriptor) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    propertyIdMarshal := propertyId is VarRef ? "ptr" : IntPtr
    propertyDescriptorMarshal := propertyDescriptor is VarRef ? "ptr*" : IntPtr
...
export JsHasProperty(_object, propertyId, hasProperty) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    propertyIdMarshal := propertyId is VarRef ? "ptr" : IntPtr
    hasPropertyMarshal := hasProperty is VarRef ? "int*" : IntPtr
...
export JsHasIndexedProperty(_object, index, result) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    indexMarshal := index is VarRef ? "ptr" : IntPtr
    resultMarshal := result is VarRef ? "int*" : IntPtr
...
export JsEquals(object1, object2, result) {
    object1Marshal := object1 is VarRef ? "ptr" : IntPtr
    object2Marshal := object2 is VarRef ? "ptr" : IntPtr
    resultMarshal := result is VarRef ? "int*" : IntPtr
...
export JsGetExternalData(_object, externalData) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    externalDataMarshal := externalData is VarRef ? "ptr*" : IntPtr
...
export JsSetExternalData(_object, externalData) {
    _objectMarshal := _object is VarRef ? "ptr" : IntPtr
    externalDataMarshal := externalData is VarRef ? "ptr" : IntPtr
    externalDataMarshal := externalData == 0 ? IntPtr : "ptr"
...
export JsCallFunction(function, arguments, argumentCount, result) {
    functionMarshal := function is VarRef ? "ptr" : IntPtr
    argumentsMarshal := arguments is VarRef ? "ptr*" : IntPtr
    resultMarshal := result is VarRef ? "ptr*" : IntPtr
    resultMarshal := result == 0 ? IntPtr : "ptr*"
...
export JsHasException(hasException) {
    hasExceptionMarshal := hasException is VarRef ? "int*" : IntPtr
...
export JsIsRuntimeExecutionDisabled(runtime, isDisabled) {
    runtimeMarshal := runtime is VarRef ? "ptr" : IntPtr
    isDisabledMarshal := isDisabled is VarRef ? "int*" : IntPtr
...
export JsIsEnumeratingHeap(isEnumeratingHeap) {
    isEnumeratingHeapMarshal := isEnumeratingHeap is VarRef ? "int*" : IntPtr
