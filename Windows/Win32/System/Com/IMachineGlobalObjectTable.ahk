...
class IMachineGlobalObjectTable extends IUnknown{
     ...
     */
    RegisterObject(clsid, identifier, object_) {
        identifier := identifier is String ? StrPtr(identifier) : identifier

        result := ComCall(3, this, "ptr", clsid, "ptr", identifier, "ptr", object_, "ptr*", &token := 0, "int")
        if(result != 0) {
    ...
    GetObject(clsid, identifier, riid) {
        ...
        result := ComCall(4, this, "ptr", clsid, "ptr", identifier, "ptr", riid, "ptr*", &ppv := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
