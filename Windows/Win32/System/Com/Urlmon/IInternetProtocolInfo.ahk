...
class IInternetProtocolInfo extends IUnknown{
    ...
    ParseUrl(pwzUrl, ParseAction, dwParseFlags, pwzResult, cchResult, pcchResult, dwReserved) {
        ...
        result := ComCall(3, this, "ptr", pwzUrl, "int", ParseAction, "uint", dwParseFlags, "ptr", pwzResult, "uint", cchResult, "uint*", pcchResult, "uint", dwReserved, "int")
        if(result != 0)
    ...
    CombineUrl(pwzBaseUrl, pwzRelativeUrl, dwCombineFlags, pwzResult, cchResult, pcchResult, dwReserved) {
        ...
        result := ComCall(4, this, "ptr", pwzBaseUrl, "ptr", pwzRelativeUrl, "uint", dwCombineFlags, "ptr", pwzResult, "uint", cchResult, "uint*", pcchResult, "uint", dwReserved, "int")
        if(result != 0)
            throw OSError(result)
    ...
    QueryInfo(pwzUrl, OueryOption, dwQueryFlags, pBuffer, cbBuffer, pcbBuf, dwReserved) {
        ...
        result := ComCall(6, this, "ptr", pwzUrl, "int", OueryOption, "uint", dwQueryFlags, "ptr", pBuffer, "uint", cbBuffer, "uint*", pcbBuf, "uint", dwReserved, "int")
        if(result != 0)
            throw OSError(result)
