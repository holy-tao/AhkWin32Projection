...
class IInternetProtocolSink extends IUnknown{
    ...
    Switch(pProtocolData) {
        result := ComCall(3, this, "ptr", pProtocolData, "int")
        if(result != 0)
    ...
    ReportProgress(ulStatusCode, szStatusText) {
        ...
        result := ComCall(4, this, "uint", ulStatusCode, "ptr", szStatusText, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ReportData(grfBSCF, ulProgress, ulProgressMax) {
        result := ComCall(5, this, "uint", grfBSCF, "uint", ulProgress, "uint", ulProgressMax, "int")
        if(result != 0)
            throw OSError(result)
    ...
    ReportResult(hrResult, dwError, szResult) {
        ...
        result := ComCall(6, this, "int", hrResult, "uint", dwError, "ptr", szResult, "int")
        if(result != 0)
            throw OSError(result)
