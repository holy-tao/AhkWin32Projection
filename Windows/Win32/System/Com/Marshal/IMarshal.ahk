 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nn-objidl-imarshal
...
class IMarshal extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-getunmarshalclass
     */
    GetUnmarshalClass(riid, pv, dwDestContext, mshlflags_) {
        static pvDestContext := 0 ;Reserved parameters must always be NULL

        pvMarshal := pv is VarRef ? "ptr" : "ptr"

        pCid := Guid()
        result := ComCall(3, this, "ptr", riid, pvMarshal, pv, "uint", dwDestContext, "ptr", pvDestContext, "uint", mshlflags_, "ptr", pCid, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-getmarshalsizemax
     */
    GetMarshalSizeMax(riid, pv, dwDestContext, mshlflags_) {
        static pvDestContext := 0 ;Reserved parameters must always be NULL

        pvMarshal := pv is VarRef ? "ptr" : "ptr"

        result := ComCall(4, this, "ptr", riid, pvMarshal, pv, "uint", dwDestContext, "ptr", pvDestContext, "uint", mshlflags_, "uint*", &pSize := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    /**
     * The IMarshal::MarshalInterface method (objidl.h) marshals an interface pointer.
     * @remarks
     * This method is called indirectly, in  a call to <a href="https://docs.microsoft.com/windows/desktop/api/combaseapi/nf-combaseapi-comarshalinterface">CoMarshalInterface</a>, by whatever code in the server process is responsible for marshaling a pointer to an interface on an object. This marshaling code is usually a stub generated by COM for one of several interfaces that can marshal a pointer to an interface implemented on an entirely different object. Examples include the <a href="https://docs.microsoft.com/windows/desktop/api/unknwnbase/nn-unknwnbase-iclassfactory">IClassFactory</a> and <a href="https://docs.microsoft.com/windows/desktop/api/oleidl/nn-oleidl-ioleitemcontainer">IOleItemContainer</a> interfaces. For purposes of discussion, the code responsible for marshaling a pointer is called the <i>marshaling stub</i>.
     * 
     * <h3><a id="Notes_to_Callers"></a><a id="notes_to_callers"></a><a id="NOTES_TO_CALLERS"></a>Notes to Callers</h3>
     ...
     * </tr>
     * </table>
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-marshalinterface
     */
    MarshalInterface(pStm, riid, pv, dwDestContext, mshlflags_) {
        static pvDestContext := 0 ;Reserved parameters must always be NULL

        pvMarshal := pv is VarRef ? "ptr" : "ptr"

        result := ComCall(5, this, "ptr", pStm, "ptr", riid, pvMarshal, pv, "uint", dwDestContext, "ptr", pvDestContext, "uint", mshlflags_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    /**
     * The IMarshal::UnmarshalInterface method (objidl.h) unmarshals an interface pointer.
     * @remarks
     * The COM library in the process where unmarshaling is to occur calls the proxy's implementation of this method.
     * 
     * <h3><a id="Notes_to_Callers"></a><a id="notes_to_callers"></a><a id="NOTES_TO_CALLERS"></a>Notes to Callers</h3>
     * You do not call this method directly. There are, however, some situations in which you might call it indirectly through a call to <a href="https://docs.microsoft.com/windows/desktop/api/combaseapi/nf-combaseapi-counmarshalinterface">CoUnmarshalInterface</a>. For example, if you are implementing a stub, your implementation would call <b>CoUnmarshalInterface</b> when the stub receives an interface pointer as a parameter in a method call.
     * 
     * <h3><a id="Notes_to_Implementers"></a><a id="notes_to_implementers"></a><a id="NOTES_TO_IMPLEMENTERS"></a>Notes to Implementers</h3>
     * The proxy's implementation should read the data written to the stream by the original object's implementation of <a href="https://docs.microsoft.com/windows/desktop/api/objidl/nf-objidl-imarshal-marshalinterface">IMarshal::MarshalInterface</a> and use that data to initialize the proxy object whose CLSID was returned by the marshaling stub's call to the original object's implementation of <a href="https://docs.microsoft.com/windows/desktop/api/objidl/nf-objidl-imarshal-getunmarshalclass">IMarshal::GetUnmarshalClass</a>.
     * 
     * To return the appropriate interface pointer, the proxy implementation can simply call <a href="https://docs.microsoft.com/windows/desktop/api/unknwn/nf-unknwn-iunknown-queryinterface(q)">QueryInterface</a> on itself, passing the <i>riid</i> and <i>ppv</i> parameters. However, your implementation of <b>UnmarshalInterface</b> is free to create a different object and, if necessary, return a pointer to it.
     * 
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-unmarshalinterface
     */
    UnmarshalInterface(pStm, riid) {
        result := ComCall(6, this, "ptr", pStm, "ptr", riid, "ptr*", &ppv := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    /**
     * The IMarshal::ReleaseMarshalData method (objidl.h) destroys a marshaled data packet.
     * @remarks
     * If an object's marshaled data packet does not get unmarshaled in the client process space and the packet is no longer needed, the client calls <b>ReleaseMarshalData</b> on the proxy's <a href="https://docs.microsoft.com/windows/desktop/api/objidl/nn-objidl-imarshal">IMarshal</a> implementation to instruct the object to destroy the data packet. The call occurs within the <a href="https://docs.microsoft.com/windows/desktop/api/combaseapi/nf-combaseapi-coreleasemarshaldata">CoReleaseMarshalData</a> function. The data packet serves as an additional reference on the object, and releasing the data is like releasing an interface pointer by calling <a href="https://docs.microsoft.com/windows/desktop/api/unknwn/nf-unknwn-iunknown-release">Release</a>.
     * 
     ...
     * @returns {HRESULT} This method can return the standard return values S_OK and E_FAIL, as well as any of the stream-access errors for the <a href="https://docs.microsoft.com/windows/desktop/api/objidl/nn-objidl-istream">IStream</a> interface.
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-releasemarshaldata
     */
    ReleaseMarshalData(pStm) {
        result := ComCall(7, this, "ptr", pStm, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    /**
     * The IMarshal::DisconnectObject method (objidl.h) releases all connections to an object prior to shutting down.
     * @remarks
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/objidl/nf-objidl-imarshal-disconnectobject
     */
    DisconnectObject(dwReserved) {
        result := ComCall(8, this, "uint", dwReserved, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
