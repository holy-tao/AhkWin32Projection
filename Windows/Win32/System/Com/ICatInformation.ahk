...
class ICatInformation extends IUnknown{
    ...
    EnumCategories(lcid, ppenumCategoryInfo) {
        result := ComCall(3, this, "uint", lcid, "ptr", ppenumCategoryInfo, "int")
        if(result != 0)
    ...
    GetCategoryDesc(rcatid, lcid, pszDesc) {
        result := ComCall(4, this, "ptr", rcatid, "uint", lcid, "ptr", pszDesc, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumClassesOfCategories(cImplemented, rgcatidImpl, cRequired, rgcatidReq, ppenumClsid) {
        result := ComCall(5, this, "uint", cImplemented, "ptr", rgcatidImpl, "uint", cRequired, "ptr", rgcatidReq, "ptr", ppenumClsid, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumImplCategoriesOfClass(rclsid, ppenumCatid) {
        result := ComCall(7, this, "ptr", rclsid, "ptr", ppenumCatid, "int")
        if(result != 0)
            throw OSError(result)
    ...
    EnumReqCategoriesOfClass(rclsid, ppenumCatid) {
        result := ComCall(8, this, "ptr", rclsid, "ptr", ppenumCatid, "int")
        if(result != 0)
            throw OSError(result)
