...
class IClientSecurity extends IUnknown{
     ...
     * @param {Pointer<UInt32>} pCapabilites 
     * @returns {HRESULT} 
     */
    QueryBlanket(pProxy, pAuthnSvc, pAuthzSvc, pServerPrincName, pAuthnLevel, pImpLevel, pAuthInfo, pCapabilites) {
        result := ComCall(3, this, "ptr", pProxy, "uint*", pAuthnSvc, "uint*", pAuthzSvc, "ushort*", pServerPrincName, "uint*", pAuthnLevel, "uint*", pImpLevel, "ptr", pAuthInfo, "uint*", pCapabilites, "int")
        if(result != 0)
    ...
    SetBlanket(pProxy, dwAuthnSvc, dwAuthzSvc, pServerPrincName, dwAuthnLevel, dwImpLevel, pAuthInfo, dwCapabilities) {
        ...
        result := ComCall(4, this, "ptr", pProxy, "uint", dwAuthnSvc, "uint", dwAuthzSvc, "ptr", pServerPrincName, "uint", dwAuthnLevel, "uint", dwImpLevel, "ptr", pAuthInfo, "uint", dwCapabilities, "int")
        if(result != 0)
            throw OSError(result)
    ...
    CopyProxy(pProxy, ppCopy) {
        result := ComCall(5, this, "ptr", pProxy, "ptr", ppCopy, "int")
        if(result != 0)
            throw OSError(result)
