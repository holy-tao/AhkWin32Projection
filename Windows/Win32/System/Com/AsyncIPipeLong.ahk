 ...
 */
export default struct AsyncIPipeLong extends IUnknown {
    /**
     * The interface identifier for AsyncIPipeLong
     * @type {Guid}
     */
    static IID := Guid("{db2f3acd-2f86-11d1-8e04-00c04fb9989a}")

    /**
     * The class identifier for AsyncIPipeLong
     * @type {Guid}
     */
    static CLSID := Guid("{db2f3acd-2f86-11d1-8e04-00c04fb9989a}")
     ...
     * used for AsyncIPipeLong interfaces
    */
    ...
    Finish_Pull(buf, pcReturned) {
        bufMarshal := buf is VarRef ? "int*" : IntPtr
    ...
    _Query(iid) {
        if (AsyncIPipeLong.IID.Equals(iid)) {
            return true
        }
