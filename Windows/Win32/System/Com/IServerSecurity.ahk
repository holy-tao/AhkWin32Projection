 ...
 */
export default struct IServerSecurity extends IUnknown {
    /**
     ...
     * used for IServerSecurity interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        QueryBlanket      : IntPtr
        ImpersonateClient : IntPtr
        RevertToSelf      : IntPtr
        IsImpersonating   : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := IServerSecurity.Vtbl()
        }
    ...
    Query(iid) {
        if (IServerSecurity.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        CallbackFree(this.vtbl.ImpersonateClient)
        CallbackFree(this.vtbl.RevertToSelf)
        CallbackFree(this.vtbl.IsImpersonating)
    }
}
