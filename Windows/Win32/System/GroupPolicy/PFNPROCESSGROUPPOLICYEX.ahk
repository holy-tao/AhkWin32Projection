 ...
 * <div> </div>
 * @see https://learn.microsoft.com/windows/win32/api/userenv/nc-userenv-pfnprocessgrouppolicyex
 * @namespace Windows.Win32.System.GroupPolicy
 */
export default struct PFNPROCESSGROUPPOLICYEX {
    value : IntPtr

    __value {
        set {
            if (value is PFNPROCESSGROUPPOLICYEX) {
     ...
     * @param {IWbemServices} pWbemServices Specifies a WMI services pointer to the RSoP namespace to which the policy data should be written. This parameter is <b>NULL</b> when RSoP logging is disabled, indicating that the extension should not log RSoP data.
     * @param {Pointer<HRESULT>} pRsopStatus Pointer to an <b>HRESULT</b> return code that indicates whether RSoP logging was successful.
     ...
     */
    Call(dwFlags, hToken, hKeyRoot, pDeletedGPOList, pChangedGPOList, pHandle, pbAbort, pStatusCallback, pWbemServices, pRsopStatus) {
        pbAbortMarshal := pbAbort is VarRef ? "int*" : IntPtr
        pStatusCallbackMarshal := pStatusCallback == 0 ? IntPtr : PFNSTATUSMESSAGECALLBACK
        pWbemServicesMarshal := pWbemServices == 0 ? IntPtr : "ptr"
        pRsopStatusMarshal := pRsopStatus is VarRef ? "int*" : IntPtr

        result := DllCall(this.value, UInt32, dwFlags, HANDLE, hToken, HKEY, hKeyRoot, GROUP_POLICY_OBJECTA.Ptr, pDeletedGPOList, GROUP_POLICY_OBJECTA.Ptr, pChangedGPOList, IntPtr, pHandle, pbAbortMarshal, pbAbort, pStatusCallbackMarshal, pStatusCallback, pWbemServicesMarshal, pWbemServices, pRsopStatusMarshal, pRsopStatus, UInt32)
        return result
    }

    /**
     * A PFNPROCESSGROUPPOLICYEX that invokes the given AHK function when called.
