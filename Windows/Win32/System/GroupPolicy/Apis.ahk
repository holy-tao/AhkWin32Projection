 ...
 */
export RefreshPolicy(bMachine) {
    A_LastError := 0

    result := DllCall("USERENV.dll\RefreshPolicy", BOOL, bMachine, BOOL)
 ...
 * @since windows6.0.6000
 */
export EnterCriticalPolicySection(bMachine) {
    A_LastError := 0

    result := DllCall("USERENV.dll\EnterCriticalPolicySection", BOOL, bMachine, HANDLE.Owned)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export LeaveCriticalPolicySection(hSection) {
    A_LastError := 0

    result := DllCall("USERENV.dll\LeaveCriticalPolicySection", HANDLE, hSection, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export RegisterGPNotification(hEvent, bMachine) {
    A_LastError := 0

    result := DllCall("USERENV.dll\RegisterGPNotification", HANDLE, hEvent, BOOL, bMachine, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export UnregisterGPNotification(hEvent) {
    A_LastError := 0

    result := DllCall("USERENV.dll\UnregisterGPNotification", HANDLE, hEvent, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export GetGPOListA(hToken, lpName, lpHostName, lpComputerName, dwFlags, pGPOList) {
 ...
 * @since windows6.0.6000
 */
export GetGPOListW(hToken, lpName, lpHostName, lpComputerName, dwFlags, pGPOList) {
    lpName := lpName is String ? StrPtr(lpName) : lpName
    lpHostName := lpHostName is String ? StrPtr(lpHostName) : lpHostName
    lpComputerName := lpComputerName is String ? StrPtr(lpComputerName) : lpComputerName
 ...
 * @since windows6.0.6000
 */
export FreeGPOListA(pGPOList) {
    A_LastError := 0

    result := DllCall("USERENV.dll\FreeGPOListA", GROUP_POLICY_OBJECTA.Ptr, pGPOList, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export FreeGPOListW(pGPOList) {
    A_LastError := 0

    result := DllCall("USERENV.dll\FreeGPOListW", GROUP_POLICY_OBJECTW.Ptr, pGPOList, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export InstallApplication(pInstallInfo) {
    result := DllCall("ADVAPI32.dll\InstallApplication", INSTALLDATA.Ptr, pInstallInfo, UInt32)
    return result
}
 ...
 * @returns {Integer} 
 */
export CommandLineFromMsiDescriptor(Descriptor, CommandLine, CommandLineLength) {
 ...
 * @since windows6.0.6000
 */
export CreateGPOLink(lpGPO, lpContainer, fHighPriority) {
    lpGPO := lpGPO is String ? StrPtr(lpGPO) : lpGPO
    lpContainer := lpContainer is String ? StrPtr(lpContainer) : lpContainer

    result := DllCall("GPEDIT.dll\CreateGPOLink", "ptr", lpGPO, "ptr", lpContainer, BOOL, fHighPriority, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export DeleteGPOLink(lpGPO, lpContainer) {
    lpGPO := lpGPO is String ? StrPtr(lpGPO) : lpGPO
    lpContainer := lpContainer is String ? StrPtr(lpContainer) : lpContainer
 ...
 * @since windows6.0.6000
 */
export DeleteAllGPOLinks(lpContainer) {
    lpContainer := lpContainer is String ? StrPtr(lpContainer) : lpContainer
 ...
 * @since windows6.0.6000
 */
export BrowseForGPO(lpBrowseInfo) {
    result := DllCall("GPEDIT.dll\BrowseForGPO", GPOBROWSEINFO.Ptr, lpBrowseInfo, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export ImportRSoPData(lpNameSpace, lpFileName) {
 ...
 * @since windows6.0.6000
 */
export ExportRSoPData(lpNameSpace, lpFileName) {
    lpNameSpace := lpNameSpace is String ? StrPtr(lpNameSpace) : lpNameSpace
    lpFileName := lpFileName is String ? StrPtr(lpFileName) : lpFileName
