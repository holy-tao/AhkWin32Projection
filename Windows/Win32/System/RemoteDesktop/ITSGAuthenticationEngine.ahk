 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/tsgauthenticationengine/nn-tsgauthenticationengine-itsgauthenticationengine
...
class ITSGAuthenticationEngine extends IUnknown{
     ...
     * @param {Pointer} context_ A pointer to  a <b>ULONG</b> that contains  a value specific to this connection. This 
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgauthenticationengine/nf-tsgauthenticationengine-itsgauthenticationengine-authenticateuser
     */
    AuthenticateUser(mainSessionId, cookieData, numCookieBytes, context_, pSink) {
        cookieDataMarshal := cookieData is VarRef ? "char*" : "ptr"

        result := ComCall(3, this, "ptr", mainSessionId, cookieDataMarshal, cookieData, "uint", numCookieBytes, "ptr", context_, "ptr", pSink, "int")
        if(result != 0) {
     ...
     * @param {Pointer} context_ A pointer to  a <b>ULONG</b> that contains  a value that identifies this connection. This value should be passed back to RD Gateway by using the methods of the  <a href="https://docs.microsoft.com/windows/desktop/api/tsgauthenticationengine/nn-tsgauthenticationengine-itsgauthenticateusersink">ITSGAuthenticateUserSink</a> interface.
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgauthenticationengine/nf-tsgauthenticationengine-itsgauthenticationengine-cancelauthentication
     */
    CancelAuthentication(mainSessionId, context_) {
        result := ComCall(4, this, "ptr", mainSessionId, "ptr", context_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
