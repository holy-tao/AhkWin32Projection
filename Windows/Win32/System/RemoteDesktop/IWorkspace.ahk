...
class IWorkspace extends IUnknown{
    ...
    /**
     * 
     * @param {Pointer<SAFEARRAY>} psaWkspNames 
     * @returns {HRESULT} 
     */
    GetWorkspaceNames(psaWkspNames) {
        result := ComCall(3, this, "ptr", psaWkspNames, "int")
        if(result != 0)
     ...
     * @param {Pointer<UInt32>} pulProcessId 
    ...
    GetProcessId(pulProcessId) {
        result := ComCall(5, this, "uint*", pulProcessId, "int")
        if(result != 0)
            throw OSError(result)
