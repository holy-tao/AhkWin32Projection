...
class IWTSSBPlugin extends IUnknown{
     ...
     * @param {Pointer<UInt32>} PluginCapabilities 
     * @returns {HRESULT} <ul>
    ...
    Initialize(PluginCapabilities) {
        result := ComCall(3, this, "uint*", PluginCapabilities, "int")
        if(result != 0)
     ...
     * @param {Pointer<Int32>} pMachineId 
     * @returns {HRESULT} 
     */
    WTSSBX_GetMostSuitableServer(UserName, DomainName, ApplicationType, FarmName, pMachineId) {
        ...
        result := ComCall(6, this, "ptr", UserName, "ptr", DomainName, "ptr", ApplicationType, "ptr", FarmName, "int*", pMachineId, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<UInt32>} pSessionId 
    ...
    WTSSBX_GetUserExternalSession(UserName, DomainName, ApplicationType, RedirectorInternalIP, pSessionId, pMachineConnectInfo) {
        ...
        result := ComCall(8, this, "ptr", UserName, "ptr", DomainName, "ptr", ApplicationType, "ptr", RedirectorInternalIP, "uint*", pSessionId, "ptr", pMachineConnectInfo, "int")
        if(result != 0)
            throw OSError(result)
