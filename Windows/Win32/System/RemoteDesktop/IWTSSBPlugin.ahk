 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nn-tssbx-iwtssbplugin
...
class IWTSSBPlugin extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-initialize
     */
    Initialize() {
        result := ComCall(3, this, "uint*", &PluginCapabilities := 0, "int")
        if(result != 0) {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-wtssbx_machinechangenotification
     */
    WTSSBX_MachineChangeNotification(NotificationType, MachineId, pMachineInfo) {
        result := ComCall(4, this, "int", NotificationType, "int", MachineId, "ptr", pMachineInfo, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} Returns <b>S_OK</b> if successful.
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-wtssbx_sessionchangenotification
     */
    WTSSBX_SessionChangeNotification(NotificationType, MachineId, NumOfSessions, SessionInfo) {
        result := ComCall(5, this, "int", NotificationType, "int", MachineId, "uint", NumOfSessions, "ptr", SessionInfo, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @param {PWSTR} ApplicationType_ A pointer to a Unicode string that contains the name of the program that Remote Desktop Services runs after it creates the session.
     ...
     * @returns {HRESULT} Returns <b>S_OK</b> if successful.
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-wtssbx_getmostsuitableserver
     */
    WTSSBX_GetMostSuitableServer(UserName, DomainName, ApplicationType_, FarmName, pMachineId) {
        UserName := UserName is String ? StrPtr(UserName) : UserName
        DomainName := DomainName is String ? StrPtr(DomainName) : DomainName
        ApplicationType_ := ApplicationType_ is String ? StrPtr(ApplicationType_) : ApplicationType_
        FarmName := FarmName is String ? StrPtr(FarmName) : FarmName

        pMachineIdMarshal := pMachineId is VarRef ? "int*" : "ptr"

        result := ComCall(6, this, "ptr", UserName, "ptr", DomainName, "ptr", ApplicationType_, "ptr", FarmName, pMachineIdMarshal, pMachineId, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * Your implementation of this method must return <b>S_OK</b> immediately if successful.
     * @returns {HRESULT} Returns <b>S_OK</b> if successful.
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-terminated
     */
    Terminated() {
     ...
     * @param {PWSTR} ApplicationType_ A pointer to a Unicode string  that contains the program that Remote Desktop Services runs after the user session is created.
     ...
     * @returns {HRESULT} Returns <b>S_OK</b> if successful.
     * @see https://learn.microsoft.com/windows/win32/api//content/tssbx/nf-tssbx-iwtssbplugin-wtssbx_getuserexternalsession
     */
    WTSSBX_GetUserExternalSession(UserName, DomainName, ApplicationType_, RedirectorInternalIP, pSessionId, pMachineConnectInfo) {
        UserName := UserName is String ? StrPtr(UserName) : UserName
        DomainName := DomainName is String ? StrPtr(DomainName) : DomainName
        ApplicationType_ := ApplicationType_ is String ? StrPtr(ApplicationType_) : ApplicationType_
        ...
        result := ComCall(8, this, "ptr", UserName, "ptr", DomainName, "ptr", ApplicationType_, "ptr", RedirectorInternalIP, pSessionIdMarshal, pSessionId, "ptr", pMachineConnectInfo, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
