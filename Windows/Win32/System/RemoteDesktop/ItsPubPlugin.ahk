...
class ItsPubPlugin extends IUnknown{
     ...
     * @param {Pointer<pluginResource>} resourceList 
     * @returns {HRESULT} 
     */
    GetResourceList(userID, pceAppListSize, resourceList) {
        userID := userID is String ? StrPtr(userID) : userID

        result := ComCall(3, this, "ptr", userID, "int*", pceAppListSize, "ptr", resourceList, "int")
        if(result != 0)
    ...
    /**
     * 
     * @param {Pointer<UInt64>} lastUpdateTime 
     * @returns {HRESULT} 
     */
    GetCacheLastUpdateTime(lastUpdateTime) {
        result := ComCall(5, this, "uint*", lastUpdateTime, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<UInt32>} resourceType 
    ...
    ResolveResource(resourceType, resourceLocation, endPointName, userID, alias) {
        ...
        result := ComCall(8, this, "uint*", resourceType, "ptr", resourceLocation, "ptr", endPointName, "ptr", userID, "ptr", alias, "int")
        if(result != 0)
            throw OSError(result)
