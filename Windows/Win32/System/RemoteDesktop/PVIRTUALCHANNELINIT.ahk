...
export default struct PVIRTUALCHANNELINIT {
    ...
    Call(ppInitHandle, pChannel, channelCount, versionRequested, pChannelInitEventProc) {
        ...
        result := DllCall(this.value, ppInitHandleMarshal, ppInitHandle, CHANNEL_DEF.Ptr, pChannel, Int32, channelCount, UInt32, versionRequested, "ptr", pChannelInitEventProc, UInt32)
        return result
    }
    ...
    struct From extends PVIRTUALCHANNELINIT {
         ...
         * @param {Func("ptr*", CHANNEL_DEF, Int32, UInt32, "ptr") => UInt32} fn the function to invoke.
         */
        __New(fn) {
            if (!HasMethod(fn, , 5)) {
                throw MethodError("Object of type " Type(fn) " is not callable with 5 parameters.", -1, fn)
            }
            this.value := CallbackCreate(fn, , ["ptr*", CHANNEL_DEF.Ptr, Int32, UInt32, "ptr", UInt32])
        }
