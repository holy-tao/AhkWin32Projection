 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/tsgpolicyengine/nn-tsgpolicyengine-itsgpolicyengine
...
class ITSGPolicyEngine extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgpolicyengine/nf-tsgpolicyengine-itsgpolicyengine-authorizeconnection
     */
    AuthorizeConnection(mainSessionId, username, authType, clientMachineIP, clientMachineName, sohData, numSOHBytes, cookieData, numCookieBytes, userToken, pSink) {
        ...
        if(clientMachineIP is String) {
            pin := BSTR.Alloc(clientMachineIP)
            clientMachineIP := pin.Value
        }
        if(clientMachineName is String) {
            pin := BSTR.Alloc(clientMachineName)
            clientMachineName := pin.Value
        }
        ...
        result := ComCall(3, this, "ptr", mainSessionId, "ptr", username, "int", authType, "ptr", clientMachineIP, "ptr", clientMachineName, sohDataMarshal, sohData, "uint", numSOHBytes, cookieDataMarshal, cookieData, "uint", numCookieBytes, "ptr", userToken, "ptr", pSink, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} If this method succeeds, it returns <b>S_OK</b>. Otherwise, it returns an <b>HRESULT</b> error code.
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgpolicyengine/nf-tsgpolicyengine-itsgpolicyengine-authorizeresource
     */
    AuthorizeResource(mainSessionId, subSessionId, username, resourceNames, numResources, alternateResourceNames, numAlternateResourceName, portNumber, operation, cookie, numBytesInCookie, pSink) {
        ...
        if(operation is String) {
            pin := BSTR.Alloc(operation)
            operation := pin.Value
        }

        cookieMarshal := cookie is VarRef ? "char*" : "ptr"

        result := ComCall(4, this, "ptr", mainSessionId, "int", subSessionId, "ptr", username, "ptr", resourceNames, "uint", numResources, "ptr", alternateResourceNames, "uint", numAlternateResourceName, "uint", portNumber, "ptr", operation, cookieMarshal, cookie, "uint", numBytesInCookie, "ptr", pSink, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgpolicyengine/nf-tsgpolicyengine-itsgpolicyengine-refresh
     */
    Refresh() {
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/tsgpolicyengine/nf-tsgpolicyengine-itsgpolicyengine-isquarantineenabled
     */
    IsQuarantineEnabled() {
        result := ComCall(6, this, "int*", &quarantineEnabled := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
