 ...
 * @version v4.0.30319
 */
class LPMAPIREADMAIL extends IUnknown {
     ...
     */
    Invoke(lhSession, ulUIParam, lpszMessageID, flFlags, ulReserved, lppMessage) {
        lpszMessageID := lpszMessageID is String ? StrPtr(lpszMessageID) : lpszMessageID

        lppMessageMarshal := lppMessage is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, "ptr", lhSession, "ptr", ulUIParam, "ptr", lpszMessageID, "uint", flFlags, "uint", ulReserved, lppMessageMarshal, lppMessage, "uint")
        return result
    }
}
