...
;@region Methods
     ...
     * @param {Pointer<Void>} context_ Optional pointer to a context that is passed to the callback.
     ...
     */
    static HcsCreateOperation(context_, callback) {
        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsCreateOperation", context_Marshal, context_, "ptr", callback, "ptr")
     ...
     */
    static HcsCreateOperationWithNotifications(eventTypes, context_, callback) {
        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsCreateOperationWithNotifications", "int", eventTypes, context_Marshal, context_, "ptr", callback, "ptr")
        resultHandle := HCS_OPERATION({Value: result}, True)
        return resultHandle
    }
     ...
     */
    static HcsSetOperationContext(operation, context_) {
        operation := operation is Win32Handle ? NumGet(operation, "ptr") : operation

        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsSetOperationContext", "ptr", operation, context_Marshal, context_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @returns {HRESULT} 
     */
    static HcsAddResourceToOperation(operation, type, uri, handle_) {
        operation := operation is Win32Handle ? NumGet(operation, "ptr") : operation
        uri := uri is String ? StrPtr(uri) : uri
        handle_ := handle_ is Win32Handle ? NumGet(handle_, "ptr") : handle_

        result := DllCall("computecore.dll\HcsAddResourceToOperation", "ptr", operation, "int", type, "ptr", uri, "ptr", handle_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    static HcsSetOperationCallback(operation, context_, callback) {
        operation := operation is Win32Handle ? NumGet(operation, "ptr") : operation

        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsSetOperationCallback", "ptr", operation, context_Marshal, context_, "ptr", callback, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     * @param {Pointer<SECURITY_DESCRIPTOR>} securityDescriptor_ Reserved for future use, must be `NULL`.
     ...
     */
    static HcsCreateComputeSystem(id, configuration, operation, securityDescriptor_) {
        ...
        result := DllCall("computecore.dll\HcsCreateComputeSystem", "ptr", id, "ptr", configuration, "ptr", operation, "ptr", securityDescriptor_, "ptr", computeSystem, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    static HcsWaitForComputeSystemExit(computeSystem, timeoutMs) {
        ...
        result := DllCall("computecore.dll\HcsWaitForComputeSystemExit", "ptr", computeSystem, "uint", timeoutMs, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    static HcsSetComputeSystemCallback(computeSystem, callbackOptions, context_, callback) {
        computeSystem := computeSystem is Win32Handle ? NumGet(computeSystem, "ptr") : computeSystem

        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsSetComputeSystemCallback", "ptr", computeSystem, "int", callbackOptions, context_Marshal, context_, "ptr", callback, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    static HcsCreateProcess(computeSystem, processParameters, operation, securityDescriptor_) {
        computeSystem := computeSystem is Win32Handle ? NumGet(computeSystem, "ptr") : computeSystem
        processParameters := processParameters is String ? StrPtr(processParameters) : processParameters
        operation := operation is Win32Handle ? NumGet(operation, "ptr") : operation

        process := HCS_PROCESS()
        result := DllCall("computecore.dll\HcsCreateProcess", "ptr", computeSystem, "ptr", processParameters, "ptr", operation, "ptr", securityDescriptor_, "ptr", process, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    static HcsSetProcessCallback(process, callbackOptions, context_, callback) {
        process := process is Win32Handle ? NumGet(process, "ptr") : process

        context_Marshal := context_ is VarRef ? "ptr" : "ptr"

        result := DllCall("computecore.dll\HcsSetProcessCallback", "ptr", process, "int", callbackOptions, context_Marshal, context_, "ptr", callback, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    static HcsWaitForProcessExit(computeSystem, timeoutMs) {
        ...
        result := DllCall("computecore.dll\HcsWaitForProcessExit", "ptr", computeSystem, "uint", timeoutMs, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    static HcsGetServiceProperties(propertyQuery) {
        ...
        result := DllCall("computecore.dll\HcsGetServiceProperties", "ptr", propertyQuery, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    static HcsModifyServiceSettings(settings) {
        ...
        result := DllCall("computecore.dll\HcsModifyServiceSettings", "ptr", settings, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
