...
;@region Methods
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/avrfsdk/nf-avrfsdk-verifierenumerateresource
     */
    static VerifierEnumerateResource(Process, Flags, ResourceType_, ResourceCallback, EnumerationContext) {
        Process := Process is Win32Handle ? NumGet(Process, "ptr") : Process

        EnumerationContextMarshal := EnumerationContext is VarRef ? "ptr" : "ptr"

        result := DllCall("verifier.dll\VerifierEnumerateResource", "ptr", Process, "uint", Flags, "uint", ResourceType_, "ptr", ResourceCallback, EnumerationContextMarshal, EnumerationContext, "uint")
        return result
    }
