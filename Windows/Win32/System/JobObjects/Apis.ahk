 ...
 * @since windows5.1.2600
 */
export OpenJobObjectW(dwDesiredAccess, bInheritHandle, lpName) {
    lpName := lpName is String ? StrPtr(lpName) : lpName
 ...
 * @since windows5.1.2600
 */
export AssignProcessToJobObject(hJob, hProcess) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\AssignProcessToJobObject", HANDLE, hJob, HANDLE, hProcess, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows10.0.10240
 */
export SetIoRateControlInformationJobObject(hJob, IoRateControlInfo) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\SetIoRateControlInformationJobObject", HANDLE, hJob, JOBOBJECT_IO_RATE_CONTROL_INFORMATION.Ptr, IoRateControlInfo, UInt32)
    if(A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export UserHandleGrantAccess(hUserHandle, hJob, bGrant) {
    A_LastError := 0

    result := DllCall("USER32.dll\UserHandleGrantAccess", HANDLE, hUserHandle, HANDLE, hJob, BOOL, bGrant, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export OpenJobObjectA(dwDesiredAccess, bInheritHandle, lpName) {
    lpName := lpName is String ? StrPtr(lpName) : lpName
