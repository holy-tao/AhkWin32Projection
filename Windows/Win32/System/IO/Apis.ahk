...
export GetOverlappedResult(hFile, lpOverlapped, lpNumberOfBytesTransferred, bWait) {
    ...
    result := DllCall("KERNEL32.dll\GetOverlappedResult", HANDLE, hFile, OVERLAPPED.Ptr, lpOverlapped, lpNumberOfBytesTransferredMarshal, lpNumberOfBytesTransferred, BOOL, bWait, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows5.1.2600
 */
export CancelIo(hFile) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\CancelIo", HANDLE, hFile, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
 ...
 * @since windows6.0.6000
 */
export CancelSynchronousIo(hThread) {
    A_LastError := 0

    result := DllCall("KERNEL32.dll\CancelSynchronousIo", HANDLE, hThread, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
