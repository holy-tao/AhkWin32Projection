...
class INetworkTransportSettings extends IUnknown{
     ...
     * @param {Pointer<Byte>} ValueIn 
     * @param {Pointer<UInt32>} LengthOut 
     * @param {Pointer<Byte>} ValueOut 
     * @returns {HRESULT} 
     */
    ApplySetting(SettingId, LengthIn, ValueIn, LengthOut, ValueOut) {
        result := ComCall(3, this, "ptr", SettingId, "uint", LengthIn, "char*", ValueIn, "uint*", LengthOut, "char*", ValueOut, "int")
        if(result != 0)
    ...
    QuerySetting(SettingId, LengthIn, ValueIn, LengthOut, ValueOut) {
        result := ComCall(4, this, "ptr", SettingId, "uint", LengthIn, "char*", ValueIn, "uint*", LengthOut, "char*", ValueOut, "int")
        if(result != 0)
            throw OSError(result)
