...
class IRTCProfile extends IUnknown{
    ...
    get_ClientBanner(pfBanner) {
        result := ComCall(10, this, "ptr", pfBanner, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<Int32>} plSupportedSessions 
     * @returns {HRESULT} 
     */
    get_SessionCapabilities(plSupportedSessions) {
        result := ComCall(19, this, "int*", plSupportedSessions, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    get_State(penState) {
        result := ComCall(20, this, "int*", penState, "int")
        if(result != 0)
            throw OSError(result)
