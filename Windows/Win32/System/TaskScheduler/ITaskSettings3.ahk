...
class ITaskSettings3 extends ITaskSettings{
    ...
    get_DisallowStartOnRemoteAppSession(pDisallowStart) {
        result := ComCall(47, this, "ptr", pDisallowStart, "int")
        if(result != 0)
    ...
    get_UseUnifiedSchedulingEngine(pUseUnifiedEngine) {
        result := ComCall(49, this, "ptr", pUseUnifiedEngine, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_MaintenanceSettings(ppMaintenanceSettings) {
        result := ComCall(51, this, "ptr", ppMaintenanceSettings, "int")
        if(result != 0)
            throw OSError(result)
    ...
    /**
     * 
     * @param {Pointer<IMaintenanceSettings>} pMaintenanceSettings 
     * @returns {HRESULT} 
     */
    ...
    CreateMaintenanceSettings(ppMaintenanceSettings) {
        result := ComCall(53, this, "ptr", ppMaintenanceSettings, "int")
        if(result != 0)
            throw OSError(result)
    ...
    get_Volatile(pVolatile) {
        result := ComCall(54, this, "ptr", pVolatile, "int")
        if(result != 0)
            throw OSError(result)
