...
class ScopeNamespace extends IDispatch{
    ...
    GetParent(Node, Parent) {
        result := ComCall(7, this, "ptr", Node, "ptr", Parent, "int")
        if(result != 0)
     ...
     */
    GetChild(Node, Child) {
        result := ComCall(8, this, "ptr", Node, "ptr", Child, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetNext(Node, Next) {
        result := ComCall(9, this, "ptr", Node, "ptr", Next, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    GetRoot(Root) {
        result := ComCall(10, this, "ptr", Root, "int")
        if(result != 0)
            throw OSError(result)
     ...
     */
    Expand(Node) {
        result := ComCall(11, this, "ptr", Node, "int")
        if(result != 0)
            throw OSError(result)
