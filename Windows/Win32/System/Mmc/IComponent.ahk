...
class IComponent extends IUnknown{
     ...
     * @param {Pointer<IConsole>} lpConsole 
     * @returns {HRESULT} <ul>
    ...
    QueryDataObject(cookie, type, ppDataObject) {
        result := ComCall(6, this, "ptr", cookie, "int", type, "ptr", ppDataObject, "int")
        if(result != 0)
            throw OSError(result)
     ...
     * @param {Pointer<Int32>} pViewOptions 
     * @returns {HRESULT} 
     */
    GetResultViewType(cookie, ppViewType, pViewOptions) {
        result := ComCall(7, this, "ptr", cookie, "ptr", ppViewType, "int*", pViewOptions, "int")
        if(result != 0)
            throw OSError(result)
