...
class _Application extends IDispatch{
     ...
     * @see https://learn.microsoft.com/windows/win32/ktop-src/windowsribbon/windowsribbon-element-helpbutton
     */
    Help() {
        ComCall(7, this)
    }
     ...
     */
    get_Document() {
        result := ComCall(9, this, "ptr*", &Document_ := 0, "int")
        if(result != 0) {
     ...
     */
    get_Visible() {
        result := ComCall(12, this, "int*", &Visible := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    get_UserControl() {
        result := ComCall(15, this, "int*", &UserControl := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    get_VersionMajor() {
        result := ComCall(17, this, "int*", &VersionMajor := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
     ...
     */
    get_VersionMinor() {
        result := ComCall(18, this, "int*", &VersionMinor := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
