#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\Com\IUnknown.ahk

/**
 * The IExtendTaskPad interface is introduced in MMC 1.1.
 * @see https://docs.microsoft.com/windows/win32/api//mmc/nn-mmc-iextendtaskpad
 * @namespace Windows.Win32.System.Mmc
 * @version v4.0.30319
 */
class IExtendTaskPad extends IUnknown{
    /**
     * The interface identifier for IExtendTaskPad
     * @type {Guid}
     */
    static IID => Guid("{8dee6511-554d-11d1-9fea-00600832db4a}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 3

    /**
     * 
     * @param {Pointer<IDataObject>} pdo 
     * @param {Pointer<VARIANT>} arg 
     * @param {Pointer<VARIANT>} param2 
     * @returns {HRESULT} 
     */
    TaskNotify(pdo, arg, param2) {
        result := ComCall(3, this, "ptr", pdo, "ptr", arg, "ptr", param2, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IDataObject>} pdo 
     * @param {PWSTR} szTaskGroup 
     * @param {Pointer<IEnumTASK>} ppEnumTASK 
     * @returns {HRESULT} 
     */
    EnumTasks(pdo, szTaskGroup, ppEnumTASK) {
        szTaskGroup := szTaskGroup is String ? StrPtr(szTaskGroup) : szTaskGroup

        result := ComCall(4, this, "ptr", pdo, "ptr", szTaskGroup, "ptr", ppEnumTASK, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} pszGroup 
     * @param {Pointer<PWSTR>} pszTitle 
     * @returns {HRESULT} 
     */
    GetTitle(pszGroup, pszTitle) {
        pszGroup := pszGroup is String ? StrPtr(pszGroup) : pszGroup

        result := ComCall(5, this, "ptr", pszGroup, "ptr", pszTitle, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} pszGroup 
     * @param {Pointer<PWSTR>} pszDescriptiveText 
     * @returns {HRESULT} 
     */
    GetDescriptiveText(pszGroup, pszDescriptiveText) {
        pszGroup := pszGroup is String ? StrPtr(pszGroup) : pszGroup

        result := ComCall(6, this, "ptr", pszGroup, "ptr", pszDescriptiveText, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} pszGroup 
     * @param {Pointer<MMC_TASK_DISPLAY_OBJECT>} pTDO 
     * @returns {HRESULT} 
     */
    GetBackground(pszGroup, pTDO) {
        pszGroup := pszGroup is String ? StrPtr(pszGroup) : pszGroup

        result := ComCall(7, this, "ptr", pszGroup, "ptr", pTDO, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {PWSTR} pszGroup 
     * @param {Pointer<MMC_LISTPAD_INFO>} lpListPadInfo 
     * @returns {HRESULT} 
     */
    GetListPadInfo(pszGroup, lpListPadInfo) {
        pszGroup := pszGroup is String ? StrPtr(pszGroup) : pszGroup

        result := ComCall(8, this, "ptr", pszGroup, "ptr", lpListPadInfo, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
