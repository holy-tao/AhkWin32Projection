 ...
 * @since windows6.0.6000
 */
export WerReportSetParameter(hReportHandle, dwparamID, pwzName, pwzValue) {
 ...
 * @since windows6.0.6000
 */
export WerReportAddFile(hReportHandle, pwzPath, repFileType, dwFileFlags) {
    pwzPath := pwzPath is String ? StrPtr(pwzPath) : pwzPath

    result := DllCall("wer.dll\WerReportAddFile", HREPORT, hReportHandle, "ptr", pwzPath, WER_FILE_TYPE, repFileType, WER_FILE, dwFileFlags, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerReportSetUIOption(hReportHandle, repUITypeID, pwzValue) {
    pwzValue := pwzValue is String ? StrPtr(pwzValue) : pwzValue

    result := DllCall("wer.dll\WerReportSetUIOption", HREPORT, hReportHandle, WER_REPORT_UI, repUITypeID, "ptr", pwzValue, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerReportSubmit(hReportHandle, consent, dwFlags) {
    result := DllCall("wer.dll\WerReportSubmit", HREPORT, hReportHandle, WER_CONSENT, consent, WER_SUBMIT_FLAGS, dwFlags, "int*", &pSubmitResult := 0, "HRESULT")
 ...
 * @since windows6.0.6000
 */
export WerReportCloseHandle(hReportHandle) {
    result := DllCall("wer.dll\WerReportCloseHandle", HREPORT, hReportHandle, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerRegisterFile(pwzFile, regFileType, dwFlags) {
    pwzFile := pwzFile is String ? StrPtr(pwzFile) : pwzFile

    result := DllCall("KERNEL32.dll\WerRegisterFile", "ptr", pwzFile, WER_REGISTER_FILE_TYPE, regFileType, WER_FILE, dwFlags, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerUnregisterFile(pwzFilePath) {
    pwzFilePath := pwzFilePath is String ? StrPtr(pwzFilePath) : pwzFilePath

    result := DllCall("KERNEL32.dll\WerUnregisterFile", "ptr", pwzFilePath, "HRESULT")
    return result
}
 ...
 * @since windows10.0.15063
 */
export WerRegisterCustomMetadata(key, value) {
    key := key is String ? StrPtr(key) : key
    value := value is String ? StrPtr(value) : value

    result := DllCall("KERNEL32.dll\WerRegisterCustomMetadata", "ptr", key, "ptr", value, "HRESULT")
    return result
}
 ...
 * @since windows10.0.15063
 */
export WerUnregisterCustomMetadata(key) {
    key := key is String ? StrPtr(key) : key

    result := DllCall("KERNEL32.dll\WerUnregisterCustomMetadata", "ptr", key, "HRESULT")
    return result
}
 ...
 */
export WerRegisterAppLocalDump(localAppDataRelativePath) {
    localAppDataRelativePath := localAppDataRelativePath is String ? StrPtr(localAppDataRelativePath) : localAppDataRelativePath

    result := DllCall("KERNEL32.dll\WerRegisterAppLocalDump", "ptr", localAppDataRelativePath, "HRESULT")
    return result
}
 ...
 * @since windows10.0.16299
 */
export WerUnregisterAppLocalDump() {
    result := DllCall("KERNEL32.dll\WerUnregisterAppLocalDump", "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerSetFlags(dwFlags) {
    result := DllCall("KERNEL32.dll\WerSetFlags", WER_FAULT_REPORTING, dwFlags, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerGetFlags(hProcess) {
    result := DllCall("KERNEL32.dll\WerGetFlags", HANDLE, hProcess, "uint*", &pdwFlags := 0, "HRESULT")
    return pdwFlags
}
 ...
 * @since windows6.0.6000
 */
export WerAddExcludedApplication(pwzExeName, bAllUsers) {
    pwzExeName := pwzExeName is String ? StrPtr(pwzExeName) : pwzExeName

    result := DllCall("wer.dll\WerAddExcludedApplication", "ptr", pwzExeName, BOOL, bAllUsers, "HRESULT")
    return result
}
 ...
 * @since windows6.0.6000
 */
export WerRemoveExcludedApplication(pwzExeName, bAllUsers) {
    pwzExeName := pwzExeName is String ? StrPtr(pwzExeName) : pwzExeName

    result := DllCall("wer.dll\WerRemoveExcludedApplication", "ptr", pwzExeName, BOOL, bAllUsers, "HRESULT")
    return result
}
 ...
 * @since windows10.0.15063
 */
export WerStoreOpen(repStoreType) {
    phReportStore := HREPORTSTORE.Owned()
    result := DllCall("wer.dll\WerStoreOpen", REPORT_STORE_TYPES, repStoreType, HREPORTSTORE.Ptr, phReportStore, "HRESULT")
 ...
 * @since windows10.0.15063
 */
export WerStoreGetFirstReportKey(_hReportStore) {
    result := DllCall("wer.dll\WerStoreGetFirstReportKey", HREPORTSTORE, _hReportStore, PWSTR.Ptr, &ppszReportKey := 0, "HRESULT")
 ...
 * @since windows10.0.15063
 */
export WerStoreGetNextReportKey(_hReportStore) {
    result := DllCall("wer.dll\WerStoreGetNextReportKey", HREPORTSTORE, _hReportStore, PWSTR.Ptr, &ppszReportKey := 0, "HRESULT")
    return ppszReportKey
}
 ...
 * @since windows10.0.15063
 */
export WerStoreQueryReportMetadataV2(_hReportStore, pszReportKey, pReportMetadata) {
    pszReportKey := pszReportKey is String ? StrPtr(pszReportKey) : pszReportKey

    result := DllCall("wer.dll\WerStoreQueryReportMetadataV2", HREPORTSTORE, _hReportStore, "ptr", pszReportKey, WER_REPORT_METADATA_V2.Ptr, pReportMetadata, "HRESULT")
    return result
}
 ...
 * @returns {HRESULT} 
 */
export WerStoreQueryReportMetadataV3(_hReportStore, pszReportKey, pReportMetadata) {
    pszReportKey := pszReportKey is String ? StrPtr(pszReportKey) : pszReportKey

    result := DllCall("wer.dll\WerStoreQueryReportMetadataV3", HREPORTSTORE, _hReportStore, "ptr", pszReportKey, WER_REPORT_METADATA_V3.Ptr, pReportMetadata, "HRESULT")
    return result
}
...
export WerStorePurge() {
    result := DllCall("wer.dll\WerStorePurge", "HRESULT")
    return result
}
 ...
 * @returns {Integer} 
 */
export WerStoreGetReportCount(_hReportStore) {
    result := DllCall("wer.dll\WerStoreGetReportCount", HREPORTSTORE, _hReportStore, "uint*", &pdwReportCount := 0, "HRESULT")
 ...
 * @returns {Integer} 
 */
export WerStoreGetSizeOnDisk(_hReportStore) {
    result := DllCall("wer.dll\WerStoreGetSizeOnDisk", HREPORTSTORE, _hReportStore, "uint*", &pqwSizeInBytes := 0, "HRESULT")
 ...
 * @returns {HRESULT} 
 */
export WerStoreQueryReportMetadataV1(_hReportStore, pszReportKey, pReportMetadata) {
    pszReportKey := pszReportKey is String ? StrPtr(pszReportKey) : pszReportKey

    result := DllCall("wer.dll\WerStoreQueryReportMetadataV1", HREPORTSTORE, _hReportStore, "ptr", pszReportKey, WER_REPORT_METADATA_V1.Ptr, pReportMetadata, "HRESULT")
    return result
}
 ...
 * @since windows5.1.2600
 */
export AddERExcludedApplicationA(szApplication) {
    szApplication := szApplication is String ? StrPtr(szApplication) : szApplication

    A_LastError := 0

    result := DllCall("faultrep.dll\AddERExcludedApplicationA", "ptr", szApplication, BOOL)
 ...
 * @since windows5.1.2600
 */
export AddERExcludedApplicationW(wszApplication) {
    wszApplication := wszApplication is String ? StrPtr(wszApplication) : wszApplication

    A_LastError := 0

    result := DllCall("faultrep.dll\AddERExcludedApplicationW", "ptr", wszApplication, BOOL)
    if(!result && A_LastError) {
        throw OSError(A_LastError)
    }
