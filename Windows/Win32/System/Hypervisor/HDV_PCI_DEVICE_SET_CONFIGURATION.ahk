 ...
 * @version v4.0.30319
 */
class HDV_PCI_DEVICE_SET_CONFIGURATION extends IUnknown {
     ...
     */
    Invoke(deviceContext, configurationValueCount, configurationValues) {
        deviceContextMarshal := deviceContext is VarRef ? "ptr" : "ptr"
        configurationValuesMarshal := configurationValues is VarRef ? "ptr*" : "ptr"

        result := ComCall(3, this, deviceContextMarshal, deviceContext, "uint", configurationValueCount, configurationValuesMarshal, configurationValues, "int")
        if(result != 0) {
