...
class ISecurityCallContext extends IDispatch{
    ...
    IsCallerInRole(bstrRole, pfInRole) {
        ...
        result := ComCall(10, this, "ptr", bstrRole, "ptr", pfInRole, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsSecurityEnabled(pfIsEnabled) {
        result := ComCall(11, this, "ptr", pfIsEnabled, "int")
        if(result != 0)
            throw OSError(result)
    ...
    IsUserInRole(pUser, bstrRole, pfInRole) {
        ...
        result := ComCall(12, this, "ptr", pUser, "ptr", bstrRole, "ptr", pfInRole, "int")
        if(result != 0)
            throw OSError(result)
