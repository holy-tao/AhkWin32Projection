 ...
 * @see https://learn.microsoft.com/windows/win32/api//content/comsvcs/nn-comsvcs-isendmethodevents
...
class ISendMethodEvents extends IUnknown{
     ...
     * @see https://learn.microsoft.com/windows/win32/api//content/comsvcs/nf-comsvcs-isendmethodevents-sendmethodcall
     */
    SendMethodCall(pIdentity, riid, dwMeth) {
        pIdentityMarshal := pIdentity is VarRef ? "ptr" : "ptr"

        result := ComCall(3, this, pIdentityMarshal, pIdentity, "ptr", riid, "uint", dwMeth, "int")
        if(result != 0) {
     ...
     * @returns {HRESULT} This method can return the standard return values E_INVALIDARG, E_OUTOFMEMORY, E_UNEXPECTED, E_FAIL, and S_OK.
     * @see https://learn.microsoft.com/windows/win32/api//content/comsvcs/nf-comsvcs-isendmethodevents-sendmethodreturn
     */
    SendMethodReturn(pIdentity, riid, dwMeth, hrCall, hrServer) {
        pIdentityMarshal := pIdentity is VarRef ? "ptr" : "ptr"

        result := ComCall(4, this, pIdentityMarshal, pIdentity, "ptr", riid, "uint", dwMeth, "int", hrCall, "int", hrServer, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
