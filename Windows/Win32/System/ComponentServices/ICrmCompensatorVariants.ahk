 ...
 * @see https://learn.microsoft.com/windows/win32/api/comsvcs/nn-comsvcs-icrmcompensatorvariants
 * @namespace Windows.Win32.System.ComponentServices
 */
export default struct ICrmCompensatorVariants extends IUnknown {
    /**
     ...
     * used for ICrmCompensatorVariants interfaces
    */
    struct Vtbl extends IUnknown.Vtbl {
        SetLogControlVariants : IntPtr
        BeginPrepareVariants  : IntPtr
        PrepareRecordVariants : IntPtr
        EndPrepareVariants    : IntPtr
        BeginCommitVariants   : IntPtr
        CommitRecordVariants  : IntPtr
        EndCommitVariants     : IntPtr
        BeginAbortVariants    : IntPtr
        AbortRecordVariants   : IntPtr
        EndAbortVariants      : IntPtr
    }

    __New(implObj := 0, flags := "") {
        if (NumGet(ObjGetDataPtr(this), 0, "ptr") == 0) {
            this.vtbl := ICrmCompensatorVariants.Vtbl()
        }
    ...
    PrepareRecordVariants(pLogRecord) {
        result := ComCall(5, this, VARIANT.Ptr, pLogRecord, VARIANT_BOOL.Ptr, &pbForget := 0, "HRESULT")
        return pbForget
    }
     ...
     */
    EndPrepareVariants() {
        result := ComCall(6, this, VARIANT_BOOL.Ptr, &pbOkToPrepare := 0, "HRESULT")
    ...
    BeginCommitVariants(bRecovery) {
        result := ComCall(7, this, VARIANT_BOOL, bRecovery, "HRESULT")
        return result
    }
    ...
    CommitRecordVariants(pLogRecord) {
        result := ComCall(8, this, VARIANT.Ptr, pLogRecord, VARIANT_BOOL.Ptr, &pbForget := 0, "HRESULT")
        return pbForget
    }
    ...
    BeginAbortVariants(bRecovery) {
        result := ComCall(10, this, VARIANT_BOOL, bRecovery, "HRESULT")
        return result
    }
    ...
    AbortRecordVariants(pLogRecord) {
        result := ComCall(11, this, VARIANT.Ptr, pLogRecord, VARIANT_BOOL.Ptr, &pbForget := 0, "HRESULT")
        return pbForget
    }
    ...
    Query(iid) {
        if (ICrmCompensatorVariants.IID.Equals(iid)) {
            return true
        }
    ...
    Dispose() {
        ...
        super.Dispose()
        CallbackFree(this.vtbl.SetLogControlVariants)
        CallbackFree(this.vtbl.BeginPrepareVariants)
        CallbackFree(this.vtbl.PrepareRecordVariants)
        CallbackFree(this.vtbl.EndPrepareVariants)
        CallbackFree(this.vtbl.BeginCommitVariants)
        CallbackFree(this.vtbl.CommitRecordVariants)
        CallbackFree(this.vtbl.EndCommitVariants)
        CallbackFree(this.vtbl.BeginAbortVariants)
        CallbackFree(this.vtbl.AbortRecordVariants)
        CallbackFree(this.vtbl.EndAbortVariants)
    }
}
