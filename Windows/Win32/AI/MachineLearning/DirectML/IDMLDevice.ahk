#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\..\Guid.ahk
#Include .\IDMLObject.ahk

/**
 * Represents a DirectML device, which is used to create operators, binding tables, command recorders, and other objects.
 * @see https://docs.microsoft.com/windows/win32/api//directml/nn-directml-idmldevice
 * @namespace Windows.Win32.AI.MachineLearning.DirectML
 * @version v4.0.30319
 */
class IDMLDevice extends IDMLObject{
    /**
     * The interface identifier for IDMLDevice
     * @type {Guid}
     */
    static IID => Guid("{6dbd6437-96fd-423f-a98c-ae5e7c2a573f}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 7

    /**
     * 
     * @param {Integer} feature 
     * @param {Integer} featureQueryDataSize 
     * @param {Pointer} featureQueryData 
     * @param {Integer} featureSupportDataSize 
     * @param {Pointer} featureSupportData 
     * @returns {HRESULT} 
     */
    CheckFeatureSupport(feature, featureQueryDataSize, featureQueryData, featureSupportDataSize, featureSupportData) {
        result := ComCall(7, this, "int", feature, "uint", featureQueryDataSize, "ptr", featureQueryData, "uint", featureSupportDataSize, "ptr", featureSupportData, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<DML_OPERATOR_DESC>} desc 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    CreateOperator(desc, riid, ppv) {
        result := ComCall(8, this, "ptr", desc, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<IDMLOperator>} op 
     * @param {Integer} flags 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    CompileOperator(op, flags, riid, ppv) {
        result := ComCall(9, this, "ptr", op, "int", flags, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} operatorCount 
     * @param {Pointer<IDMLCompiledOperator>} operators 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    CreateOperatorInitializer(operatorCount, operators, riid, ppv) {
        result := ComCall(10, this, "uint", operatorCount, "ptr", operators, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    CreateCommandRecorder(riid, ppv) {
        result := ComCall(11, this, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<DML_BINDING_TABLE_DESC>} desc 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    CreateBindingTable(desc, riid, ppv) {
        result := ComCall(12, this, "ptr", desc, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} count 
     * @param {Pointer<IDMLPageable>} ppObjects 
     * @returns {HRESULT} 
     */
    Evict(count, ppObjects) {
        result := ComCall(13, this, "uint", count, "ptr", ppObjects, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Integer} count 
     * @param {Pointer<IDMLPageable>} ppObjects 
     * @returns {HRESULT} 
     */
    MakeResident(count, ppObjects) {
        result := ComCall(14, this, "uint", count, "ptr", ppObjects, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @returns {HRESULT} 
     */
    GetDeviceRemovedReason() {
        result := ComCall(15, this, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }

    /**
     * 
     * @param {Pointer<Guid>} riid 
     * @param {Pointer<Void>} ppv 
     * @returns {HRESULT} 
     */
    GetParentDevice(riid, ppv) {
        result := ComCall(16, this, "ptr", riid, "ptr", ppv, "int")
        if(result != 0)
            throw OSError(result)

        return result
    }
}
