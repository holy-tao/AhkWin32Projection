...
class IMLOperatorShapeInferenceContext extends IMLOperatorAttributes{
    ...
    GetInputTensorDimensionCount(inputIndex, dimensionCount) {
        result := ComCall(12, this, "uint", inputIndex, "uint*", dimensionCount, "int")
        if(result != 0)
            throw OSError(result)
    ...
    GetInputTensorShape(inputIndex, dimensionCount, dimensions) {
        result := ComCall(13, this, "uint", inputIndex, "uint", dimensionCount, "uint*", dimensions, "int")
        if(result != 0)
            throw OSError(result)
    ...
    SetOutputTensorShape(outputIndex, dimensionCount, dimensions) {
        result := ComCall(14, this, "uint", outputIndex, "uint", dimensionCount, "uint*", dimensions, "int")
        if(result != 0)
            throw OSError(result)
