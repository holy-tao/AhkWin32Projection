#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Security\Credentials\PasswordCredential.ahk
#Include ..\..\Win32\System\WinRT\HSTRING.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Networking.Vpn
 * @version WindowsRuntime 1.4
 */
class IVpnPickedCredential extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IVpnPickedCredential
     * @type {Guid}
     */
    static IID => Guid("{9a793ac7-8854-4e52-ad97-24dd9a842bce}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_PasskeyCredential", "get_AdditionalPin", "get_OldPasswordCredential"]

    /**
     * @type {PasswordCredential} 
     */
    PasskeyCredential {
        get => this.get_PasskeyCredential()
    }

    /**
     * @type {HSTRING} 
     */
    AdditionalPin {
        get => this.get_AdditionalPin()
    }

    /**
     * @type {PasswordCredential} 
     */
    OldPasswordCredential {
        get => this.get_OldPasswordCredential()
    }

    /**
     * 
     * @returns {PasswordCredential} 
     */
    get_PasskeyCredential() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return PasswordCredential(value)
    }

    /**
     * 
     * @returns {HSTRING} 
     */
    get_AdditionalPin() {
        value := HSTRING()
        result := ComCall(7, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @returns {PasswordCredential} 
     */
    get_OldPasswordCredential() {
        result := ComCall(8, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return PasswordCredential(value)
    }
}
