#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include .\WwanConnectionProfileDetails.ahk
#Include .\WlanConnectionProfileDetails.ahk
#Include ..\..\Foundation\IReference.ahk
#Include ..\..\Foundation\IPropertyValue.ahk
#Include ..\..\Foundation\IAsyncOperation.ahk
#Include ..\..\Foundation\Collections\IVectorView.ahk
#Include .\NetworkUsage.ahk
#Include .\ConnectivityInterval.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Networking.Connectivity
 * @version WindowsRuntime 1.4
 */
class IConnectionProfile2 extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IConnectionProfile2
     * @type {Guid}
     */
    static IID => Guid("{e2045145-4c9f-400c-9150-7ec7d6e2888a}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_IsWwanConnectionProfile", "get_IsWlanConnectionProfile", "get_WwanConnectionProfileDetails", "get_WlanConnectionProfileDetails", "get_ServiceProviderGuid", "GetSignalBars", "GetDomainConnectivityLevel", "GetNetworkUsageAsync", "GetConnectivityIntervalsAsync"]

    /**
     * @type {Boolean} 
     */
    IsWwanConnectionProfile {
        get => this.get_IsWwanConnectionProfile()
    }

    /**
     * @type {Boolean} 
     */
    IsWlanConnectionProfile {
        get => this.get_IsWlanConnectionProfile()
    }

    /**
     * @type {WwanConnectionProfileDetails} 
     */
    WwanConnectionProfileDetails {
        get => this.get_WwanConnectionProfileDetails()
    }

    /**
     * @type {WlanConnectionProfileDetails} 
     */
    WlanConnectionProfileDetails {
        get => this.get_WlanConnectionProfileDetails()
    }

    /**
     * @type {IReference<Guid>} 
     */
    ServiceProviderGuid {
        get => this.get_ServiceProviderGuid()
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_IsWwanConnectionProfile() {
        result := ComCall(6, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_IsWlanConnectionProfile() {
        result := ComCall(7, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @returns {WwanConnectionProfileDetails} 
     */
    get_WwanConnectionProfileDetails() {
        result := ComCall(8, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return WwanConnectionProfileDetails(value)
    }

    /**
     * 
     * @returns {WlanConnectionProfileDetails} 
     */
    get_WlanConnectionProfileDetails() {
        result := ComCall(9, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return WlanConnectionProfileDetails(value)
    }

    /**
     * 
     * @returns {IReference<Guid>} 
     */
    get_ServiceProviderGuid() {
        result := ComCall(10, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetGuid(), value)
    }

    /**
     * 
     * @returns {IReference<Integer>} 
     */
    GetSignalBars() {
        result := ComCall(11, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetByte(), value)
    }

    /**
     * 
     * @returns {Integer} 
     */
    GetDomainConnectivityLevel() {
        result := ComCall(12, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @param {DateTime} startTime 
     * @param {DateTime} endTime 
     * @param {Integer} granularity 
     * @param {NetworkUsageStates} states 
     * @returns {IAsyncOperation<IVectorView<NetworkUsage>>} 
     */
    GetNetworkUsageAsync(startTime, endTime, granularity, states) {
        result := ComCall(13, this, "ptr", startTime, "ptr", endTime, "int", granularity, "ptr", states, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(IVectorView.Call.Bind(IVectorView, NetworkUsage), value)
    }

    /**
     * 
     * @param {DateTime} startTime 
     * @param {DateTime} endTime 
     * @param {NetworkUsageStates} states 
     * @returns {IAsyncOperation<IVectorView<ConnectivityInterval>>} 
     */
    GetConnectivityIntervalsAsync(startTime, endTime, states) {
        result := ComCall(14, this, "ptr", startTime, "ptr", endTime, "ptr", states, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(IVectorView.Call.Bind(IVectorView, ConnectivityInterval), value)
    }
}
