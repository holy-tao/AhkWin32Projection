#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\Collections\IVectorView.ahk
#Include ..\..\Foundation\Uri.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Networking.Connectivity
 * @version WindowsRuntime 1.4
 */
class IProxyConfiguration extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IProxyConfiguration
     * @type {Guid}
     */
    static IID => Guid("{ef3a60b4-9004-4dd6-b7d8-b3e502f4aad0}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_ProxyUris", "get_CanConnectDirectly"]

    /**
     * @type {IVectorView<Uri>} 
     */
    ProxyUris {
        get => this.get_ProxyUris()
    }

    /**
     * @type {Boolean} 
     */
    CanConnectDirectly {
        get => this.get_CanConnectDirectly()
    }

    /**
     * 
     * @returns {IVectorView<Uri>} 
     */
    get_ProxyUris() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IVectorView(Uri, value)
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_CanConnectDirectly() {
        result := ComCall(7, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }
}
