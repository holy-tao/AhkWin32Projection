#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\IReference.ahk
#Include ..\..\Foundation\IPropertyValue.ahk
#Include ..\..\Storage\Streams\IBuffer.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Networking.Connectivity
 * @version WindowsRuntime 1.4
 */
class IConnectionProfileFilter2 extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IConnectionProfileFilter2
     * @type {Guid}
     */
    static IID => Guid("{cd068ee1-c3fc-4fad-9ddc-593faa4b7885}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["put_IsRoaming", "get_IsRoaming", "put_IsOverDataLimit", "get_IsOverDataLimit", "put_IsBackgroundDataUsageRestricted", "get_IsBackgroundDataUsageRestricted", "get_RawData"]

    /**
     * @type {IReference<Boolean>} 
     */
    IsRoaming {
        get => this.get_IsRoaming()
        set => this.put_IsRoaming(value)
    }

    /**
     * @type {IReference<Boolean>} 
     */
    IsOverDataLimit {
        get => this.get_IsOverDataLimit()
        set => this.put_IsOverDataLimit(value)
    }

    /**
     * @type {IReference<Boolean>} 
     */
    IsBackgroundDataUsageRestricted {
        get => this.get_IsBackgroundDataUsageRestricted()
        set => this.put_IsBackgroundDataUsageRestricted(value)
    }

    /**
     * @type {IBuffer} 
     */
    RawData {
        get => this.get_RawData()
    }

    /**
     * 
     * @param {IReference<Boolean>} value 
     * @returns {HRESULT} 
     */
    put_IsRoaming(value) {
        result := ComCall(6, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {IReference<Boolean>} 
     */
    get_IsRoaming() {
        result := ComCall(7, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetBoolean(), value)
    }

    /**
     * 
     * @param {IReference<Boolean>} value 
     * @returns {HRESULT} 
     */
    put_IsOverDataLimit(value) {
        result := ComCall(8, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {IReference<Boolean>} 
     */
    get_IsOverDataLimit() {
        result := ComCall(9, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetBoolean(), value)
    }

    /**
     * 
     * @param {IReference<Boolean>} value 
     * @returns {HRESULT} 
     */
    put_IsBackgroundDataUsageRestricted(value) {
        result := ComCall(10, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {IReference<Boolean>} 
     */
    get_IsBackgroundDataUsageRestricted() {
        result := ComCall(11, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetBoolean(), value)
    }

    /**
     * 
     * @returns {IBuffer} 
     */
    get_RawData() {
        result := ComCall(12, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IBuffer(value)
    }
}
