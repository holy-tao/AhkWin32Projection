#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.UI.Input
 * @version WindowsRuntime 1.4
 */
class IPointerVisualizationSettings extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IPointerVisualizationSettings
     * @type {Guid}
     */
    static IID => Guid("{4d1e6461-84f7-499d-bd91-2a36e2b7aaa2}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["put_IsContactFeedbackEnabled", "get_IsContactFeedbackEnabled", "put_IsBarrelButtonFeedbackEnabled", "get_IsBarrelButtonFeedbackEnabled"]

    /**
     * @type {Boolean} 
     */
    IsContactFeedbackEnabled {
        get => this.get_IsContactFeedbackEnabled()
        set => this.put_IsContactFeedbackEnabled(value)
    }

    /**
     * @type {Boolean} 
     */
    IsBarrelButtonFeedbackEnabled {
        get => this.get_IsBarrelButtonFeedbackEnabled()
        set => this.put_IsBarrelButtonFeedbackEnabled(value)
    }

    /**
     * 
     * @param {Boolean} value 
     * @returns {HRESULT} 
     */
    put_IsContactFeedbackEnabled(value) {
        result := ComCall(6, this, "int", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_IsContactFeedbackEnabled() {
        result := ComCall(7, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @param {Boolean} value 
     * @returns {HRESULT} 
     */
    put_IsBarrelButtonFeedbackEnabled(value) {
        result := ComCall(8, this, "int", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_IsBarrelButtonFeedbackEnabled() {
        result := ComCall(9, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }
}
