...
class IToastNotifier extends IInspectable{
    ...
    /**
     * The interface identifier for IToastNotifier
     * @type {Guid}
     */
    static IID => Guid("{75927b93-03f3-41ec-91d3-6e5bac1b38e7}")
     ...
     * @type {Array<String>}
     */
    static VTableNames => ["Show", "Hide", "get_Setting", "AddToSchedule", "RemoveFromSchedule", "GetScheduledToastNotifications"]
     ...
     * @param {ToastNotification} notification_ 
     ...
     */
    Hide(notification_) {
        result := ComCall(7, this, "ptr", notification_, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    /**
     * 
     * @param {ScheduledToastNotification} scheduledToast 
     * @returns {HRESULT} 
     */
    AddToSchedule(scheduledToast) {
        result := ComCall(9, this, "ptr", scheduledToast, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    RemoveFromSchedule(scheduledToast) {
        result := ComCall(10, this, "ptr", scheduledToast, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    GetScheduledToastNotifications() {
        ...
        return IVectorView(ScheduledToastNotification, result_)
    }
}
