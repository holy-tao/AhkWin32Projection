#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\IReference.ahk
#Include ..\..\Foundation\IPropertyValue.ahk
#Include .\NotificationVisual.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * Receives notification messages when an app is triggered through a toast from the action center.
 * @see https://learn.microsoft.com/windows/win32/api//content/notificationactivationcallback/nn-notificationactivationcallback-inotificationactivationcallback
 * @namespace Windows.UI.Notifications
 * @version WindowsRuntime 1.4
 */
class INotification extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for INotification
     * @type {Guid}
     */
    static IID => Guid("{108037fe-eb76-4f82-97bc-da07530a2e20}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_ExpirationTime", "put_ExpirationTime", "get_Visual", "put_Visual"]

    /**
     * @type {IReference<DateTime>} 
     */
    ExpirationTime {
        get => this.get_ExpirationTime()
        set => this.put_ExpirationTime(value)
    }

    /**
     * @type {NotificationVisual} 
     */
    Visual {
        get => this.get_Visual()
        set => this.put_Visual(value)
    }

    /**
     * 
     * @returns {IReference<DateTime>} 
     */
    get_ExpirationTime() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IReference((ptr) => IPropertyValue(ptr).GetDateTime(), value)
    }

    /**
     * 
     * @param {IReference<DateTime>} value 
     * @returns {HRESULT} 
     */
    put_ExpirationTime(value) {
        result := ComCall(7, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {NotificationVisual} 
     */
    get_Visual() {
        result := ComCall(8, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return NotificationVisual(value)
    }

    /**
     * 
     * @param {NotificationVisual} value 
     * @returns {HRESULT} 
     */
    put_Visual(value) {
        result := ComCall(9, this, "ptr", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }
}
