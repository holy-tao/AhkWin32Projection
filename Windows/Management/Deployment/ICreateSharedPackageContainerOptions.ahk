#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\Collections\IVector.ahk
#Include .\SharedPackageContainerMember.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Management.Deployment
 * @version WindowsRuntime 1.4
 */
class ICreateSharedPackageContainerOptions extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for ICreateSharedPackageContainerOptions
     * @type {Guid}
     */
    static IID => Guid("{c2ab6ece-f664-5c8e-a4b3-2a33276d3dde}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_Members", "get_ForceAppShutdown", "put_ForceAppShutdown", "get_CreateCollisionOption", "put_CreateCollisionOption"]

    /**
     * @type {IVector<SharedPackageContainerMember>} 
     */
    Members {
        get => this.get_Members()
    }

    /**
     * @type {Boolean} 
     */
    ForceAppShutdown {
        get => this.get_ForceAppShutdown()
        set => this.put_ForceAppShutdown(value)
    }

    /**
     * @type {Integer} 
     */
    CreateCollisionOption {
        get => this.get_CreateCollisionOption()
        set => this.put_CreateCollisionOption(value)
    }

    /**
     * 
     * @returns {IVector<SharedPackageContainerMember>} 
     */
    get_Members() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IVector(SharedPackageContainerMember, value)
    }

    /**
     * 
     * @returns {Boolean} 
     */
    get_ForceAppShutdown() {
        result := ComCall(7, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @param {Boolean} value 
     * @returns {HRESULT} 
     */
    put_ForceAppShutdown(value) {
        result := ComCall(8, this, "int", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }

    /**
     * 
     * @returns {Integer} 
     */
    get_CreateCollisionOption() {
        result := ComCall(9, this, "int*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return value
    }

    /**
     * 
     * @param {Integer} value 
     * @returns {HRESULT} 
     */
    put_CreateCollisionOption(value) {
        result := ComCall(10, this, "int", value, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }
}
