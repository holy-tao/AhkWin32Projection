#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\IAsyncOperation.ahk
#Include .\KeyCredentialOperationResult.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Security.Credentials
 * @version WindowsRuntime 1.4
 */
class IKeyCredentialWithWindow extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IKeyCredentialWithWindow
     * @type {Guid}
     */
    static IID => Guid("{050deb3b-b19c-4635-9df6-5650d66c62b1}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["RequestSignForWindowAsync"]

    /**
     * 
     * @param {WindowId} window_ 
     * @param {IBuffer} data 
     * @returns {IAsyncOperation<KeyCredentialOperationResult>} 
     */
    RequestSignForWindowAsync(window_, data) {
        window_ := window_ is Win32Handle ? NumGet(window_, "ptr") : window_

        result := ComCall(6, this, "ptr", window_, "ptr", data, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(KeyCredentialOperationResult, value)
    }
}
