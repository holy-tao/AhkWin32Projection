#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Foundation\Collections\IVector.ahk
#Include ..\..\Foundation\IPropertyValue.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Storage.Search
 * @version WindowsRuntime 1.4
 */
class IQueryOptionsWithProviderFilter extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IQueryOptionsWithProviderFilter
     * @type {Guid}
     */
    static IID => Guid("{5b9d1026-15c4-44dd-b89a-47a59b7d7c4f}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_StorageProviderIdFilter"]

    /**
     * @type {IVector<HSTRING>} 
     */
    StorageProviderIdFilter {
        get => this.get_StorageProviderIdFilter()
    }

    /**
     * 
     * @returns {IVector<HSTRING>} 
     */
    get_StorageProviderIdFilter() {
        result := ComCall(6, this, "ptr*", &value := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IVector((ptr) => HSTRING({ Value: ptr }), value)
    }
}
