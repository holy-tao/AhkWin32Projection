#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Win32\System\WinRT\HSTRING.ahk
#Include .\JsonValue.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Data.Json
 * @version WindowsRuntime 1.4
 */
class IJsonValueStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IJsonValueStatics
     * @type {Guid}
     */
    static IID => Guid("{5f6b544a-2f53-48e1-91a3-f78b50a6345c}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["Parse", "TryParse", "CreateBooleanValue", "CreateNumberValue", "CreateStringValue"]

    /**
     * Parses a text value and returns its value in a given type using the culture of the application.
     * @remarks
     * Use **Parse** only for converting from string to date/time and number types. For general type conversions, use the **Convert** function. Keep in mind that there is a certain performance overhead in parsing the string value.
     * @param {HSTRING} input_ 
     * @returns {JsonValue} 
     * @see https://learn.microsoft.com/office/client-developer/ocs/docs/access/parse-function-access-custom-web-app
     */
    Parse(input_) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        input_ := input_ is Win32Handle ? NumGet(input_, "ptr") : input_

        result := ComCall(6, this, "ptr", input_, "ptr*", &jsonValue_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return JsonValue(jsonValue_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @param {Pointer<JsonValue>} result_ 
     * @returns {Boolean} 
     */
    TryParse(input_, result_) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        input_ := input_ is Win32Handle ? NumGet(input_, "ptr") : input_

        result := ComCall(7, this, "ptr", input_, "ptr", result_, "int*", &succeeded := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return succeeded
    }

    /**
     * 
     * @param {Boolean} input_ 
     * @returns {JsonValue} 
     */
    CreateBooleanValue(input_) {
        result := ComCall(8, this, "int", input_, "ptr*", &jsonValue_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return JsonValue(jsonValue_)
    }

    /**
     * 
     * @param {Float} input_ 
     * @returns {JsonValue} 
     */
    CreateNumberValue(input_) {
        result := ComCall(9, this, "double", input_, "ptr*", &jsonValue_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return JsonValue(jsonValue_)
    }

    /**
     * 
     * @param {HSTRING} input_ 
     * @returns {JsonValue} 
     */
    CreateStringValue(input_) {
        if(input_ is String) {
            pin := HSTRING.Create(input_)
            input_ := pin.Value
        }
        input_ := input_ is Win32Handle ? NumGet(input_, "ptr") : input_

        result := ComCall(10, this, "ptr", input_, "ptr*", &jsonValue_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return JsonValue(jsonValue_)
    }
}
