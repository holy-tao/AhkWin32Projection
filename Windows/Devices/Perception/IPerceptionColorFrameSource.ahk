 ...
 */
class IPerceptionColorFrameSource extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IPerceptionColorFrameSource
     * @type {Guid}
     */
    static IID => Guid("{dc6dba7c-0b58-468d-9ca1-6db04cc0477c}")
    ...
    AcquireControlSession() {
        ...
        return PerceptionControlSession(value)
    }
    ...
    /**
     * 
     * @param {PerceptionDepthFrameSource} correlatedDepthFrameSource 
    ...
    TryGetDepthCorrelatedCameraIntrinsicsAsync(correlatedDepthFrameSource) {
        result := ComCall(31, this, "ptr", correlatedDepthFrameSource, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    TryGetDepthCorrelatedCoordinateMapperAsync(targetSourceId, correlatedDepthFrameSource) {
        if(targetSourceId is String) {
            pin := HSTRING.Create(targetSourceId)
            targetSourceId := pin.Value
        }
        targetSourceId := targetSourceId is Win32Handle ? NumGet(targetSourceId, "ptr") : targetSourceId

        result := ComCall(32, this, "ptr", targetSourceId, "ptr", correlatedDepthFrameSource, "ptr*", &result_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
    ...
    OpenReader() {
        ...
        return PerceptionColorFrameReader(result_)
    }
}
