#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\Win32ComInterface.ahk
#Include ..\..\..\Guid.ahk
#Include ..\..\Win32\System\WinRT\HSTRING.ahk
#Include ..\..\Foundation\IAsyncOperation.ahk
#Include .\SmsDevice.ahk
#Include ..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Devices.Sms
 * @version WindowsRuntime 1.4
 */
class ISmsDeviceStatics extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for ISmsDeviceStatics
     * @type {Guid}
     */
    static IID => Guid("{f88d07ea-d815-4dd1-a234-4520ce4604a4}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["GetDeviceSelector", "FromIdAsync", "GetDefaultAsync"]

    /**
     * 
     * @returns {HSTRING} 
     */
    GetDeviceSelector() {
        phstrDeviceClassSelector := HSTRING()
        result := ComCall(6, this, "ptr", phstrDeviceClassSelector, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return phstrDeviceClassSelector
    }

    /**
     * 
     * @param {HSTRING} deviceId 
     * @returns {IAsyncOperation<SmsDevice>} 
     */
    FromIdAsync(deviceId) {
        if(deviceId is String) {
            pin := HSTRING.Create(deviceId)
            deviceId := pin.Value
        }
        deviceId := deviceId is Win32Handle ? NumGet(deviceId, "ptr") : deviceId

        result := ComCall(7, this, "ptr", deviceId, "ptr*", &asyncInfo := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(SmsDevice, asyncInfo)
    }

    /**
     * 
     * @returns {IAsyncOperation<SmsDevice>} 
     */
    GetDefaultAsync() {
        result := ComCall(8, this, "ptr*", &asyncInfo := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(SmsDevice, asyncInfo)
    }
}
