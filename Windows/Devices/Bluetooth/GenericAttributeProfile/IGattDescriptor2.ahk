#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\..\Foundation\IAsyncOperation.ahk
#Include .\GattWriteResult.ahk
#Include ..\..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Devices.Bluetooth.GenericAttributeProfile
 * @version WindowsRuntime 1.4
 */
class IGattDescriptor2 extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IGattDescriptor2
     * @type {Guid}
     */
    static IID => Guid("{8f563d39-d630-406c-ba11-10cdd16b0e5e}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["WriteValueWithResultAsync"]

    /**
     * 
     * @param {IBuffer} value 
     * @returns {IAsyncOperation<GattWriteResult>} 
     */
    WriteValueWithResultAsync(value) {
        result := ComCall(6, this, "ptr", value, "ptr*", &operation := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IAsyncOperation(GattWriteResult, operation)
    }
}
