#Requires AutoHotkey v2.0.0 64-bit
#Include ..\..\..\..\Win32ComInterface.ahk
#Include ..\..\..\..\Guid.ahk
#Include ..\..\..\Foundation\EventRegistrationToken.ahk
#Include ..\..\..\Win32\System\WinRT\IInspectable.ahk

/**
 * @namespace Windows.Devices.Printers.Extensions
 * @version WindowsRuntime 1.4
 */
class IPrintTaskConfiguration extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IPrintTaskConfiguration
     * @type {Guid}
     */
    static IID => Guid("{e3c22451-3aa4-4885-9240-311f5f8fbe9d}")

    /**
     * The offset into the COM object's virtual function table at which this interface's methods begin.
     * @type {Integer}
     */
    static vTableOffset => 6

    /**
     * @readonly used when implementing interfaces to order function pointers
     * @type {Array<String>}
     */
    static VTableNames => ["get_PrinterExtensionContext", "add_SaveRequested", "remove_SaveRequested"]

    /**
     * @type {IInspectable} 
     */
    PrinterExtensionContext {
        get => this.get_PrinterExtensionContext()
    }

    /**
     * 
     * @returns {IInspectable} 
     */
    get_PrinterExtensionContext() {
        result := ComCall(6, this, "ptr*", &context_ := 0, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return IInspectable(context_)
    }

    /**
     * 
     * @param {TypedEventHandler<PrintTaskConfiguration, PrintTaskConfigurationSaveRequestedEventArgs>} eventHandler 
     * @returns {EventRegistrationToken} 
     */
    add_SaveRequested(eventHandler) {
        eventCookie := EventRegistrationToken()
        result := ComCall(7, this, "ptr", eventHandler, "ptr", eventCookie, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return eventCookie
    }

    /**
     * 
     * @param {EventRegistrationToken} eventCookie 
     * @returns {HRESULT} 
     */
    remove_SaveRequested(eventCookie) {
        eventCookie := eventCookie is Win32Handle ? NumGet(eventCookie, "ptr") : eventCookie

        result := ComCall(8, this, "ptr", eventCookie, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }

        return result
    }
}
