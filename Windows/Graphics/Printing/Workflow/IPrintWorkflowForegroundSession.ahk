 ...
 */
class IPrintWorkflowForegroundSession extends IInspectable{

    static sizeof => A_PtrSize
    /**
     * The interface identifier for IPrintWorkflowForegroundSession
     * @type {Guid}
     */
    static IID => Guid("{c79b63d0-f8ec-4ceb-953a-c8876157dd33}")
     ...
     * @type {Array<String>}
     */
    static VTableNames => ["add_SetupRequested", "remove_SetupRequested", "add_XpsDataAvailable", "remove_XpsDataAvailable", "get_Status", "Start"]
    ...
    add_SetupRequested(setupEventHandler) {
        ...
        result := ComCall(6, this, "ptr", setupEventHandler, "ptr", token, "int")
        if(result != 0) {
    ...
    add_XpsDataAvailable(xpsDataAvailableEventHandler) {
        token := EventRegistrationToken()
        result := ComCall(8, this, "ptr", xpsDataAvailableEventHandler, "ptr", token, "int")
        if(result != 0) {
            throw OSError(A_LastError || result)
        }
